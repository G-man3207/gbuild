...
// The `Shared` summarizer reuses gBuild's full-replace summarization core
...
/// `FullReplace` strategy (default): gBuild's full-replace — summarize the
...
/// preserves user intent itself, matching gBuild. The reduction and
    ...
    P: CompactionSampler<Item = T> + ?Sized,
{
    ...
    // 2. Summarize the whole conversation through gBuild's shared core.
    ...
    P: CompactionSampler<Item = T> + ?Sized,
{
    ...
    let summary_text = match policy.summarizer {
        ...
        // New (default): gBuild's shared summarization core from
...
/// — gBuild's summarization core (`build_summary_prompt` + bounded retry +
...
/// intra and gBuild stay in lock-step. Outcome mapping:
...
where
    ...
    P: CompactionSampler<Item = T> + ?Sized,
{
    // gBuild appends the summarization prompt as the final user message;
    ...
    .await
    {
        // gBuild returns the raw summary and cleans it in its assembler;
