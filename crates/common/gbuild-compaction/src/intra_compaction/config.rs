...
///
/// - `FullReplace` (default): gBuild's full-replace strategy — summarize
...
///
/// Uses **percentage** thresholds (borrowed from gbuild-shell's
...
/// time (token threshold only, matching gBuild) and also ignores
...
pub struct IntraCompactionConfig {
    ...
    /// as gBuild full-replace auto-compact). Worthless early passes are
