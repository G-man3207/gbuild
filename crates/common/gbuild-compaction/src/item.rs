...
//! gBuild implements it for `gbuild_sampling_types::ConversationItem`.
//!
...
/// `ConversationItem` variants (gBuild).
...
/// constructs the typed harness items that make up gBuild's rebuilt
/// history.
///
...
/// that impl. They are also gBuild-specific (Grok chat's tail-keep path
...
///
/// The gBuild implementor (`ConversationItem`) maps each constructor to the
