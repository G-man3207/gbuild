...
//!
//! The canonical `sample → classify → retry` loop, used by **both** gBuild's
...
//! the winning summary (gBuild cleans in its assembler; intra cleans via
...
pub enum SampleRetryError {
    ...
    Failure {
        ...
        /// signal the gBuild host uses to step down its input size).
