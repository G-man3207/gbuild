//! gBuild's full-replace compaction pass.
//!
//! gBuild does not select a tail to keep; it summarizes the whole
...
//! ```
//!
//! Per-harness concerns stay in the product host (for example `gbuild-shell`): the triggers, the
...
pub struct FullReplaceContext<T> {
    ...
    /// turn). gBuild keeps this; pass empty to drop it.
...
///
/// Returned by [`sample_full_replace_summary`] for harnesses (gBuild's
...
/// Run gBuild's full-replace compaction pass and return the rebuilt
...
///
/// This is the seam gBuild's shell uses: it drives the verbatim → fitted →
    ...
    O: FullReplaceObserver + ?Sized,
{
    ...
    let prompt = CompactionPrompt {
        // gBuild appends the summarization prompt as the final user
...
mod tests {
    ...
    /// returns a structured summary must produce gBuild's exact compacted
    /// history shape, with the LLM output cleaned and the agent-state reminder
    /// carried through as the final item.
    #[tokio::test]
    async fn full_replace_produces_gbuild_history_shape() {
        let llm_turns = vec![
