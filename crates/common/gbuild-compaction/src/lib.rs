...
//! stay in each product host (for example `gbuild-shell`).
//!
//! The crate depends on **neither** a conversation-type crate nor
//! `gbuild-sampling-types`. It is decoupled from both Grok chat and
//! gBuild hosts through a small set of trait seams:
//!
...
//!
//! - [`code_compaction`] — gBuild's whole-session **full-replace**
...
//! gBuild; hosts still own snapshotting and host-only sections).
...
// gBuild's full-replace subsystem now lives under `code_compaction`;
