...
const PRODUCTION_ENDPOINTS: GBuildEndpoints = GBuildEndpoints {
...
pub enum GBuildEnvironment {
    #[default]
    Production,
}
impl GBuildEnvironment {
    pub fn from_flags(_dev: bool, _staging: bool) -> Self {
        GBuildEnvironment::Production
    }
    /// Indicator string for display; `None` for Production.
    pub fn indicator(&self) -> Option<&'static str> {
        match self {
            GBuildEnvironment::Production => None,
        }
    }
    pub fn is_production(&self) -> bool {
        matches!(self, GBuildEnvironment::Production)
    }
    fn env_prefix(&self) -> &'static str {
        match self {
            GBuildEnvironment::Production => "GBUILD_PRODUCTION",
        }
    }
    /// Compiled endpoint set for this environment (production by default).
    pub fn endpoints(&self) -> GBuildEndpoints {
        match self {
            GBuildEnvironment::Production => PRODUCTION_ENDPOINTS,
        }
    }
    ...
    /// `GBUILD_GATEWAY_URL` opt-in takes precedence.
    pub fn gateway_ws_url(&self) -> String {
        self.resolve("_GATEWAY_WS_URL", self.endpoints().gateway_ws_url)
    }
}
impl std::fmt::Display for GBuildEnvironment {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            GBuildEnvironment::Production => write!(f, "production"),
        }
    }
}
...
mod tests {
    ...
    fn test_env_prefix() {
        assert_eq!(
            GBuildEnvironment::Production.env_prefix(),
            "GBUILD_PRODUCTION"
        );
    }
    #[test]
    fn env_var_guard_set_value_updates_then_restores_on_drop() {
        const KEY: &str = "XAI_GBUILD_ENV_VAR_GUARD_SET_VALUE_PROBE";
    ...
    fn relay_and_gateway_urls_are_distinct() {
        assert_ne!(
            GBuildEnvironment::Production.relay_ws_url(),
            GBuildEnvironment::Production.gateway_ws_url(),
        );
    }
    #[test]
    fn test_from_flags() {
        assert_eq!(
            GBuildEnvironment::from_flags(false, false),
            GBuildEnvironment::Production
        );
    }
}
