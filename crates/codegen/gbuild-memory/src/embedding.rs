...
pub(super) fn build_middleware_client(
    ...
    gbuild_http::with_auth_retry(gbuild_http::shared_client(), credentials)
}
...
fn build_static_middleware_client(
    ...
    let provider: std::sync::Arc<dyn gbuild_auth::AuthCredentialProvider> = std::sync::Arc::new(
        gbuild_auth::StaticAuthCredentialProvider::new(Box::new(NoopHttpAuth), api_key),
    );
...
impl gbuild_auth::HttpAuth for NoopHttpAuth {
    ...
    async fn embed_batch(
        ...
        for batch in texts.chunks(self.max_batch_size) {
            ...
            for attempt in 0..MAX_RETRIES {
                ...
                let request = gbuild_http::shared_client()
