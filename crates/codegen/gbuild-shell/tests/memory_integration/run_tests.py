...
Usage:
    ...
    python3 crates/codegen/gbuild-shell/tests/memory_integration/run_tests.py

    # Run only fast (no-model) tests:
    python3 crates/codegen/gbuild-shell/tests/memory_integration/run_tests.py --fast

    # Run a single test by name:
    python3 crates/codegen/gbuild-shell/tests/memory_integration/run_tests.py test_fts_search_quality
...
class AcpClient:
    """Talks ACP (JSON-RPC over NDJSON) to a gbuild agent stdio subprocess."""
...
class IsolatedEnv:
    """Creates a fully isolated gBuild environment with custom $HOME.

    The agent subprocess gets a fake $HOME with:
      ~/.gbuild/auth.json   (copied from real home)
      ~/.gbuild/memory/     (pre-populated by tests)
      ~/.gbuild/logs/       (memory.log appears here)

    And a workspace directory used as cwd when spawning the agent.
    """

    def __init__(self, workspace_name="test-project"):
        self.root = tempfile.mkdtemp(prefix="gbuild-memtest-")
        ...
        # Create .gbuild dirs in fake home
        self.gbuild_home = os.path.join(self.fake_home, ".gbuild")
        self.memory_dir = os.path.join(self.gbuild_home, "memory")
        self.logs_dir = os.path.join(self.gbuild_home, "logs")
        os.makedirs(self.memory_dir)
        os.makedirs(self.logs_dir)

        # Copy auth from real home
        real_auth = os.path.expanduser("~/.gbuild/auth.json")
        if os.path.isfile(real_auth):
            shutil.copy2(real_auth, os.path.join(self.gbuild_home, "auth.json"))

    def write_config(self, toml_str):
        """Write global ~/.gbuild/config.toml (where the agent reads config)."""
        with open(os.path.join(self.gbuild_home, "config.toml"), "w") as f:
            f.write(toml_str)
...
def _default_config() -> str:
    embed_model = os.environ.get("GBUILD_MEMORY_EMBEDDING_MODEL", "").strip()
...
""")
        ...
        client = env.spawn_agent(extra_env={"GBUILD_MEMORY": "0"})
        init_session(client)
        time.sleep(2)
...
def test_memory_enabled_env_override():
    """GBUILD_MEMORY=1 env var enables memory even without config."""
    section("13. GBUILD_MEMORY Env Var Override")
    env = IsolatedEnv()
    try:
        # Config enables memory
        env.write_config(INDEXING_ONLY_CONFIG)
        env.write_global_memory("# Env test content\n* Key value here\n")
        client = env.spawn_agent(extra_env={"GBUILD_MEMORY": "1"})
        init_session(client)
        time.sleep(3)

        db = env.get_db_path()
        if db:
            ok("index created with GBUILD_MEMORY=1")
        else:
            fail("no index despite GBUILD_MEMORY=1 and config enabled")
...
def test_multiple_workspace_isolation():
    ...
    try:
        ...
        # Config is already in global ~/.gbuild/config.toml from env1.write_config

        # Start agent in workspace beta (reuse env1's fake home)
        binary = os.environ.get("GBUILD_BINARY", "gbuild")
        env_vars = os.environ.copy()
        env_vars["HOME"] = env1.fake_home
        env_vars["GBUILD_MEMORY"] = "1"
        env_vars["RUST_LOG"] = "warn"
        env_vars["GBUILD_NO_LEADER"] = "1"
...
def build_binary():
    ...
    # Find rg for GBUILD_SHELL_BUNDLE_RG_PATH
    rg = shutil.which("rg")
    if not rg:
    ...
    env = os.environ.copy()
    env["GBUILD_SHELL_BUNDLE_RG_PATH"] = rg
