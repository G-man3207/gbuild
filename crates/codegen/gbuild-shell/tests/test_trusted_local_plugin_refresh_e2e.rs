...
//! 4. Smoke-validate session JSON under `$GBUILD_HOME/sessions/` after exit.
//!
//! Requires a built `gbuild` binary (`GBUILD_BINARY` or cargo-built pager) for the
//! ignored headless test.
//!
//! ```bash
//! cargo test -p gbuild-shell --test test_trusted_local_plugin_refresh_e2e
//! cargo test -p gbuild-shell --test test_trusted_local_plugin_refresh_e2e -- --ignored
//! ```

use std::collections::HashMap;
use std::path::{Path, PathBuf};

use gbuild_agent::plugins::SharedPluginRegistryHandle;
use gbuild_agent::plugins::discovery::DiscoveryConfig;
use gbuild_agent::plugins::git_install::{InstallSource, install_from_source};
...
fn trusted_local_refresh_surfaces_new_agent_via_discovery() {
    ...
    let _gbuild_guard = EnvVarGuard::set("GBUILD_HOME", &gbuild_home);
    ...
    let mut registry = InstallRegistry::empty(gbuild_home.join("installed-plugins"));
    ...
    // gbuild_home() caches the first GBUILD_HOME per process; a separate test
    ...
    assert_eq!(
        plugin.scope,
        gbuild_agent::plugins::PluginScope::CliOverride
    );
...
#[tokio::test]
#[ignore = "requires pre-built gBuild binary; run with --ignored"]
#[serial]
async fn headless_session_refreshes_trusted_local_plugin_and_writes_session_json() {
    ...
    let home = dunce::canonicalize(home_tmp.path()).unwrap();
    let gbuild_home = home.join(".gbuild");
    std::fs::create_dir_all(&gbuild_home).unwrap();
    ...
    // The spawned binary gets HOME/GBUILD_HOME via `cmd.env` below; this global env
    // is only for the in-process post-run discovery assertion (which resolves the
    // registry via gbuild_home()). `#[serial]` keeps it from racing other tests.
    let _home_guard = EnvVarGuard::set("HOME", &home);
    let _gbuild_guard = EnvVarGuard::set("GBUILD_HOME", &gbuild_home);
    ...
    sandbox
        ...
        .set_env("GBUILD_HOME", &gbuild_home);
    ...
    // Smoke: session storage under GBUILD_HOME/sessions has JSON artifacts.
    let sessions_root = gbuild_home.join("sessions");
    assert!(
