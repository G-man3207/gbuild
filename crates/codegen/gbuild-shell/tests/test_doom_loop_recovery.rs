...
use gbuild_sampler::RetryPolicy;
use gbuild_sampling_types::doom_loop::{DoomLoopSignalKind, SAMPLE_CHECK_EVENT_DATA_CUMULATIVE};
use gbuild_shell::sampling::{
...
use gbuild_test_support::{MockInferenceServer, MockModelEntry, ScriptedResponse};
...
/// ```bash
/// cargo test -p gbuild-shell --test test_doom_loop_recovery -- --ignored
/// ```
#[tokio::test]
...
async fn headless_config_enables_doom_loop_check_header() {
    ...
    let workdir = gbuild_test_support::git_workdir();
    let sandbox = gbuild_test_support::TestSandbox::builder()
        .mock_url(server.url())
        .build();
    ...
    let mut cmd = tokio::process::Command::new(gbuild_test_support::gbuild_binary());
    ...
    let result = gbuild_test_support::run_headless_in_sandbox(cmd, sandbox).await;
    gbuild_test_support::assert_headless_success(&result, "doom-loop header e2e", Some(&server));
