...
/// Shared `GBUILD_HOME` for the entire test binary. The `OnceLock` in
/// `gbuild-config` only allows one value per process, so all tests share
...
fn test_home() -> &'static PathBuf {
    ...
    HOME.get_or_init(|| {
        ...
        unsafe { std::env::set_var("GBUILD_HOME", &path) };
        path
    })
}
...
async fn mcp_tool_grant_persists_and_short_circuits_next_request() {
    run_actor_test(ClientType::GBuildPager, |handle, gw, cwd| async move {
...
async fn dont_ask_policy_denies_without_prompting() {
    use gbuild_workspace::permission::types::{PermissionConfig, PromptPolicy};
