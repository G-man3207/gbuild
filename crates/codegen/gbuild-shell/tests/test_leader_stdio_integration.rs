...
use gbuild_shell::cpu_profile::ControlErrorCode;
...
async fn setup_control_test_server(
    temp: &TempDir,
) -> (std::path::PathBuf, gbuild_shell::leader::ServerHandle) {
...
async fn test_set_model_broadcasts_to_session_subscribers() {
    ...
    let broadcast = format!(
        r#"{{"jsonrpc":"2.0","method":"gbuild/session_notification","params":{{"sessionId":"{}","update":{{"sessionUpdate":"model_changed","model_id":"grok-4","reasoning_effort":"high"}}}}}}"#,
        shared_sid
    );
    ...
    assert_eq!(inv1["method"], "gbuild/session_notification");
    ...
    assert_eq!(f["method"], "gbuild/session_notification");
...
async fn test_extension_method_roundtrip() {
    ...
    let ext_call = r#"{"jsonrpc":"2.0","id":50,"method":"_gbuild/search/fuzzy/open","params":{"sessionId":"sess-123","hidden":false}}"#;
    ...
    assert_eq!(json["method"], "_gbuild/search/fuzzy/open");
...
async fn test_session_ownership_cleanup_on_disconnect() {
    use gbuild_shell::leader::run_leader_server;
    ...
    tokio::spawn(async move {
        ...
        let (shutdown_tx, _shutdown_rx) =
            tokio::sync::watch::channel(gbuild_shell::leader::ShutdownReason::Manual);
        ...
        let _ = run_leader_server(
            ...
            client_count,
            agent_busy,
            gbuild_shell::agent::activity::AgentActivity::default(),
            ready_rx,
    ...
    assert_eq!(eviction_json["method"], "gbuild/internal/evict_sessions");
    ...
    let probe = r#"{"jsonrpc":"2.0","method":"gbuild/probe","params":{"ping":true}}"#;
...
async fn test_code_status_ext_request_forwarded_to_agent() {
    ...
    let status_req = r#"{"jsonrpc":"2.0","id":42,"method":"extensions/ext","params":{"method":"gbuild/code/status","params":{"sessionId":"sess-web-1","cwd":"/repo"}}}"#;
    ...
    assert_eq!(json["params"]["method"], "gbuild/code/status");
...
async fn test_raw_registration_handshake_not_ready_then_ready() {
    ...
    tokio::spawn(async move {
        ...
        let _ = run_leader_server(
            ...
            watch::channel(gbuild_shell::leader::ShutdownReason::Manual).0,
            None,
            control_state,
        )
        .await;
...
async fn test_version_mismatch_notification_sent_to_client() {
    use gbuild_shell::leader::{ClientCapabilities, ClientMode, LeaderClient, run_leader_server};
    use std::sync::Arc;
    use std::sync::atomic::{AtomicBool, AtomicUsize};
    use tokio::sync::{mpsc, watch};
    use tokio_util::sync::CancellationToken;
...
async fn test_auto_update_shutdown_reason_reaches_client() {
    use gbuild_shell::leader::{ClientCapabilities, ClientMode, LeaderClient, ShutdownReason};
    ...
    tokio::time::timeout(
        ...
        disconnect_rx.wait_for(|r| *r != gbuild_shell::leader::DisconnectReason::Connected),
    )
    .await
    ...
    assert_eq!(
        *disconnect_rx.borrow(),
        gbuild_shell::leader::DisconnectReason::LeaderShutdown
    );
}
...
async fn test_leader_code_nav_isolation_end_to_end() {
    ...
    let status_with_session = r#"{"jsonrpc":"2.0","id":10,"method":"extensions/ext","params":{"method":"gbuild/code/status","params":{"sessionId":"web-session","cwd":"/repo"}}}"#;
    ...
    assert_eq!(status_json["params"]["method"], "gbuild/code/status");
...
async fn setup_persistent_test_server(
    ...
    tokio::spawn(async move {
        ...
        let _ = run_leader_server(
            ...
            tokio::sync::watch::channel(false).0,
            tokio::sync::watch::channel(gbuild_shell::leader::ShutdownReason::Manual).0,
            None,
            control_state,
        )
        .await;
...
///
/// This binary does not sandbox GBUILD_HOME, so on a dev machine these entries
/// land in the real `~/.gbuild` log — accepted: the server already writes
...
fn orphan_log_count(request_id: &str) -> usize {
    let Some(bytes) = gbuild_telemetry::unified_log::snapshot_log() else {
        return 0;
    };
...
async fn wait_for_client_disconnected_log(client_id: u64) -> bool {
    let deadline = tokio::time::Instant::now() + Duration::from_secs(5);
    loop {
        let seen = gbuild_telemetry::unified_log::snapshot_log().is_some_and(|bytes| {
...
async fn test_hung_agent_leaves_transport_healthy_and_forwards_cancel() {
    ...
    assert_eq!(
        *client.disconnect_reason().borrow(),
        gbuild_shell::leader::DisconnectReason::Connected,
...
async fn test_sever_mid_rpc_orphans_response_and_replay_recovers() {
    ...
    assert_eq!(evict_json["method"], "gbuild/internal/evict_sessions");
