...
/// All tests share this as GBUILD_HOME (the `OnceLock` in gbuild-config
...
async fn update_config_does_not_leak_requirements_into_user_config() {
    ...
    let effective = gbuild_shell::config::load_effective_config().unwrap();
    let effective_cfg = gbuild_shell::util::config::load_config_from_toml(&effective);
    assert_eq!(
    ...
    gbuild_shell::util::config::update_config(|cfg| {
    ...
    let user_cfg = gbuild_shell::util::config::load_config_from_toml(&user_toml);
