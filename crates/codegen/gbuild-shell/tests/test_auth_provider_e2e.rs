...
//!
//! Runs the built gBuild binary headless against the mock inference server with
...
//! ```bash
//! cargo test -p gbuild-shell --test test_auth_provider_e2e -- --ignored
//! ```
//!
...
async fn provider_backed_model_sends_minted_token_on_the_wire() {
    ...
    sandbox.remove_env("GBUILD_LEADER_SOCKET");

    let gbuild_home = sandbox.gbuild_home().to_path_buf();
    std::fs::create_dir_all(&gbuild_home).expect("create .gbuild home");

    let counter = gbuild_home.join("mint-count");
    let helper = gbuild_home.join("mock-auth.sh");
    std::fs::write(
        &helper,
        format!(
...
async fn provider_with_args_and_json_output_sends_minted_token() {
    ...
    let seen_args = gbuild_home.join("seen-args");
    let helper = gbuild_home.join("mock-auth-json.sh");
    std::fs::write(
        &helper,
        format!(
            "#!/bin/sh\nprintf '%s' \"$*\" > {}\n\
