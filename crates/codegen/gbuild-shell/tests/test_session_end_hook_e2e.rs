...
//! debug build):
//! ```bash
//! cargo test -p gbuild-shell --test test_session_end_hook_e2e -- --ignored
//! ```
//!
//! CI coverage of the same machinery without a built binary lives in
//! `gbuild_shell::agent::activity` tests (the flush quiesce loop and its
//! grace expiry) and `gbuild_pager::acp::spawn` tests (the worker join:
...
async fn run_with_session_end_hook() -> (HeadlessResult, MockInferenceServer, tempfile::TempDir) {
    ...
    let hooks_dir = sandbox.gbuild_home().join("hooks");
