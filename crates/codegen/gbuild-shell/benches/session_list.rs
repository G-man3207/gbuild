...
//!
//! Run: `cargo bench -p gbuild-shell --bench session_list`
...
use gbuild_shell::session::persistence::Summary;
use gbuild_shell::session::storage::{JsonlStorageAdapter, StorageAdapter};
use gbuild_shell::session::unified_list::{ListReq, UnifiedListResult, build_unified_list};
...
impl Fixture {
    fn new(home: TempDir) -> Self {
        ...
        for workspace_index in 0..WORKSPACE_COUNT {
            ...
            let encoded = gbuild_shell::util::gbuild_home::encode_cwd_dirname(&cwd);
    ...
    fn assert_correct(&self, runtime: &tokio::runtime::Runtime) {
        let discovered = gbuild_shell::session::worktree::candidate_worktree_cwds_for_same_repo(
...
fn create_same_repo_cwds(home: &Path) -> SameRepoTopology {
    ...
    let worktree_base = gbuild_shell::session::worktree::worktree_base_dir(&main);
    ...
    for (index, (path, label)) in linked.iter().enumerate().take(DB_OVERLAP_WORKTREE_COUNT) {
        db.register(&WorktreeRecord {
            ...
            metadata: Some(gbuild_shell::session::worktree::build_label_metadata(
                label, false,
            )),
        })
...
fn bench_session_list(c: &mut Criterion) {
    ...
    unsafe {
        std::env::set_var("GBUILD_HOME", home.path());
    }
    assert_eq!(gbuild_shell::util::gbuild_home::gbuild_home(), home.path());
