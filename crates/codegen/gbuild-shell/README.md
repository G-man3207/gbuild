...
Credentials are stored in `~/.gbuild/auth.json` and persist across sessions. Tokens expire after 7 days; gBuild will prompt you to re-authenticate when needed.
...
export GBUILD_OIDC_ISSUER="https://acme.okta.com"
export GBUILD_OIDC_CLIENT_ID="0oa1b2c3d4e5f6g7h8i9"
```

Customers typically also override the API endpoint to point at their own proxy:
```bash
export GBUILD_CLI_CHAT_PROXY_BASE_URL="https://grok-proxy.acme.com/v1"
```

**3. Run `gbuild`.** The CLI discovers endpoints via `{issuer}/.well-known/openid-configuration`, opens the IdP login page, and stores tokens in `~/.gbuild/auth.json`. The OIDC token is sent as `Authorization: Bearer` to the configured proxy. Tokens auto-refresh silently via the stored `refresh_token`.
...
4. **stdout** → captured by gBuild and saved to `~/.gbuild/auth.json` as the access token
...
```bash
# Or via environment variables
export GBUILD_AUTH_PROVIDER_COMMAND="/usr/local/bin/my-auth-provider"
export GBUILD_AUTH_PROVIDER_LABEL="Acme Corp"   # optional
export GBUILD_AUTH_TOKEN_TTL=3600               # optional
```
...
When gBuild needs to refresh an expired token, it re-runs your binary with `GBUILD_AUTH_EXPIRED=1` set in the environment. Your binary can use this to take a faster silent-refresh path:

```bash
#!/bin/sh
if [ "$GBUILD_AUTH_EXPIRED" = "1" ]; then
...
`GBUILD_AUTH_EXPIRED` is optional — if your binary ignores it, gBuild still works. It just runs the same flow for both login and refresh.
...
export GBUILD_AUTH_EARLY_INVALIDATION_SECS=300
```
...
```bash
gbuild --debug-file /tmp/gbuild-auth.log -p "hello"
tail -f /tmp/gbuild-auth.log
```
...
- `GBUILD_AUTH_EXPIRED=1` is set whenever gBuild re-mints over a token still cached in memory, whether from near-expiry rotation or a rejection. The first mint on a cold cache runs without it.

**Token lifecycle:**

- Tokens are cached in memory per provider and shared by every model referencing the provider; nothing is written to disk. The command is a credential helper: it owns durable storage and OAuth2 refresh (keychain, its own dotdir, etc.), exactly like `gcloud auth print-access-token` or a git credential helper. On an in-session re-mint the last credential is handed back via `GBUILD_AUTH_PROVIDER_ACCESS_TOKEN` (and, when present, `GBUILD_AUTH_PROVIDER_REFRESH_TOKEN` / `GBUILD_AUTH_PROVIDER_EXPIRES_AT`), so a refresh-grant command can refresh instead of re-authenticating. The command must be non-interactive and fast; do any interactive login out of band, and gBuild re-runs the command on restart to re-mint.
...
**Security:** provider commands execute code, so they are honored only from trusted config layers (`~/.gbuild/config.toml`, managed config, requirements). A project's `.gbuild/config.toml` can never define one. Whatever layer sets a model's `base_url` decides where that model's minted token is sent, and `base_url` (unlike the provider table) is not stripped from remote or campaign patches, the same as for a static `env_key`. Keep provider tables and the model `base_url` in layers you trust. The command inherits gBuild's environment (so it sees `PATH`, `HOME`, and any other secrets there), but gBuild's own first-party credentials (`XAI_API_KEY`, `GBUILD_DEPLOYMENT_KEY`, and related keys) are removed so a BYOK helper never receives them; write helpers that read only what they need, and prefer the `GBUILD_AUTH_PROVIDER_*` handback for the prior credential.
...
curl -s -N -X POST "https://cli-chat-proxy.grok.com/v1/chat/completions" \
  ...
  -H "Authorization: Bearer $(jq -r '."https://accounts.x.ai/sign-in".key' ~/.gbuild/auth.json)" \
...
| -------------------------------- | -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Authorization: Bearer <token>`  | Yes      | Session token from `~/.gbuild/auth.json` (set by `gbuild login`)                                                                                                                              |
...
# Usage
async def main():
    client = await GBuildACPChat(cwd=".").init()
...
// Usage
const client = await new GBuildACPChat(".").init();
...
gBuild reads configuration from `~/.gbuild/config.toml`. If the file doesn't exist, gBuild uses sensible defaults. You only need to specify values you want to override.
...
telemetry = false                      # anonymous usage telemetry (env: GBUILD_TELEMETRY_ENABLED)
feedback = false                       # feedback system (env: GBUILD_FEEDBACK_ENABLED)
...
[tools]
respect_gitignore = true               # filter gitignored files from tools (env: GBUILD_RESPECT_GITIGNORE)
...
```toml
[telemetry]
events_url = "https://example.com/events"  # env: GBUILD_TELEMETRY_EVENTS_URL
events_api_key = "..."                      # env: GBUILD_TELEMETRY_EVENTS_API_KEY
mixpanel_token = "..."                      # env: GBUILD_TELEMETRY_MIXPANEL_TOKEN
mixpanel_enabled = true                     # env: GBUILD_TELEMETRY_MIXPANEL_ENABLED
trace_upload = true                         # env: GBUILD_TELEMETRY_TRACE_UPLOAD
```

When building from source, defaults can also be baked into the binary at compile time by setting `GBUILD_TELEMETRY_BUILD_EVENTS_URL`, `GBUILD_TELEMETRY_BUILD_EVENTS_API_KEY`, and `GBUILD_TELEMETRY_BUILD_MIXPANEL_TOKEN` in the build environment (providing a Mixpanel token this way also enables Mixpanel by default). Config-file and runtime env values override build-time defaults.
...
- project config: `<repo>/.gbuild/lsp.json`
- user config: `~/.gbuild/lsp.json`
...
- LSP tools are enabled (`GBUILD_LSP_TOOLS=1` or `[features] lsp_tools = true`)
- the merged LSP configuration is non-empty

Enable the tool for one run:

```bash
GBUILD_LSP_TOOLS=1 gbuild
```
...
- Passive diagnostics do **not** require `GBUILD_LSP_TOOLS=1`; they run whenever an applicable server is configured and starts successfully.
...
| --------------------------- | ----- | -------- |
| `./.gbuild/skills/`           | Local | Highest  |
| `<repo_root>/.gbuild/skills/` | Repo  | Medium   |
| `~/.gbuild/skills/`           | User  | Lowest   |
| `~/.claude/skills/`         | User  | Lowest   |

Skills with the same name are deduplicated — higher priority locations override lower ones.

Repo-scoped skills (Local and Repo) respect `.gitignore` and are filtered out if ignored. User-scoped skills (`~/.gbuild/skills/`) are outside the repo and never filtered.
...
gBuild discovers agent definitions from `.gbuild/agents/` (project), `~/.gbuild/agents/` (user), and built-in agents. Priority (highest wins):

1. `--agent-profile <PATH>` CLI flag
2. `[agent]` section in `config.toml`
3. `GBUILD_AGENT` env var
4. Default `gbuild` agent
...
# or
export GBUILD_AGENT="my-custom-agent"
```
...
```bash
export GBUILD_SUBAGENTS=0              # Environment variable
```
...
model = "grok-build"
prompt_file = ".gbuild/prompts/researcher.md"

[subagents.personas.concise]
instructions = "Be extremely concise. No filler words."
# instructions_file = ".gbuild/personas/concise.md"  # or load from file
```

Both are also discovered from `.gbuild/roles/*.toml` and `.gbuild/personas/*.toml` files respectively. If a requested persona is not found, the spawn fails (fail-closed).
...
gBuild discovers hooks from `.gbuild/hooks/` in the project directory. Manage them with:
...
**Web search model:** Set `[models] web_search`, `GBUILD_WEB_SEARCH_MODEL`, or `--web-search-model` to point the `web_search` tool at a different model. The target endpoint must support the Responses API and web search.
...
**Combining with `[endpoints]` config:** You can also set endpoints in `~/.gbuild/config.toml`:
...
MCP servers are configured in `~/.gbuild/config.toml`:
...
MCP servers can also be configured per-project in `.gbuild/config.toml`. gBuild walks from the current directory up to the git repo root, loading `.gbuild/config.toml` at each level:

| Location                        | Scope             | Priority |
| ------------------------------- | ----------------- | -------- |
| `~/.gbuild/config.toml`           | All projects      | Lowest   |
| `<repo-root>/.gbuild/config.toml` | This repository   | ↑        |
| `<cwd>/.gbuild/config.toml`       | Current directory | Highest  |
...
**Example:** commit a `.gbuild/config.toml` in your repo to share MCP servers across the team:
...
If you also have a `linear` server in `~/.gbuild/config.toml`, the project version replaces it entirely.

> **Note:** Only `[mcp_servers]` is supported in project-scoped `.gbuild/config.toml`. Other config sections (models, shortcuts, etc.) are only read from `~/.gbuild/config.toml`.
...
> **Experimental:** requires `--experimental-memory` (or `GBUILD_MEMORY=1` / `[memory] enabled = true` in config).
...
Memory is stored as Markdown files under `~/.gbuild/memory/`:
- **Global** (`~/.gbuild/memory/MEMORY.md`) — facts that apply across all your projects
- **Workspace** (`~/.gbuild/memory/<project-slug>-<hash8>/MEMORY.md`) — project-specific conventions and context
- **Session logs** (`~/.gbuild/memory/<project-slug>-<hash8>/sessions/`) — per-session summaries
...
For richer capture — decisions, patterns, debugging workflows, API discoveries — use `/flush` in the TUI. This triggers an LLM-generated summary of the current session's most important content and writes it to a dated session log under `~/.gbuild/memory/<project-slug>-<hash8>/sessions/`, where it is indexed and searchable in future sessions.
...
Key options under `[memory]` in `~/.gbuild/config.toml`:
...
| `watcher.enabled` | `true` | Watch `~/.gbuild/memory/` for external edits and reindex on search |
...
When first-turn memory injection runs, gBuild emits the `gbuild-shell-memory_injection`
...
| `workspace`     | Everywhere         | CWD + `/tmp` + `~/.gbuild/` | Allowed       | Normal development       |
| `read-only`     | Everywhere         | `~/.gbuild/` only           | Blocked       | Exploration, code review |
| `strict`        | CWD + system paths | CWD + `/tmp` + `~/.gbuild/` | Blocked       | Untrusted code           |

Sensitive paths (`~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.gbuild/auth/`) are always
write-protected regardless of profile.

### Custom Profiles

Create `~/.gbuild/sandbox.toml` (global) or `.gbuild/sandbox.toml` (per-project):
...
Sandbox events (profile applied, violations) are logged to `~/.gbuild/sandbox-events.jsonl`
...
- **Skills** — from `.gbuild/skills/`, `~/.gbuild/skills/`, plugins, and config paths
...
gBuild automatically discovers configuration from Claude Code directories alongside native `.gbuild/` paths. No extra setup is needed.

### What is picked up

| Component         | Claude Code location                                 | How gBuild uses it               |
| ----------------- | ---------------------------------------------------- | -------------------------------- |
| **Skills**        | `.claude/skills/`, `~/.claude/skills/`               | Loaded as skills (same as `.gbuild/skills/`) |
...
Fetch a specific URL and return its content as markdown. **Disabled by default** — enable with `GBUILD_WEB_FETCH=1`. 
...
Sessions are stored under `~/.gbuild/sessions/`, organized by URL-encoded working directory:

```
~/.gbuild/sessions/<encoded-cwd>/<session-id>/
...
| Path                  | Description                                         |
| --------------------- | --------------------------------------------------- |
| `~/.gbuild/config.toml` | Configuration file                                  |
| `~/.gbuild/sessions/`   | Persisted sessions (organized by working directory) |
| `~/.gbuild/auth.json`   | Authentication credentials (auto-managed)           |
| `~/.gbuild/memory/`     | Cross-session memory files and index                |
| `~/.gbuild/skills/`     | User-scoped skill definitions                       |
| `~/.gbuild/plugins/`    | User-scoped plugins                                 |
| `~/.gbuild/agents/`     | User-scoped agent definitions                       |
| `.gbuild/config.toml`   | Project-scoped config (MCP servers)                 |
| `.gbuild/skills/`       | Project-scoped skill definitions                    |
| `.gbuild/plugins/`      | Project-scoped plugins                              |
| `.gbuild/agents/`       | Project-scoped agent definitions                    |
| `.gbuild/hooks/`        | Project-scoped hooks                                |
| `.gbuild/lsp.json`      | LSP server configuration                            |
...
| `GBUILD_AUTH_EARLY_INVALIDATION_SECS` | Seconds before `expires_at` to consider a token expired (default: `300`). See [Automatic Credential Refresh](#automatic-credential-refresh) |
| `GBUILD_OIDC_ISSUER`              | OIDC issuer URL (alternative to config file). See [OIDC](#oidc-customer-sso)                             |
| `GBUILD_OIDC_CLIENT_ID`           | OIDC client ID (alternative to config file). See [OIDC](#oidc-customer-sso)                              |
| `GBUILD_HOME`                     | Override config directory (default: `~/.gbuild`)                                                           |
| `GBUILD_SUBAGENTS`                | Enable (`1`) or disable (`0`) subagent/task tool support                                                 |
| `GBUILD_MEMORY`                   | Enable (`1`) or disable (`0`) cross-session memory                                                       |
| `GBUILD_AGENT`                    | Custom agent definition path or name (see [Agent Profiles](#agent-profiles))                             |
| `GBUILD_WEB_FETCH`                | Enable (`1`) or disable (`0`) the `web_fetch` tool                                                       |
| `GBUILD_WEB_FETCH_PROXY`          | Egress proxy URL for `web_fetch` requests (overridden by `[toolset.web_fetch] proxy_endpoint`)           |
| `GBUILD_RESPECT_GITIGNORE`        | Disable `.gitignore` filtering in tools when set to `0`                                                  |
| `GBUILD_FEEDBACK_ENABLED`         | Enable (`1`) or disable (`0`) feedback system independently from telemetry                               |
| `GBUILD_DEPLOYMENT_KEY`           | Management API key for enterprise deployments                                                            |
| `GBUILD_LOG_FILE`                 | Enable file logging by providing a file path (the value is used verbatim as the path)                    |
| `GBUILD_DEBUG_LOG`                | Debug firehose (set by `--debug`): truthy routes per-session logs to `~/.gbuild/debug/<sessionId>.txt`, a path writes that one file |
| `RUST_LOG`                      | Log filter for stderr (headless `-p` defaults to `off`, other non-TUI modes to `error`; TUI captures stderr) and for the `GBUILD_LOG_FILE` log; the `--debug` firehose ignores it |
...
```bash
mkdir -p ~/.gbuild/completions/bash
gbuild completions bash > ~/.gbuild/completions/bash/gbuild.bash
```

Add to `~/.bashrc`:

```bash
[[ -r "$HOME/.gbuild/completions/bash/gbuild.bash" ]] && source "$HOME/.gbuild/completions/bash/gbuild.bash"
```
...
```bash
mkdir -p ~/.gbuild/completions/zsh
gbuild completions zsh > ~/.gbuild/completions/zsh/_gbuild
```

Add to `~/.zshrc`:

```zsh
fpath=("$HOME/.gbuild/completions/zsh" $fpath)
autoload -Uz compinit
compinit
```
...
Write logs to a file for debugging. The TUI captures stderr, so `RUST_LOG` alone won't produce visible output in production — use `gbuild --debug` or `GBUILD_LOG_FILE` instead:

```bash
# Per-session debug log (~/.gbuild/debug/<sessionId>.txt)
gbuild --debug

# Log to a custom path
GBUILD_LOG_FILE=/tmp/gbuild-debug.log gbuild

# Tail the most-recently-opened session's log in another terminal (Unix symlink)
tail -f ~/.gbuild/debug/latest.txt
```

The `--debug` firehose uses a fixed filter (first-party crates at `debug`) and is not narrowed by `RUST_LOG`. A `GBUILD_LOG_FILE` log defaults to `debug` and honors `RUST_LOG`, so you can set module-level filters for targeted debugging:

```bash
# Debug auth, info for everything else
GBUILD_LOG_FILE=/tmp/gbuild-debug.log RUST_LOG="info,gbuild_shell::auth=debug" gbuild
```
...
cat ~/.gbuild/sessions/<encoded-cwd>/<session-id>/summary.json | jq .

# View conversation history
cat ~/.gbuild/sessions/<encoded-cwd>/<session-id>/updates.jsonl | head -20

# Count turns in a session
wc -l ~/.gbuild/sessions/<encoded-cwd>/<session-id>/chat_history.jsonl
```
