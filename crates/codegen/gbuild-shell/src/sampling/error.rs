...
//!
//! The canonical error types now live in `gbuild_sampling_types::error`.
//! This module re-exports them and adds `map_sampling_err_to_acp` which
//! depends on `agent_client_protocol::Error` (a gbuild-shell dependency).

// Re-export everything from the standalone crate.
pub use gbuild_sampling_types::error::*;
...
pub const FREE_USAGE_USER_MESSAGE: &str = "You\u{2019}ve reached your free gBuild usage limit for now. Get SuperGrok for much higher limits, or try again later: https://grok.com/supergrok?referrer=grok-build";
...
/// This stays in gbuild-shell because it depends on `agent_client_protocol::Error`.
    ...
    match err {
                    ...
                    && crate::agent::auth_method::has_xai_api_key_env()
                {
                    format!(
                        "{message}\n\nYou have an API key set (XAI_API_KEY). \
                         ...
                         To use your API key, run `gbuild logout` or type /logout in the TUI."
                    )
                } else {
                    message
                };
        ...
        SamplingError::MaxTokensTruncation => {
            acp::Error::internal_error().data(terminal_error_data(
                err.to_string(),
                None,
                gbuild_sampler::SamplingErrorKind::MaxTokensTruncation,
            ))
        }
...
pub fn terminal_error_data(
    ...
    kind: gbuild_sampler::SamplingErrorKind,
) -> serde_json::Value {
    if kind != gbuild_sampler::SamplingErrorKind::MaxTokensTruncation {
...
pub fn stop_reason_for_turn_error(err: &acp::Error) -> &'static str {
    let is_max_tokens = err
        ...
        .and_then(|v| v.as_str())
        .is_some_and(|k| k == gbuild_sampler::SamplingErrorKind::MaxTokensTruncation.as_str());
...
mod tests {
    ...
    fn attach_prompt_usage_preserves_error_kind_and_round_trips() {
        ...
        ledger.record_main_loop_call(
            "m",
            &gbuild_sampling_types::TokenUsage {
                prompt_tokens: 3,
                completion_tokens: 1,
                total_tokens: 4,
                reasoning_tokens: 0,
    ...
    fn forbidden_subscription_error_includes_api_key_hint_when_env_set() {
        with_api_key_env(Some("xai-test"), || {
            ...
            assert!(
                msg.contains("gbuild logout"),
                "should suggest gbuild logout when API key is available: {msg}"
            );
            assert!(
                msg.contains("/logout"),
