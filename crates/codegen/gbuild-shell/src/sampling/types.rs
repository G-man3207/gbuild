// Re-export all types from the standalone gbuild-sampling-types crate.
// This keeps all existing `crate::sampling::types::*` imports working.
pub use gbuild_sampling_types::types::*;

// `CreateResponseWrapper` and `MessagesRequestWrapper` previously lived
// here. They were moved into `gbuild-sampling-types::types` (and
// are re-exported above via the wildcard) so the new
// `gbuild-sampler` crate can reference them without a circular
// dep on `gbuild-shell`.

// Tests for the types now live in gbuild-sampling-types crate.

use gbuild_tools::types::output::ImageContent as ToolsImageContent;
...
///
/// Lives in the shell (rather than `gbuild-sampling-types` or
/// `gbuild-tools`) so neither of those crates needs to depend on
