...
pub use self::types::*;
pub use gbuild_sampler::ApiBackend;
pub use gbuild_sampler::SamplingClient as Client;

// Re-export async-openai Responses API types under `rs` namespace
pub use async_openai::types::responses as rs;

// ---------------------------------------------------------------------------
// gbuild-sampler re-exports
// ---------------------------------------------------------------------------
//
// The actual streaming / retry / HTTP-client logic lives in the
// `gbuild-sampler` crate. We re-export the public surface here so
...
// `gbuild_sampler::*`. The shell-side `sampling::client::Config`
