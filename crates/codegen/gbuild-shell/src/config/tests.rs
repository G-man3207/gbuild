...
fn expands_env_vars_in_toml_strings() {
    with_env_var(
        "GBUILD_TEST_CONFIG_EXPAND",
        "expanded",
        || {
            let toml_str = r#"
[mcp_servers.test]
command = "$GBUILD_TEST_CONFIG_EXPAND/bin/server"
args = ["--path", "${GBUILD_TEST_CONFIG_EXPAND}/data"]
"#;
...
fn leaves_missing_env_vars_unchanged() {
    let toml_str = r#"
[mcp_servers.test]
command = "$GBUILD_TEST_CONFIG_MISSING/bin/server"
"#;
    let mut value = toml::from_str::<toml::Value>(toml_str).unwrap();
    expand_env_vars_in_toml(&mut value);
    let toml::Value::Table(table) = value else {
    ...
    let command = test.get("command").and_then(|v| v.as_str()).unwrap();
    assert_eq!(command, "$GBUILD_TEST_CONFIG_MISSING/bin/server");
}
#[test]
...
/// Mutex to serialize tests that touch the GBUILD_MEMORY env var.
...
/// Run `f` with GBUILD_MEMORY explicitly unset.
fn without_gbuild_memory<T>(f: impl FnOnce() -> T) -> T {
    let _guard = MEMORY_ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
    with_env_var_opt("GBUILD_MEMORY", None, f)
}
/// Run `f` with GBUILD_MEMORY set to a specific value.
fn with_gbuild_memory<T>(value: &str, f: impl FnOnce() -> T) -> T {
    let _guard = MEMORY_ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
    with_env_var_opt("GBUILD_MEMORY", Some(value), f)
}
#[test]
fn memory_config_default_disabled() {
    without_gbuild_memory(|| {
...
fn memory_config_env_var_zero_does_not_enable() {
    with_gbuild_memory(
        ...
        || {
            let config = toml::Value::Table(toml::map::Map::new());
            let mem = MemoryConfig::resolve(false, false, &config, None);
            assert!(!mem.enabled, "GBUILD_MEMORY=0 should not enable memory");
        },
    );
}
#[test]
fn memory_config_env_var_false_does_not_enable() {
    with_gbuild_memory(
        ...
        || {
            let config = toml::Value::Table(toml::map::Map::new());
            let mem = MemoryConfig::resolve(false, false, &config, None);
            assert!(!mem.enabled, "GBUILD_MEMORY=false should not enable memory");
        },
    );
}
#[test]
...
fn memory_config_env_zero_force_disables_toml_enabled() {
    with_gbuild_memory(
        ...
        || {
            ...
            assert!(
                !mem.enabled,
                "GBUILD_MEMORY=0 should force-disable even when TOML enables memory"
            );
        },
    );
}
...
fn memory_config_env_false_force_disables_toml_enabled() {
    with_gbuild_memory(
        ...
        || {
            ...
            assert!(
                !mem.enabled,
                "GBUILD_MEMORY=false should force-disable even when TOML enables memory"
            );
        },
    );
}
...
fn memory_config_cli_flag_overrides_env_disable() {
    with_gbuild_memory(
        ...
        || {
            ...
            assert!(
                mem.enabled,
                "CLI --experimental-memory should override GBUILD_MEMORY=0"
            );
        },
    );
}
...
fn memory_config_no_memory_overrides_all() {
    with_gbuild_memory(
        ...
        || {
            ...
            assert!(
                !mem.enabled,
                "--no-memory should override --experimental-memory, GBUILD_MEMORY=1, and TOML enabled=true"
            );
        },
    );
}
...
fn memory_config_no_memory_overrides_env_enable() {
    with_gbuild_memory(
        ...
        || {
            let config = toml::Value::Table(toml::map::Map::new());
            let mem = MemoryConfig::resolve(false, true, &config, None);
            assert!(!mem.enabled, "--no-memory should override GBUILD_MEMORY=1");
        },
    );
}
#[test]
        ...
        || {
            with_env_var(
                ...
                || {
                    let toml_str = r#"
[mcp_servers.test]
command = "$GBUILD_TEST_USER $GBUILD_TEST_ROOT"
"#;
                    let mut value = toml::from_str::<toml::Value>(toml_str).unwrap();
                    expand_env_vars_in_toml(&mut value);
                    let toml::Value::Table(table) = value else {
...
/// Mutex to serialize tests that touch the GBUILD_SUBAGENTS env var.
static SUBAGENTS_ENV_LOCK: std::sync::Mutex<()> = std::sync::Mutex::new(());
/// Run `f` with GBUILD_SUBAGENTS explicitly unset.
fn without_gbuild_subagents<T>(f: impl FnOnce() -> T) -> T {
    let _guard = SUBAGENTS_ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
    with_env_var_opt("GBUILD_SUBAGENTS", None, f)
}
/// Run `f` with GBUILD_SUBAGENTS set to a specific value.
fn with_gbuild_subagents<T>(value: &str, f: impl FnOnce() -> T) -> T {
    let _guard = SUBAGENTS_ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
    with_env_var_opt("GBUILD_SUBAGENTS", Some(value), f)
}
#[test]
fn subagents_config_default_enabled() {
    without_gbuild_subagents(|| {
        let config = toml::Value::Table(toml::map::Map::new());
    ...
    with_gbuild_subagents(
        ...
        || {
            let config: toml::Value = toml::from_str("[subagents]\nenabled = true")
                .unwrap();
            let sa = SubagentsConfig::resolve(false, &config);
            assert!(!sa.enabled, "GBUILD_SUBAGENTS=0 should override config file");
        },
    );
}
#[test]
...
fn subagents_config_env_var_disables_default() {
    with_gbuild_subagents(
        ...
        || {
            ...
            assert!(
                !sa.enabled,
                "GBUILD_SUBAGENTS=0 should override the enabled default"
            );
        },
    );
}
...
fn subagents_config_cli_flag_overrides_env_var() {
    with_gbuild_subagents(
        ...
        || {
            ...
            assert!(
                sa.enabled,
                "--subagents CLI flag should override GBUILD_SUBAGENTS=0"
            );
        },
    );
}
    ...
    with_gbuild_subagents(
        ...
        || {
            let config: toml::Value = toml::from_str(
                ...
                .unwrap();
            let sa = SubagentsConfig::resolve(false, &config);
            assert!(sa.enabled, "GBUILD_SUBAGENTS=1 should enable");
            assert_eq!(sa.models.get("explore").unwrap(), "grok-3-fast");
        },
    );
}
...
fn with_managed_mcp_env<T>(
    ...
    with_env_var_opt(
        ...
        managed_mcps,
        || with_env_var_opt("GBUILD_MANAGED_MCP_GATEWAY_TOOLS_ENABLED", gateway_tools, f),
    )
}
#[test]
#[serial_test::serial]
...
fn with_model_overrides_env_full<T>(
    ...
    let _guard = LOCK.lock().unwrap_or_else(|e| e.into_inner());
    with_env_var_opt(
        "GBUILD_WEB_SEARCH_MODEL",
        ws,
        || with_env_var_opt(
            "GBUILD_SESSION_SUMMARY_MODEL",
            ss,
            || with_env_var_opt(
                "GBUILD_IMAGE_DESCRIPTION_MODEL",
                id,
                || with_env_var_opt("GBUILD_PROMPT_SUGGESTIONS_MODEL", ps, f),
            ),
        ),
    )
}
...
#[test]
fn model_overrides_default_image_description_is_gbuild() {
    with_model_overrides_env(
        None,
        None,
        None,
...
#[test]
fn model_overrides_default_session_summary_is_gbuild() {
    with_model_overrides_env(
        None,
        None,
        None,
...
fn with_tools_env<T>(
    ...
    with_env_var_opt(
        "GBUILD_RESPECT_GITIGNORE",
        respect_gitignore,
        || with_env_var_opt("GBUILD_DISABLE_ZDR_INCOMPATIBLE_TOOLS", disable_zdr, f),
    )
}
fn without_gbuild_respect_gitignore<T>(f: impl FnOnce() -> T) -> T {
    with_tools_env(None, None, f)
}
fn with_gbuild_respect_gitignore<T>(value: &str, f: impl FnOnce() -> T) -> T {
    with_tools_env(Some(value), None, f)
}
#[test]
fn tools_config_default_disabled() {
    without_gbuild_respect_gitignore(|| {
        let config = toml::Value::Table(toml::map::Map::new());
    ...
    with_gbuild_respect_gitignore(
        ...
        || {
            ...
            assert!(
                ...
                "GBUILD_RESPECT_GITIGNORE=false should override config file"
            );
        },
    );
}
...
fn roles_parse_from_toml() {
    let toml_str = r#"
            ...
            prompt_file = ".gbuild/prompts/impl.md"
        "#;
...
fn validate_roles_accepts_valid_prompt_file() {
    let toml_str = r#"
            ...
            prompt_file = ".gbuild/prompts/ok.md"
        "#;
    let cfg: SubagentsConfig = toml::from_str(toml_str).unwrap();
    assert!(cfg.validate_roles().is_empty());
}
...
fn discover_roles_loads_from_directory() {
    ...
    let roles_dir = tmp.path().join(".gbuild").join("roles");
...
fn personas_parse_from_toml() {
    ...
    assert_eq!(
            ...
            Some(".gbuild/personas/concise.md")
        );
}
#[test]
...
fn discover_personas_loads_from_directory() {
    let tmp = tempfile::TempDir::new().unwrap();
    let dir = tmp.path().join(".gbuild").join("personas");
...
fn bundled_personas_and_roles_have_lowest_priority_in_resolve_order() {
    ...
    let bundled = home.join(".gbuild").join("bundled");
    std::fs::create_dir_all(workspace.join(".gbuild").join("roles")).unwrap();
    std::fs::create_dir_all(workspace.join(".gbuild").join("personas")).unwrap();
    std::fs::create_dir_all(home.join(".gbuild").join("roles")).unwrap();
    std::fs::create_dir_all(home.join(".gbuild").join("personas")).unwrap();
    ...
    std::fs::write(
        ...
        .unwrap();
    std::fs::write(
            home.join(".gbuild/roles/reviewer.toml"),
            r#"description = "User reviewer""#,
        )
        .unwrap();
    std::fs::write(
            home.join(".gbuild/personas/reviewer.toml"),
            r#"instructions = "User persona""#,
        )
        .unwrap();
    std::fs::write(
            workspace.join(".gbuild/roles/reviewer.toml"),
            r#"description = "Project reviewer""#,
        )
        .unwrap();
    std::fs::write(
            workspace.join(".gbuild/personas/reviewer.toml"),
    ...
    std::fs::remove_file(workspace.join(".gbuild/roles/reviewer.toml")).unwrap();
    std::fs::remove_file(workspace.join(".gbuild/personas/reviewer.toml")).unwrap();
    let config = toml::from_str::<
        toml::Value,
    >(r#"
            [subagents]
    ...
    std::fs::remove_file(home.join(".gbuild/roles/reviewer.toml")).unwrap();
    std::fs::remove_file(home.join(".gbuild/personas/reviewer.toml")).unwrap();
    let config = toml::from_str::<
        toml::Value,
    >(r#"
            [subagents]
...
fn render_io_summary_shows_bundled_for_bundled_personas() {
    let persona = SubagentPersona {
        ...
        source_path: Some("/tmp/home/.gbuild/bundled/personas/reviewer.toml".to_string()),
        ..Default::default()
    };
...
/// a repo `.gbuild/config.toml` structurally cannot supply one.)
#[test]
...
fn enterprise_two_file_merge_routes_deployment_key_to_proxy() {
    for k in [
        ...
        "GBUILD_TRACE_UPLOAD_ENDPOINT_URL",
    ] {
...
/// RCE guard: a project `.gbuild/config.toml` must never source
...
#[test]
fn validate_hooks_path_rejects_outside_gbuild_home() {
    let result = validate_hooks_path("/tmp/evil-hooks");
    assert!(result.is_err());
    let msg = result.unwrap_err().to_string();
    assert!(
            msg.contains("must be under ~/.gbuild/"),
            "should mention ~/.gbuild/ restriction, got: {msg}"
        );
}
#[test]
fn validate_hooks_path_rejects_traversal_attack() {
    let gbuild_home = crate::util::gbuild_home::gbuild_home();
    let traversal = format!("{}/../evil", gbuild_home.display());
...
#[test]
fn validate_hooks_path_accepts_gbuild_hooks_subdir() {
    let gbuild_home = crate::util::gbuild_home::gbuild_home();
    let valid_path = gbuild_home.join("hooks").join("my-hooks");
    let _ = std::fs::create_dir_all(&valid_path);
    let result = validate_hooks_path(valid_path.to_str().unwrap());
    assert!(result.is_ok(), "path under ~/.gbuild/ should be accepted");
}
#[test]
fn managed_settings_disables_features_and_requirements_overrides() {
    use gbuild_workspace::permission::resolution::ManagedSettingsFeatures;
    let mut cfg = crate::agent::config::Config::default();
...
/// disableBypassPermissionsMode (-> features.disable_yolo) must NOT clamp the user's own gbuild yolo.
#[test]
...
fn base_resolver_without_project_cwd_keeps_project_files_out() {
    let tmp = tempfile::tempdir().unwrap();
    write_subagent_definitions(&tmp.path().join(".gbuild"), &[("project", "Project")]);
    let base = SubagentsConfig::resolve_base_with_sources(
        false,
...
#[test]
fn explicit_gbuild_root_is_the_only_user_source() {
    let tmp = tempfile::tempdir().unwrap();
    let ambient = tmp.path().join("ambient-home/.gbuild");
    let configured = tmp.path().join("configured-gbuild-home");
