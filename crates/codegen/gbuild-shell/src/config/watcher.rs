...
pub enum ConfigChangeEvent {
    AuthChanged,
    GlobalConfigChanged,
    /// `~/.gbuild/models_cache.json` changed — the on-disk `/v1/models`
    /// catalog cache was rewritten, possibly by **another** gbuild process
    /// sharing the same `~/.gbuild` (the writer may also be this process;
...
/// Watches `~/.gbuild/` for `auth.json`, `config.toml`, and `models_cache.json`
/// changes, plus any extra paths (project `.gbuild/config.toml`, `.mcp.json`,
...
impl ConfigFileWatcher {
    ...
    /// `<cwd>/.gbuild/`. Use [`Self::watch_path`] later to register additional
    ...
    pub fn start(
        ...
        let gbuild_home_buf = gbuild_home.to_path_buf();
        ...
        let mut debouncer = new_filtered_debouncer(debounce, move |res: DebounceEventResult| {
            ...
            for event in events {
                ...
                let change = match name {
                    Some("auth.json") if parent == Some(gbuild_home_buf.as_path()) => {
                        Some(ConfigChangeEvent::AuthChanged)
                    }
                    Some("config.toml") if parent == Some(gbuild_home_buf.as_path()) => {
                        Some(ConfigChangeEvent::GlobalConfigChanged)
                    }
                    Some("models_cache.json") if parent == Some(gbuild_home_buf.as_path()) => {
        ...
        debouncer
            .watcher()
            .watch(gbuild_home, RecursiveMode::NonRecursive)
            .map_err(|e| {
                tracing::warn!(
                    path = %gbuild_home.display(),
                    error = %e,
                    "failed to watch gbuild home directory"
                )
            })
            .ok()?;
        ...
        // includes `<cwd>/.gbuild/config.toml` so the loop above
        // watches `<cwd>/.gbuild/`), the call below installs a
        ...
        tracing::info!(
            gbuild_home = %gbuild_home.display(),
    ...
    /// Register `<cwd>/` and `<cwd>/.gbuild/` as **non-recursive** watch
    ...
    /// `<cwd>/.mcp.json` and `<cwd>/.gbuild/config.toml` trigger a
    ...
    /// `<cwd>/.gbuild/`) previously registered for `cwd` via
...
///
/// **Known limitation:** if `<cwd>/.gbuild/` does not yet
/// exist at session-open time, the `.gbuild/` watch fails ENOENT and is
/// swallowed at `debug!`. A later `mkdir <cwd>/.gbuild/` followed by a
/// write to `<cwd>/.gbuild/config.toml` will NOT be observed — the
...
fn watch_cwd_dirs(debouncer: &mut Debouncer<AccessFilteredWatcher>, cwd: &Path) {
    ...
    let gbuild_dir = cwd.join(".gbuild");
    if let Err(e) = debouncer
        .watcher()
        .watch(&gbuild_dir, RecursiveMode::NonRecursive)
    {
        log_watch_error(
            &e,
            "failed to watch project .gbuild directory (non-recursive)",
        );
    }
}
...
fn unwatch_cwd_dirs(debouncer: &mut Debouncer<AccessFilteredWatcher>, cwd: &Path) {
    ...
    let gbuild_dir = cwd.join(".gbuild");
    if let Err(e) = debouncer.watcher().unwatch(&gbuild_dir) {
        tracing::debug!(error = %e, "failed to unwatch project .gbuild directory");
    }
}

/// Log a `notify` watch failure, distinguishing the benign
/// "directory doesn't exist yet" case (logged at `debug!` — it's
/// expected for a freshly-opened session whose `<cwd>/.gbuild/` hasn't
...
fn discovery_change_for_path(path: &Path) -> Option<DiscoveryChange> {
    if path.file_name().is_some_and(|name| name == ".gbuild") {
...
/// recursively: `gbuild_home` (`~/.gbuild`, or `$GBUILD_HOME`) or a known vendor dir
/// directly under `$HOME` ([`HOME_VENDOR_DIRS`]).
///
/// These hold large non-skill trees — `~/.gbuild` alone has `worktrees/`,
...
fn is_global_config_dir(dir: &Path, gbuild_home: &Path) -> bool {
    #[allow(deprecated)]
    let home = std::env::home_dir();
    is_global_config_dir_impl(dir, gbuild_home, home.as_deref())
}
...
const HOME_VENDOR_DIRS: &[&str] = &[".gbuild", ".agents", ".claude", ".cursor"];

/// Testable core of [`is_global_config_dir`] with `$HOME` injected.
fn is_global_config_dir_impl(dir: &Path, gbuild_home: &Path, home: Option<&Path>) -> bool {
    let canon = |p: &Path| dunce::canonicalize(p).unwrap_or_else(|_| p.to_path_buf());
    if canon(dir) == canon(gbuild_home) {
        return true;
    }
...
impl ProjectDiscoveryWatcher {
    pub fn start(cwd: &Path) -> Option<(Self, mpsc::UnboundedReceiver<DiscoveryChange>)> {
        ...
        let project_gbuild = project_root.join(".gbuild");
        let workflows = project_gbuild.join("workflows");
        let (tx, rx) = mpsc::unbounded_channel();
        let project_gbuild_for_events = project_gbuild.clone();
        let mut debouncer =
            new_filtered_debouncer(SKILLS_DEBOUNCE, move |res: DebounceEventResult| {
                ...
                for event in events
                    .iter()
                    .filter(|event| event.path.starts_with(&project_gbuild_for_events))
                {
        ...
        let initial = if project_gbuild.is_dir() {
            project_gbuild.clone()
        } else {
            project_root.clone()
        };
        ...
        let refresh_dirs = vec![
            (project_gbuild, RecursiveMode::NonRecursive),
            (
                project_root.join(".gbuild").join("skills"),
                RecursiveMode::Recursive,
            ),
            (
                project_root.join(".gbuild").join("commands"),
...
impl SkillsFileWatcher {
    ///
    /// Uses [`collect_skill_config_dirs`](gbuild_agent::prompt::skills::collect_skill_config_dirs)
    ...
    pub fn start(
        ...
        let gbuild_home = gbuild_tools::util::gbuild_home::gbuild_home();
        ...
        let dirs_to_watch = gbuild_agent::prompt::skills::collect_skill_config_dirs(
            cwd,
            monorepo_user_dir,
            &gbuild_home,
            config_paths,
        ...
        for dir in &dirs_to_watch {
            if is_global_config_dir(dir, &gbuild_home) {
        ...
        let mut refresh_dirs = vec![(gbuild_home.join("workflows"), RecursiveMode::NonRecursive)];
        if let Some(cwd) = cwd {
            let project_root = crate::session::workflow::registry::project_root(cwd);
            let project_gbuild = project_root.join(".gbuild");
            let parent_watch = if project_gbuild.is_dir() {
                project_gbuild.clone()
            } else {
                project_root
            };
            ...
            refresh_dirs.push((project_gbuild.clone(), RecursiveMode::NonRecursive));
            refresh_dirs.push((
                project_gbuild.join("workflows"),
                RecursiveMode::NonRecursive,
            ));
        }
...
mod tests {
    ...
    /// `is_global_config_dir` must scope down only gbuild_home and the known
    ...
    /// recursively watched.
    #[test]
    fn is_global_config_dir_matches_only_gbuild_home_and_vendor_dirs() {
        let home = TempDir::new().unwrap();
        let home = home.path();
        let gbuild_home = home.join(".gbuild");

        let g = |dir: &Path| is_global_config_dir_impl(dir, &gbuild_home, Some(home));

        // gbuild_home and vendor dirs directly under $HOME: scoped (global).
        assert!(g(&gbuild_home));
        ...
        assert!(!g(&home.join("repo").join(".gbuild")));
    }

    /// Regression for the ~/.gbuild inotify-exhaustion / worktree-noise bug: a
    /// `SKILL.md` under a sibling subtree (e.g. `~/.gbuild/worktrees/`) must not
    ...
    fn workflow_change_classifies_missing_directory_creation() {
        let gbuild = Path::new("/tmp/project/.gbuild");
        assert_eq!(
            discovery_change_for_path(gbuild),
            Some(DiscoveryChange::Skills),
            "first .gbuild creation must take the broader skills reload path"
        );
        assert_eq!(
            discovery_change_for_path(&gbuild.join("workflows")),
            Some(DiscoveryChange::Workflows)
        );
        assert_eq!(
            discovery_change_for_path(&gbuild.join("workflows/review.rhai")),
            Some(DiscoveryChange::Workflows)
        );
        assert_eq!(
            discovery_change_for_path(&gbuild.join("skills/review/SKILL.md")),
            Some(DiscoveryChange::Skills)
        );
    }
    ...
    /// A write to `<gbuild_home>/models_cache.json` must surface as
    /// `ConfigChangeEvent::ModelsCacheChanged` so a long-running leader can
    /// hot-load a catalog fetched by another gbuild process.
    #[test]
    #[cfg_attr(
        target_os = "macos",
        ignore = "flaky on macOS: FSEvents does not reliably deliver events in test harness"
    ...
    /// A write to `<cwd>/.gbuild/config.toml` must surface as
    ...
    fn project_cwd_toml_triggers_reload() {
        let gbuild_home = TempDir::new().unwrap();
        let cwd = TempDir::new().unwrap();
        let project_gbuild = cwd.path().join(".gbuild");
        fs::create_dir_all(&project_gbuild).unwrap();
        // Seed the file before the watcher starts so we observe the
        // modification rather than the creation event.
        fs::write(project_gbuild.join("config.toml"), "").unwrap();
        ...
        fs::write(
            project_gbuild.join("config.toml"),
            "[mcp_servers.x]\ncommand = \"/bin/true\"",
        )
        .unwrap();
        ...
        assert!(
            found,
            "expected ProjectConfigChanged for <cwd>/.gbuild/config.toml within 2s"
        );
    }
    ...
    /// `start` must light up `<new_cwd>/.gbuild/config.toml` writes
    ...
    fn watch_path_dynamic_registration() {
        ...
        let project_gbuild = new_cwd.path().join(".gbuild");
        fs::create_dir_all(&project_gbuild).unwrap();
        fs::write(project_gbuild.join("config.toml"), "").unwrap();
