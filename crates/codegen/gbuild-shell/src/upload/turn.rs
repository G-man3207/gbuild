//! Turn-scoped helpers shared by the agent and subagent paths.
...
pub(crate) fn parse_agent_profile_from_meta(
...
) -> Option<gbuild_agent::AgentDefinition> {
    let value = meta?.get("agentProfile")?;
    if value.is_object() {
        return match gbuild_agent::AgentDefinition::from_json(value) {
            Ok(def) => {
                tracing::info!(
                    agent_name = %def.name,
    ...
    if let Some(name) = value.as_str() {
        ...
        return gbuild_agent::discovery::by_name(name);
    }
    tracing::warn!(
...
mod tests {
    ...
    fn parse_ask_user_question_returns_false_when_disabled() {
        let meta: agent_client_protocol::Meta = serde_json::json!({"askUserQuestion": false})
            .as_object()
            .unwrap()
            .clone();
        assert_eq!(parse_ask_user_question_from_meta(Some(&meta)), Some(false));
    }

    #[test]
    fn parse_ask_user_question_returns_true_when_enabled() {
        let meta: agent_client_protocol::Meta = serde_json::json!({"askUserQuestion": true})
            .as_object()
            .unwrap()
            .clone();
        assert_eq!(parse_ask_user_question_from_meta(Some(&meta)), Some(true));
    }

    #[test]
    fn parse_ask_user_question_returns_none_when_absent() {
        let meta: agent_client_protocol::Meta = serde_json::json!({})
            .as_object()
            .unwrap()
            .clone();
        assert_eq!(parse_ask_user_question_from_meta(Some(&meta)), None);
    }
    ...
    fn parse_ask_user_question_ignores_non_bool() {
        let meta: agent_client_protocol::Meta = serde_json::json!({"askUserQuestion": "yes"})
            .as_object()
            .unwrap()
            .clone();
        assert_eq!(parse_ask_user_question_from_meta(Some(&meta)), None);
