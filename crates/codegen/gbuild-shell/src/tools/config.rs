use crate::models;
use gbuild_sampler::SamplerConfig;
use gbuild_tools::implementations::gbuild;
use gbuild_tools::registry::types::ToolConfig;
use serde::{Deserialize, Serialize};

/// Production gBuild foreground command-timeout ceiling (seconds). The
...
impl BashToolConfig {
    ...
    pub fn to_bash_params_json(
        ...
        // production gBuild opts up to 10h by sending it explicitly
...
pub struct WebFetchToolConfig {
    ...
    /// this URL. Resolution: TOML > `GBUILD_WEB_FETCH_PROXY` env > remote settings > None.
    ...
    /// Resolution: TOML > `GBUILD_WEB_FETCH_ALLOW_LOCAL` env > false.
...
impl WebFetchToolConfig {
    ...
    pub fn resolve_params(
    ...
    ) -> gbuild_tools::implementations::gbuild::web_fetch::WebFetchParams {
        ...
        let proxy_endpoint = self
            ...
            .as_ref()
            .cloned()
            .or_else(|| env_string("GBUILD_WEB_FETCH_PROXY"))
        ...
        let allow_local = self
            .allow_local
            .or_else(|| gbuild_config::env_bool("GBUILD_WEB_FETCH_ALLOW_LOCAL"));
...
/// from `gbuild_tools::registry::types::ToolsetConfig` which holds
...
///
/// Selects between the standard `GBuild` toolset (`read_file`,
/// `search_replace`, `grep`) and the anchor-based `GBuildHashline`
        ...
        match self {
            ...
            Self::Hashline => {
                ...
                Ok(vec![
                    ...
                    ToolConfig {
                        id: "GBuildHashline:hashline_edit".to_owned(),
                        params: params_map.clone(),
                        name_override: None,
                        params_name_overrides: None,
                        description_override: None,
                    ...
                    ToolConfig {
                        id: "GBuildHashline:hashline_grep".to_owned(),
                        params: params_map,
                        name_override: None,
                        params_name_overrides: None,
                        description_override: None,
...
mod tests {
    ...
    fn standard_toolset_configs() {
        ...
        assert!(ids.contains(&"GBuild:read_file"));
        assert!(ids.contains(&"GBuild:search_replace"));
        assert!(ids.contains(&"GBuild:grep"));
    }
    ...
    fn hashline_toolset_configs() {
        ...
        assert_eq!(configs.len(), 3);
        let ids: Vec<&str> = configs.iter().map(|c| c.id.as_str()).collect();
        assert!(ids.contains(&"GBuildHashline:hashline_read"));
        assert!(ids.contains(&"GBuildHashline:hashline_edit"));
        assert!(ids.contains(&"GBuildHashline:hashline_grep"));
    }
    ...
    fn file_toolset_override_never_grants_edit_to_read_only_toolsets() {
        ...
        for mut def in [
            gbuild_agent::config::AgentDefinition::plan(),
            gbuild_agent::config::AgentDefinition::explore(),
        ] {
            ...
            assert!(
                !def.tool_config
                    .tools
                    .iter()
                    .any(|t| t.id == "GBuild:search_replace"),
            ...
            assert!(
                ids.contains(&"GBuildHashline:hashline_read"),
                "{name}: {ids:?}"
            );
            assert!(!ids.contains(&"GBuild:read_file"), "{name}: {ids:?}");
            // ...but never grants the edit slot.
            assert!(
                !ids.contains(&"GBuildHashline:hashline_edit"),
    ...
    fn max_timeout_secs_defaults_to_production_10h() {
        ...
        assert_eq!(
            ...
            "production gBuild must set the 10h foreground ceiling"
        );
    }
