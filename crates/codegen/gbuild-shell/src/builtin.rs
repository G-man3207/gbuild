//! Built-in files extracted to `~/.gbuild/` on startup.

const BUILTIN_FILES: &[(&str, &str)] = &[("README.md", include_str!("../README.md"))];

/// Extract built-in metadata files to `~/.gbuild/` on startup.
///
/// User skills under `~/.gbuild/skills/` are never managed here. Platform skills
/// are delivered separately through the bundled skill cache.
pub fn extract_builtin_files(gbuild_home: &std::path::Path) {
    let version = gbuild_version::VERSION;
    let marker = gbuild_home.join(".metadata_version");
...
mod tests {
    ...
    fn same_version_does_not_restore_missing_or_delete_legacy_skills() {
        ...
        std::fs::write(home.join(".metadata_version"), gbuild_version::VERSION).unwrap();
