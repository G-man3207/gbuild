// claude_import.rs
// Scans Claude settings and generates TOML patches for .gbuild/config.toml.
//
...
use gbuild_workspace::permission::claude_settings::{
    find_claude_settings_paths, load_claude_settings,
};
use gbuild_workspace::permission::rules::parse_permission_rule;
use gbuild_workspace::permission::types::{PatternMode, PermissionRule, RuleAction, ToolFilter};
...
pub enum ImportScope {
    /// User-level: writes to `~/.gbuild/config.toml`.
    Global,
    /// Project-level: writes to `<repo>/.gbuild/config.toml`.
    Project,
}
...
pub struct ImportPlan {
    /// Items to write to `~/.gbuild/config.toml`.
    pub global_items: Vec<ImportableItem>,
    /// Items to write to `<repo>/.gbuild/config.toml`.
...
//
// The marker `[claude_compat] imported = true` in `~/.gbuild/config.toml` is
...
///
/// Reads `[claude_compat] imported = true` from `~/.gbuild/config.toml` once
...
/// Write `[claude_compat] imported = true` to `~/.gbuild/config.toml`.
///
...
pub fn mark_claude_imported() -> anyhow::Result<()> {
    let path = crate::util::gbuild_home::gbuild_home().join("config.toml");
...
/// overwritten).
///
/// Project items are written to `<repo_root>/.gbuild/config.toml` (discovered
/// via `git2::Repository::discover`), not `cwd/.gbuild/config.toml`, to avoid
...
pub fn apply_import(plan: &ImportPlan, cwd: &Path) -> anyhow::Result<ImportResult> {
    ...
    if !plan.global_items.is_empty() {
        let global_path = crate::util::gbuild_home::gbuild_home().join("config.toml");
        ...
        // Hooks are written separately to ~/.gbuild/hooks/imported-from-claude.json.
        let hooks_dir = crate::util::gbuild_home::gbuild_home().join("hooks");
    ...
    if !plan.project_items.is_empty() {
        let project_root = find_project_root(cwd);
        let project_path = project_root.join(".gbuild").join("config.toml");
        ...
        let hooks_dir = project_root.join(".gbuild").join("hooks");
...
fn apply_items_to_config(config_path: &Path, items: &[ImportableItem]) -> anyhow::Result<usize> {
    ...
    for item in items {
        match item {
            ...
            // Hooks are written to .gbuild/hooks/ JSON files in apply_hooks_to_dir,
...
///
/// The output JSON is the same shape that `gbuild-hooks` natively understands
/// (Claude-compatible). The native hooks loader scans `.gbuild/hooks/*.json`
...
fn apply_hooks_to_dir(hooks_dir: &Path, items: &[ImportableItem]) -> anyhow::Result<usize> {
    ...
    if count > 0 || dirty {
        ...
        info!(
            path = %target.display(),
            count,
            "Wrote imported hooks to .gbuild/hooks/imported-from-claude.json"
        );
    }
...
mod tests {
    ...
    impl Drop for MarkerGuard {
        fn drop(&mut self) {
            ...
            unsafe { std::env::remove_var("_GBUILD_CLAUDE_MARKER_OVERRIDE") };
        }
    }
    ...
    fn discover_hooks_honors_claude_compat_gate() {
        ...
        // test host (from the non-injectable ~/.claude, ~/.gbuild) don't interfere.
        let has_probe = |reg: &gbuild_hooks::discovery::HookRegistry| {
...
"#;
    ...
    fn scan_claude_path_dirs_dedupes_global_and_project_when_same() {
        ...
        // requires `GBUILD_HOME` plumbing which is intentionally out of scope.
    ...
    async fn gate_resolve_permissions_with_provenance_skips_claude_when_marker_set() {
        ...
        unsafe { std::env::set_var("_GBUILD_CLAUDE_MARKER_OVERRIDE", "1") };
        let dir = tempfile::tempdir().unwrap();
        ...
        // managed settings, and the developer's real `~/.gbuild/config.toml`.
        // We can't isolate `gbuild_home()` because it's `OnceLock`-cached.
        // Instead, assert on rule *provenance*: no rule should originate from
        // our tempdir's `.claude/settings.json`. The dev's real ~/.gbuild
        ...
        if let Some(r) = resolved {
            ...
            use gbuild_workspace::permission::types::RequirementSource;
    ...
    fn gate_marker_cache_unset_means_uses_disk() {
        ...
        // dev's real `~/.gbuild/config.toml` may legitimately have the marker
        // set during local testing, and we can't override `gbuild_home()`
