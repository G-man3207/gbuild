//! Leader-follower IPC architecture for gbuild-shell.
//!
...
//! │  └────────────────────────┬────────────────────────────────┘│
//! └───────────────────────────┼──────────────────────────────────┘
//!                             │ IPC (Unix socket at ~/.gbuild/leader.sock)
//!         ┌───────────────────┼───────────────────┐
//!         ▼                   ▼                   ▼
//! ┌───────────────┐   ┌───────────────┐   ┌───────────────┐
...
//!
//! ```ignore
//! use gbuild_shell::leader::{connect_or_spawn, ClientCapabilities, ClientMode};
//!
...
mod transport;
use crate::env::GBuildEnvironment;
...
const CLIENT_LEADER_VERSION: &str = gbuild_version::VERSION;
...
/// How long the SAME live gBuild flock-holder may stay unconnectable before
...
/// Maximum reconnection attempts for bounded mode (headless/`gbuild -p`).
...
pub struct LeaderDescriptor {
    ...
    pub environment: Option<GBuildEnvironment>,
...
pub enum LeaderTarget {
    Environment(GBuildEnvironment),
    WsUrl(String),
    Pid(u32),
}
fn known_environment_for_ws_url(ws_url: &str) -> Option<GBuildEnvironment> {
    let environments: &[GBuildEnvironment] = &[GBuildEnvironment::Production];
    environments
        .iter()
        .copied()
...
fn known_environment_for_suffix(ws_url_suffix: &str) -> Option<GBuildEnvironment> {
    let environments: &[GBuildEnvironment] = &[GBuildEnvironment::Production];
    environments
        .iter()
        .copied()
...
pub async fn discover_leaders() -> Vec<LeaderDescriptor> {
    discover_leaders_in(&crate::util::gbuild_home::gbuild_home()).await
}
                ...
                Ok(()) => {
                    ...
                    crate::unified_log::warn(
                        ...
                        Some(serde_json::json!({
                            ...
                            "reason": reason,
                            "killer_ver": gbuild_version::VERSION,
                        })),
                    );
                }
                Err(e) => {
...
pub enum ReconnectPolicy {
    ...
    /// Suitable for headless/`gbuild -p` where hanging forever is unacceptable.
...
///     "gbuild-tui", ClientMode::Stdio, env_urls, caps, status_tx,
/// );
///
...
enum ZombieAction {
    ...
    Clear,
    /// A live gBuild holder is still unconnectable; timer (re)armed, keep waiting.
    Wait,
...
/// The live *gBuild* PID that ACTUALLY holds the flock on the lock file, if any.
/// `None` for a dead / non-gBuild PID, OR when the file PID can't be confirmed to be
...
/// stricter (name-matching) gBuild check since this drives the auto-kill path.
///
...
fn live_gbuild_lock_holder(lock: &LeaderLock) -> Option<u32> {
    let file_pid = lock.read_pid()?;
    let pid = evictable_holder(file_pid, confirmed_flock_holder(lock.lock_path()))?;
    (crate::util::is_process_alive(pid) && crate::util::is_gbuild_process_strict(pid))
        .then_some(pid)
}
...
/// # Arguments
///
/// * `client_type` - Identifier for the client type (e.g., "gbuild-tui", "vscode")
    ...
    loop {
        ...
        match wait_for_socket_connectable(&sock_path, client_type, mode, capabilities.clone()).await
        {
            ...
            Err(e) if is_connect_level_failure(&e) => {
                let holder = live_gbuild_lock_holder(&lock);
...
///
/// For a **managed install** — the running binary lives under `gbuild_home`
/// (e.g. `~/.gbuild/...`) — prefer the managed `~/.gbuild/bin/gbuild` symlink. After an
...
/// `gbuild_update::auto_update::resolve_restart_exe`.
///
/// For a **dev / out-of-tree binary** (`cargo run`, integration tests, installs
/// not under `gbuild_home`), keep `current_exe()` so the spawned leader matches the
/// calling binary.
///
/// Falls back to `~/.gbuild/bin/gbuild` only when `current_exe()` is unavailable.
fn resolve_exe_for_spawn() -> Result<std::path::PathBuf, ConnectionError> {
    resolve_binary_with_home(&crate::util::gbuild_home::gbuild_home())
}
fn resolve_binary_with_home(gbuild_home: &Path) -> Result<std::path::PathBuf, ConnectionError> {
    resolve_binary_impl(gbuild_home, std::env::current_exe().ok())
}
/// Binary file name for the managed gBuild install.
fn managed_gbuild_bin_name() -> &'static str {
    if cfg!(windows) {
        "gbuild.exe"
    } else {
        "gbuild"
...
fn resolve_binary_impl(
    ...
    let managed_bin = gbuild_home.join("bin").join(managed_gbuild_bin_name());
    if let Some(ref exe) = current_exe
        && path_is_under(exe, gbuild_home)
        && managed_bin.exists()
    {
...
fn spawn_leader_subprocess(env_urls: &LeaderEnvUrls) -> Result<u32, ConnectionError> {
    ...
    let log_path = crate::util::gbuild_home::gbuild_home().join("leader.log");
    ...
    let leader_log = std::env::var("GBUILD_LEADER_LOG")
        .or_else(|_| std::env::var("RUST_LOG"))
        .unwrap_or_else(|_| "gbuild_shell=info,xai_acp_lib=warn,gbuild_mcp=warn".into());
...
mod tests {
    ...
    /// No live gBuild holder → `Clear`, and any pending timer is reset.
    #[test]
...
";
    ...
    /// `live_gbuild_lock_holder` returns `None` for a missing or dead PID, so the
    /// zombie net never times/kills a recycled or unrelated PID.
    #[test]
    fn live_gbuild_lock_holder_none_for_missing_or_dead_pid() {
        let temp = TempDir::new().unwrap();
        let lock = LeaderLock::from_paths(
            temp.path().join("leader.lock"),
            temp.path().join("leader.sock"),
        );
        assert_eq!(live_gbuild_lock_holder(&lock), None);
    ...
    fn resolve_binary_prefers_current_exe_for_out_of_tree_install() {
        ...
        let bin_dir = temp.path().join("bin");
        std::fs::create_dir_all(&bin_dir).unwrap();
        std::fs::write(bin_dir.join(managed_gbuild_bin_name()), "managed").unwrap();
    ...
    fn resolve_binary_falls_back_to_managed_when_no_current_exe() {
        ...
        let bin_dir = temp.path().join("bin");
        std::fs::create_dir_all(&bin_dir).unwrap();
        let managed = bin_dir.join(managed_gbuild_bin_name());
