...
use fs2::FileExt;
use gbuild_workspace::util::is_lock_contended;
...
/// `~/.gbuild/leader.sock` — e.g. run a local branch build's leader without
...
/// bypassed entirely.
pub const LEADER_SOCKET_ENV: &str = "GBUILD_LEADER_SOCKET";
...
pub fn lock_path_for_ws_url(ws_url: &str) -> PathBuf {
    resolve_lock_path(leader_socket_override(), &gbuild_home(), ws_url)
}
...
pub fn socket_path_for_ws_url(ws_url: &str) -> PathBuf {
    resolve_socket_path(leader_socket_override(), &gbuild_home(), ws_url)
}
...
impl LeaderLock {
    /// Create a new LeaderLock using the default paths in gBuild home.
...
mod tests {
    ...
    fn override_socket_path_wins_over_ws_url_derivation() {
        let root = Path::new("/home/u/.gbuild");
        let override_sock = PathBuf::from("/home/u/.gbuild/leader-branch.sock");
        ...
        assert_eq!(
            ...
            PathBuf::from("/home/u/.gbuild/leader-branch.lock")
        );
    }
