...
use gbuild_workspace::WorkspaceHandle;
...
struct ClientState {
    ...
    /// The client type string from IPC registration (e.g., "gbuild-tui", "grok-code-extension").
            ...
            "params": {
                ...
                "message": format!(
                    "Client version {client_version} differs from leader version \
                     {leader_version}. Restart the gBuild binary to use the same version."
                )
            }
        })
        .to_string(),
...
mod tests {
    ...
    async fn workspace_start_errors_when_cancelled_before_auth() {
        let state = default_test_control_state(Path::new("/tmp/gbuild-ws-auth-test.sock"));
        let cancel = CancellationToken::new();
        cancel.cancel();
    ...
    /// registration (devbox / `gbuild agent headless` flow) flips the watch so
    ...
    async fn relay_demand_signals_only_on_headless_registration() {
        ...
        tokio::time::sleep(Duration::from_millis(50)).await;
        let _stdio =
            connect_and_register_with_mode(&sock_path, "gbuild-tui", ClientMode::Stdio).await;
        ...
        let _headless =
            connect_and_register_with_mode(&sock_path, "gbuild-headless", ClientMode::Headless)
                .await;
    ...
    async fn initialize_gets_client_identifier_injected() {
        ...
        assert_eq!(
            json["params"]["_meta"]["clientIdentifier"], "gbuild-tui",
    ...
    fn inject_capabilities_does_not_override_existing_client_identifier() {
        ...
        let caps = ClientCapabilities::default();
        let mut json = pv(&payload);
        inject_capabilities_into_session_new(&mut json, &caps, "gbuild-tui", ClientId(1));
    ...
    fn inject_yolo_notification_adds_client_identifier() {
        ...
        assert_eq!(json["params"]["clientIdentifier"], "gbuild-tui");
    ...
    async fn high_throughput_replay_no_drops() {
        ...
        let (sock_path, cancel, response_tx, mut acp_rx) =
            setup_persistent_server_with_agent(&temp).await;
        let (mut reader, mut writer) = connect_and_register(&sock_path, "gbuild-tui").await;
    ...
    fn inject_capabilities_sets_code_nav_enabled_false() {
        ...
        let payload = r#"{"jsonrpc":"2.0","method":"session/new","id":1,"params":{"cwd":"/repo","_meta":{"clientIdentifier":"gbuild-tui"}}}"#;
        let mut json = pv(payload);
        inject_capabilities_into_session_new(&mut json, &caps, "gbuild-tui", ClientId(1));
        assert_eq!(
            json["params"]["_meta"]["codeNavEnabled"],
    ...
    fn inject_capabilities_two_clients_stay_isolated() {
        ...
        inject_capabilities_into_session_new(&mut tui_json, &tui_caps, "gbuild-tui", ClientId(2));
        assert_eq!(
