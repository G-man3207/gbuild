...
pub enum ClientMode {
    /// Headless mode (gbuild agent, gbuild agent headless) - uses websocket relay.
    /// Leader connects to websocket relay once and forwards messages.
    Headless,
    /// Stdio mode (gbuild agent stdio, gbuild -p) - uses local IPC.
...
pub struct LeaderCapabilities {
    ...
    /// (driven by `gbuild update`). Old leaders default to `false`, so a new
...
pub enum ControlCommand {
    ...
    /// `gbuild update`). The leader stops admitting new turns, waits a bounded
    ...
    ///
    /// `to_version` is the version `gbuild update` just installed; the leader uses
...
mod tests {
    ...
    fn workspace_status_payload_roundtrip() {
        let payload = ControlPayload::WorkspaceStatus {
            ...
            uptime_ms: 4200,
            active_tool_calls: 2,
            sessions: vec!["gbuild-a".into(), "gbuild-b".into()],
            pid: 4242,
        };
