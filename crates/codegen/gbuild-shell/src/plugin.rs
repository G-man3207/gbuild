...
use gbuild_agent::plugins::discovery::PluginScope;
use gbuild_agent::plugins::git_install::{self, UpdateStatus};
use gbuild_agent::plugins::install_registry::{
    InstallError, InstallKind, InstallRegistry, InstalledRepo, MarketplaceProvenance,
};
use gbuild_plugin_marketplace::git::{self, SourceCacheLease};
use gbuild_plugin_marketplace::{
...
/// When `keep_data` is true, `~/.gbuild/plugin-data/<id>/` is preserved.
...
fn marketplace_root_for_provenance(
    provenance: &gbuild_agent::plugins::install_registry::MarketplaceProvenance,
                ...
                if registered.is_empty() {
                    write!(
                        ...
                        "Unknown marketplace \"{qualifier}\". No marketplaces are registered; \
                         add one with `gbuild plugin marketplace add`."
                    )
                } else {
            ...
            Self::NameNotFound {
                ...
                write!(
                    ...
                    "No marketplace plugin named \"{name}\" in any registered marketplace.\n\
                     Install a local directory with `gbuild plugin install ./{name}`, or add a \
                     source with `gbuild plugin marketplace add`."
                )?;
            ...
            Self::NameAmbiguous { name, candidates } => {
                ...
                write!(
                    ...
                    "Multiple marketplaces provide a plugin named \"{name}\":\n{list}\n\
                     Pin one with `gbuild plugin install {name}@<qualifier>`."
                )
            }
            Self::PartialScan {
                name,
                ...
                write!(
                    ...
                    "Couldn't scan every marketplace while resolving \"{name}\", so it can't be \
                     ...
                     Retry, or pin the source explicitly with `gbuild plugin install {name}@<qualifier>`."
                )
            }
            Self::Sync {
                source_display,
...
pub fn marketplace_require_sha() -> bool {
    gbuild_config::load_effective_config_disk_only()
        .map(|c| gbuild_plugin_marketplace::load_require_sha(&c))
        .unwrap_or_else(|_| gbuild_plugin_marketplace::env_require_sha())
}
...
fn filter_sources_by_allowlist(
    ...
    allowlist: &gbuild_workspace::permission::resolution::MarketplaceAllowlist,
...
/// `~/.gbuild/` and `~/.claude/`. Returns `true` if removed from at least one file.
pub fn try_remove_source_from_json_files(source_url_or_path: &str) -> bool {
    // Resolve user gbuild via user_gbuild_home() (None when no home resolves) and
    // home separately, so removal still runs from $GBUILD_HOME when no home dir
    // exists, and never touches a cwd-relative .gbuild.
    let home = dirs::home_dir();
    let gbuild = gbuild_config::user_gbuild_home();

    let mut settings_candidates: Vec<std::path::PathBuf> = Vec::new();
    if let Some(ref gbuild) = gbuild {
        settings_candidates.push(gbuild.join("settings.local.json"));
        settings_candidates.push(gbuild.join("settings.json"));
    }
    ...
    if let Some(ref gbuild) = gbuild {
        known_candidates.push(gbuild.join("plugins").join("known_marketplaces.json"));
    }
    if let Some(ref home) = home {
...
mod tests {
    ...
    fn name_not_found_error_hints_local_dir_and_add_source() {
        ...
        let msg = err.to_string();
        assert!(msg.contains("gbuild plugin install ./sentry"), "{msg}");
        assert!(msg.contains("gbuild plugin marketplace add"), "{msg}");
    ...
    fn name_ambiguous_error_lists_candidates_and_pin_hint() {
        ...
        assert!(
            msg.contains("gbuild plugin install sentry@<qualifier>"),
            "{msg}"
        );
    }
    ...
    fn marketplace_allowlist(
        urls: &[&str],
    ) -> gbuild_workspace::permission::resolution::MarketplaceAllowlist {
        gbuild_workspace::permission::resolution::MarketplaceAllowlist {
    ...
    fn filter_sources_by_allowlist_unrestricted_passes_everything() {
        let allowlist = gbuild_workspace::permission::resolution::MarketplaceAllowlist::default();
        let sources = vec![
