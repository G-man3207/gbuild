//! `gbuild mcp doctor` -- runtime health check for MCP servers.
...
fn discover_servers(cwd: &Path) -> (Vec<ConfigSourceStatus>, Vec<DiscoveredServer>) {
    ...
    let user_config = gbuild_home.join("config.toml");
    if user_config.is_file() {
        sources.push(ConfigSourceStatus {
            path: "~/.gbuild/config.toml".to_string(),
...
async fn try_discover_managed_servers() -> (ConfigSourceStatus, Vec<DiscoveredServer>) {
    ...
    let auth_manager = Arc::new(crate::auth::AuthManager::new(&gbuild_home, grok_com_config));
    ...
    let token = match auth_manager.get_valid_token().await {
        Ok(key) => key,
        Err(_) => return managed_skipped("auth expired — run `gbuild login`"),
    };
...
async fn check_tools_list(service: &mcp_servers::McpService) -> Check {
    use gbuild_mcp::rmcp::model::PaginatedRequestParams;
    match service
...
pub fn print_report(report: &DoctorReport) {
    ...
    if report.servers.is_empty() {
        ...
        println!("  Run `gbuild mcp add --help` to get started.");
        println!();
        return;
    }
    ...
    println!(
        ...
        if report.failing_count > 0 {
            " Run `gbuild mcp doctor --json` for full diagnostics."
        } else {
            ""
        }
    );
