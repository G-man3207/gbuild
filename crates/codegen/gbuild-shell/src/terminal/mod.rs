...
/// Resolved absolute path to bash. On Unix uses the `gbuild_config` shell
/// resolution cascade (`$GBUILD_SHELL` > `$SHELL` > `which` > common dirs >
...
pub fn default_shell_path() -> &'static str {
    #[cfg(unix)]
    {
        gbuild_config::shell::unix_shell_path(gbuild_config::shell::UnixShellKind::Bash)
    }
    #[cfg(not(unix))]
    {
        "/bin/bash"
...
/// Delegates to the canonical implementation in `gbuild-tools`.
pub use gbuild_tools::util::pager_env;
...
/// gbuild-pager which renders its own UI and doesn't need raw ANSI codes).
///
