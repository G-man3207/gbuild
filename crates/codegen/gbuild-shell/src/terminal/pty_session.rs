...
const NOTIFICATION_METHOD: &str = "gbuild/terminal/pty/notification";
...
/// `GBUILD_SHELL`) since `$SHELL` is absent.
fn resolve_pty_shell(shell: Option<&str>) -> (String, Vec<String>) {
    ...
    #[cfg(not(unix))]
    {
        use gbuild_config::shell::{WindowsShell, detect_windows_shell};
