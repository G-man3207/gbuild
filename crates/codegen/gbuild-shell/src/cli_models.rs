//! Data APIs for `gbuild models`. Clients own display.
...
/// Status for the `gbuild models` banner (display order ≠ sampling priority; see [`AuthStatus::resolve`]).
...
mod tests {
    ...
    ///
    /// Uses `GBUILD_AUTH_PATH` (not `GBUILD_HOME`) so a OnceLock-cached real home
    ...
    fn isolate_auth_sources() -> (tempfile::TempDir, [EnvGuard; 6]) {
        ...
        let guards = [
            ...
            EnvGuard::unset("GBUILD_AUTH"),
            EnvGuard::set("GBUILD_AUTH_PATH", auth_path.to_str().unwrap()),
            EnvGuard::unset("GBUILD_DEPLOYMENT_KEY"),
            EnvGuard::unset("GBUILD_WS_ORIGIN"),
            EnvGuard::unset("GBUILD_DISABLE_API_KEY_AUTH"),
        ];
        (dir, guards)
    }
    ...
    fn resolve_oauth_session() {
        ...
        let _auth = EnvGuard::set("GBUILD_AUTH", &json);
