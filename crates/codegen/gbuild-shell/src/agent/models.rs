...
use gbuild_sampling_types::{ReasoningEffort, ReasoningEffortOption};
...
impl Default for ModelsManager {
    fn default() -> Self {
        ...
        let auth_manager = Arc::new(AuthManager::new(&gbuild_home, GrokComConfig::default()));
        Self::new(
            None,
            IndexMap::new(),
...
impl ModelsManager {
    ...
    pub fn model_compactions_remaining(
        &self,
        model_id: &str,
    ) -> Option<gbuild_sampling_types::CompactionsRemaining> {
        self.inner
            .models
            .read()
            .get(model_id)
    ...
    pub fn model_compaction_at_tokens(
        &self,
        model_id: &str,
    ) -> Option<gbuild_sampling_types::CompactionAtTokens> {
        self.inner
            .models
            .read()
            .get(model_id)
    ...
    /// Hot-reload the catalog from `~/.gbuild/models_cache.json` after an
    ...
    /// When another gBuild process sharing `~/.gbuild` (a `--no-leader` run, a
    ...
    /// `ModelsCacheManager` path is fixed to `gbuild_home()`, a process-wide
    /// `OnceLock`).
...
impl ModelsCacheManager {
    fn new() -> Self {
        Self {
            path: crate::util::gbuild_home::gbuild_home().join(MODELS_CACHE_FILE),
            ttl: CACHE_TTL,
        }
    }
    ...
    fn load_fresh(
        ...
        if cache.gbuild_version.as_deref() != Some(gbuild_version::VERSION) {
    ...
    fn persist(
        ...
        let cache = ModelsCache {
            fetched_at: Utc::now(),
            gbuild_version: Some(gbuild_version::VERSION.to_string()),
...
fn resolve_prefetch_env(grok_com_config: Option<GrokComConfig>) -> Option<PrefetchEnv> {
    let gbuild_home = crate::util::gbuild_home::gbuild_home();
    let auth_manager = AuthManager::new(&gbuild_home, grok_com_config.unwrap_or_default());
...
mod tests {
    ...
    fn test_manager() -> ModelsManager {
        ...
        let tmp = std::env::temp_dir().join("gbuild-test-models-manager");
    ...
    fn current_reasoning_effort_seeded_from_config() {
        let tmp = std::env::temp_dir().join("gbuild-test-models-manager-seed");
        let auth_manager = Arc::new(AuthManager::new(&tmp, GrokComConfig::default()));
    ...
    fn reasoning_effort_override_skips_models_that_do_not_offer_level() {
        use gbuild_sampling_types::ReasoningEffortOption;
        use indexmap::IndexMap;
    ...
    fn config_menu_only_model_derives_support_and_default() {
        ...
        let tmp = std::env::temp_dir().join("gbuild-test-models-manager-menu-only");
        let auth_manager = Arc::new(AuthManager::new(&tmp, GrokComConfig::default()));
        let mgr = ModelsManager::new(
            None,
            catalog,
