...
use gbuild_tools::implementations::{gbuild, opencode};
...
/// `gbuild-tools` coordinator actor and reached only through `reporter`.
#[tracing::instrument(
...
pub(crate) async fn run_shell_child(
    ...
    let mut effective_runtime = gbuild_subagent_resolution::resolve_runtime_config(
        &request.subagent_type,
    ...
    if let Some(ref source) = resume_source {
        ...
        if let Err(e) = gbuild_subagent_resolution::validate_resume_identity(
            &request.subagent_type,
    ...
    let worktree_path = if let Some(ref source) = resume_source {
        ...
        let dest = match crate::session::worktree::worktree_base_dir_for_source(&source_cwd) {
            ...
            Err(e) => {
                ...
                std::env::temp_dir()
                    .join("gbuild-subagent-worktrees")
                    .join(&request.id)
            }
        };
    ...
    effective_runtime.capability_mode = gbuild_subagent_resolution::intersect_capability_modes(
    ...
    let allow_nested_subagents =
        child_depth < gbuild_tools::implementations::gbuild::task::MAX_SUBAGENT_DEPTH;
    gbuild_subagent_resolution::apply_child_tool_policy(
        &mut definition,
        effective_runtime.capability_mode,
    ...
    {
        let reminder = gbuild_sampling_types::conversation::ConversationItem::system_reminder(
    ...
    let child_cwd_abs = gbuild_paths::AbsPathBuf::new(child_cwd).unwrap_or_else(|_| {
        gbuild_paths::AbsPathBuf::new(std::env::current_dir().unwrap_or_default())
    ...
    let attribution_callback: Option<gbuild_sampler::SharedAttributionCallback> =
    ...
    let yolo_policy_block = gbuild_workspace::permission::resolution::yolo_disabled_by_policy();
    ...
    if let Some(scope) = agent_memory_scope {
        let memory_tools: Vec<gbuild_tools::registry::types::ToolConfig> = vec![
            ...
            && let Ok(content) = std::fs::read_to_string(&memory_md)
        {
            ...
            let truncated: String = content
                ...
                .join("\n");
            let truncated =
                gbuild_tools::util::truncate::truncate_str(&truncated, MAX_BYTES).to_string();
        ...
        if is_plugin_agent {
            ...
            if !specs.is_empty() {
                let specs: Vec<_> = specs
                    .into_iter()
                    .map(|mut s| {
                        if s.event == gbuild_hooks::event::HookEventName::Stop {
                            s.event = gbuild_hooks::event::HookEventName::SubagentStop;
                        }
                        s
                    })
                    .collect();
    ...
    let agent_mcp_servers: Vec<_> = if !agent_owned_mcp_servers_allowed(is_plugin_agent) {
        ...
        definition
                ...
                .filter_map(|entry| match entry {
                    gbuild_agent::config::McpServerRef::Named(name) => {
                    ...
                    gbuild_agent::config::McpServerRef::Inline { name, config } => {
    ...
    if inherit_skills && ctx.parent_skills.is_none() {
        ...
        ctx.parent_skills = Some(
            gbuild_agent::prompt::skills::list_skills_with_plugins(
                Some(&parent_cwd_str),
    ...
    gbuild_telemetry::session_ctx::log_event(gbuild_telemetry::events::SubagentLaunched {
    ...
    let spawn_result = session::spawn_session_on_thread(
        ...
        gbuild_agent::DEFAULT_SYSTEM_PROMPT_LABEL.to_string(),
        ...
        std::sync::Arc::new(parking_lot::Mutex::new(
            gbuild_workspace::file_system::CodebaseIndexManager::new(),
        )),
        false,
        subagent_fs_watch,
        None,
        ...
        if inherit_skills {
            ...
            gbuild_agent::prompt::skills::SkillsConfig::default()
        },
        if inherit_skills {
        ...
        ctx.yolo_mode
            || matches!(
                ...
                gbuild_agent::config::PermissionMode::BypassPermissions
            ),
        false,
        None,
        ...
        Vec::new(),
        gbuild_agent::prompt::context::PromptAudience::Subagent,
    ...
    let outcome = if result.success {
        gbuild_telemetry::events::Outcome::Completed
    } else if result.cancelled {
        gbuild_telemetry::events::Outcome::Cancelled
    } else {
        gbuild_telemetry::events::Outcome::Error
    };
    gbuild_telemetry::session_ctx::log_event(gbuild_telemetry::events::SubagentCompleted {
        subagent_id: request.id.clone(),
        parent_session_id: request.parent_session_id.clone(),
        outcome,
    ...
    if let Some(ref wt_path) = worktree_path {
        if snapshot_dispose_enabled {
            let ref_name = format!("refs/gbuild/subagents/{}", request.id);
            let source_repo = resolve_subagent_source_repo(&ctx);
