...
//! `gbuild-tools` coordinator actor. This module keeps shell-specific
...
use gbuild_subagent_resolution::ResumeSourceData;
use gbuild_tools::implementations::gbuild::task::coordinator::{
    ChildCompletion, ChildControl, ChildReporter, ChildRunOutput, LocalBoxFuture, StartedChild,
    SubagentProgress,
};
use gbuild_tools::implementations::gbuild::task::types::*;
use gbuild_tools::types::tool::ToolKind;
use gbuild_workspace::file_system::AsyncFileSystem;
...
pub(crate) struct SubagentSpawnContext {
    ...
    pub lsp: Option<std::sync::Arc<dyn gbuild_tools::implementations::lsp::LspBackend>>,
    ...
    pub sampling_config: gbuild_sampler::SamplerConfig,
    ...
    pub inherited_tool_overrides: Option<gbuild_sampling_types::ToolOverrides>,
    pub yolo_mode: bool,
    ...
    pub parent_terminal_backend: Option<Arc<dyn gbuild_tools::computer::types::TerminalBackend>>,
    ...
    pub parent_notification_handle:
        Option<gbuild_tools::notification::types::ToolNotificationHandle>,
    ...
    pub web_search_sampling_config: Option<gbuild_sampler::SamplerConfig>,
    /// Resolved config for web fetch.
    pub web_fetch_config: gbuild_tools::implementations::gbuild::web_fetch::WebFetchConfig,
    /// Image generation config (parent-inherited).
    pub image_gen_config: gbuild_tools::implementations::gbuild::image_gen::ImageGenConfig,
    /// Resolved config for video generation.
    pub video_gen_config: gbuild_tools::implementations::gbuild::video_gen::VideoGenConfig,
    ...
    pub plugin_registry: Option<std::sync::Arc<gbuild_agent::plugins::PluginRegistry>>,
    ...
    pub file_tool_overrides: Option<Vec<gbuild_tools::registry::types::ToolConfig>>,
    ...
    pub api_key_provider: Option<gbuild_tools::types::SharedApiKeyProvider>,
    pub image_description_model: String,
    /// Dual-mode workspace operations handle.
    pub workspace_ops: gbuild_workspace::WorkspaceOps,
    ...
    pub attribution_callback: Option<gbuild_sampler::SharedAttributionCallback>,
    /// Parent session's agent name (e.g. "gBuild").
    ...
    pub parent_tool_definitions: Option<Vec<gbuild_sampling_types::ToolSpec>>,
    /// Pre-discovered skills from the parent session, captured at spawn time.
    pub parent_skills: Option<Vec<gbuild_tools::implementations::skills::types::SkillInfo>>,
    /// Parent's skills config for the child's SkillManager.
    pub parent_skills_config: gbuild_agent::prompt::skills::SkillsConfig,
    ...
    pub parent_compat: gbuild_tools::types::compat::CompatConfig,
...
impl SubagentSpawnContext {
    ...
    fn apply_session_cli_overrides(&self, def: &mut gbuild_agent::config::AgentDefinition) {
    ...
    pub fn resolve_compaction_verbatim_input(&self) -> bool {
        crate::agent::config::BoolFlag::env("GBUILD_COMPACTION_VERBATIM_INPUT")
            .config(
                self.agent_config
                    .as_ref()
    ...
    pub fn resolve_subagent_worktree_snapshot_enabled(&self) -> bool {
        crate::agent::config::BoolFlag::env("GBUILD_SUBAGENT_WORKTREE_SNAPSHOT")
            .config(
                self.agent_config
                    .as_ref()
...
async fn resolve_subagent_sampling_config(
    agent_name: &str,
    agent_model: &gbuild_agent::config::ModelOverride,
    ctx: &SubagentSpawnContext,
) -> (gbuild_sampler::SamplerConfig, acp::ModelId) {
...
async fn resolve_effective_model_config(
    ...
    subagent_type: &str,
    definition_model: &gbuild_agent::config::ModelOverride,
    ctx: &SubagentSpawnContext,
) -> (gbuild_sampler::SamplerConfig, acp::ModelId) {
...
fn log_subagent_model_resolution(
    agent_name: &str,
    priority: &str,
    resolved: &gbuild_sampler::SamplerConfig,
    resolved_id: &acp::ModelId,
    parent: &gbuild_sampler::SamplerConfig,
) {
...
async fn read_parent_sampling_config(
    ...
    if let Some(ref chat_state) = ctx.parent_chat_state {
        if let Some(cfg) = chat_state.get_sampling_config().await {
            ...
            let auth_scheme = crate::agent::config::try_resolve_model_credentials(&cfg.model, None)
                .map(|r| r.auth_scheme)
                .unwrap_or_default();
            let inherited = gbuild_sampler::SamplerConfig {
    ...
    gbuild_telemetry::unified_log::warn(
...
fn resolve_model_override_to_config(
    model_id: &str,
    ctx: &SubagentSpawnContext,
) -> Option<(gbuild_sampler::SamplerConfig, acp::ModelId)> {
...
pub(crate) fn resume_inherited_prefix_len(
    conversation: &[gbuild_sampling_types::conversation::ConversationItem],
) -> usize {
    conversation
        .iter()
...
struct InitialContext {
    ...
    conversation: Vec<gbuild_sampling_types::conversation::ConversationItem>,
...
fn forked_initial_context(
    mut items: Vec<gbuild_sampling_types::conversation::ConversationItem>,
) -> InitialContext {
    ...
    let (conversation, prefix_len) =
        gbuild_subagent_resolution::context::normalize_forked_context(items);
    if prefix_len < 2 {
        return InitialContext {
            source: InitialContextSource::New,
...
fn conversation_tail_is_complete(
    items: &[gbuild_sampling_types::conversation::ConversationItem],
) -> bool {
    matches!(
        items.last(),
...
fn verbatim_or_normalize_fork(
    items: Vec<gbuild_sampling_types::conversation::ConversationItem>,
    ...
    let (conversation, prefix_len) =
        gbuild_subagent_resolution::context::normalize_forked_context(filtered);
    InitialContext {
        source: InitialContextSource::Forked,
        copy_error: None,
        prefix_len: Some(prefix_len),
...
fn resolve_inherited_mcp_pool(
    ...
    inheritance: &gbuild_agent::config::McpInheritance,
...
fn resolve_agent_definition(
    subagent_type: &str,
    ctx: &SubagentSpawnContext,
) -> Option<gbuild_agent::config::AgentDefinition> {
    let cli_agents = ctx
        .agent_config
        .as_ref()
        .map(|config| config.cli_agents.as_slice())
        .unwrap_or_default();
    let resolution_context = gbuild_subagent_resolution::DefinitionResolutionContext {
        cwd: &ctx.parent_cwd,
    ...
    let mut def =
        gbuild_subagent_resolution::discover_agent_definition(subagent_type, &resolution_context)?;
...
fn available_agent_names(ctx: &SubagentSpawnContext) -> Vec<String> {
    let cli_agents = ctx
        ...
        .map(|config| config.cli_agents.as_slice())
        .unwrap_or_default();
    gbuild_subagent_resolution::available_agent_names(
        &gbuild_subagent_resolution::DefinitionResolutionContext {
            cwd: &ctx.parent_cwd,
            plugins: ctx.plugin_registry.as_deref(),
            cli_agents,
            toggles: &ctx.subagent_toggle,
...
pub(crate) struct SubagentValidationContext {
    pub parent_cwd: PathBuf,
    pub plugin_registry: Option<Arc<gbuild_agent::plugins::PluginRegistry>>,
...
pub(crate) fn validate_subagent_type(
    ...
    let context = gbuild_subagent_resolution::DefinitionValidationContext {
        cwd: &ctx.parent_cwd,
        plugins: ctx.plugin_registry.as_deref(),
    ...
    match gbuild_subagent_resolution::validate_agent_name(subagent_type, &context) {
        Ok(()) => SubagentValidateTypeOutcome::Ok,
        Err(gbuild_subagent_resolution::ResolutionError::Unknown { available, .. }) => {
            SubagentValidateTypeOutcome::Unknown { available }
        }
        Err(gbuild_subagent_resolution::ResolutionError::Disabled { .. }) => {
            SubagentValidateTypeOutcome::Disabled
        }
        Err(gbuild_subagent_resolution::ResolutionError::NotAllowed { allowed, .. }) => {
            SubagentValidateTypeOutcome::NotAllowed { allowed }
        }
        Err(
            gbuild_subagent_resolution::ResolutionError::PersonaResolution(_)
            | gbuild_subagent_resolution::ResolutionError::ResumeValidation(_),
...
fn gate_subagent_type(
    ...
    match gbuild_subagent_resolution::gate_agent_definition(subagent_type, &resolution_context) {
        Ok(()) => SubagentValidateTypeOutcome::Ok,
        Err(gbuild_subagent_resolution::ResolutionError::Disabled { .. }) => {
            SubagentValidateTypeOutcome::Disabled
        }
        ...
        Err(
            gbuild_subagent_resolution::ResolutionError::Unknown { .. }
            | gbuild_subagent_resolution::ResolutionError::PersonaResolution(_)
            | gbuild_subagent_resolution::ResolutionError::ResumeValidation(_),
        ) => SubagentValidateTypeOutcome::ValidationUnavailable,
    }
...
pub(crate) fn subagent_harness_flavor_is_representable(agent_type: &str) -> bool {
    gbuild_subagent_resolution::subagent_harness_flavor_is_representable(agent_type)
}
/// Apply the harness-dependent toolset/prompt re-selection to a resolved
/// agent definition.
///
/// The harness flavor (alternate vs gBuild) normally follows the PARENT
/// agent: `GBuildOrchestrator` parents give children
...
/// (so a gBuild session can run an alternate-harness verifier and vice-versa);
...
fn resolve_subagent_toolset(
    ...
    ctx: &SubagentSpawnContext,
    definition: &mut gbuild_agent::config::AgentDefinition,
) {
    let resolution_context = gbuild_subagent_resolution::HarnessToolsetContext {
    ...
    gbuild_subagent_resolution::apply_harness_toolset(
        subagent_type,
        &resolution_context,
        definition,
    );
...
fn summarize_tool_config(
    config: &gbuild_tools::registry::types::ToolServerConfig,
) -> SubagentTypeSummary {
...
fn resolve_subagent_permission_mode(
    requested: gbuild_agent::config::PermissionMode,
    is_plugin: bool,
    policy_block: Option<&'static str>,
) -> gbuild_agent::config::PermissionMode {
    if is_plugin {
...
fn resolve_subagent_source_repo(ctx: &SubagentSpawnContext) -> std::path::PathBuf {
    ...
    gbuild_workspace::session::git::find_main_repo_root_from_path(&source_cwd).unwrap_or(source_cwd)
}
...
fn inject_subagent_completed_prompt(
    ...
    task_completion_reservations: &Option<
        gbuild_tools::reminders::task_completion::TaskCompletionReservations,
    >,
    parent_cmd_tx: Option<&mpsc::UnboundedSender<SessionCommand>>,
    ...
    let summary = gbuild_tools::implementations::gbuild::task::completion_summary(request, result);
    let message = gbuild_tools::reminders::task_completion::format_subagent_completion(
        &summary,
...
pub(crate) async fn reconcile_orphaned_subagents_with_backend(
    ...
    backend: &gbuild_tools::implementations::gbuild::task::backend::ChannelBackend,
    session_dir: &Path,
    parent_session_id: &str,
    gateway: &GatewaySender,
    parent_cmd_tx: Option<&mpsc::UnboundedSender<SessionCommand>>,
