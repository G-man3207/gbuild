...
use gbuild_subagent_resolution::resolve_effective_overrides;
...
async fn subagent_inherits_session_cli_overrides() {
    use gbuild_agent::config::{AgentDefinition, PermissionMode};
...
fn subagent_bypass_permission_mode_gated_by_policy_pin() {
    use gbuild_agent::config::PermissionMode;
    const PIN: &str = gbuild_workspace::permission::resolution::YOLO_PIN_REASON_REQUIREMENTS;
    assert_eq!(
...
fn resume_worktree_action_covers_three_outcomes() {
    ...
    assert_eq!(
            resume_worktree_action(true, Some("refs/gbuild/subagents/x")),
            ResumeWorktreeAction::Rehydrate
        );
    assert_eq!(
            resume_worktree_action(false, Some("refs/gbuild/subagents/x")),
            ResumeWorktreeAction::Rehydrate
        );
    assert_eq!(
...
fn subagent_inherits_parent_lsp_via_context() {
    let parent: std::sync::Arc<dyn gbuild_tools::implementations::lsp::LspBackend> = Arc::new(
        DummyLspDispatch,
    );
    let mut ctx = ctx_with_toggle(HashMap::new());
...
fn explicit_override_takes_precedence_over_role() {
    ...
    let role = gbuild_subagent_resolution::config::SubagentRole {
...
fn persona_resolved_from_config() {
    ...
    personas
        .insert(
            ...
            gbuild_subagent_resolution::config::SubagentPersona {
...
fn model_precedence_explicit_over_role_over_persona() {
    ...
    let role_no_model = gbuild_subagent_resolution::config::SubagentRole {
        description: "test".into(),
        ..Default::default()
    };
...
fn reasoning_effort_precedence_explicit_over_role_over_persona() {
    ...
    let role_no_re = gbuild_subagent_resolution::config::SubagentRole {
        description: "test".into(),
        ..Default::default()
    };
    let r = resolve_effective_overrides(
...
fn forked_initial_context_inherits_parent_across_reasoning() {
    ...
    let items = vec![
            ...
            ConversationItem::Reasoning(gbuild_sampling_types::synthesized_reasoning_item(
                "deliberating",
            )),
...
fn verbatim_fork_keeps_items_byte_for_byte_when_small() {
    use gbuild_sampling_types::conversation::{
...
fn verbatim_fork_falls_back_to_summary_when_oversize() {
    use gbuild_sampling_types::conversation::{ContentPart, ConversationItem};
    let items = vec![
            ConversationItem::system("parent system"),
...
fn summarize_tool_config_uses_name_override_and_strips_namespace() {
    ...
    let mut read = ToolConfig::from_id("GBuild:read_file");
...
/// Regression: on the DEFAULT gBuild host —
...
fn subagent_keeps_default_flavor_when_parent_model_is_non_strict() {
    ...
    ctx.parent_model_agent_type = Some(
        BuiltinAgentName::GBuildPlan.as_ref().to_string(),
    );
...
fn fresh_tool_model_rejects_unknown_and_nonavailable_entries() {
    ...
    for requested in [
        ...
        assert!(!error.contains("gbuild models"));
    }
    assert!(
            super::handle_request::task_model_override_error(
                Some("oauth-only"),
...
fn test_sampling_config(model_slug: &str) -> gbuild_sampling_types::SamplingConfig {
