    ...
    W: tokio::io::AsyncWrite + Unpin + Send + 'static,
{
    ...
    let workspace_user_dir = gbuild_agent::prompt::workspace_user::optional_workspace_user_dir();
...
pub async fn run_stdio_agent(
    ...
    // Log the client that launched us (set by grok-desktop when spawning `gbuild agent stdio`).
    // This appears early in unified.jsonl and is extremely useful for auth diagnostics.
    if let Ok(version) = std::env::var("GBUILD_CLIENT_VERSION") {
        crate::unified_log::info(
            "GBUILD_CLIENT_VERSION",
            None,
    ...
    let result = local_set
        .run_until(async move {
            ...
            // `gbuild agent stdio` is a local/interactive entrypoint (spawned by
...
async fn run_headless_inner(
    ...
    register_fs_watch_runtime();
    gbuild_telemetry::unified_log::set_version(gbuild_version::VERSION);
    // `gbuild agent [headless]` serves non-TUI automation; stamp proxy requests
    // as headless. IDE-facing `gbuild agent stdio` stays interactive.
    ...
    let (mut auth, did_browser_flow) = if no_browser {
        ...
        let auth_manager = Arc::new(AuthManager::new(&gbuild_home::gbuild_home(), ctx.clone()));
        run_auth_flow(
            &auth_manager,
            ctx,
            true,
    ...
    // Capture the gBuild URL for the first-connection callback
    ...
    let on_first_connect: Box<dyn FnOnce() + Send + 'static> = Box::new(move || {
        if !did_browser_flow && !no_browser {
            ...
            eprintln!();
            eprintln!(
                "Open gBuild: {} (press Enter to open in browser)",
                grok_code_url
            );
            eprintln!();
...
///
/// * `relay_on_demand == false` (default — explicit `gbuild agent leader`
...
pub async fn run_leader(
    ...
    local_set
        .run_until(async move {
            ...
            // `gbuild agent leader` (devbox / systemd) has no local IPC clients
            ...
            // Gated on user_gbuild_home() so a cwd-relative .gbuild/auth.json is never
            // read as the user auth store when no home resolves.
            let initial_auth_key_hash = gbuild_config::user_gbuild_home()
                ...
                while let Some(update) = config_update_rx.recv().await {
                    match update {
                        ...
                        ConfigUpdate::ModelsCacheChanged => {
                            // External write to ~/.gbuild/models_cache.json
                            // (another gBuild process fetched a fresher /v1/models
...
mod tests {
    ...
    /// `gbuild agent leader` (devbox/systemd — no local IPC clients,
        ...
        local
            .run_until(async {
                ...
                spawn_leader_relay(
                    slot.clone(),
                    config,
                    false, // eager: explicit `gbuild agent leader` invocation
                    demand_rx,
                    ws_to_agent_tx,
                    agent_to_ws_tx.clone(),
                    cancel.clone(),
