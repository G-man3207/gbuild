...
//! `.gbuild/config.toml` including `[permission]` / `[mcp_servers]` /
//! `[plugins].paths`, `~/.claude.json` `projects.<cwd>`, project `.gbuild/lsp.json`).
...
//! interactive prompt, and the durable [`gbuild_workspace::trust::TrustStore`]
//! reads/writes — lives in `gbuild-workspace` (client-side); this module keeps
...
//! on [`gbuild_workspace::folder_trust::decide`]; the consume-side nuance is
...
use gbuild_workspace::trust::{TrustStore, is_unsafe_trust_root, workspace_key};
use parking_lot::Mutex;

// Decision-side (scan/decide/prompt/store) relocated to `gbuild-workspace`
// (client crate). `grant_folder_trust` is the ONLY moved item referenced from
// OUTSIDE this module (shell call sites + the pager's
// `gbuild_shell::agent::folder_trust::grant_folder_trust`), so only it is
...
pub use gbuild_workspace::folder_trust::grant_folder_trust;
...
// NOTE: this folder-trust store (`~/.gbuild/trusted_folders.toml`) is SEPARATE
// from the pre-existing per-plugin trust store
// (`gbuild_agent::plugins::TrustStore` at `~/.gbuild/trusted-plugins`, plus the
...
/// [`gbuild_workspace::folder_trust::revoke_folder_trust_store`].
///
...
pub fn revoke_folder_trust(cwd: &Path) -> bool {
    ...
    let was_trusted = gbuild_workspace::folder_trust::revoke_folder_trust_store(cwd);
...
/// ([`gbuild_workspace::folder_trust::repo_config_kinds`] /
...
/// folder is gated (so an `.gbuild/lsp.json`-only repo still has a non-empty reason
...
pub(crate) fn detected_config_kinds(cwd: &Path) -> Vec<String> {
    gbuild_workspace::folder_trust::repo_config_kinds(cwd)
        .into_iter()
        .map(str::to_string)
        .collect()
}
...
pub(crate) fn agent_inline_hooks_allowed(
    scope: gbuild_agent::config::AgentScope,
    trusted: impl FnOnce() -> bool,
) -> bool {
    scope != gbuild_agent::config::AgentScope::Project || trusted()
}
...
/// whose cwd differs from the launch dir, `gbuild mcp doctor`) passes `false`, so
...
///   `store_trusted`: a `gbuild --trust` grant issued AFTER this workspace was
...
/// (session merge, the session-less agent pool, `gbuild mcp doctor`). It MUST
...
///
/// Sources: project `.gbuild/config.toml [mcp_servers]` (NOT the user-tier global
...
/// `~/.gbuild/config.toml` is dropped when untrusted. This is intended — untrusted
...
pub fn project_scoped_mcp_names(cwd: &Path) -> HashSet<String> {
    ...
    // `.gbuild/config.toml [mcp_servers]` entries tagged project (the loader's key
...
pub fn filter_untrusted_project_lsp(
    ...
    sourced: std::collections::BTreeMap<
        String,
        (
            gbuild_tools::implementations::lsp::config::LspServerConfig,
            gbuild_tools::types::config_source::ConfigSource,
        ),
    >,
) -> std::collections::BTreeMap<String, gbuild_tools::implementations::lsp::config::LspServerConfig>
{
    gbuild_tools::implementations::lsp::config::filter_project_lsp_when_untrusted(
        sourced,
...
mod tests {
    ...
    use gbuild_workspace::folder_trust::repo_configs_present;
    ...
    fn simulate_release_build() -> EnvGuard {
        EnvGuard::set(gbuild_version::TEST_VERSION_ENV, "0.0.0-sim")
    }
    ...
    fn revoke_folder_trust_downgrades_cache() {
        ...
        // true; GBUILD_HOME-isolated so the seed can't touch the real user file and
        // `#[serial]` because GBUILD_HOME is process-global.
    ...
    fn revoke_never_trusted_folder_writes_no_deny_and_preserves_cascade() {
        ...
        // mid-session untrust. GBUILD_HOME-isolated so the grant writes to a temp
        // store; `#[serial]` because GBUILD_HOME is global.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("GBUILD_HOME", home.path());
    ...
    fn revoke_ancestor_cascade_trusted_child_writes_explicit_untrust() {
        ...
        // the cache. GBUILD_HOME-isolated so the grant writes to a temp store;
        // `#[serial]` because GBUILD_HOME is process-global.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("GBUILD_HOME", home.path());
    ...
    fn revoke_on_unrecordable_home_root_records_no_deny() {
        ...
        // home; GBUILD_HOME-isolated store; GBUILD_FOLDER_TRUST unset so the
        // default-on flag applies.
        let home = tempfile::tempdir().unwrap();
        let _home = EnvGuard::set("HOME", home.path());
        let gbuild_home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("GBUILD_HOME", gbuild_home.path());
        ...
        std::fs::create_dir_all(home.path().join(".gbuild").join("hooks")).unwrap();
    ...
    fn envrc_gate_drops_untrusted_then_loads_when_store_trusted() {
        ...
        // actually reads `.envrc`. GBUILD_HOME-isolated so the trust store is empty;
        // GBUILD_FOLDER_TRUST unset so the default-on feature flag applies.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("GBUILD_HOME", home.path());
        let _flag = EnvGuard::unset("GBUILD_FOLDER_TRUST");
        let tmp = repo_tmp();
    ...
    fn claude_env_gate_drops_project_env_when_untrusted() {
        ...
        // folder merges it. GBUILD_HOME-isolated so the trust store is empty;
        // GBUILD_FOLDER_TRUST unset so the default-on feature flag applies.
        use gbuild_workspace::permission::claude_settings::load_claude_env_with_project;
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("GBUILD_HOME", home.path());
        let _flag = EnvGuard::unset("GBUILD_FOLDER_TRUST");
        let tmp = repo_tmp();
    ...
    fn claude_env_gate_drops_subdir_project_env_when_untrusted() {
        ...
        // this). GBUILD_HOME-isolated so the trust store is empty.
        use gbuild_workspace::permission::claude_settings::load_claude_env_with_project;
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("GBUILD_HOME", home.path());
        let _flag = EnvGuard::unset("GBUILD_FOLDER_TRUST");
    ...
    fn project_agent_inline_hooks_gated_when_untrusted_but_user_kept() {
        ...
        // predicate used at mvp_agent/subagent. GBUILD_HOME-isolated (empty store).
        use gbuild_agent::config::AgentScope;
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("GBUILD_HOME", home.path());
        let _flag = EnvGuard::unset("GBUILD_FOLDER_TRUST");
        let tmp = repo_tmp();
        let agents = tmp.path().join(".gbuild").join("agents");
        ...
        .unwrap();
        let def = gbuild_agent::discovery::by_name_in_cwd("explore", tmp.path())
    ...
    fn project_scope_allowed_denies_untrusted_repo_with_configs() {
        ...
        // code-exec config (here `.gbuild/hooks`), and no store grant must be
        // DENIED — even though no verdict was recorded first (the gate re-resolves
        // fail-closed rather than defaulting open). GBUILD_HOME-isolated (empty
        // store); GBUILD_FOLDER_TRUST unset so the default-on flag applies.
        let _sim = simulate_release_build();
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("GBUILD_HOME", home.path());
        let _flag = EnvGuard::unset("GBUILD_FOLDER_TRUST");
    ...
    fn project_scope_allowed_allows_repo_without_configs() {
        ...
        // would wrongly deny it. Release-stamped + GBUILD_HOME-isolated so the
    ...
    fn project_scope_allowed_allows_store_trusted_repo() {
        ...
        // configs present. GBUILD_HOME-isolated so the seeded store is the temp one;
        // GBUILD_FOLDER_TRUST unset so the default-on flag applies.
        let _sim = simulate_release_build();
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("GBUILD_HOME", home.path());
        let _flag = EnvGuard::unset("GBUILD_FOLDER_TRUST");
    ...
    fn project_scope_allowed_denies_untrusted_plugin_only_repo() {
        // A plugin-only untrusted repo (just `.gbuild/plugins/<x>/`, no
        ...
        // discover_plugins/build_for_cwd/reload. GBUILD_HOME-isolated (empty store);
        // GBUILD_FOLDER_TRUST unset so the default-on flag applies.
        let _sim = simulate_release_build();
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("GBUILD_HOME", home.path());
        let _flag = EnvGuard::unset("GBUILD_FOLDER_TRUST");
        let tmp = repo_tmp();
        std::fs::create_dir_all(tmp.path().join(".gbuild").join("plugins").join("evil")).unwrap();
        assert!(
            !project_scope_allowed(tmp.path()),
    ...
    fn project_scope_allowed_denies_untrusted_permission_only_repo() {
        // Bridge: a clone whose ONLY repo-local config is `.gbuild/config.toml`
    ...
    fn kill_switch_allows_untrusted_repo_after_authoritative_resolve() {
        ...
        // GBUILD_HOME-isolated (empty store); GBUILD_FOLDER_TRUST unset so the kill-switch
        // is the only signal.
        let _sim = simulate_release_build();
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("GBUILD_HOME", home.path());
        ...
        let cold = repo_tmp();
        std::fs::create_dir_all(cold.path().join(".gbuild").join("hooks")).unwrap();
        assert!(
    ...
    fn build_for_cwd_with_trust_verdict_gates_active_project_plugin() {
        ...
        // un-gating here.
        //
        // GBUILD_HOME-isolated so both the folder-trust store and the plugin trust
        // store start empty (deterministic untrusted); GBUILD_FOLDER_TRUST unset so
        // the default-on flag applies; `#[serial]` because both are process-global.
        use gbuild_agent::plugins::discovery::DiscoveryConfig;
        use gbuild_agent::plugins::{PluginRegistry, SharedPluginRegistryHandle};
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("GBUILD_HOME", home.path());
        let _flag = EnvGuard::unset("GBUILD_FOLDER_TRUST");
        let tmp = repo_tmp();
        ...
        let plugin = tmp.path().join(".gbuild").join("plugins").join("trustgate");
    ...
    fn discover_hooks_gates_then_loads_project_hook_via_trust_verdict() {
        ...
        // GBUILD_HOME-isolated so the grant writes to a temp store; GBUILD_FOLDER_TRUST unset
        // so the default-on flag applies.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("GBUILD_HOME", home.path());
        let _flag = EnvGuard::unset("GBUILD_FOLDER_TRUST");
        let tmp = repo_tmp();
        let hooks_dir = tmp.path().join(".gbuild").join("hooks");
        ...
        let has_project_hook = |reg: &gbuild_hooks::discovery::HookRegistry| {
            reg.all_hooks()
                .iter()
        ...
        let git_root = gbuild_workspace::session::git::find_git_root_from_path(tmp.path()).ok();
        let (reg, _errs) = crate::util::hooks::discover_hooks(
            git_root.as_deref(),
            &gbuild_tools::types::compat::CompatConfig::default(),
            untrusted,
        );
        assert!(
    ...
    fn filter_untrusted_project_lsp_drops_only_project() {
        use gbuild_tools::implementations::lsp::config::LspServerConfig;
        use gbuild_tools::types::config_source::ConfigSource;
    ...
    fn load_servers_sourced_tags_project_lsp_json() {
        use gbuild_tools::implementations::lsp::config::load_servers_with_plugins_sourced;
        use gbuild_tools::types::config_source::ConfigSource;

        // A `<cwd>/.gbuild/lsp.json` server must be tagged `Project` so the gate
        // can distinguish it from user/plugin servers. Asserts on the specific
        // key, so any real `~/.gbuild/lsp.json` on the test host is irrelevant.
        let tmp = repo_tmp();
        let gbuild = tmp.path().join(".gbuild");
        std::fs::create_dir_all(&gbuild).unwrap();
        std::fs::write(
            gbuild.join("lsp.json"),
    ...
    fn untrusted_workspace_drops_loaded_project_lsp() {
        ...
        // from `<cwd>/.gbuild/lsp.json` is dropped once the workspace is untrusted.
        let tmp = repo_tmp();
        let gbuild = tmp.path().join(".gbuild");
        std::fs::create_dir_all(&gbuild).unwrap();
        std::fs::write(
    ...
    /// (`.mcp.json`) and `projtoml` (`.gbuild/config.toml [mcp_servers]`).
    ...
    /// `.gbuild/config.toml`, `.mcp.json`, and `.cursor/mcp.json` must appear in
    ...
    fn cached_untrusted_is_upgraded_by_a_later_grant() {
        ...
        record(&key, false);
        assert!(!project_scope_allowed(tmp.path()));
        // Simulate a `gbuild --trust` grant landing in the store after the
    ...
    fn explicit_child_untrust_survives_reload_despite_ancestor_trust() {
        ...
        let tmp = tempfile::tempdir().unwrap();
        let store_path = tmp.path().join(gbuild_workspace::trust::TRUST_FILE_NAME);
    ...
    fn resolve_launch_dir_trust_matches_resolve_and_record() {
        ...
        // GBUILD_HOME so the store is empty/seeded in temp. `#[serial]` because both
        // vars are process-global.
        let _feature = EnvGuard::set("GBUILD_FOLDER_TRUST", "1");
        let _sim = simulate_release_build();
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("GBUILD_HOME", home.path());
        ...
        let untrusted = repo_tmp();
        std::fs::create_dir_all(untrusted.path().join(".gbuild").join("hooks")).unwrap();
        ...
        let trusted = repo_tmp();
        std::fs::create_dir_all(trusted.path().join(".gbuild").join("hooks")).unwrap();
        let mut store = TrustStore::load();
    ...
    fn prompt_warranted_true_for_untrusted_repo_with_configs() {
        ...
        // => the GUI prompt is warranted. GBUILD_HOME-isolated so the store starts
        // empty; `#[serial]` because GBUILD_HOME is process-global.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("GBUILD_HOME", home.path());
        let tmp = repo_tmp();
    ...
    fn detected_config_kinds_summarizes_present_markers() {
        ...
        std::fs::write(tmp.path().join(".mcp.json"), "{}").unwrap();
        std::fs::create_dir_all(tmp.path().join(".gbuild").join("hooks")).unwrap();
        std::fs::write(tmp.path().join(".gbuild").join("lsp.json"), "{}").unwrap();
    ...
    fn detected_config_kinds_reports_lsp_only_repo() {
        ...
        // `.gbuild/lsp.json` must still produce a non-empty reason list.
        let tmp = repo_tmp();
        std::fs::create_dir_all(tmp.path().join(".gbuild")).unwrap();
        std::fs::write(tmp.path().join(".gbuild").join("lsp.json"), "{}").unwrap();
        let kinds = detected_config_kinds(tmp.path());
