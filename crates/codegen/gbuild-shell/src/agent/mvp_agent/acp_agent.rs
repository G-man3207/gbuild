...
) -> Option<gbuild_sampling_types::ToolOverrides> {
...
fn insert_applied_tool_overrides(
    ...
    echo: Option<&gbuild_sampling_types::ToolOverrides>,
) {
...
impl acp::Agent for MvpAgent {
    ...
    async fn initialize(
        ...
        gbuild_telemetry::unified_log::info("agent initialized", None, None);
        ...
        {
            let root = crate::util::gbuild_home::gbuild_home();
        ...
        CLEANUP_PERMISSIONS_ONCE
            .call_once(|| {
                tokio::task::spawn(
                    gbuild_workspace::permission::cleanup_stale_permission_state(
        ...
        *self.buffering_settings.borrow_mut() = buffering_settings;
        ...
        gbuild_telemetry::unified_log::info(
            ...
            Some(
                serde_json::json!({
                "gbuild_home": crate::util::gbuild_home::gbuild_home().display().to_string(),
        ...
        Ok(
            acp::InitializeResponse::new(acp::ProtocolVersion::V1)
                ...
                .meta({
                    let metadata = parse_json_object_env("GBUILD_AGENT_METADATA");
                    serde_json::json!({
                    "grokShell": true,
                    ...
                    "agentVersion": gbuild_version::VERSION,
                    "agentId": agent_id(),
    ...
    async fn authenticate(
        ...
        match arguments.method_id.0.as_ref() {
            auth_method::XAI_API_KEY_METHOD_ID => {
                ...
                log_event(gbuild_telemetry::events::Login {
    ...
    async fn new_session(
        ...
        gbuild_telemetry::session_ctx::log_session_event(crate::agent::session_metrics::SessionStarted {
        ...
        if product_analytics || gbuild_telemetry::external::is_active() {
            ...
            tokio::spawn(async move {
                let git = gbuild_telemetry::context::collect_git_context(&cwd_str);
                let ev = gbuild_telemetry::events::SessionNew {
                    session_id: sid,
        ...
        let (git_root, is_git_repo, discovery_failed) = match gbuild_workspace::session::git::discover_git_root(
            cwd.as_path(),
        ) {
    ...
    async fn load_session(
            ...
            Some(s) => {
                match s.state {
                    crate::session::plan_mode::PlanModeState::Inactive => {
                        gbuild_telemetry::events::PlanModeState::Inactive
                    }
                    crate::session::plan_mode::PlanModeState::Pending => {
                        gbuild_telemetry::events::PlanModeState::Pending
                    }
                    crate::session::plan_mode::PlanModeState::Active
                    | crate::session::plan_mode::PlanModeState::ExitPending => {
                        gbuild_telemetry::events::PlanModeState::Active
                    }
                }
            }
            None => gbuild_telemetry::events::PlanModeState::Inactive,
        };
        ...
        if restore_code_requested && registry_client_for_restore.is_none() {
            gbuild_workspace::session::git::warn_registry_disabled_restore(
                session_id.0.as_ref(),
            );
        }
        let restore_checkout_allowed = gbuild_workspace::session::git::restore_code_checkout_allowed(
            cwd.as_path(),
        ...
        {
            tracing::warn!(
                target: gbuild_workspace::session::git::RESTORE_CODE_LOG,
                session_id = %session_id.0,
                supplied_cwd = %cwd.as_str(),
                persisted_cwd = %summary.info.cwd,
                target_sha = %target_sha,
                "restore_code: skipping session HEAD checkout — supplied cwd is neither a gbuild worktree nor the session's persisted cwd (refusing to detach the source repo)"
            );
            gbuild_telemetry::unified_log::warn(
        ...
        if restore_code_requested && restore_checkout_allowed
            && let Some(ref target_sha) = summary.head_commit
        {
            use gbuild_workspace::session::git::RestoreKind;
            let outcome = gbuild_workspace::session::git::checkout_session_commit(
                    cwd.as_path(),
                    target_sha,
                    true,
                    session_id.0.as_ref(),
        ...
        let is_gbuild = persisted_model.0.starts_with("grok-build");
        let same_family_fallback = if is_gbuild {
        ...
        if summary.head_commit.is_some() && let Some(ref cwd) = session_cwd
            && summary
                ...
                .as_deref()
                .is_none_or(|root| {
                    gbuild_workspace::session::git::find_git_root_from_path(
                ...
                })
        {
            ...
            let current_head = gbuild_workspace::session::git::git_cli(
                    cwd_path,
                    &["rev-parse", "HEAD"],
                )
                .await
                .ok();
            if let Some(divergence) = gbuild_workspace::session::git::detect_head_divergence(
        ...
        if self.product_analytics_enabled() {
            log_event(gbuild_telemetry::events::SessionLoad {
                session_id: session_id.0.to_string(),
    ...
    async fn prompt(
            ...
            .and_then(|m| m.get("toolOverrides"))
        {
            None => None,
            Some(value) => {
                match gbuild_sampling_types::ToolOverridesUpdate::parse(value) {
            ...
            Ok(turn_ok) => {
                let crate::session::commands::PromptTurnOk {
                    ...
                    }
                    {
                        ...
                        tokio::spawn(async move {
                            let head = gbuild_workspace::session::git::get_current_commit(
                                    std::path::Path::new(&cwd),
                                )
                                .await;
                            let branch = gbuild_workspace::session::git::get_branch(
                                    std::path::Path::new(&cwd),
                                )
                                .await;
                            let _ = cmd_tx
        ...
        let result = match method.as_ref() {
            ...
            "gbuild/cloud/terminate" => {
                crate::extensions::auth_gate::require_xai_auth(
                    ...
                    "Run `gbuild login` to authenticate.",
                )?;
    ...
    async fn ext_notification(
        ...
        if args.method.as_ref() == "gbuild/telemetry/non_git_decision" {
            ...
            >(args.params.get()) {
                ...
                gbuild_telemetry::session_ctx::log_event(gbuild_telemetry::events::NonGitDecisionEvent {
                    decision: params.decision,
                    session_id: params.session_id,
                    client_version: params.client_version,
                });
        ...
        if args.method.as_ref() == "gbuild/telemetry/multi_agent_followup" {
            ...
            >(args.params.get()) {
                ...
                gbuild_telemetry::session_ctx::log_event(gbuild_telemetry::events::MultiAgentFollowup {
                    ...
                    other_agents: params
                        .other_agents
                        .into_iter()
                        .map(|(l, s, m)| gbuild_telemetry::events::AgentInfo {
                            label: l.to_string(),
                            session_id: s,
                            model_id: m,
                        })
        ...
        if args.method.as_ref() == "gbuild/telemetry/multi_agent_apply" {
            ...
            >(args.params.get()) {
                ...
                gbuild_telemetry::session_ctx::log_event(gbuild_telemetry::events::MultiAgentApply {
        ...
        if args.method.as_ref() == "gbuild/telemetry/multi_agent_discard" {
            ...
            >(args.params.get()) {
                ...
                gbuild_telemetry::session_ctx::log_event(gbuild_telemetry::events::MultiAgentDiscard {
                    discarded_agents: params
                        .discarded_agents
                        .into_iter()
                        .map(|(l, s, m)| gbuild_telemetry::events::AgentInfo {
        ...
        if args.method.as_ref() == gbuild_telemetry::unified_log::LOG_METHOD
            && let Ok(params) = serde_json::from_str::<
                gbuild_telemetry::unified_log::LogNotificationParams,
            >(args.params.get())
        {
            gbuild_telemetry::unified_log::ingest_client_entries(
                params.src,
                &params.entries,
            );
        }
