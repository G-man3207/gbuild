...
fn harness_pair(id: &str) -> Vec<gbuild_sampling_types::conversation::ConversationItem> {
    use gbuild_sampling_types::ToolCall;
...
#[test]
#[serial_test::serial]
fn resolve_agent_definition_defaults_to_gbuild() {
    let prev = std::env::var("GBUILD_AGENT").ok();
    unsafe {
        std::env::remove_var("GBUILD_AGENT");
    }
    ...
    if let Some(v) = prev {
        unsafe { std::env::set_var("GBUILD_AGENT", v) }
    }
}
/// When model_agent_type = Some("codex"), the codex agent is selected even
/// though the default chain would return gbuild.
#[test]
#[serial_test::serial]
...
fn resolve_agent_definition_acp_profile_wins_when_model_agent_type_is_default() {
    ...
    let tmp = tempfile::tempdir().unwrap();
    let acp_profile = gbuild_agent::AgentDefinition::from_json(&serde_json::json!(
...
/// `gbuild-plan`, models in the catalog that still declare
/// `agent_type = "gbuild"` explicitly must NOT preempt an ACP
/// profile. Any value in the `gbuild*` family is the stock harness
/// with no strict requirement.
#[test]
#[serial_test::serial]
fn resolve_agent_definition_acp_profile_wins_for_explicit_gbuild_family() {
    let prev = std::env::var("GBUILD_AGENT").ok();
    unsafe {
        std::env::remove_var("GBUILD_AGENT");
    }
    ...
    .expect("agent definition must parse");
    for family_variant in ["gbuild", "gbuild-plan", "gbuild-concise"] {
        let def = MvpAgent::resolve_agent_definition(
            tmp.path(),
            None,
            &config::AgentSelectionConfig::default(),
        ...
        assert_eq!(
            def.name, "custom-devbox-profile",
            "ACP profile must win for gbuild family variant `{family_variant}`"
        );
    }
    if let Some(v) = prev {
        unsafe { std::env::set_var("GBUILD_AGENT", v) }
...
fn inherited_harness_template_skips_nonstrict_model() {
    use gbuild_agent::prompt::user_message::UserMessageTemplate;
    let tmp = tempfile::tempdir().unwrap();
    assert!(
...
fn resolve_agent_definition_agent_profile_with_model_override() {
    ...
    let tmp = tempfile::tempdir().unwrap();
    let agents_dir = tmp.path().join(".gbuild").join("agents");
    ...
    assert_eq!(
        def.model,
        gbuild_agent::config::ModelOverride::Override("test-model-123".to_string()),
        "agent profile model override must be preserved through resolution"
    );
    match prev {
        Some(v) => unsafe { std::env::set_var("GBUILD_AGENT", v) },
        None => unsafe { std::env::remove_var("GBUILD_AGENT") },
    }
}
#[test]
...
/// flow: a zero-turn switch from `gbuild` (a client profile name) to
/// `gbuild-plan` (the default model agent_type) must be
...
fn harnesses_are_compatible_for_stock_family_pairs() {
    assert!(harnesses_are_compatible("gbuild", "gbuild-plan"));
    assert!(harnesses_are_compatible("gbuild-plan", "gbuild"));
    assert!(harnesses_are_compatible("gbuild", "gbuild"));
...
fn harnesses_are_compatible_rejects_strict_mismatches() {
    ...
    assert!(!harnesses_are_compatible("gbuild-plan", "codex"));
}
#[test]
fn explicit_agent_type_wins_over_session_default() {
    assert_eq!(
        resolve_required_agent_type(Some("cursor"), "gbuild-plan"),
        "cursor"
    );
}
#[test]
fn null_agent_type_falls_back_to_session_default_gbuild_plan() {
    assert_eq!(
        resolve_required_agent_type(None, "gbuild-plan"),
        "gbuild-plan"
    );
}
#[test]
fn null_agent_type_falls_back_to_session_default_gbuild() {
    assert_eq!(
        resolve_required_agent_type(None, "gbuild"),
        "gbuild"
    );
}
#[test]
fn null_agent_type_returns_to_session_default_after_cursor_switch() {
    let session_default = "gbuild-plan";
    let required_after_null = resolve_required_agent_type(None, session_default);
    assert_eq!(required_after_null, "gbuild-plan");
...
fn agent_name_unchanged_without_harness_rebuild() {
    let unchanged = agent_name_after_model_switch(false, "gbuild-plan", "remote-sidebar");
    assert_eq!(
...
async fn file_toolset_override_e2e_to_finalized_toolset() {
    ...
    use gbuild_tools::computer::local::{LocalFs, LocalTerminalBackend};
    ...
    let builder = gbuild_tools::registry::types::ToolRegistryBuilder::new();
    let ctx = SessionContext {
        ...
        memory_backend: None,
        web_search_config: gbuild_tools::implementations::web_search::WebSearchConfig::default(),
        web_fetch_config: Default::default(),
        lsp: None,
        image_gen_config: gbuild_tools::implementations::gbuild::image_gen::ImageGenConfig::default(),
        video_gen_config: gbuild_tools::implementations::gbuild::video_gen::VideoGenConfig::default(),
        app_builder_deployer_config: gbuild_tools::implementations::gbuild::deploy_app::AppBuilderDeployerConfig::default(),
...
fn make_test_handle(
    ...
    crate::session::SessionHandle {
        ...
        tool_context: crate::tools::ToolContext::new_local_context(
            gbuild_paths::AbsPathBuf::new(std::path::PathBuf::from("/tmp")).unwrap(),
            std::sync::Arc::new(gbuild_workspace::file_system::LocalFs::new(
                std::path::PathBuf::from("/tmp"),
            )),
        ...
        permission_handle: gbuild_workspace::permission::PermissionHandle::allow_all(),
        attribution_callback: None,
        agent_name: "gbuild".to_string(),
...
async fn model_state_prefers_session_reasoning_effort_over_global() {
    ...
    use gbuild_sampling_types::{REASONING_EFFORT_META_KEY, ReasoningEffort};
    let agent = build_minimal_agent_for_tests();
...
async fn yolo_toggle_scoped_by_client_identifier() {
    ...
    let mut sessions: HashMap<acp::SessionId, crate::session::SessionHandle> = [
        (
            sid_tui.clone(),
            make_test_handle("grok-3", false, Some("gbuild-tui")),
        ),
        (
            sid_vscode.clone(),
    ...
    .into();
    let updated = apply_yolo_mode_to_matching_sessions(&mut sessions, Some("gbuild-tui"), true);
...
async fn yolo_toggle_can_disable_session_started_with_yolo_enabled() {
    ...
    let mut sessions: HashMap<acp::SessionId, crate::session::SessionHandle> = [
        (
            sid_tui.clone(),
            make_test_handle("grok-3", true, Some("gbuild-tui")),
        ),
        (
            sid_other.clone(),
    ...
    .into();
    let updated = apply_yolo_mode_to_matching_sessions(&mut sessions, Some("gbuild-tui"), false);
...
async fn test_per_session_code_nav_isolation() {
    ...
    let tui_handle = {
        let mut h = make_test_handle("model", false, Some("gbuild-tui"));
...
async fn test_tui_session_is_rejected() {
    ...
    let mut handle = make_test_handle("model", false, Some("gbuild-tui"));
    handle.code_nav_enabled = true;
    let sessions = [(sid.clone(), handle)].into();
    assert_eq!(
...
async fn test_leader_mode_two_sessions_stay_isolated() {
    ...
    let mut tui_handle = make_test_handle("model", false, Some("gbuild-tui"));
...
mod eligibility_gates {
    ...
    fn tui_client_rejected() {
        assert_eq!(
            check_gates(ClientType::GBuildTui, true, true),
            Err(CodeNavEligibility::ClientNotWeb)
        );
    }
    #[test]
...
async fn cached_token_fallthrough_prefers_api_key_for_deployment_key() {
    ...
    use gbuild_test_support::EnvGuard;
    let _lockdown = EnvGuard::unset("GBUILD_DISABLE_API_KEY_AUTH");
...
async fn prepare_image_gen_config_fails_open_without_auth() {
    use gbuild_tools::implementations::gbuild::image_gen::ImageGenConfig;
    let agent = build_minimal_agent_for_tests();
    agent.sampling_config.borrow_mut().api_key = Some("test-key".to_string());
...
fn chat_session_spawn_options_matches_thin_profile() {
    ...
    let cwd = gbuild_paths::AbsPathBuf::new(std::env::temp_dir()).expect("temp cwd");
...
async fn remove_session_releases_workspace_binding_and_side_maps() {
    ...
    let ops = gbuild_workspace::WorkspaceOps::for_test();
    let toolset =
        std::sync::Arc::new(gbuild_tools::registry::types::FinalizedToolset::empty_for_test());
    ...
    *agent.workspace_ops.borrow_mut() = Some(ops);
    ...
    let (_permission_tx, permission_rx) =
        tokio::sync::mpsc::unbounded_channel::<gbuild_workspace::permission::PermissionEvent>();
    agent
...
fn ext_method_rewind_uses_local_dispatch_without_bridge() {
    use acp::Agent as _;
    let _env = crate::env::EnvVarGuard::remove(crate::env::GBUILD_DISABLE_CUSTOM_BRIDGE_ENV);
...
fn make_live_session_handle(
    ...
    let mut handle = make_test_handle("test-model", false, Some("gbuild-tui"));
...
/// observe (and answer) agent→client reverse-requests.
...
fn write_project_subagent_definitions(cwd: &std::path::Path) {
    let roles = cwd.join(".gbuild/roles");
    let personas = cwd.join(".gbuild/personas");
...
#[test]
#[serial_test::serial]
fn subagent_spawn_context_loads_project_definitions() {
    let repo = tempfile::tempdir().unwrap();
    git2::Repository::init(repo.path()).unwrap();
    write_project_subagent_definitions(repo.path());
    run_local_for_bridge_test(|| async {
        let (agent, _rx) = build_agent_with_gateway_rx();
        let sid = acp::SessionId::new("roles-personas-project-defs");
        ...
        agent.sessions.borrow_mut().insert(sid.clone(), handle);
        {
            ...
            cfg.subagent_roles.insert(
                "refreshed".into(),
                gbuild_subagent_resolution::config::SubagentRole {
        ...
        assert!(ctx.subagent_personas.contains_key("probe"));
        assert_eq!(
            ctx.subagent_roles
                .get("refreshed")
                .map(|role| role.description.as_str()),
        ...
        assert_eq!(ctx.subagent_toggle.get("probe"), Some(&false));
    });
}
fn ann(id: &str) -> gbuild_announcements::RemoteAnnouncement {
    gbuild_announcements::RemoteAnnouncement {
...
fn announcements_push_gate_ignores_expired_only_addition() {
    let now = test_now();
    let expired = gbuild_announcements::RemoteAnnouncement {
...
fn announcements_push_gate_emits_on_expiry_crossing() {
    let expiring = gbuild_announcements::RemoteAnnouncement {
