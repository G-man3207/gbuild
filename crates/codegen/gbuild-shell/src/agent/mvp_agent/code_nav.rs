...
impl MvpAgent {
    ...
    pub(crate) fn parse_code_nav_capability(init: &acp::InitializeRequest) -> bool {
        init.client_capabilities
            .meta
            .as_ref()
            .and_then(|m| m.get("gbuild/codeNavigation"))
    ...
    pub(super) fn code_nav_eligibility_inner(
        ...
        let git_root = gbuild_workspace::session::git::find_git_root_from_path(cwd).ok();
        match &setting {
    ...
    pub(super) fn indexed_roots_for(&self, cwd: &std::path::Path) -> Vec<String> {
        ...
        if let Ok(git_root) = gbuild_workspace::session::git::find_git_root_from_path(cwd)
