...
use super::*;
use gbuild_tools::implementations::gbuild::task::backend::SubagentBackend;
...
impl MvpAgent {
    ...
    pub(super) fn ensure_telemetry_client(&self) {
        ...
        if !mode.is_disabled() {
            ...
            gbuild_telemetry::client::init_if_needed(
                cfg.telemetry.clone(),
                mode,
                user_id,
                team_id,
                ...
                gbuild_version::VERSION.to_owned(),
                subscription_tier,
    ...
    pub(crate) fn plugin_registry_handle(
        &self,
    ) -> &gbuild_agent::plugins::SharedPluginRegistryHandle {
    ...
    fn ensure_local_workspace_ops(
        &self,
    ) -> Result<gbuild_workspace::WorkspaceOps, acp::Error> {
        ...
        let workspace_identity = self
            ...
            .map(|a| match a.team_id.filter(|t| !t.is_empty()) {
                Some(team) => {
                    gbuild_workspace::WorkspaceIdentity::team(a.user_id, team)
                }
                None => {
                    gbuild_workspace::WorkspaceIdentity::new(
                        a.user_id,
                        a.principal_type,
                        a.principal_id,
                    )
    ...
    pub(super) fn apply_polled_announcements(
        ...
        pre_fetch: Option<Vec<gbuild_announcements::RemoteAnnouncement>>,
    ) {
        let mut cfg = self.cfg.borrow_mut();
    ...
    pub(super) fn prepare_image_gen_config(
        &self,
    ) -> gbuild_tools::implementations::gbuild::image_gen::ImageGenConfig {
        ...
        let version = cfg
            .client_version
            .clone()
            .unwrap_or_else(|| gbuild_version::VERSION.to_string());
        let alpha_test_key = cfg.endpoints.alpha_test_key.clone();
    ...
    pub(super) fn prepare_app_builder_deployer_config(
        ...
        use gbuild_tools::implementations::gbuild::deploy_app::AppBuilderDeployerConfig;
    ...
    pub(super) fn prepare_video_gen_config(
        &self,
    ) -> gbuild_tools::implementations::gbuild::video_gen::VideoGenConfig {
    ...
    ///
    /// Enabled gate: `disable_web_search` kill-switch > `GBUILD_WEB_FETCH` env >
    /// remote settings `web_fetch_enabled` > default (false).
    ///
    /// Params resolution (TOML > env > remote settings > default):
    /// - `proxy_endpoint`: `[toolset.web_fetch] proxy_endpoint` > `GBUILD_WEB_FETCH_PROXY` > remote settings > None
    /// - `allowed_domains`: `[toolset.web_fetch] allowed_domains` > remote settings > built-in defaults
    /// - `allow_local`: `[toolset.web_fetch] allow_local` > `GBUILD_WEB_FETCH_ALLOW_LOCAL` > false
    pub(super) fn prepare_web_fetch_config(
        &self,
    ) -> gbuild_tools::implementations::gbuild::web_fetch::WebFetchConfig {
    ...
    pub fn with_models(
        ...
        if relay_sync_enabled {
            tracing::info!("[gBuild] Relay sync: ENABLED");
        ...
        let instance = Self {
            ...
            monitor_event_buffer: gbuild_tools::implementations::gbuild::task::types::MonitorEventBuffer::default(),
    ...
    pub async fn kill_background_task(
        ...
        session_id: &str,
        task_id: &str,
    ) -> Result<gbuild_tools::types::KillOutcome, String> {
        let sid = acp::SessionId::new(session_id);
        if let Some(handle) = self.get_session_handle(&sid) {
    ...
    pub async fn cancel_subagent(
        &self,
        subagent_id: &str,
    ) -> gbuild_tools::implementations::gbuild::task::types::SubagentCancelOutcome {
        gbuild_tools::implementations::gbuild::task::backend::ChannelBackend::new(
    ...
    pub(crate) async fn list_running_subagents(
        ...
        gbuild_tools::implementations::gbuild::task::types::SubagentInspection,
    > {
        gbuild_tools::implementations::gbuild::task::backend::ChannelBackend::new(
                self.subagent_event_tx.clone(),
            )
    ...
    pub(crate) async fn query_subagent(
        ...
        gbuild_tools::implementations::gbuild::task::types::SubagentSnapshot,
    > {
        gbuild_tools::implementations::gbuild::task::backend::ChannelBackend::new(
                self.subagent_event_tx.clone(),
            )
    ...
    pub async fn list_tasks(
        &self,
        session_id: &str,
    ) -> Option<Vec<gbuild_tools::types::TaskSnapshot>> {
        let sid = acp::SessionId::new(session_id);
        if let Some(handle) = self.get_session_handle(&sid) {
    ...
    /// 2. Config file/env enables it ([relay] enabled or GBUILD_RELAY_SYNC_ENABLED)
    ...
    pub fn plugin_registry_snapshot(
        &self,
    ) -> Option<std::sync::Arc<gbuild_agent::plugins::PluginRegistry>> {
    ...
    pub(super) fn session_config_meta(
        ...
        let detail = session_config::GBuildSessionDetail::build(
    ...
    pub(super) async fn upload_harness_trace_turns(
        ...
        model: &str,
        turns: Vec<Vec<gbuild_sampling_types::conversation::ConversationItem>>,
    ) {
    ...
    pub(crate) async fn get_trace_context(
        ...
        let upload_method = match upload_method {
            Some(method) => method,
            None => {
                gbuild_telemetry::session_ctx::log_session_event(crate::agent::session_metrics::TraceUploadSkipped {
    ...
    /// 5. `GBUILD_AGENT` env var.
    /// 6. Built-in default agent.
    ///
    /// `GBUILD_AGENT` and an explicit `[agent] name` bypass step 1.
    /// Strict-harness classification is structural — see
    /// [`gbuild_agent::config::is_strict_harness_agent_type`].
    ///
    ...
    pub fn resolve_agent_definition(
        ...
        acp_agent_profile: Option<gbuild_agent::AgentDefinition>,
        model_agent_type: Option<&str>,
    ) -> gbuild_agent::AgentDefinition {
        use gbuild_agent::AgentDefinition;
        let gbuild_agent_env_set = std::env::var("GBUILD_AGENT")
            .ok()
            .is_some_and(|s| !s.trim().is_empty());
        let config_agent_explicitly_set = agent_config.name.is_some();
        let model_requires_strict_harness = model_agent_type
            .is_some_and(gbuild_agent::config::is_strict_harness_agent_type);
        if !gbuild_agent_env_set && !config_agent_explicitly_set
            && model_requires_strict_harness && let Some(required) = model_agent_type
            && let Some(def) = gbuild_agent::discovery::by_name_in_cwd(required, cwd)
        {
            tracing::info!(
                agent_name = %def.name,
        ...
        if let Some(ref name) = agent_config.name {
            ...
            if let Some(def) = gbuild_agent::discovery::by_name_in_cwd(name, cwd) {
                return def;
            }
            tracing::warn!(
                agent_name = %name,
        ...
        let agent_name = std::env::var("GBUILD_AGENT").ok();
        let resolved = match agent_name.as_deref() {
            Some("browser-use") | Some("browser_use") => AgentDefinition::browser_use(),
            Some("gbuild-concise") | Some("gbuild_concise") => {
                AgentDefinition::gbuild_concise()
            }
            Some(path) if std::path::Path::new(path).is_absolute() => {
                match AgentDefinition::from_file(path) {
                    ...
                    Err(e) => {
                        ...
                        AgentDefinition::gbuild_plan()
                    }
                }
            }
            Some(name) => {
                gbuild_agent::discovery::by_name_in_cwd(name, cwd)
                    .unwrap_or_else(AgentDefinition::gbuild_plan)
            }
            None => AgentDefinition::gbuild_plan(),
        };
        if !gbuild_agent_env_set && !config_agent_explicitly_set
            && model_requires_strict_harness && let Some(required) = model_agent_type
            && resolved.name != required
        {
            ...
            if let Some(def) = gbuild_agent::discovery::by_name_in_cwd(required, cwd) {
                return def;
            }
            tracing::warn!(
                model_agent_type = %required,
    ...
    pub(super) async fn spawn_and_register_session(
        ...
        let fs: Arc<dyn gbuild_workspace::file_system::AsyncFileSystem> = if use_acp_fs {
        ...
        let loc_tracking_enabled = hunk_tracking_enabled && has_xai_auth
            && (self
                ...
                .unwrap_or(false)
                || std::env::var("GBUILD_LOC_TRACKING")
        ...
        let pinned_model: Option<(acp::ModelId, ModelEntry)> = match &agent_definition
            .model
        {
            gbuild_agent::config::ModelOverride::Override(id) => {
            ...
            gbuild_agent::config::ModelOverride::Inherit => None,
        };
        ...
        if lsp_tools_enabled && tool_ctx.lsp.is_none() {
            ...
            let (
                ...
                .unzip();
            let sourced = gbuild_tools::implementations::lsp::config::load_servers_with_plugins_sourced(
            ...
            if servers.is_empty() {
                let user_path = gbuild_tools::util::gbuild_home::gbuild_home()
                    .join("lsp.json");
                let project_path = tool_ctx.cwd.as_path().join(".gbuild").join("lsp.json");
                tracing::warn!(
                    cwd = %tool_ctx.cwd,
        ...
        let (mut handle, permission_events_rx, agent_system_prompt, session_thread) = {
            ...
            let attribution_callback: Option<
                gbuild_sampler::SharedAttributionCallback,
            > = Some(
            ...
            let agent_hook_registry_override = agent_definition
                ...
                .and_then(|hooks_config| {
                    ...
                    let (specs, errors) = gbuild_hooks::config::parse_hooks_from_value_with_dir(
                        ...
                        &format!(
                        "{}{}",
                        gbuild_hooks::config::AGENT_HOOK_PREFIX,
            ...
            spawn_session_on_thread(
                    ...
                    subagent_toggle,
                    Vec::new(),
                    gbuild_agent::prompt::context::PromptAudience::Primary,
                    None,
                    None,
                    disable_web_search,
