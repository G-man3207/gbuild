...
use gbuild_telemetry::id::{agent_id, agent_instance_id};
use gbuild_telemetry::session_ctx::log_event;
use gbuild_workspace::file_system::{AcpSessionFs, CodebaseIndexManager, LocalFs};
use gbuild_workspace::permission::{ClientType, PermissionEvent};
...
use gbuild_workspace::session::git::GitDiscoveryResult;
...
pub(crate) struct PromptResponseMetaArgs<'a> {
    ...
    pub total_tokens: u64,
    pub model_id: &'a str,
    pub last_turn_usage: Option<&'a gbuild_sampling_types::TokenUsage>,
...
pub struct MvpAgent {
    ...
    /// **non-recursive** watch on `<cwd>/` and `<cwd>/.gbuild/`).
    ///
    ...
    /// Local session-registry override: `GBUILD_SESSION_REGISTRY` env, else
    ...
    monitor_event_buffer: gbuild_tools::implementations::gbuild::task::types::MonitorEventBuffer,
    ...
    workspace_ops: RefCell<Option<gbuild_workspace::WorkspaceOps>>,
...
pub(crate) fn inherited_harness_template(
    current: &gbuild_agent::prompt::user_message::UserMessageTemplate,
    pinned_model_agent_type: Option<&str>,
    cwd: &std::path::Path,
) -> Option<gbuild_agent::prompt::user_message::UserMessageTemplate> {
    ...
    if !gbuild_agent::config::is_strict_harness_agent_type(agent_type) {
        return None;
    }
    let harness = gbuild_agent::discovery::by_name_in_cwd(agent_type, cwd)?;
...
/// unchanged — compatible stock switches (e.g. `gBuild` →
/// `gbuild-plan`) intentionally preserve the session's original ACP
/// `agentProfile`.
...
/// default wire format and toolset, so switching e.g. `gBuild` →
/// `gbuild-plan` doesn't require rebuilding the harness and would
...
pub(crate) fn harnesses_are_compatible(active: &str, required: &str) -> bool {
    use gbuild_agent::config::is_strict_harness_agent_type;
    match (
...
impl MvpAgent {
    ...
    pub(super) fn reconcile_stale_background_tasks(
        ...
        for task in &orphaned {
            let snapshot = gbuild_tools::types::TaskSnapshot {
    ...
    pub(super) async fn enforce_grok_code_access(&self, auth: &crate::auth::GrokAuth) {
        ...
        if !allow {
            tracing::info!(
                "auth: user blocked by allow_access (remote settings gbuild_access_gate)"
            );
    ...
    ///    against `~/.gbuild/bundled/` and the manifest.
...
/// absent — matching the `gbuild_access_gate` flag's server-side default.
///
