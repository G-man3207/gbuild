...
/// `GBUILD_DISABLE_API_KEY_AUTH`) is the admin kill switch: when true the
...
pub const PREFERRED_OIDC_UNAVAILABLE: &str =
    "preferred_method=oidc but no session is available. Run `gbuild login` to authenticate.";
...
pub fn cached_token_auth_method() -> acp::AuthMethod {
    acp::AuthMethod::Agent(
        ...
        .description(Some("Cached token from ~/.gbuild/auth.json".to_string())),
    )
}
...
mod tests {
    ...
    /// `~/.gbuild/config.toml` skeleton from the bug report, walks it through
    ...
    // -- gbuild login --legacy regression coverage ------------------------
    //
    // `gbuild login --legacy` produces a GrokAuth with `auth_mode: WebLogin`,
    // `oidc_issuer: None`, and no `expires_at` (30-day hardcoded TTL).
    // When this token is present via the `GBUILD_AUTH` env var (or via legacy
    ...
    // to-end. A regression in GBUILD_AUTH JSON parsing or in auth method
    // ordering would send legacy-token users to the login screen.

    /// END-TO-END REGRESSION TEST: a legacy auth token (WebLogin, no
    /// expires_at) present in the `GBUILD_AUTH` env var, with no other auth
    ...
    fn grok_login_legacy_token_does_not_require_login() {
        ...
        let _g1 = EnvGuard::unset("GBUILD_AUTH_PATH");
        let _g2 = EnvGuard::unset(XAI_API_KEY_ENV_VAR);

        // Construct a legacy-style token exactly as `gbuild login --legacy`
        ...
        // Provide it via GBUILD_AUTH env var (highest priority code path in
        ...
        let _g = EnvGuard::set("GBUILD_AUTH", &legacy_json);
        ...
        assert!(
            current.is_some(),
            "legacy token in GBUILD_AUTH env MUST be loaded directly -- if this fails, \
    ...
    fn no_legacy_token_means_no_cached_token_advertised() {
        ...
        let _g1 = EnvGuard::unset("GBUILD_AUTH");
        let _g2 = EnvGuard::unset("GBUILD_AUTH_PATH");
