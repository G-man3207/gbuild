...
use gbuild_agent::prompt::skills::SkillsConfig;
use gbuild_sampler::{AuthScheme, SamplerConfig};
...
pub const DEFAULT_AGENT_TYPE: &str = "gbuild-plan";
...
pub struct EndpointsConfig {
    ...
    /// Env: `GBUILD_MODELS_BASE_URL`. Enables custom endpoint mode.
    /// List URL defaults to `{models_base_url}/models`.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub models_base_url: Option<String>,
    /// Env: `GBUILD_MODELS_LIST_URL`. Overrides the default `{base}/models` list URL.
    #[serde(alias = "models_endpoint", skip_serializing_if = "Option::is_none")]
    pub models_list_url: Option<String>,
    /// Env: `GBUILD_FEEDBACK_BASE_URL`. Where feedback submissions go.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub feedback_base_url: Option<String>,
    /// Env: `GBUILD_TRACE_UPLOAD_URL`. Where trace uploads go.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub trace_upload_url: Option<String>,
    /// Env: `GBUILD_TRACE_UPLOAD_BUCKET`. Direct bucket (`gs://` or `s3://`), bypasses proxy.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub trace_upload_bucket: Option<String>,
    /// Env: `GBUILD_TRACE_UPLOAD_REGION`. AWS region (S3 only).
    #[serde(skip_serializing_if = "Option::is_none")]
    pub trace_upload_region: Option<String>,
    /// Env: `GBUILD_TRACE_UPLOAD_CREDENTIALS_FILE`. Path to GCS SA key or AWS credentials file.
    #[serde(skip_serializing_if = "Option::is_none")]
    ...
    /// Env: `GBUILD_TRACE_UPLOAD_ENDPOINT_URL`. Custom S3-compatible endpoint.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub trace_upload_endpoint_url: Option<String>,
    /// Env: `GBUILD_DEPLOYMENT_KEY`. Management API key for enterprise deployments.
    /// Sent on telemetry and service requests for deployment-level attribution.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub deployment_key: Option<String>,
    /// Env: `GBUILD_MANAGED_CONFIG_URL`. Override the managed config endpoint.
    ...
    /// favor of `GBUILD_INTERNAL_OTLP_TRACES_ENDPOINT`, and ignored by the internal
    /// pipeline when `GBUILD_EXTERNAL_OTEL` is set (the standard `OTEL_*` vars then
    ...
    /// Env: `GBUILD_INTERNAL_OTLP_TRACES_ENDPOINT`. Full INTERNAL traces endpoint,
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub gbuild_internal_otlp_traces_endpoint: Option<String>,
    /// Env: `GBUILD_INTERNAL_OTLP_HEADERS`. `k=v,k2=v2` extra headers for the
    /// internal export (debug). Wins over the legacy `OTEL_EXPORTER_OTLP_HEADERS`.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub gbuild_internal_otlp_headers: Option<String>,
    /// External-OTEL master switch, captured at construction via
    /// [`external_otel_master_switch_resolved`] — the same layered resolution
    /// (requirement pin > `GBUILD_EXTERNAL_OTEL` env > `[telemetry].otel_enabled`
...
pub(crate) fn default_asset_server_url() -> String {
    std::env::var("GBUILD_ASSET_SERVER_URL").unwrap_or_else(|_| ASSET_SERVER_URL_DEFAULT.to_owned())
}
...
/// format, shared with `GBUILD_INTERNAL_OTLP_HEADERS`): split on `,`,
...
impl EndpointsConfig {
    ...
    /// Managed deployment-config URL (`gbuild setup`): explicit `managed_config_url`,
    ...
    /// 1. `gbuild_internal_otlp_traces_endpoint` (verbatim)
    ...
    pub fn resolve_otlp_traces_endpoint(&self) -> String {
        if let Some(full) = blank_as_unset(&self.gbuild_internal_otlp_traces_endpoint) {
            ...
            && let Some(legacy) = self.legacy_internal_otlp_traces_endpoint()
        {
            tracing::warn!(
                "Repointing the internal trace pipeline via OTEL_EXPORTER_OTLP_ENDPOINT / \
                 ...
                 GBUILD_INTERNAL_OTLP_TRACES_ENDPOINT instead — the standard OTEL_* vars will \
    ...
    /// Extra headers for the INTERNAL export: `gbuild_internal_otlp_headers`
    ...
    pub fn resolve_otlp_headers(&self) -> Vec<(String, String)> {
        if let Some(headers) = blank_as_unset(&self.gbuild_internal_otlp_headers) {
    ...
    /// `gbuild_internal_otlp_*` override shadowed that half.
    ///
    ...
    pub fn internal_otlp_consumed_standard_vars(&self) -> bool {
        ...
        let endpoint_consumed = blank_as_unset(&self.gbuild_internal_otlp_traces_endpoint)
            .is_none()
            && self.legacy_internal_otlp_traces_endpoint().is_some();
        let headers_consumed = blank_as_unset(&self.gbuild_internal_otlp_headers).is_none()
    ...
    /// still honored by the internal pipeline even with `GBUILD_EXTERNAL_OTEL`
...
impl Default for EndpointsConfig {
    fn default() -> Self {
        Self {
            cli_chat_proxy_base_url: std::env::var("GBUILD_CLI_CHAT_PROXY_BASE_URL").ok(),
            xai_api_base_url: std::env::var("GBUILD_XAI_API_BASE_URL")
                .unwrap_or_else(|_| XAI_API_BASE_URL_DEFAULT.to_owned()),
            alpha_test_key: None,
            models_base_url: env_string("GBUILD_MODELS_BASE_URL"),
            models_list_url: env_string("GBUILD_MODELS_LIST_URL"),
            feedback_base_url: env_string("GBUILD_FEEDBACK_BASE_URL"),
            trace_upload_url: env_string("GBUILD_TRACE_UPLOAD_URL"),
            trace_upload_bucket: env_string("GBUILD_TRACE_UPLOAD_BUCKET"),
            trace_upload_region: env_string("GBUILD_TRACE_UPLOAD_REGION"),
            trace_upload_credentials_file: env_string("GBUILD_TRACE_UPLOAD_CREDENTIALS_FILE"),
            trace_upload_credentials: None,
            trace_upload_endpoint_url: env_string("GBUILD_TRACE_UPLOAD_ENDPOINT_URL"),
            deployment_key: env_string("GBUILD_DEPLOYMENT_KEY"),
            managed_config_url: env_string("GBUILD_MANAGED_CONFIG_URL"),
            ...
            gbuild_internal_otlp_traces_endpoint: env_string(
                "GBUILD_INTERNAL_OTLP_TRACES_ENDPOINT",
            ),
            gbuild_internal_otlp_headers: env_string("GBUILD_INTERNAL_OTLP_HEADERS"),
...
pub use gbuild_config_types::{BoolFlag, ConfigSource, LazinessDetectorPerModelConfig, Resolved};
...
/// environment so it can't inherit gBuild's first-party keys.
/// requests. Keep in sync with every first-party credential env read across the
/// crate: `auth::manager` (`GBUILD_AUTH`/`GBUILD_AUTH_PATH`), `auth_method`
/// (`XAI_API_KEY`), and the credential-bearing `env_string(...)` reads in
...
pub use gbuild_config::env_bool;
...
fn resolve_compat_cell(
    ...
    resolve_compat_cell_with_env(gbuild_config::env_bool(env), cfg, remote, default)
}
...
pub(crate) fn compat_config_cell(
    ...
    cell: gbuild_tools::types::compat::CompatCell,
...
pub(crate) use gbuild_telemetry::config::env_telemetry_mode;
pub use gbuild_telemetry::config::{TelemetryConfig, TelemetryMode};
...
impl PluginsConfig {
    ...
    /// Native `.gbuild/config.toml` entries already present take precedence:
    ...
    pub fn merge_claude_enabled_plugins(&mut self, _cwd: Option<&std::path::Path>) {
        ...
        for path in &paths {
            let (claude_enabled, claude_disabled) =
                gbuild_agent::plugins::marketplace::load_enabled_disabled_plugins(path);
    ...
    pub fn to_discovery_config(&self) -> gbuild_agent::plugins::discovery::DiscoveryConfig {
        gbuild_agent::plugins::discovery::DiscoveryConfig {
...
pub struct CliConfig {
    ...
    /// Env `GBUILD_MINIMUM_VERSION`. See [`crate::util::config::VersionPolicy`] for
    /// the version-policy knobs. (Unrelated to
    /// `version_overrides[].maximum_version`, which gates config patches.)
    #[serde(skip_serializing_if = "Option::is_none")]
    pub minimum_version: Option<String>,
    /// Env `GBUILD_MAXIMUM_VERSION`. See [`crate::util::config::VersionPolicy`].
    #[serde(skip_serializing_if = "Option::is_none")]
    pub maximum_version: Option<String>,
    /// Env `GBUILD_REQUIRED_MINIMUM_VERSION`. See [`crate::util::config::VersionPolicy`].
    #[serde(skip_serializing_if = "Option::is_none")]
    pub required_minimum_version: Option<String>,
    /// Env `GBUILD_REQUIRED_MAXIMUM_VERSION`. See [`crate::util::config::VersionPolicy`].
    #[serde(skip_serializing_if = "Option::is_none")]
...
/// (`gbuild workspace` / leader `with_default_hub_url`). Does **not** enable
...
pub struct HubConfig {
    ...
    /// `gbuild workspace start` when the CLI does not pass `--hub-url`.
    #[serde(skip_serializing_if = "Option::is_none")]
...
impl SandboxSettingsConfig {
    ...
    pub fn resolve_profile(
        ...
        resolve_string_flag(cli_arg, "GBUILD_SANDBOX", self.profile.as_deref(), None)
    ...
    pub fn resolve_auto_allow_bash(&self, requirement: Option<bool>) -> Resolved<bool> {
        BoolFlag::env("GBUILD_SANDBOX_AUTO_ALLOW_BASH")
...
impl SuggestionsConfig {
    pub fn resolve_enabled(
        ...
        BoolFlag::env("GBUILD_SUGGESTIONS")
            .config(self.enabled)
    ...
    pub fn resolve_ai_enabled(
        ...
        BoolFlag::env("GBUILD_SUGGESTIONS_AI")
...
///
/// These supplement the built-in scan locations (`.gbuild/skills/`,
/// `.agents/skills/`, `~/.gbuild/skills/`). They're written by `/import-claude`
...
pub struct Config {
    ...
    #[serde(default, skip_serializing)]
    pub announcements: Vec<gbuild_announcements::RemoteAnnouncement>,
    ...
    /// `[marketplace]` — also read by `gbuild_plugin_marketplace::load_sources()`.
    #[serde(default, skip_serializing)]
    ...
    /// Defaults to reading from GBUILD_STORAGE_MODE env var.
    #[serde(skip)]
    ...
    #[serde(skip)]
    pub cli_agents: Vec<gbuild_agent::config::AgentDefinition>,
    #[serde(skip)]
    pub cli_agent_overrides: CliAgentOverrides,
    /// Whether subagent (task tool) support is enabled. Enabled by default;
    /// disabled only via `GBUILD_SUBAGENTS=0` or `[subagents] enabled = false`.
    /// Not remotely gated.
    #[serde(skip)]
    ...
    #[serde(skip)]
    pub subagent_roles:
        std::collections::HashMap<String, gbuild_subagent_resolution::config::SubagentRole>,
    /// Trust-independent personas from inline, user, and bundled sources.
    #[serde(skip)]
    pub subagent_personas:
        std::collections::HashMap<String, gbuild_subagent_resolution::config::SubagentPersona>,
    ...
    pub zdr_video_output_s3:
        Option<gbuild_tools::implementations::gbuild::video_gen::ZdrVideoOutputS3Config>,
...
pub struct CliAgentOverrides {
    ...
    pub permission_rules: Vec<gbuild_workspace::permission::types::PermissionRule>,
    pub max_turns: Option<u32>,
    pub permission_mode: Option<gbuild_agent::config::PermissionMode>,
}
impl CliAgentOverrides {
    ...
    pub fn apply_to_definition(&self, def: &mut gbuild_agent::config::AgentDefinition) {
    ...
    pub fn apply_to_subagent_definition(&self, def: &mut gbuild_agent::config::AgentDefinition) {
...
pub use gbuild_agent::config::AgentDefinition;
pub use gbuild_agent::config::Effort;
pub use gbuild_agent::config::PermissionMode;
pub use gbuild_shared::ui_config::{ContextualHints, UiConfig};
...
/// ```toml
/// [agent]
/// # Use a named agent (looked up via discovery: .gbuild/agents/, ~/.gbuild/agents/, built-ins)
...
/// 4. `GBUILD_AGENT` env var
/// 5. Default `gbuild` agent
#[derive(Debug, Clone, Default, Serialize, Deserialize)]
#[serde(default)]
pub struct AgentSelectionConfig {
    /// Name of a built-in or discovered agent definition.
    /// Looked up via `gbuild_agent::discovery::by_name_in_cwd()`.
    /// Examples: "gbuild", "browser-use", or a custom agent name.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub name: Option<String>,
    /// Path to an agent definition file (.md with YAML frontmatter).
    /// When set, the agent is loaded from this file.
    /// Supports environment variable expansion (e.g., `$HOME/.gbuild/agents/my-agent.md`).
    #[serde(skip_serializing_if = "Option::is_none")]
...
impl Default for Config {
    fn default() -> Self {
        ...
        let mut cfg = Self {
            ...
            client_version: Some(gbuild_version::VERSION.to_string()),
...
/// (surfaced by `gbuild inspect`) and is skipped, so it fails closed for the
...
impl Config {
    ...
    pub fn resolve_runtime_fields(&mut self, ctx: &RuntimeResolutionContext<'_>) {
        ...
        self.auto_wake_enabled = BoolFlag::env("GBUILD_AUTO_WAKE")
    ...
    fn apply_env_overrides(&mut self) {
        ...
        if let Some(mode) = env_telemetry_mode("GBUILD_TELEMETRY_ENABLED") {
    ...
    /// config.toml key, or `GBUILD_TWO_PASS_COMPACTION` env.
    ...
    pub(crate) fn resolve_trace_upload(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("GBUILD_TELEMETRY_TRACE_UPLOAD")
    ...
    pub(crate) fn trace_upload_decision_debug(&self) -> serde_json::Value {
        ...
        serde_json::json!({
            ...
            "in_env_trace_upload": std::env::var("GBUILD_TELEMETRY_TRACE_UPLOAD").ok(),
            "in_env_telemetry_enabled": std::env::var("GBUILD_TELEMETRY_ENABLED").ok(),
    ...
    pub(crate) fn resolve_feedback(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("GBUILD_FEEDBACK_ENABLED")
    ...
    pub(crate) fn resolve_two_pass_compaction(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("GBUILD_TWO_PASS_COMPACTION")
    ...
    /// `GBUILD_DOOM_LOOP_RECOVERY` > TOML `enabled` > remote `enabled` >
    ...
    pub(crate) fn resolve_doom_loop_recovery(
        &self,
    ) -> Option<gbuild_sampling_types::DoomLoopRecoveryPolicy> {
        use gbuild_sampling_types::DoomLoopRecoveryPolicy as Policy;
        let remote = self
            .remote_settings
            .as_ref()
            .and_then(|s| s.doom_loop_recovery.as_ref());
        let enabled = BoolFlag::env("GBUILD_DOOM_LOOP_RECOVERY")
    ...
    /// Precedence: env `GBUILD_OFFICIAL_MARKETPLACE_AUTO_REGISTER` > remote settings >
    ...
    pub(crate) fn resolve_official_marketplace_auto_register(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("GBUILD_OFFICIAL_MARKETPLACE_AUTO_REGISTER")
            .feature_flag(ff)
            .default(false)
            .resolve()
    }
    ...
    pub(crate) fn resolve_ask_user_question(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("GBUILD_ASK_USER_QUESTION")
    ...
    /// Precedence: env `GBUILD_IMAGE_GEN_MODEL_OVERRIDE` > `[features]
    ...
    pub(crate) fn resolve_image_gen_model_override(&self) -> Option<String> {
        resolve_string_flag(
            None,
            "GBUILD_IMAGE_GEN_MODEL_OVERRIDE",
    ...
    pub(crate) fn resolve_image_edit_model_override(&self) -> Option<String> {
        resolve_string_flag(
            None,
            "GBUILD_IMAGE_EDIT_MODEL_OVERRIDE",
    ...
    pub(crate) fn resolve_goal(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("GBUILD_GOAL")
            .config(self.goal.enabled)
            .feature_flag(ff)
            .default(true)
            .resolve()
    }
    /// Background workflows (`workflow` tool, `.gbuild/workflows/*.rhai`,
    ...
    pub(crate) fn resolve_goal_classifier_enabled(&self, goal_enabled: bool) -> Resolved<bool> {
        BoolFlag::env("GBUILD_GOAL_CLASSIFIER")
    ...
    pub(crate) fn resolve_goal_planner_enabled(&self, goal_enabled: bool) -> Resolved<bool> {
        BoolFlag::env("GBUILD_GOAL_PLANNER")
    ...
    pub(crate) fn resolve_goal_summary_enabled(&self, goal_enabled: bool) -> Resolved<bool> {
        BoolFlag::env("GBUILD_GOAL_SUMMARY")
    ...
    pub(crate) fn resolve_goal_use_current_model_only(&self) -> Resolved<bool> {
        BoolFlag::env("GBUILD_GOAL_USE_CURRENT_MODEL_ONLY")
    ...
    pub(crate) fn resolve_write_file(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("GBUILD_WRITE_FILE")
    ...
    pub(crate) fn resolve_backend_tools(&self) -> Resolved<bool> {
        BoolFlag::env("GBUILD_BACKEND_SEARCH")
            .config(self.features.backend_tools)
            .default(true)
            .resolve()
    }
    /// Resolve the mode (env `GBUILD_COMPACTION_MODE` > config > remote settings >
    ...
    pub(crate) fn resolve_compaction_mode(&self) -> xai_chat_state::CompactionMode {
        resolve_compaction_mode_from(
            env_string("GBUILD_COMPACTION_MODE").as_deref(),
    ...
    /// Resolve verbatim-input flag: env `GBUILD_COMPACTION_VERBATIM_INPUT` > config > remote settings > default `true`.
    pub(crate) fn resolve_compaction_verbatim_input(&self) -> bool {
        BoolFlag::env("GBUILD_COMPACTION_VERBATIM_INPUT")
    ...
    /// Precedence: env `GBUILD_COMPACTION_DETAIL`, then config
    ...
    fn resolve_compaction_detail(&self) -> xai_chat_state::CompactionDetail {
        resolve_compaction_detail_from(
            env_string("GBUILD_COMPACTION_DETAIL").as_deref(),
    ...
    pub fn resolve_cancel_rewind(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("GBUILD_CANCEL_REWIND")
    ...
    ///
    /// Priority: `--oauth` > GBUILD_OAUTH_ENABLED env > default (true = OAuth).
    pub fn resolve_grok_oauth(&self, cli_oidc: Option<bool>) -> Resolved<bool> {
        BoolFlag::env("GBUILD_OAUTH_ENABLED")
            .cli(cli_oidc)
            .default(true)
            .resolve()
    }
    ...
    /// narrow non-recursive watches for `<cwd>/` and `<cwd>/.gbuild/`.
    ///
    /// Thin delegate to the canonical
...
///
/// `requirement > cli > env (GBUILD_MCP_LIVENESS_WATCHERS) > config >
...
pub fn resolve_mcp_liveness_watchers(
    ...
    BoolFlag::env("GBUILD_MCP_LIVENESS_WATCHERS")
        .requirement(requirement)
        .cli(cli)
        .config(config)
        .managed(managed)
...
/// `BoolFlag` precedence:
///
/// `requirement > cli > env (GBUILD_MCP_AUTO_RESTART) > config >
/// managed > feature_flag > default (true)`.
///
...
/// precedence is single-sourced.
///
/// Recovery is on by default; opt out via `GBUILD_MCP_AUTO_RESTART=false`,
...
pub fn resolve_mcp_auto_restart(
    ...
    BoolFlag::env("GBUILD_MCP_AUTO_RESTART")
        .requirement(requirement)
        .cli(cli)
        .config(config)
        .managed(managed)
...
///
/// `requirement > cli > env (GBUILD_MCP_PUSH_SERVER_STATUS) > config >
/// managed > feature_flag > default (true)`.
///
...
pub fn resolve_mcp_push_server_status(
    ...
    BoolFlag::env("GBUILD_MCP_PUSH_SERVER_STATUS")
        .requirement(requirement)
        .cli(cli)
        .config(config)
        .managed(managed)
...
/// [`resolve_mcp_liveness_watchers`]:
///
/// `requirement > cli > env (GBUILD_MCP_RECURSIVE_CONFIG_WATCH) >
...
/// operators flip this flag (e.g. via `GBUILD_MCP_RECURSIVE_CONFIG_
...
pub fn resolve_mcp_recursive_config_watch(
    ...
    BoolFlag::env("GBUILD_MCP_RECURSIVE_CONFIG_WATCH")
        .requirement(requirement)
        .cli(cli)
        .config(config)
        .managed(managed)
...
impl SyncBoolFlag {
    ...
    /// Either-direction env resolver (typically `GBUILD_*`). Returns
...
pub fn is_telemetry_disabled_sync() -> bool {
    !SyncBoolFlag::new(telemetry_enabled_from_toml)
        ...
        .enable_env(gbuild_telemetry_env_enabled)
        .resolve()
}
...
pub fn is_error_reporting_disabled_sync() -> bool {
    !SyncBoolFlag::new(error_reporting_enabled_from_toml)
        ...
        .enable_env(|| env_bool("GBUILD_ERROR_REPORTING"))
...
/// `GBUILD_TELEMETRY_ENABLED` resolved through `TelemetryMode::parse` so the
/// extended string forms (e.g. `"session_metrics"`) are accepted.
fn gbuild_telemetry_env_enabled() -> Option<bool> {
    env_telemetry_mode("GBUILD_TELEMETRY_ENABLED").map(|m| !m.is_disabled())
}
/// Load `~/.gbuild/requirements.toml` standalone so the admin pin can beat
/// env vars. The merged config layer can't express that — last-merge-wins
/// loses provenance.
pub(crate) fn read_requirements_toml() -> Option<toml::Value> {
    let path = crate::util::gbuild_home::gbuild_home().join("requirements.toml");
    let content = std::fs::read_to_string(&path).ok()?;
    toml::from_str(&content).ok()
}
/// Resolve the external-OTEL master switch exactly the way the external
/// stream's activation does: **requirement pin > `GBUILD_EXTERNAL_OTEL` env >
...
pub(crate) fn external_otel_master_switch_resolved() -> bool {
    external_otel_master_switch_from(
        gbuild_config::load_merged_requirements().as_ref(),
        env_bool("GBUILD_EXTERNAL_OTEL"),
...
/// by `gbuild setup`) sit under the env vars, requirements pins are applied on
...
pub fn resolve_external_otel_config(
    client: gbuild_telemetry::external::config::ExternalClientInfo,
) -> Option<gbuild_telemetry::external::ExternalOtelConfig> {
...
pub(crate) fn resolve_external_otel_config_with(
    ...
    let file_cfg: Option<gbuild_telemetry::external::ExternalOtelFileConfig> = effective_config
        .and_then(|cfg| cfg.get("telemetry"))
        .map(|t| gbuild_telemetry::external::ExternalOtelFileConfig {
    ...
    let getenv_pinned = |name: &str| -> Option<String> {
        let pin = match name {
            gbuild_telemetry::external::config::ENV_MASTER_SWITCH => req_enabled,
    ...
    let mut resolved = gbuild_telemetry::external::ExternalOtelConfig::resolve_with(
        getenv_pinned,
        file_cfg.as_ref(),
    )?;
...
pub fn apply_external_otel_remote_policy(settings: Option<&crate::util::config::RemoteSettings>) {
    ...
    let policy = gbuild_telemetry::external::ExternalOtelRemotePolicy {
    ...
    if policy.force_disable || policy.lock_content_gates {
        gbuild_telemetry::external::apply_remote_policy(policy);
    }
}
...
pub fn apply_remote_settings_side_effects(settings: Option<&crate::util::config::RemoteSettings>) {
    if let Some(s) = settings {
        ...
        gbuild_config::signed_policy::apply_remote_managed_config_signature_verification(
...
fn managed_settings_env_flag(key: &str) -> Option<bool> {
    let path = gbuild_config::claude_managed_settings_path()?;
    let content = std::fs::read_to_string(&path).ok()?;
    let json: serde_json::Value = serde_json::from_str(&content).ok()?;
    gbuild_workspace::permission::resolution::json_env_flag(json.get("env"), key)
}
...
pub struct ModelEntryConfig {
    ...
    /// Can also be set via the `GBUILD_MAX_RETRIES` environment variable.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub max_retries: Option<u32>,
...
pub struct ModelInfo {
    ...
    /// Always has a value; defaults to `"gbuild-plan"` when the server
...
pub struct AutoModeConfig {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub prompt_type: Option<gbuild_workspace::permission::ClassifierPromptType>,
...
pub struct Features {
    ...
    /// Show a blocking warning when gBuild starts outside a Git repository.
    /// Default: false. Used as the local fallback when the `non_git_warning` remote settings
    /// flag in `gbuild_settings` is absent. When the remote flag is present it takes
    ...
    /// env (`GBUILD_COMPACTION_MODE`). Parsed via `CompactionMode::parse`.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub compaction_mode: Option<String>,
    /// `none` | `minimal` | `balanced` | `verbose` (default). `None` = defer to
    /// env (`GBUILD_COMPACTION_DETAIL`). The `segments` verbatim detail level.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    ...
    /// `GBUILD_MCP_AUTO_RESTART` to opt out).
    ///
...
pub fn enforce_disable_api_key_auth(
        ...
        && crate::util::is_xai_api_url(&creds.base_url)
    {
        ...
        gbuild_telemetry::unified_log::debug(
...
pub use gbuild_telemetry::config::deployment_id_from_key;
...
mod tests {
    ...
    fn main_cli_tools_override_preserves_profile_injection_policy() {
        ...
        let mut cases = vec![(AgentDefinition::default_gbuild(), true)];
    ...
    fn auto_mode_config_parses_from_toml_and_json_equivalently() {
        use gbuild_workspace::permission::ClassifierPromptType;
        let toml_src = r#"
enabled = true
...
"#;
    ...
    #[test]
    fn resolve_aux_model_honors_gbuild_override() {
        let endpoints = EndpointsConfig::default();
    ...
    fn session_resolver_is_not_stamped_onto_third_party_samplers() {
        ...
        impl gbuild_sampler::BearerResolver for SessionResolver {
    ...
    fn shell_environment_policy_known_keys_track_the_policy_struct() {
        let gbuild_tools::util::ShellEnvironmentPolicy {
            inherit: _,
            ignore_default_excludes: _,
            exclude: _,
            set: _,
            include_only: _,
        } = gbuild_tools::util::ShellEnvironmentPolicy::default();
    ...
    async fn set_env_key_shadows_warm_provider_at_resolve_time() {
        use gbuild_test_support::EnvGuard;
        let var = "GBUILD_TEST_ENVKEY_SHADOW";
    ...
    fn env_keys_resolve_first_set_wins() {
        let keys = EnvKeys::new([
            "GBUILD_TEST_ENV_KEY_PRIMARY",
            "GBUILD_TEST_ENV_KEY_FALLBACK",
        ]);
        assert_eq!(keys.resolve_value_with(|_| None), None, "none set");
        assert_eq!(
            keys.resolve_value_with(
                |n| (n == "GBUILD_TEST_ENV_KEY_FALLBACK").then(|| "from-fallback".into())
            ),
            Some("from-fallback".into())
        );
        assert_eq!(
            keys.resolve_value_with(|n| match n {
                "GBUILD_TEST_ENV_KEY_PRIMARY" => Some("from-primary".into()),
                "GBUILD_TEST_ENV_KEY_FALLBACK" => Some("from-fallback".into()),
                _ => None,
            }),
        ...
        assert_eq!(
            keys.resolve_value_with(|n| match n {
                "GBUILD_TEST_ENV_KEY_PRIMARY" => Some(String::new()),
                "GBUILD_TEST_ENV_KEY_FALLBACK" => Some("from-fallback".into()),
                _ => None,
            }),
            Some("from-fallback".into())
    ...
    fn env_keys_resolve_skips_whitespace_only_value() {
        let keys = EnvKeys::new(["GBUILD_TEST_WS_PRIMARY", "GBUILD_TEST_WS_FALLBACK"]);
        assert_eq!(
            keys.resolve_value_with(|n| match n {
                "GBUILD_TEST_WS_PRIMARY" => Some("   ".into()),
                "GBUILD_TEST_WS_FALLBACK" => Some("real".into()),
                _ => None,
            }),
            Some("real".into())
        );
        assert_eq!(
            EnvKeys::single("GBUILD_TEST_WS_ONLY").resolve_value_with(|_| Some("   ".into())),
            None
        );
        assert_eq!(
            EnvKeys::single("GBUILD_TEST_WS_PAD").resolve_value_with(|_| Some("  tok  ".into())),
            Some("  tok  ".into())
        );
    }
    #[test]
    ...
    fn resolve_credentials_multi_env_key_uses_lc_alias() {
        use xai_chat_state::AuthType;
        let primary = "GBUILD_TEST_MULTI_ENV_PRIMARY";
        let alias = "GBUILD_TEST_MULTI_ENV_LC_ALIAS";
        unsafe {
    ...
    fn resolve_credentials_empty_env_key_does_not_send_session_to_custom_origin() {
        use gbuild_test_support::EnvGuard;
        use xai_chat_state::AuthType;
        let primary = "GBUILD_TEST_EMPTY_ENV_PRIMARY";
        let alias = "GBUILD_TEST_EMPTY_ENV_LC_ALIAS";
    ...
    fn resolve_credentials_empty_env_key_does_not_send_global_key_to_custom_origin() {
        ...
        let primary = "GBUILD_TEST_EMPTY_ENV_GLOBAL_PRIMARY";
        let alias = "GBUILD_TEST_EMPTY_ENV_GLOBAL_ALIAS";
        let _primary = EnvGuard::set(primary, "");
        let _alias = EnvGuard::set(alias, "");
    ...
    fn x_api_key_auth_scheme_flows_from_config_to_sampler() {
        ...
        assert_eq!(config.api_backend, ApiBackend::Messages);
        let client = gbuild_sampler::SamplingClient::new(config).expect("client should build");
    ...
    fn user_override_parses_compaction_at_tokens_from_toml() {
        use gbuild_sampling_types::CompactionAtTokens;
        let dm = crate::models::default_model();
        let raw_config: toml::Value = toml::from_str(&format!(
            r#"
            [model."{dm}"]
    ...
    fn user_override_parses_compactions_remaining_from_toml() {
        use gbuild_sampling_types::CompactionsRemaining;
        let dm = crate::models::default_model();
        let raw_config: toml::Value = toml::from_str(&format!(
            r#"
            [model."{dm}"]
    ...
    /// gbuild-shell/src/agent/models.rs:857 for any BYOK Claude config.
    #[test]
    ...
    #[test]
    fn model_agent_type_defaults_to_gbuild() {
        let raw_config: toml::Value = toml::from_str(
            r#"
            [model.my-model]
            model = "my-model"
    ...
    fn config_models_default_is_not_overwritten_by_default_models_json() {
        ...
        let resolved = resolve_string_flag(
            None,
            "GBUILD_DEFAULT_MODEL_TEST_NONEXISTENT",
            config_default,
    ...
    fn unset_endpoint_env_vars() {
        for k in [
            ...
            "GBUILD_FEEDBACK_BASE_URL",
            "GBUILD_TRACE_UPLOAD_URL",
            "GBUILD_MANAGED_CONFIG_URL",
            "GBUILD_MODELS_BASE_URL",
            "GBUILD_MODELS_LIST_URL",
            "OTEL_EXPORTER_OTLP_ENDPOINT",
            "OTEL_EXPORTER_OTLP_TRACES_ENDPOINT",
            "OTEL_EXPORTER_OTLP_HEADERS",
            "GBUILD_INTERNAL_OTLP_TRACES_ENDPOINT",
            "GBUILD_INTERNAL_OTLP_HEADERS",
            "GBUILD_EXTERNAL_OTEL",
        ] {
    ...
    fn resolve_feedback_defaults_to_false_when_unset() {
        unsafe { std::env::remove_var("GBUILD_FEEDBACK_ENABLED") };
        unsafe { std::env::remove_var("GBUILD_TELEMETRY_ENABLED") };
        let cfg = Config::default();
    ...
    fn resolve_session_recap_defaults_to_true_when_unset() {
        unsafe { std::env::remove_var("GBUILD_SESSION_RECAP") };
        let cfg = Config::default();
    ...
    fn resolve_session_recap_env_off_overrides_default() {
        unsafe { std::env::set_var("GBUILD_SESSION_RECAP", "0") };
        let cfg = Config::default();
        let r = cfg.resolve_session_recap();
        assert!(!r.value);
    ...
    fn resolve_two_pass_compaction_precedence() {
        unsafe { std::env::remove_var("GBUILD_TWO_PASS_COMPACTION") };
        ...
        assert!(r.value);
        assert_eq!(r.source, ConfigSource::Config);
        unsafe { std::env::set_var("GBUILD_TWO_PASS_COMPACTION", "0") };
        let r = config_over_remote.resolve_two_pass_compaction();
    ...
    fn resolve_doom_loop_recovery_precedence() {
        ...
        unsafe { std::env::remove_var("GBUILD_DOOM_LOOP_RECOVERY") };
        let default_cfg = Config::default();
        assert!(
        ...
        unsafe { std::env::set_var("GBUILD_DOOM_LOOP_RECOVERY", "0") };
        assert!(
    ...
    fn resolve_feedback_env_overrides_all() {
        unsafe { std::env::set_var("GBUILD_FEEDBACK_ENABLED", "true") };
        let mut cfg = Config::default();
    ...
    fn resolve_trace_upload_disabled_when_telemetry_off_despite_remote_flag() {
        unsafe { std::env::remove_var("GBUILD_TELEMETRY_ENABLED") };
        unsafe { std::env::remove_var("GBUILD_TELEMETRY_TRACE_UPLOAD") };
        let mut cfg = Config::default();
    ...
    fn resolve_goal_defaults_to_true_when_unset() {
        unsafe { std::env::remove_var("GBUILD_GOAL") };
        let cfg = Config::default();
    ...
    fn resolve_goal_env_overrides_config_without_remote_kill_switch() {
        unsafe { std::env::set_var("GBUILD_GOAL", "1") };
        let mut cfg = Config::default();
    ...
    fn background_workflows_default_on_without_affecting_goal() {
        unsafe { std::env::remove_var("GBUILD_WORKFLOWS") };
        let cfg = Config::default();
    ...
    fn resolve_workflows_remote_false_kills_local_opt_in() {
        unsafe { std::env::set_var("GBUILD_WORKFLOWS", "1") };
        let mut cfg = Config::default();
    ...
    fn resolve_workflows_env_wins() {
        unsafe { std::env::set_var("GBUILD_WORKFLOWS", "0") };
        let cfg = Config::default();
        let r = cfg.resolve_workflows();
        assert_eq!(r.source, ConfigSource::Env);
        assert!(
    ...
    fn resolve_ask_user_question_defaults_to_true_when_unset() {
        unsafe { std::env::remove_var("GBUILD_ASK_USER_QUESTION") };
        let cfg = Config::default();
    ...
    fn resolve_ask_user_question_env_overrides_remote_settings() {
        unsafe { std::env::set_var("GBUILD_ASK_USER_QUESTION", "1") };
        let cfg = Config {
            remote_settings: Some(crate::util::config::RemoteSettings {
    ...
    fn resolve_image_gen_model_override_remote_settings_or_config() {
        unsafe { std::env::remove_var("GBUILD_IMAGE_GEN_MODEL_OVERRIDE") };
    ...
    fn resolve_image_edit_model_override_remote_settings_or_config() {
        unsafe { std::env::remove_var("GBUILD_IMAGE_EDIT_MODEL_OVERRIDE") };
        let with = |config: Option<&str>, gb: Option<&str>| Config {
            features: Features {
    ...
    fn clear_goal_envs() {
        unsafe {
            std::env::remove_var("GBUILD_GOAL");
            std::env::remove_var("GBUILD_GOAL_CLASSIFIER");
            std::env::remove_var("GBUILD_GOAL_PLANNER");
            std::env::remove_var("GBUILD_GOAL_SUMMARY");
            std::env::remove_var("GBUILD_GOAL_VERIFIER_N");
            std::env::remove_var("GBUILD_GOAL_CLASSIFIER_MAX");
            std::env::remove_var("GBUILD_GOAL_STRATEGIST_EVERY");
            std::env::remove_var("GBUILD_GOAL_REVERIFY_AFTER");
        }
    }
    ...
    fn resolve_goal_classifier_env_overrides_default_and_remote() {
        clear_goal_envs();
        unsafe { std::env::set_var("GBUILD_GOAL_CLASSIFIER", "0") };
        let r = cfg_with_goal_and_remote(true, remote_classifier(true))
            .resolve_goal_classifier_enabled(true);
        assert!(!r.value);
        assert_eq!(r.source, ConfigSource::Env);
        unsafe { std::env::set_var("GBUILD_GOAL_CLASSIFIER", "1") };
    ...
    fn resolve_goal_planner_env_overrides_default_and_remote() {
        clear_goal_envs();
        unsafe { std::env::set_var("GBUILD_GOAL_PLANNER", "0") };
        let r =
            cfg_with_goal_and_remote(true, remote_planner(true)).resolve_goal_planner_enabled(true);
        assert!(!r.value);
        assert_eq!(r.source, ConfigSource::Env);
        unsafe { std::env::set_var("GBUILD_GOAL_PLANNER", "1") };
    ...
    fn resolve_goal_summary_env_overrides_default_and_remote() {
        clear_goal_envs();
        unsafe { std::env::set_var("GBUILD_GOAL_SUMMARY", "0") };
        let r =
...
"#,
    ...
    const GOAL_USE_CURRENT_ENV: &str = "GBUILD_GOAL_USE_CURRENT_MODEL_ONLY";
...
[goal]
enabled = true
planner_model = { model = "grok-build", agent_type = "gbuild-plan" }
...
planner_model = { agent_type = "gbuild-plan" }
"#;
        let raw: toml::Value = toml::from_str(toml_str).unwrap();
        let cfg = Config::new_from_toml_cfg(&raw)
...
"#,
        ...
        let gbuild = crate::util::config::GoalRoleModel {
            model: "grok-build".into(),
            agent_type: "gbuild-plan".into(),
        };
        ...
        assert_eq!(planner.value, GoalRoleModelChoice::Explicit(gbuild.clone()));
        ...
        assert_eq!(
            ...
            vec![
                GoalRoleModelChoice::Explicit(gbuild),
    ...
    fn config_accepts_all_known_sections() {
        let unused = unused_keys_from_toml(
            ...
            command = "/opt/bin/gbuild-identity"
            [repo_changes_dedup]
            enabled = false
            [relay]
            enabled = false
    ...
    fn internal_otlp_test_config() -> EndpointsConfig {
        EndpointsConfig {
            ...
            gbuild_internal_otlp_traces_endpoint: None,
            gbuild_internal_otlp_headers: None,
            external_otel_master_switch: false,
            ..Default::default()
        }
    }
    /// `gbuild_internal_otlp_traces_endpoint` wins over the legacy `OTEL_*`
    /// fields regardless of the master switch.
    #[test]
    fn internal_otlp_endpoint_grok_internal_wins_regardless_of_switch() {
        for switch in [false, true] {
            let cfg = EndpointsConfig {
                gbuild_internal_otlp_traces_endpoint: Some(
            ...
            assert_eq!(
                ...
                "switch={switch}: GBUILD_INTERNAL_OTLP_TRACES_ENDPOINT must win verbatim (trailing / trimmed)"
            );
        }
    }
    ...
    fn internal_otlp_consumed_standard_vars_cases() {
        ...
        for case in cases {
            let cfg = EndpointsConfig {
                ...
                gbuild_internal_otlp_traces_endpoint: case
                    .internal_ep
                    .then(|| "https://internal.example/traces".to_string()),
                gbuild_internal_otlp_headers: case.internal_headers.then(|| "ik=iv".to_string()),
                ..internal_otlp_test_config()
            };
            assert_eq!(
    ...
    /// Headers precedence: `gbuild_internal_otlp_headers` wins; legacy
    /// `otel_exporter_otlp_headers` only when the master switch is unset.
    #[test]
    fn internal_otlp_headers_precedence() {
        for switch in [false, true] {
            let cfg = EndpointsConfig {
                gbuild_internal_otlp_headers: Some("x-debug=1".to_string()),
    ...
    fn ext_client() -> gbuild_telemetry::external::config::ExternalClientInfo {
        gbuild_telemetry::external::config::ExternalClientInfo::default()
    }
    #[test]
    fn external_otel_default_off_and_double_opt_in() {
        ...
        assert!(
            resolve_external_otel_config_with(
                None,
                None,
                ext_env(&[("GBUILD_EXTERNAL_OTEL", "1")]),
                ext_client(),
                false,
            )
            .is_none()
    ...
    fn external_otel_file_table_layered_under_env() {
        ...
        assert!(
            resolve_external_otel_config_with(
                Some(&effective),
                None,
                ext_env(&[("GBUILD_EXTERNAL_OTEL", "0")]),
                ext_client(),
                false,
            )
            .is_none()
    ...
    /// (managed config / requirements — no `GBUILD_EXTERNAL_OTEL` env var) must
    ...
    fn clear_runtime_env_vars() {
        unsafe {
            std::env::remove_var("GBUILD_SUBAGENTS");
            std::env::remove_var("GBUILD_RESPECT_GITIGNORE");
            std::env::remove_var("GBUILD_WEB_SEARCH_MODEL");
            std::env::remove_var("GBUILD_SESSION_SUMMARY_MODEL");
            std::env::remove_var("GBUILD_CURSOR_SKILLS_ENABLED");
            std::env::remove_var("GBUILD_CURSOR_RULES_ENABLED");
            std::env::remove_var("GBUILD_CURSOR_AGENTS_ENABLED");
            std::env::remove_var("GBUILD_CLAUDE_SKILLS_ENABLED");
            std::env::remove_var("GBUILD_CLAUDE_RULES_ENABLED");
            std::env::remove_var("GBUILD_CLAUDE_AGENTS_ENABLED");
        }
    }
    fn clear_managed_mcp_env_vars() {
        unsafe {
            std::env::remove_var("GBUILD_MANAGED_MCPS_ENABLED");
            std::env::remove_var("GBUILD_MANAGED_MCP_GATEWAY_TOOLS_ENABLED");
        }
    }
...
"#,
    ...
    fn resolve_raw_compat_sessions_load_failure_allows_env_override() {
        let _env = isolate_compat_env();
        let _codex = EnvGuard::set("GBUILD_CODEX_SESSIONS_ENABLED", "true");
        let resolved = resolve_compat_sessions_from_raw(Err(()), None);
        assert!(!resolved.cursor.sessions);
        assert!(!resolved.claude.sessions);
        assert!(resolved.codex.sessions);
    ...
    fn resolve_compat_env_sessions_disable_independently() {
        ...
        for (vendor, env_var) in [
            (CompatVendor::Cursor, "GBUILD_CURSOR_SESSIONS_ENABLED"),
            (CompatVendor::Claude, "GBUILD_CLAUDE_SESSIONS_ENABLED"),
            (CompatVendor::Codex, "GBUILD_CODEX_SESSIONS_ENABLED"),
        ] {
    ...
    fn resolve_compat_precedence_and_reserved_codex_hook() {
        ...
        let _session = EnvGuard::set("GBUILD_CURSOR_SESSIONS_ENABLED", "true");
        let _hook = EnvGuard::set("GBUILD_CODEX_HOOKS_ENABLED", "true");
        let resolved = resolve_compat_config(&config, Some(&remote));
        assert!(resolved.cursor.sessions);
        assert!(resolved.codex.hooks);
    }
    ...
    fn resolve_runtime_fields_compat_asymmetric_sources() {
        let _env = isolate_compat_env();
        let _cursor = EnvGuard::set("GBUILD_CURSOR_SESSIONS_ENABLED", "false");
        let raw: toml::Value =
    ...
    fn resolve_runtime_fields_gitignore_from_env() {
        clear_runtime_env_vars();
        unsafe { std::env::set_var("GBUILD_RESPECT_GITIGNORE", "0") };
        let raw = empty_config();
        let mut cfg = Config::new_from_toml_cfg(&raw).unwrap();
        cfg.resolve_runtime_fields(&RuntimeResolutionContext {
            raw_config: &raw,
...
"#,
    ...
    fn is_telemetry_explicitly_disabled_sync_env_signals() {
        unsafe { std::env::set_var("GBUILD_TELEMETRY_ENABLED", "0") };
        unsafe { std::env::remove_var("DISABLE_TELEMETRY") };
        assert!(is_telemetry_explicitly_disabled_sync());
        unsafe { std::env::set_var("GBUILD_TELEMETRY_ENABLED", "1") };
        assert!(!is_telemetry_explicitly_disabled_sync());
        unsafe { std::env::remove_var("GBUILD_TELEMETRY_ENABLED") };
...
"#,
        ...
        gbuild_config::apply_version_overrides(&mut value, &v).unwrap();
    ...
    fn mcp_liveness_watchers_default_is_true() {
        unsafe { std::env::remove_var("GBUILD_MCP_LIVENESS_WATCHERS") };
    ...
    fn mcp_liveness_watchers_requirement_wins_over_everything() {
        unsafe { std::env::set_var("GBUILD_MCP_LIVENESS_WATCHERS", "true") };
    ...
    fn mcp_liveness_watchers_env_wins_over_config_and_below() {
        unsafe { std::env::set_var("GBUILD_MCP_LIVENESS_WATCHERS", "false") };
    ...
    fn mcp_auto_restart_default_is_true() {
        unsafe { std::env::remove_var("GBUILD_MCP_AUTO_RESTART") };
    ...
    fn mcp_auto_restart_requirement_wins_over_everything() {
        unsafe { std::env::set_var("GBUILD_MCP_AUTO_RESTART", "false") };
    ...
    fn mcp_auto_restart_env_wins_over_config_and_below() {
        unsafe { std::env::set_var("GBUILD_MCP_AUTO_RESTART", "true") };
    ...
    fn mcp_push_server_status_default_is_true() {
        unsafe { std::env::remove_var("GBUILD_MCP_PUSH_SERVER_STATUS") };
    ...
    fn mcp_push_server_status_requirement_wins_over_everything() {
        unsafe { std::env::set_var("GBUILD_MCP_PUSH_SERVER_STATUS", "true") };
    ...
    fn mcp_push_server_status_env_wins_over_config_and_below() {
        unsafe { std::env::set_var("GBUILD_MCP_PUSH_SERVER_STATUS", "false") };
    ...
    fn mcp_recursive_config_watch_default_is_true() {
        unsafe { std::env::remove_var("GBUILD_MCP_RECURSIVE_CONFIG_WATCH") };
    ...
    fn mcp_recursive_config_watch_requirement_wins_over_everything() {
        unsafe { std::env::set_var("GBUILD_MCP_RECURSIVE_CONFIG_WATCH", "true") };
    ...
    fn mcp_recursive_config_watch_env_wins_over_config_and_below() {
        unsafe { std::env::set_var("GBUILD_MCP_RECURSIVE_CONFIG_WATCH", "false") };
    ...
    fn remote_settings_disarm_managed_config_signatures() {
        ...
        assert!(gbuild_config::signed_policy::verification_active());
    ...
    fn remote_settings_disarm_requires_prod_proxy_when_keys_embedded() {
        ...
        unsafe {
            std::env::remove_var("GBUILD_CLI_CHAT_PROXY_BASE_URL");
        }
        apply_remote_settings_side_effects(Some(&settings));
        assert!(
