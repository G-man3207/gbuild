...
/// auth.json scope key for plain API key auth (desktop login, `gbuild login --api-key`).
...
pub enum AuthMode {
    ...
    External,
    /// Plain API key (e.g. from grok-desktop login or `gbuild login --api-key`)
    ApiKey,
}
...
///
/// Legacy `WebLogin` tokens (from the pre-OIDC `gbuild login --legacy`
...
/// Early-invalidation buffer. Override with `GBUILD_AUTH_EARLY_INVALIDATION_SECS`
/// for testing (e.g. `=5` to shrink the buffer to 5 seconds).
pub(super) fn early_invalidation() -> Duration {
    std::env::var("GBUILD_AUTH_EARLY_INVALIDATION_SECS")
        .ok()
