...
fn api_key_id_for(auth: Option<&crate::auth::GrokAuth>) -> Option<String> {
    auth.filter(|a| matches!(a.auth_mode, crate::auth::AuthMode::ApiKey))
        .map(|a| gbuild_telemetry::config::deployment_id_from_key(&a.key))
}
...
pub(crate) fn embedding_session_credentials(
...
) -> gbuild_memory::EndpointScopedCredentials {
    ...
    gbuild_memory::EndpointScopedCredentials::for_endpoint(
        embed_base_url,
...
///
/// - `"gbuild-shell"`   — classic gBuild / TUI (gbuild-shell)
/// - `"gbuild-pager"`   — gBuild pager / TUI (gbuild-pager)
...
pub fn build_storage_client_for_proxy(
    ...
    if let Some(am) = auth_manager {
        ...
        .with_client_identity(gbuild_version::VERSION, client_identifier)
...
pub fn sync_external_otel_identity() {
    if let Some(provider) = OTEL_PROVIDER.get() {
        ...
        gbuild_telemetry::external::set_identity(
            gbuild_telemetry::external::IdentityAttrs::from_snapshot(&snapshot),
        );
    }
}
...
mod tests {
    ...
    use gbuild_auth::AuthCredentialProvider;
    use std::sync::Mutex;
    /// Serializes tests that pin `GBUILD_AUTH_EARLY_INVALIDATION_SECS`, since
    /// env vars are process-global and parallel tests would race.
    static EARLY_INVALIDATION_LOCK: Mutex<()> = Mutex::new(());
    /// RAII guard: pins `GBUILD_AUTH_EARLY_INVALIDATION_SECS` to the production
    ...
    impl EarlyInvalidationGuard {
        fn pin_to_default() -> Self {
            ...
            let previous = std::env::var("GBUILD_AUTH_EARLY_INVALIDATION_SECS").ok();
            unsafe { std::env::set_var("GBUILD_AUTH_EARLY_INVALIDATION_SECS", "300") };
            Self {
                _lock: lock,
                previous,
            }
        ...
        fn drop(&mut self) {
            unsafe {
                match self.previous.take() {
                    Some(prev) => std::env::set_var("GBUILD_AUTH_EARLY_INVALIDATION_SECS", prev),
                    None => std::env::remove_var("GBUILD_AUTH_EARLY_INVALIDATION_SECS"),
                }
            }
        }
    }
    ...
    fn embedding_session_credentials_scopes_to_first_party() {
        ...
        let api_key_provider: gbuild_tools::types::SharedApiKeyProvider =
    ...
    fn snapshot_populates_tenant_id_per_auth_mode() {
        use gbuild_telemetry::config::deployment_id_from_key;
        let _guard = EarlyInvalidationGuard::pin_to_default();
        let dir = tempfile::tempdir().unwrap();
