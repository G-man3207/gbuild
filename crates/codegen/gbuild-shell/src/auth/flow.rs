...
fn resolve_device_flow(
    ...
    crate::agent::config::BoolFlag::env("GBUILD_LOGIN_DEVICE_FLOW")
...
///
/// Precedence: CLI (`--oauth`/`--device-auth`) > `GBUILD_LOGIN_DEVICE_FLOW` env >
/// `[auth] login_device_flow` config > `gbuild_login_device_flow` remote feature flag > loopback.
async fn should_use_device_flow(login_override: LoginTransportOverride) -> bool {
    ...
    let resolved = if login_override.as_cli_bool().is_some() {
        ...
        let env = crate::agent::config::env_bool("GBUILD_LOGIN_DEVICE_FLOW");
...
async fn run_auth_flow_inner(
    ...
    // `gbuild login --devbox` uses run_devbox_login and is not gated here.
    ...
    // --device-auth / GBUILD_LOGIN_DEVICE_FLOW / [auth] login_device_flow.
    ...
    anyhow::bail!(
        "No OAuth2 configuration available. Run `gbuild login` to authenticate, or contact your administrator if you use enterprise SSO."
    )
}
...
pub async fn try_ensure_fresh_auth(grok_com_config: &GrokComConfig) -> Option<GrokAuth> {
    let gbuild_home = gbuild_home::gbuild_home();
    let auth_manager = std::sync::Arc::new(AuthManager::new(&gbuild_home, grok_com_config.clone()));
...
pub(crate) async fn try_ensure_session_noninteractive(
    ...
    let gbuild_home = gbuild_home::gbuild_home();
    let auth_manager = Arc::new(AuthManager::new(&gbuild_home, grok_com_config.clone()));
...
/// Unified `gbuild login` handler for CLI entry points (tui, pager).
///
/// Precedence: `--oauth` forces loopback, `--device-auth` forces device,
/// otherwise `GBUILD_LOGIN_DEVICE_FLOW` env / `[auth] login_device_flow` config /
...
pub async fn run_cli_login(
    ...
    // supports the device flow. Without this guard, `gbuild login` on an
...
pub struct LogoutResult {
    ...
    /// `true` if the `XAI_API_KEY` env var is set.
...
pub fn perform_logout(
    ...
    if was_logged_in {
        ...
        gbuild_telemetry::external::set_identity(
            gbuild_telemetry::external::IdentityAttrs::default(),
        );
        gbuild_telemetry::external::flush();
...
/// `gbuild logout` CLI handler. Calls [`perform_logout`] and formats
/// the result to stderr.
pub fn run_cli_logout(config: &crate::agent::config::Config) -> anyhow::Result<()> {
    let gbuild_home = gbuild_home::gbuild_home();
    let auth_manager = AuthManager::new(&gbuild_home, config.grok_com_config.clone());
...
mod tests {
    ...
    /// Run `f` with `GBUILD_LOGIN_DEVICE_FLOW` set to `value` (unset for `None`).
    ...
    fn with_device_flow_env<T>(value: Option<bool>, f: impl FnOnce() -> T) -> T {
        let _guard = match value {
            Some(true) => EnvVarGuard::set("GBUILD_LOGIN_DEVICE_FLOW", "true"),
            Some(false) => EnvVarGuard::set("GBUILD_LOGIN_DEVICE_FLOW", "false"),
            None => EnvVarGuard::remove("GBUILD_LOGIN_DEVICE_FLOW"),
        };
        f()
    }
    ...
    async fn persist_or_use_minted_returns_token_when_save_fails() {
        ...
        // Read-only gbuild_home: reading a missing auth.json succeeds (empty), but
    ...
    async fn preresolved_bypasses_resolver_and_is_never_cli() {
        ...
        {
            let _guard = EnvVarGuard::set("GBUILD_LOGIN_DEVICE_FLOW", "false");
            assert!(
        ...
        {
            let _guard = EnvVarGuard::set("GBUILD_LOGIN_DEVICE_FLOW", "true");
            assert!(
    ...
    async fn enterprise_oidc_never_uses_device_flow() {
        // oidc=Some, oauth2=None: `gbuild login` must use loopback, not device —
    ...
    async fn run_auth_flow_falls_through_when_no_refresh_token() {
        ...
        // GBUILD_LOGIN_DEVICE_FLOW / the real config file (the CLI override
    ...
    /// CLI `gbuild login` passes `on_stderr=None`; stderr must be inherited so
