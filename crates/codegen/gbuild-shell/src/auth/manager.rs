...
use gbuild_telemetry::events::ManualAuthSurface;
...
impl AuthManager {
    pub fn new(gbuild_home: &Path, grok_com_config: GrokComConfig) -> Self {
        ...
        gbuild_telemetry::unified_log::info(
            ...
            Some(serde_json::json!({
                "scope": &scope,
                "gbuild_home": gbuild_home.display().to_string(),
                "HOME": std::env::var("HOME").unwrap_or_else(|_| "(unset)".into()),
                "GBUILD_HOME": std::env::var("GBUILD_HOME").unwrap_or_else(|_| "(unset)".into()),
                "GBUILD_AUTH_PATH": std::env::var("GBUILD_AUTH_PATH").unwrap_or_else(|_| "(unset)".into()),
                "GBUILD_AUTH": std::env::var("GBUILD_AUTH").map(|_| "(set)".to_string()).unwrap_or_else(|_| "(unset)".into()),
            })),
        );

        // GBUILD_AUTH: inline JSON credentials (highest priority, read-only).
        if let Ok(inline_json) = std::env::var("GBUILD_AUTH") {
            ...
            tracing::warn!("GBUILD_AUTH set but failed to parse as JSON, falling back to file");
        }

        // GBUILD_AUTH_PATH: custom file path (overrides default $GBUILD_HOME/auth.json).
        let path = std::env::var("GBUILD_AUTH_PATH")
            .map(PathBuf::from)
            .unwrap_or_else(|_| gbuild_home.join("auth.json"));
    ...
    /// (inline `GBUILD_AUTH` vs. on-disk `auth.json`), which differ only in the
    ...
    pub(crate) async fn update(self: &Arc<Self>, auth: GrokAuth) -> std::io::Result<GrokAuth> {
        ...
        match &write_result {
            Ok(()) => gbuild_telemetry::unified_log::info(
            ...
            Err(e) => gbuild_telemetry::unified_log::error(
    ...
    fn observe_disk_state(
        ...
        match new_state {
            // Recovery (or first observation in GBUILD_AUTH mode).
            DiskAuthState::Ok => {
                gbuild_telemetry::unified_log::info(
            ...
            | DiskAuthState::EntryMissing
            | DiskAuthState::Unreadable => {
                gbuild_telemetry::unified_log::warn("auth disk state: entry lost", None, Some(ctx));
            }
        }
    }
    ...
    async fn auth_dispatch(self: &Arc<Self>) -> Result<GrokAuth, AuthError> {
        ...
        let result = match token_type {
            ...
            TokenType::LegacySession => {
                ...
                // assumption that a sibling process (`gbuild login` from
    ...
    pub(crate) async fn try_devbox_recovery(self: &Arc<Self>) -> Result<GrokAuth, AuthError> {
        ...
        gbuild_telemetry::unified_log::info("auth: devbox recovery starting", None, None);
    ...
    #[cfg(test)]
    pub(crate) fn manual_auth_last_emit(&self) -> Option<gbuild_telemetry::events::ManualAuth> {
...
impl gbuild_tools::types::ApiKeyProvider for SharedAuthKeyProvider {
...
) -> gbuild_tools::types::SharedApiKeyProvider {
