...
//!
//! 1. [`gbuild_telemetry::unified_log::warn`] for the local
//!    `~/.gbuild/logs/unified.jsonl` file (best-effort; ships to GCS
...
//!
//! [`gbuild_sampler`] is intentionally decoupled from this crate. It
//! invokes the trait [`gbuild_sampler::Auth401AttributionCallback`] at
//! its six 401 arms; this module provides [`ShellAttribution`], the
//! concrete impl that the shell wires into
//! [`gbuild_sampler::SamplerConfig::attribution_callback`] at every
...
use gbuild_sampler::{Auth401AttributionCallback, SamplingConsumer};
use gbuild_tools::{Auth401AttributionCallback as ToolAuth401AttributionCallback, ToolConsumer};
...
impl ShellAttribution {
    ...
    /// [`gbuild_sampler::SamplerConfig::attribution_callback`].
    ///
    /// (Returns `Arc<dyn Trait>` rather than `Self` because the
    /// `gbuild_sampler::SamplerConfig` field expects exactly that;
    ...
    /// `Arc<dyn gbuild_tools::Auth401AttributionCallback>` for the
...
impl Auth401AttributionCallback for ShellAttribution {
    fn record_401(&self, consumer: SamplingConsumer, sent_bearer_prefix: Option<&str>) {
        ...
        // to `gbuild_sampler::SENT_BEARER_PREFIX_LEN` characters
...
/// in `gbuild-tools` emits a 401 attribution event through this
...
/// All 401 emit sites in `gbuild-shell` go through this helper -- the
...
pub(crate) fn record_auth_401(
    ...
    // Sink 1 -- local file (~/.gbuild/logs/unified.jsonl) + scrubbed
    ...
    gbuild_telemetry::unified_log::warn("auth 401 attribution", session_id, Some(payload.clone()));
...
mod tests {
    ...
    /// nothing from a developer's actual `~/.gbuild/auth.json` leaks in.
    ...
    /// `ShellAttribution` implements `gbuild_tools::Auth401AttributionCallback`
