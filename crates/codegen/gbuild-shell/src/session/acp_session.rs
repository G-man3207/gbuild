...
use gbuild_agent::prompt::agents_md::LEGACY_AGENTS_MD_REMINDER_PREFIX;
use gbuild_agent::prompt::skills::SkillsConfig;
use gbuild_sampler::SamplerConfig as SamplingConfig;
use gbuild_sampling_types::truncate_bytes;
...
use gbuild_tools::types::compat::CompatConfig;
use gbuild_tools::types::output::{
    BashOutput, ReadFileOutput, ToolOutput as ToolsToolOutput, ToolRunResult,
};
use gbuild_workspace::file_system::CodebaseIndexManager;
use gbuild_workspace::permission::{
    AccessKind, ClientType, Decision, PermissionEvent, PermissionHandle,
};
use gbuild_workspace::session::file_state::{FileStateHandle, FileStateTracker};
...
pub(crate) struct InputItem {
    ...
    pub(crate) tool_overrides_update: Option<gbuild_sampling_types::ToolOverridesUpdate>,
...
impl gbuild_tools::types::resources::ManagedGatewayToolCaller for ShellManagedGatewayToolClient {
    async fn call_tool(
        &self,
        call_id: &str,
        ...
        gbuild_tools::types::resources::ManagedGatewayToolCallResponse,
...
pub(crate) struct SessionActor {
    ...
    /// `gbuild-sampler`. Threaded into every `SamplerConfig`
    /// reconstructed by `reconstruct_full_config`. `None` when the
    /// session was spawned without an `AuthManager` (BYOK direct
    /// mode, test fixtures).
    pub(crate) attribution_callback: Option<gbuild_sampler::SharedAttributionCallback>,
    ...
    pub(crate) tool_overrides: std::cell::RefCell<Option<gbuild_sampling_types::ToolOverrides>>,
    ...
    pub(crate) resolved_tool_overrides:
        std::sync::Arc<arc_swap::ArcSwapOption<gbuild_sampling_types::ToolOverrides>>,
    pub(crate) compactions_remaining:
        std::cell::Cell<Option<gbuild_sampling_types::CompactionsRemaining>>,
    pub(crate) compaction_at_tokens:
        std::cell::Cell<Option<gbuild_sampling_types::CompactionAtTokens>>,
    ...
    pub(crate) doom_loop_recovery: Option<gbuild_sampling_types::DoomLoopRecoveryPolicy>,
    ...
    pub(crate) agent: std::cell::RefCell<gbuild_agent::Agent>,
    ...
    /// Kill-switch (`GBUILD_GOAL_USE_CURRENT_MODEL_ONLY` / `[features]
    ...
    pub(crate) pending_classifier_completions: parking_lot::Mutex<
        VecDeque<gbuild_tools::implementations::gbuild::update_goal::UpdateGoalInput>,
    >,
    ...
    pub(crate) mcp_announced_servers:
        Mutex<HashMap<String, gbuild_tools::implementations::search_tool::ServerFingerprint>>,
    ...
    pub(crate) plugin_registry:
        std::cell::RefCell<Option<std::sync::Arc<gbuild_agent::plugins::PluginRegistry>>>,
    /// Shared handle to the agent-level plugin registry.
    /// Used by `/plugins reload` to trigger a rebuild that new sessions see.
    pub(crate) plugin_registry_handle: Option<gbuild_agent::plugins::SharedPluginRegistryHandle>,
    ...
    /// Handle to the per-session `gbuild-sampler` actor.
    ///
    ...
    pub(crate) sampler_handle: gbuild_sampler::SamplerHandle,
    /// Cached recipe for constructing this session's [`gbuild_agent::Agent`].
    ///
    /// Populated once at session spawn and then reused by
    /// `handle_rebuild_agent_for_definition` to build a fresh `Agent`
    /// (system prompt, [`gbuild_tools::bridge::ToolBridge`], tool
    ...
    pub(crate) workspace_ops: gbuild_workspace::WorkspaceOps,
    ...
    fn build_command_availability(
        ...
        slash_commands::CommandAvailability {
            ...
            scheduler: tool_names
                .iter()
                .any(|n| n == gbuild_tools::implementations::gbuild::SCHEDULER_CREATE_TOOL_NAME),
            hooks: self.hook_registry.borrow().is_some(),
            plugins: self.plugin_registry.borrow().is_some(),
            goal,
            workflows: tool_names
                .iter()
                .any(|n| n == gbuild_tools::implementations::gbuild::workflow::WORKFLOW_TOOL_NAME),
...
impl SessionActor {
    ...
    fn tool_bridge_handle(&self) -> Arc<gbuild_tools::bridge::ToolBridge> {
...
/// The saved JSON enables deterministic re-rendering, `gbuild prompt --json`
/// inspection, and post-hoc debugging of what went into a session's system prompt.
fn save_prompt_context(session_info: &SessionInfo, prompt_context: &gbuild_agent::PromptContext) {
...
) -> Option<gbuild_agent::PromptContext> {
    let dir = crate::session::persistence::session_dir(session_info);
...
mod managed_gateway_descriptor_tests {
    use super::*;
    use gbuild_tools::types::output::{MCPOutput, ToolOutput};
    use gbuild_tools::types::tool::{ToolKind, ToolNamespace};
    #[derive(Debug, Default)]
    struct FixtureMcpTool;
    impl gbuild_tools::types::tool_metadata::ToolMetadata for FixtureMcpTool {
...
mod tool_meta_stamp_tests {
    ...
    use super::*;
    use gbuild_tools::registry::types::ToolConfig;
    use gbuild_tools::tool_taxonomy::TOOL_META_KEY;
    use gbuild_workspace::permission::PermissionCommand;
    use tokio::sync::mpsc;
...
mod managed_gateway_tool_tests {
    ...
    use gbuild_tools::types::output::{MCPOutput, ToolOutput};
    use gbuild_tools::types::tool::{ToolKind, ToolNamespace};
    use gbuild_tools::types::tool_metadata::ToolMetadata;
    #[derive(Debug)]
    struct FixtureMcpTool;
    ...
    async fn refresh_snapshot_seeds_only_admitted_gateway_catalog_entries() {
        ...
        let catalog = bridge
            .read_resource::<gbuild_tools::types::resources::ManagedGatewayToolCatalog>()
            .await
