...
pub(crate) struct RoleToolNames {
    ...
    /// (the default gBuild host's `search_replace` mutator) when `Write`
...
impl RoleToolNames {
    ...
    /// `search_replace` on the default gBuild host) instead of the literal
    ...
    pub(crate) fn from_summary(
        ...
        Self::from_parts(
            ...
            get(ToolKind::Search),
            // The default gBuild host's pre-spawn describe probe exposes only
...
pub(crate) mod tests {
    ...
    #[test]
    fn from_summary_uses_gbuild_names() {
        // A gBuild toolset: client names match the literal defaults.
    ...
    fn web_tools_fall_back_when_absent_from_the_toolset() {
        ...
        // on the default gBuild host (the stock `web_search`/`web_fetch`).
    ...
    #[test]
    fn from_summary_write_falls_back_to_edit_on_default_gbuild_host() {
        // Default gBuild host: the pre-spawn describe probe exposes only
    ...
    #[test]
    fn parent_and_summary_renders_agree_on_default_gbuild_mutator() {
        ...
        // default gBuild host (Edit-only toolset), so a fail-open retry
