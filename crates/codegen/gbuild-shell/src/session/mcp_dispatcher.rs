...
//!
//! Receives [`gbuild_mcp::servers::McpClientEvent`]s emitted by:
//! - per-client transport-liveness watchers
//!   ([`gbuild_mcp::liveness`]),
//! - the [`gbuild_mcp::servers::GBuildClientHandler`] (server-pushed
...
pub struct McpServerStatusPayload {
    ...
    /// or `local` (user `.gbuild/config.toml`).
...
pub enum McpServerStatus {
    /// Client is in [`gbuild_mcp::servers::ClientStateKind::Ready`]
...
/// [`gbuild_mcp::servers::McpClientEventKind`]). Used as the
    ...
    let (status, reason, detail) = match (kind, event) {
        ...
        (McpClientEventKind::HandshakeFailed, McpClientEvent::HandshakeFailed { reason, .. })
            if source == McpServerSource::Managed
                && gbuild_mcp::servers::is_auth_rejection_message(reason) =>
        {
            (
...
mod tests {
    ...
    async fn dispatcher_drops_dead_clients_on_transport_closed() {
        use gbuild_mcp::servers::{McpClient, McpState};
