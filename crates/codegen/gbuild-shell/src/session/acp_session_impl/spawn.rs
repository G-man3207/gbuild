...
fn drop_cli_catchall_allows(
    rules: Vec<gbuild_workspace::permission::types::PermissionRule>,
    policy_block: Option<&'static str>,
) -> (
    Vec<gbuild_workspace::permission::types::PermissionRule>,
    Vec<gbuild_workspace::permission::types::PermissionRule>,
) {
    ...
    for rule in rules {
        if gbuild_workspace::permission::resolution::is_catchall_allow(&rule) {
            dropped.push(rule);
        } else {
            kept.push(rule);
        }
...
mod runtime_containment_tests {
    ...
    const CHILD_ENV: &str = "XAI_GBUILD_SHELL_RUNTIME_CONTAINMENT_CHILD";
...
mod cli_catchall_drop_tests {
    ...
    use gbuild_workspace::permission::resolution::YOLO_PIN_REASON_REQUIREMENTS;
    use gbuild_workspace::permission::rules::parse_permission_rule;
    use gbuild_workspace::permission::types::{PermissionRule, RuleAction, ToolFilter};
...
pub(crate) async fn spawn_session_actor(
    ...
    attribution_callback: Option<gbuild_sampler::SharedAttributionCallback>,
    ...
    web_search_sampling_config: Option<gbuild_sampler::SamplerConfig>,
    ...
    app_builder_deployer_config: gbuild_tools::implementations::gbuild::deploy_app::AppBuilderDeployerConfig,
    ...
    prompt_audience: gbuild_agent::prompt::context::PromptAudience,
    ...
    plugin_registry: Option<std::sync::Arc<gbuild_agent::plugins::PluginRegistry>>,
    plugin_registry_handle: Option<gbuild_agent::plugins::SharedPluginRegistryHandle>,
    models_manager: crate::agent::models::ModelsManager,
    inherited_permission_handle: Option<gbuild_workspace::permission::PermissionHandle>,
    api_key_provider: Option<gbuild_tools::types::SharedApiKeyProvider>,
    image_description_model: String,
    hook_registry_override: Option<std::sync::Arc<gbuild_hooks::discovery::HookRegistry>>,
    workspace_ops: gbuild_workspace::WorkspaceOps,
    cli_permission_rules: Vec<gbuild_workspace::permission::types::PermissionRule>,
    todo_gate: bool,
    ...
    parent_terminal_backend: Option<
        std::sync::Arc<dyn gbuild_tools::computer::types::TerminalBackend>,
    >,
    parent_scheduler_handle: Option<
        gbuild_tools::implementations::gbuild::scheduler::types::SchedulerHandle,
    >,
    ...
    gbuild_agent::AgentBuildError,
> {
    if max_turns == Some(0) {
        return Err(gbuild_agent::AgentBuildError::InvalidConfig(
        ...
        inherited_permission_handle
    {
            ...
            && !dropped_catchalls.is_empty()
        {
            ...
            if startup_hints.non_interactive {
                eprintln!("gbuild: --allow catch-all ignored: {reason}");
            }
        }
        if !cli_permission_rules.is_empty() {
            match &mut permission_config {
                ...
                None => {
                    permission_config =
                        Some(gbuild_workspace::permission::types::PermissionConfig::new(
        ...
        let deny_read_globs = permission_config
            .as_ref()
            .map(gbuild_workspace::permission::resolution::deny_read_globs_from_config)
            .unwrap_or_default();
        let hub_permission = if gbuild_workspace::permission::hitl_permission_live_enabled() {
            ...
            let transport = server
                .and_then(|server| {
                    gbuild_workspace::permission::ToolServerPermissionTransport::from_session_id(
                        server,
                ...
                .map(|t| {
                    std::sync::Arc::new(t)
                        as std::sync::Arc<dyn gbuild_workspace::permission::PermissionHookTransport>
                });
        ...
        let (permissions, permission_events_rx) =
            gbuild_workspace::permission::spawn_permission_manager_with_hub(
    ...
    let web_search_config = if disable_web_search {
        gbuild_tools::implementations::WebSearchConfig::Disabled
    } else if let Some(cfg) = web_search_sampling_config {
        if let Some(api_key) = cfg.api_key {
            gbuild_tools::implementations::WebSearchConfig::Enabled {
                api_key,
    ...
    let context_window_override = std::env::var("GBUILD_DEBUG_CONTEXT_WINDOW")
        .ok()
    ...
    if let Some(cw) = context_window_override {
        tracing::warn!(
            ...
            "GBUILD_DEBUG_CONTEXT_WINDOW override active"
        );
    }
    let chat_state_sampling_config = gbuild_sampling_types::SamplingConfig {
    ...
    let task_completion_reservations =
        gbuild_tools::reminders::task_completion::TaskCompletionReservations::default();
    ...
    *synthetic_trace_tx_shared.lock().unwrap() = tool_context.synthetic_trace_tx.clone();
    ...
    let resolve_search_shadows = || {
        ...
        gbuild_tools::computer::local::SearchShadowConfig {
            find_bfs,
            grep_ugrep,
        }
    };
    let resolve_policy = || crate::util::config::resolve_shell_env_policy(effective_cfg.as_ref());
    let terminal_backend: std::sync::Arc<dyn gbuild_tools::computer::types::TerminalBackend> =
        match terminal_backend_kind {
            ...
            TerminalBackendKind::AcpClient => {
                std::sync::Arc::new(crate::terminal::AcpTerminalAdapter::new(
                    ...
                    as std::sync::Arc<dyn gbuild_tools::computer::types::TerminalBackend>
            }
    ...
    let fs_backend: std::sync::Arc<dyn gbuild_tools::computer::types::AsyncFileSystem> =
        if client_fs_capable && tool_context.gateway.is_some() {
            std::sync::Arc::new(gbuild_workspace::file_system::AcpFsAdapter::new(
                tool_context.gateway.clone().unwrap(),
                tool_context.session_id.clone().unwrap(),
            ))
        } else {
            std::sync::Arc::new(gbuild_tools::computer::local::LocalFs)
        };
    ...
    let harness_metrics = if telemetry_enabled || gbuild_telemetry::external::is_active() {
        ...
        Some(super::telemetry::SessionHarnessMetrics {
            ...
            permission_mode: if session_yolo_mode {
                gbuild_telemetry::enums::PermissionMode::AlwaysApprove
            } else if session_auto_mode
                && crate::util::config::auto_permission_mode_enabled_from_disk()
            {
                gbuild_telemetry::enums::PermissionMode::Auto
            } else {
                gbuild_telemetry::enums::PermissionMode::Ask
            },
    ...
    let compaction_policy = gbuild_agent::CompactionPolicy {
    ...
    let (user_question_tx, user_question_rx) = tokio::sync::mpsc::unbounded_channel::<
        gbuild_tools::implementations::gbuild::ask_user_question::types::UserQuestionRequest,
    >();
    ...
    let memory_backend_for_spec: Option<
        std::sync::Arc<dyn gbuild_tools::types::memory_backend::MemoryBackend>,
    > = if let Some(ref storage) = memory_storage_for_session {
        if let Err(e) = storage.ensure_initialized() {
            tracing::warn!(
                target: gbuild_telemetry::memory_log::TARGET,
                error = %e,
        ...
        let backend: std::sync::Arc<dyn gbuild_tools::types::memory_backend::MemoryBackend> =
        ...
        gbuild_telemetry::session_ctx::log_event(
            gbuild_telemetry::memory_telemetry::MemorySessionInit {
                session_id: session_info.id.to_string(),
                memory_enabled: true,
                watcher_config_enabled: watcher_config.enabled,
                watcher_started,
    ...
    let managed_gateway_tool_client = auth_manager.as_ref().map(|am| {
        gbuild_tools::types::resources::ManagedGatewayToolClient(Arc::new(
    ...
    let resolved_task_output =
        gbuild_tools::reminders::task_completion::resolve_task_output_tool_name(
            agent.tool_bridge(),
        )
        .await;
    let resolved_read =
        gbuild_tools::reminders::task_completion::resolve_read_tool_name(agent.tool_bridge()).await;
    ...
    let scheduler_handle_for_handle = {
        ...
        res.get::<gbuild_tools::implementations::gbuild::scheduler::types::SchedulerHandle>()
            .cloned()
    };
    ...
    let feedback_client_type = match client_type {
        ...
        ClientType::GBuildPager => prod_mc_cli_chat_proxy_types::feedback_types::ClientType::Tui,
    };
    ...
    let sampler_retry_policy = gbuild_sampler::RetryPolicy {
    ...
    let (sampler_event_tx, sampler_event_rx) =
        tokio::sync::mpsc::unbounded_channel::<gbuild_sampler::SamplingEvent>();
    let sampler_handle = gbuild_sampler::SamplerActor::spawn(
    ...
    let mut hook_discovery_errors: Vec<gbuild_hooks::error::HookError> = Vec::new();
    let built_hook_registry: Option<Arc<gbuild_hooks::discovery::HookRegistry>> =
    ...
    let (workflow_launch_tx, mut workflow_launch_rx) = tokio::sync::mpsc::unbounded_channel::<
        gbuild_tools::implementations::gbuild::workflow::WorkflowLaunchEnvelope,
    >();
    {
        ...
        tokio::spawn(async move {
            ...
            use gbuild_tools::implementations::gbuild::workflow::WorkflowLaunchAck;
                ...
                if !background_workflows_enabled {
                    let _ = ack.send(WorkflowLaunchAck::Rejected {
                        ...
                        detail: "Background workflows are disabled for this session \
                                 ([workflows] enabled = false / GBUILD_WORKFLOWS=0 / remote flag)."
                            .into(),
                    });
                    continue;
                }
    ...
    let resolved_tool_overrides: std::sync::Arc<
        arc_swap::ArcSwapOption<gbuild_sampling_types::ToolOverrides>,
    ...
    let session = Arc::new_cyclic(|weak: &std::sync::Weak<SessionActor>| SessionActor {
        ...
        max_turns,
        max_retries: gbuild_sampler::resolve_max_retries(max_retries),
        ...
        vcs_kind: {
            ...
            match gbuild_workspace::session::git::discover_git_root(root) {
                gbuild_workspace::session::git::GitDiscoveryResult::Found(git_root) => {
                    gbuild_workspace::session::git::detect_vcs_kind(&git_root)
                }
                _ => gbuild_workspace::session::git::VcsKind::None,
            }
        },
    ...
    }
    {
        ...
        session
            ...
            .borrow()
            .tool_bridge()
            .update_resource(gbuild_tools::types::tool_index::ToolIndex(
    ...
    }
    {
        ...
        session
            ...
            .borrow()
            .tool_bridge()
            .update_resource(gbuild_tools::types::resources::PlanFilePath(plan_path))
            .await;
    }
    ...
    session
        ...
        .tool_bridge()
        .update_resource(
            gbuild_tools::implementations::gbuild::workflow::WorkflowLaunchHandle(
    ...
    if !background_workflows_enabled {
        session
            ...
            .tool_bridge()
            .update_resource(
                gbuild_tools::implementations::gbuild::update_goal::GoalUpdateHandle(
    ...
    if let Some(storage) = session.memory.storage() {
        ...
        tokio::task::spawn_local(async move {
            ...
            ) && let Ok(files) = storage.list_memory_files()
            {
                ...
                gbuild_telemetry::session_ctx::log_event(
                    gbuild_telemetry::memory_telemetry::MemoryReindex {
    ...
    }
    {
        ...
        use gbuild_tools::implementations::gbuild::ask_user_question::{
        ...
        tokio::task::spawn_local(async move {
            while let Some(mut request) = user_question_rx.recv().await {
                use gbuild_tools::implementations::gbuild::ask_user_question::AskUserQuestionMode;
    ...
    let telemetry_ctx = gbuild_telemetry::session_ctx::TelemetryCtx::new(
    ...
    tokio::task::spawn_local(async move {
        gbuild_telemetry::session_ctx::with_session_ctx(
            telemetry_ctx,
            run_session(
                session,
                cmd_rx,
...
> {
    let (init_tx, init_rx) =
        tokio::sync::oneshot::channel::<Result<SessionInitResult, gbuild_agent::AgentBuildError>>();
        ...
        .spawn(move || {
            ...
            let rt = match build_session_runtime() {
                ...
                Err(e) => {
                    ...
                    let _ = init_tx.send(Err(gbuild_agent::AgentBuildError::RuntimeBuild(e)));
                    return;
                }
            };
    ...
    conversation.iter().any(|item| {
        ...
        let contains = |needle: &str| {
            u.content.iter().any(|part| {
                matches!(
                    part,
                    gbuild_sampling_types::conversation::ContentPart::Text { text }
