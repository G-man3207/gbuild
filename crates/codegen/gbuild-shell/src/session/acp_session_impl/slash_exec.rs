...
impl SessionActor {
    ...
    pub(super) async fn execute_builtin_slash_command(
        ...
        gbuild_telemetry::session_ctx::log_event(gbuild_telemetry::events::SlashCommandUsed {
            ...
            BuiltinAction::HooksAdd { path } => {
                if path.is_empty() {
                    self.send_host_turn_slash_command_output(
                        "Usage: /hooks add <path>\nProvide a path to a hook JSON file or directory under ~/.gbuild/.",
                    )
                    .await;
                } else {
                    // CWE-427: Use shared add_hooks_path() which validates
                    // paths are under ~/.gbuild/ to prevent hook path injection.
                    match crate::config::add_hooks_path(&path) {
                        Ok(()) => {
                            gbuild_telemetry::session_ctx::log_event(
                                gbuild_telemetry::events::HookAdded { success: true },
                            );
                        ...
                        Err(e) => {
                            gbuild_telemetry::session_ctx::log_event(
                                gbuild_telemetry::events::HookAdded { success: false },
                            );
                            self.send_host_turn_slash_command_output(&format!(
            ...
            BuiltinAction::HooksRemove { path } => {
                if path.is_empty() {
                    ...
                    match crate::config::remove_hooks_path(&path) {
                        Ok(()) => {
                            gbuild_telemetry::session_ctx::log_event(
                                gbuild_telemetry::events::HookRemoved { success: true },
                            );
                            self.send_host_turn_slash_command_output(&format!(
                        ...
                        Err(e) => {
                            gbuild_telemetry::session_ctx::log_event(
                                gbuild_telemetry::events::HookRemoved { success: false },
                            );
                            self.send_host_turn_slash_command_output(&format!(
            ...
            BuiltinAction::PluginsReload => {
                match &self.plugin_registry_handle {
                    Some(handle) => {
                        ...
                        gbuild_telemetry::session_ctx::log_event(
                            gbuild_telemetry::events::PluginReloaded { success: true },
                        );
                        self.send_host_turn_slash_command_output(&msg).await;
                    }
                    None => {
                        gbuild_telemetry::session_ctx::log_event(
                            gbuild_telemetry::events::PluginReloaded { success: false },
                        );
                        self.send_host_turn_slash_command_output(
            ...
            BuiltinAction::PluginsAdd { path } => {
                if path.is_empty() {
                    ...
                    match crate::config::add_plugin_path(&path_str) {
                        Ok(()) => {
                            gbuild_telemetry::session_ctx::log_event(
                                gbuild_telemetry::events::PluginAdded {
                                    source: gbuild_telemetry::events::PluginSource::LocalPath,
                                    success: true,
                                },
                            );
            ...
            BuiltinAction::PluginsRemove { path } => {
                if path.is_empty() {
                    ...
                    match crate::config::remove_plugin_path(&path_str) {
                        Ok(()) => {
                            gbuild_telemetry::session_ctx::log_event(
                                gbuild_telemetry::events::PluginRemoved { success: true },
                            );
                        ...
                        Err(e) => {
                            gbuild_telemetry::session_ctx::log_event(
                                gbuild_telemetry::events::PluginRemoved { success: false },
                            );
                            self.send_host_turn_slash_command_output(&format!(
                ...
                if source.is_empty() {
                    ...
                    if !trust {
                        ...
                        let source_desc = match &install_source {
                            gbuild_agent::plugins::git_install::InstallSource::Git {
                                url, ..
                            } => {
                                format!("remote git repo: {url}")
                            }
                            gbuild_agent::plugins::git_install::InstallSource::Local {
                                path,
                                ..
                            } => {
                        ...
                        match crate::plugin::install_plugin(&source, cwd) {
                            Ok(outcome) => {
                                ...
                                gbuild_telemetry::session_ctx::log_event(
                                    gbuild_telemetry::events::PluginInstalled {
                                        install_kind: kind,
                                        success: true,
                                        trust: true,
                                        error_category: None,
            ...
            BuiltinAction::PluginsUninstall { name, confirm } => {
                if name.is_empty() {
                    ...
                    match crate::plugin::uninstall_plugin(&name, confirm, false) {
                        Ok(outcome) => {
                            gbuild_telemetry::session_ctx::log_event(
                                gbuild_telemetry::events::PluginUninstalled {
                                    confirmed: true,
                                    success: true,
                                },
                            );
                        ...
                        if let Err(e) = storage.ensure_initialized() {
                            ...
                            *self.memory.search_counter.borrow_mut() =
                            ...
                            let backend: std::sync::Arc<
                                dyn gbuild_tools::types::memory_backend::MemoryBackend,
