...
impl SessionActor {
    ...
    pub(super) async fn maybe_notify_git_branch(&self) {
        ...
        let (worktree_info, branch) = tokio::join!(
            gbuild_workspace::session::git::get_worktree_info(cwd),
            gbuild_workspace::session::git::get_branch(cwd),
        );
        ...
        let params = gbuild_workspace::session::git::GitHeadChanged {
        ...
        if let Ok(raw) = serde_json::value::to_raw_value(&params) {
            let notification = acp::ExtNotification::new("gbuild/git_head_changed", raw.into());
            self.notifications
                .gateway
    ...
    pub(super) async fn outstanding_reply_for_prompt(
        &self,
        prompt_id: &str,
    ) -> Option<gbuild_tools::implementations::gbuild::task::types::SubagentOutstandingReply> {
    ...
    pub(super) fn classify_turn_error(err: &acp::Error) -> String {
        use gbuild_hooks::event::StopFailureKind as K;
    ...
    pub(super) fn stop_failure_error_type(
        err: &acp::Error,
    ) -> gbuild_hooks::event::StopFailureKind {
        use gbuild_hooks::event::StopFailureKind as K;
    ...
    pub(super) fn classify_install_error(
        err: &gbuild_agent::plugins::install_registry::InstallError,
    ) -> String {
