...
/// `SamplingError::is_auth_error` in gbuild-sampling-types: 403 is
...
pub(super) async fn call_with_auth_retry<F, Fut>(
    ...
    tool_name: &str,
    mut call: F,
) -> Result<gbuild_tools::types::output::ToolRunResult, xai_tool_runtime::ToolError>
where
    F: FnMut() -> Fut,
    Fut: std::future::Future<
            Output = Result<
                gbuild_tools::types::output::ToolRunResult,
...
impl SessionActor {
    ...
    fn resolve_hosted(
        ...
        Vec<gbuild_sampling_types::HostedTool>,
        gbuild_sampling_types::ToolOverrides,
    ) {
        let mut tools = self.agent.borrow().hosted_tools().to_vec();
        let applied = gbuild_sampling_types::apply_tool_overrides(
            &mut tools,
    ...
    pub(crate) fn effective_hosted_tools(&self) -> Vec<gbuild_sampling_types::HostedTool> {
        self.resolve_hosted().0
    }
    pub(crate) fn hosted_tools_for_turn(&self) -> Vec<gbuild_sampling_types::HostedTool> {
    ...
    pub(crate) fn effective_tool_overrides(&self) -> Option<gbuild_sampling_types::ToolOverrides> {
        if !self.backend_search_active() {
            return None;
        }
    ...
    /// inherits it.
    pub(crate) fn set_tool_overrides(&self, overrides: gbuild_sampling_types::ToolOverrides) {
    ...
    pub(crate) fn apply_tool_overrides_update(
        &self,
        update: Option<gbuild_sampling_types::ToolOverridesUpdate>,
    ) {
    ...
    pub(super) async fn reconstruct_full_config(&self) -> SamplingConfig {
        ...
        impl gbuild_sampler::HeaderInjector for TraceContextInjector {
        ...
        impl gbuild_sampler::BearerResolver for AuthManagerBearerResolver {
        ...
        let cfg = self
            ...
            .get_sampling_config()
            .await
            .unwrap_or_else(|| gbuild_sampling_types::SamplingConfig {
                base_url: String::new(),
                model: String::new(),
        ...
        SamplingConfig {
            ...
            bearer_resolver: if use_bearer_resolver {
                self.auth_manager
                    .as_ref()
                    .map(|am| -> gbuild_sampler::SharedBearerResolver {
    ...
    pub(crate) async fn wire_permission_auto_llm_classifier(self: &Arc<Self>) {
        ...
        let (tx, mut rx) = tokio::sync::mpsc::unbounded_channel::<(
            Vec<gbuild_workspace::permission::ClassifierMessage>,
            tokio::sync::oneshot::Sender<
                Result<String, gbuild_workspace::permission::ClassifierFailure>,
            >,
        )>();
            ...
            while let Some((messages, respond_to)) = rx.recv().await {
                let result = async {
                        ...
                        None => {
                            let client =
                                session.prepare_chat_completion(false).await.map_err(|e| {
                                    gbuild_workspace::permission::ClassifierFailure::TransportError(
                                        e.to_string(),
                                    )
                                })?;
                            let model = session
                    ...
                    let items = messages
                        .into_iter()
                        .map(|m| match m.role {
                            gbuild_workspace::permission::ClassifierMessageRole::System => {
                                ConversationItem::system(m.text)
                            }
                            gbuild_workspace::permission::ClassifierMessageRole::User => {
                    ...
                    let request = ConversationRequest {
                        ...
                        json_schema: Some(
                            gbuild_workspace::permission::classifier_output_json_schema(),
                        ),
                    ...
                    let response = tokio::time::timeout(classify_timeout, fut)
                        .await
                        .map_err(|_| gbuild_workspace::permission::ClassifierFailure::Timeout)?
                        .map_err(|e| {
                            gbuild_workspace::permission::ClassifierFailure::TransportError(
                                e.to_string(),
                            )
        ...
        let clf =
            gbuild_workspace::permission::LlmPermissionClassifier::with_channel(tx, prompt_type);
        debug_assert!(
            clf.has_side_query(),
    ...
    pub(super) async fn resolve_aux_sampler_config(
        &self,
        slug: &str,
    ) -> Option<gbuild_sampler::SamplerConfig> {
        let creds = self.chat_state_handle.get_credentials().await;
    ...
    async fn resolve_auto_classifier_sampler(
        &self,
        slug: &str,
    ) -> Option<(gbuild_sampler::SamplingClient, String)> {
        ...
        let client = gbuild_sampler::SamplingClient::new(cfg)
            .map_err(|e| {
    ...
    pub(super) async fn prepare_chat_completion(
        &self,
        force_http1: bool,
    ) -> Result<gbuild_sampler::SamplingClient, acp::Error> {
        self.refresh_token_if_expired().await;
        let mut full_config = self.reconstruct_full_config().await;
        full_config.force_http1 = force_http1;
        let sampling_client =
            gbuild_sampler::SamplingClient::new(full_config).map_err(|e| self.to_acp_error(e))?;
        Ok(sampling_client)
    }
    ...
    /// `gbuild-sampler` instead of constructing a new
    /// `OaiCompatClient`.
    ///
    ...
    pub(crate) async fn handle_sampling_failure(
        self: &Arc<Self>,
        error: gbuild_sampler::SamplingErrorInfo,
    ) -> Result<SamplerFailureRecovery, acp::Error> {
        use gbuild_sampler::SamplingErrorKind;
        if self.tool_context.task_output_token_budget.is_some() {
        ...
        let client_version = gbuild_version::VERSION;
        if auth_mode == crate::auth::AuthMode::WebLogin {
            let msg = format!(
                "{detailed_message}\n\n\
                 ...
                 To fix: run `gbuild logout` then `gbuild login` to re-authenticate with OAuth2.\n\n\
        ...
        let error_type = if gbuild_sampling_types::is_context_length_error(&error.message) {
            "context_length"
        } else {
            error.kind.as_str()
        };
    ...
    pub(crate) async fn run_turn_via_sampler(
        ...
        let request_id = gbuild_sampler::RequestId::random();
            ...
            .await
        {
            ...
            Err(rich_err) => {
                self.turn_stream_drained.lock().take();
                let info = gbuild_sampler::SamplingErrorInfo::from(&rich_err);
...
fn resolve_configured_cutoff(
    seed: Option<gbuild_sampling_types::ToolOverrides>,
    base: Option<&gbuild_sampling_types::ToolOverrides>,
) -> gbuild_sampling_types::ToolOverrides {
    use gbuild_sampling_types::{ToolOverrides, WebSearchOptions, XSearchOptions};
    let ToolOverrides {
        x_search: seed_x,
        web_search: seed_w,
...
mod configured_cutoff_tests {
    use gbuild_sampling_types::{SearchDateBound, ToolOverrides, WebSearchOptions, XSearchOptions};
    ...
    fn inherited_cutoff_agrees_with_the_wire_echo() {
        use gbuild_sampling_types::{HostedTool, apply_tool_overrides};
