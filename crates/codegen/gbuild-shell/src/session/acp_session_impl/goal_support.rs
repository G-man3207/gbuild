...
impl DrainSource {
    ...
    pub(super) fn into_parts(
        ...
        gbuild_tools::implementations::gbuild::update_goal::UpdateGoalInput,
        Option<
...
pub(super) fn try_send_ack(
    ack_tx: Option<
        ...
        >,
    >,
    ack: gbuild_tools::implementations::gbuild::update_goal::UpdateGoalAck,
) {
...
/// `Some` only when the planner is enabled (`GBUILD_GOAL_PLANNER`) and a plan
...
/// Override with `GBUILD_GOAL_REVERIFY_AFTER` (floored at 1).
...
pub(crate) fn goal_slash_and_harness_available(goal_enabled: bool, tool_names: &[String]) -> bool {
    use gbuild_tools::implementations::gbuild::UPDATE_GOAL_TOOL_NAME;
...
impl SessionActor {
    ...
    pub(super) async fn set_goal_loop_active_resource(&self, active: bool) {
        ...
        self.agent
            ...
            .tool_bridge()
            .update_resource(
                gbuild_tools::implementations::gbuild::task::types::GoalLoopActive(active),
            )
            .await;
    }
}
