...
impl UsageDrainOutcome {
    ...
    pub(super) fn from_outstanding_reply(
        reply: Option<
            &gbuild_tools::implementations::gbuild::task::types::SubagentOutstandingReply,
        >,
    ) -> Self {
        match reply {
            None => Self {
...
impl SessionActor {
    ...
    pub(super) async fn handle_prompt(
        ...
        *self.turn_prompt_mode.lock() = prompt_mode;
        ...
        let prompt_blocks = match slash_commands::resolve(
            ...
            Err(SlashCommandOutcome::Builtin(action)) => {
                ...
                let slash_used = gbuild_telemetry::events::SlashCommandUsed {
                ...
                match action {
                    BuiltinAction::GoalSet {
                        ...
                        gbuild_telemetry::session_ctx::log_event(slash_used);
            ...
            Err(SlashCommandOutcome::InvokeSkill {
                ...
                for sk in &parsed_skills {
                    gbuild_telemetry::session_ctx::log_event(
                        gbuild_telemetry::events::SlashCommandUsed {
                    ...
                    if let Some(ref pname) = sk.plugin_name {
                        gbuild_telemetry::session_ctx::log_event(
                            gbuild_telemetry::events::PluginUsed {
        ...
        gbuild_telemetry::session_ctx::log_session_event(crate::agent::session_metrics::Turn {
            session_id: self.session_info.id.0.to_string(),
            turn_number: turn_idx,
        });
        let current_prompt_index = self.chat_state_handle.get_prompt_index().await;
        gbuild_telemetry::session_ctx::begin_prompt_id();
        let origin = super::super::PromptOrigin::from_prompt_id(prompt_id);
        let mut chunk_meta = serde_json::Map::new();
        ...
        *self.tool_context.prompt_index.lock().await = current_prompt_index;
        ...
        let (query, extra_images) = if !self.is_cursor_harness() {
            let extraction = gbuild_tools::util::base64_images::extract_base64_images(query);
        ...
        let model_id = self
            ...
            .map(|c| c.model)
            .unwrap_or_default();
        if self.telemetry_enabled || gbuild_telemetry::external::is_active() {
            let effective_client_identifier =
                prompt_client_identifier.or_else(|| self.client_identifier.clone());
            let ev = gbuild_telemetry::events::PromptSubmitted {
        ...
        self.tool_bridge_handle()
            .update_resource(gbuild_tools::types::resources::AttachedImages(
                attached_image_refs,
            ))
            .await;
        ...
        self.dispatch_hook(
            gbuild_hooks::event::HookEventName::UserPromptSubmit,
            gbuild_hooks::event::HookPayload::UserPromptSubmit {
        ...
        match &result {
            Ok(TurnOutcome::Completed { refusal, .. }) => {
                ...
                if let Some(explanation) = refusal {
                    ...
                    self.dispatch_hook(
                        gbuild_hooks::event::HookEventName::StopFailure,
                        gbuild_hooks::event::HookPayload::StopFailure {
                            error: gbuild_hooks::event::StopFailureKind::InvalidRequest,
                ...
                .await;
                gbuild_telemetry::session_ctx::log_event(gbuild_telemetry::events::TurnCompleted {
                    outcome: gbuild_telemetry::events::Outcome::Completed,
                    duration_ms: turn_duration_ms,
                    tool_call_count: turn_tool_count,
            ...
            Ok(TurnOutcome::Cancelled { category, context }) => {
                ...
                .await;
                gbuild_telemetry::session_ctx::log_event(gbuild_telemetry::events::TurnCompleted {
                    outcome: gbuild_telemetry::events::Outcome::Cancelled,
                    duration_ms: turn_duration_ms,
                    tool_call_count: turn_tool_count,
                    model_id: turn_model_id,
            ...
            Err(err) => {
                ...
                gbuild_telemetry::session_ctx::log_session_event(
                    gbuild_telemetry::events::ApiError {
                ...
                gbuild_telemetry::session_ctx::log_event(gbuild_telemetry::events::TurnCompleted {
                    outcome: gbuild_telemetry::events::Outcome::Error,
                    duration_ms: turn_duration_ms,
                    tool_call_count: turn_tool_count,
                    model_id: turn_model_id,
                    cancellation_category: None,
    ...
    pub(crate) async fn inject_pending_monitor_events(&self) {
        ...
        let mine = gbuild_tools::implementations::gbuild::task::types::drain_owned(
            buffer,
            Some(self.session_info.id.0.as_ref()),
        );
        if mine.is_empty() {
        ...
        let Some(body) = gbuild_tools::reminders::task_completion::format_monitor_events(
            &mine,
            Some(&self.tool_context.task_output_tool_name),
        ) else {
            return;
        };
        let wrapped = gbuild_tools::reminders::wrap_reminder(&body);
        self.chat_state_handle
    ...
    pub(super) async fn process_conversation_turn_with_recovery(
        ...
        let completion_req = match agent_ref.completion_requirement() {
            ...
            None => {
                return self
                    .process_conversation_turn(req_id, json_schema)
                    .await;
            }
        };
        ...
        loop {
            ...
            result = self
                .process_conversation_turn(req_id, None)
                .await;
            if matches!(
                result,
                Ok(TurnOutcome::MaxTurnsReached { .. }) | Ok(TurnOutcome::StationarityEnded { .. })
    ...
    pub(crate) async fn first_turn_memory_reminder(&self) -> Option<String> {
        ...
        use gbuild_tools::types::memory_backend::MemoryBackend as _;
        ...
        gbuild_telemetry::session_ctx::log_event(
            gbuild_telemetry::memory_telemetry::MemoryInjection {
    ...
    async fn handle_structured_output_tool_call(
        &self,
        tool_calls: &mut Vec<gbuild_sampling_types::conversation::ToolCall>,
        validator: &Result<jsonschema::Validator, String>,
        retries: &mut u32,
    ...
    async fn is_run_true_step(
        &self,
        tool_calls: &[gbuild_sampling_types::conversation::ToolCall],
    ) -> bool {
    ...
    async fn finalize_turn_bookkeeping(
        ...
        if let Some(snap) = snapshot.as_mut() {
            ...
            for pr in &snap.delta.prs_created_this_turn {
                gbuild_telemetry::session_ctx::log_event(gbuild_telemetry::events::PrCreated {
                    source: pr.source,
    ...
    async fn process_conversation_turn(
        ...
        loop {
            ...
            if identical_tool_calls.run_len >= identical_tool_calls.hard_stop_threshold() {
                ...
                gbuild_telemetry::session_ctx::log_event(
                    gbuild_telemetry::events::ActionStationarityStop {
                        true_noop,
                        run_len,
            ...
            request.x_grok_agent_id = Some(gbuild_telemetry::id::agent_id());
            ...
            let model_duration_ms = model_timer.elapsed().as_millis() as u64;
            {
                let model_id = self.current_model_id().await;
                gbuild_telemetry::session_ctx::log_event(
                    gbuild_telemetry::events::ModelResponseReceived {
                        model_id,
            ...
            let turn_refused =
                stop_reason == Some(gbuild_sampling_types::StopReason::ContentFilter);
            ...
            for item in response.items {
                match item {
                    gbuild_sampling_types::ConversationItem::Assistant(_) => {
            ...
            if is_true_noop {
                gbuild_telemetry::session_ctx::log_event(gbuild_telemetry::events::ShellTrueNoop {
