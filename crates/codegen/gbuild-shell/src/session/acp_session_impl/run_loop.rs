...
pub(super) async fn run_session(
    ...
    if !session.startup_hints.is_subagent && liveness_watchers_enabled {
        let (event_tx, event_rx) =
            tokio::sync::mpsc::unbounded_channel::<gbuild_mcp::servers::McpClientEvent>();
        {
        ...
        tokio::select! {
                ...
                _ = &mut idle_flush_sleep, if session.idle_flush_timeout.is_some()
                    ...
                    if current_len > last_len {
                        tracing::info!(target: gbuild_telemetry::memory_log::TARGET,
                        ...
                        tracing::debug!(target: gbuild_telemetry::memory_log::TARGET,
                ...
                maybe_cmd = cmd_rx.recv() => {
                    let Some(cmd) = maybe_cmd else {
                        ...
                        let envelope = session.fire_hook(
                            gbuild_hooks::event::HookEventName::SessionEnd,
                            None,
                            gbuild_hooks::event::HookPayload::SessionEnd {
                        ...
                        if let Some(registry) = session.hook_registry.borrow().clone() {
                            ...
                            let results = gbuild_hooks::dispatcher::dispatch_non_blocking(
                                &registry,
                                gbuild_hooks::event::HookEventName::SessionEnd,
                                &envelope,
                                &ctx,
                        ...
                        }
                        {
                            ...
                            if let Some(signals) = session.signals_handle().snapshot().await {
                                gbuild_telemetry::session_ctx::log_event(
                                    gbuild_telemetry::events::SessionEnded {
                    ...
                    match cmd {
                        ...
                        SessionCommand::InjectNotification { prompt_id, prompt_blocks, priority, source } => {
                            ...
                            if is_turn_active && priority == NotificationPriority::Next {
                                ...
                                if let Some(buffer) = &session.tool_context.monitor_event_buffer {
                                    ...
                                    buffer.push_capped(
                                        gbuild_tools::implementations::gbuild::task::types::MonitorEventNotification {
                        ...
                        SessionCommand::RefreshSkillBaseline => {
                            ...
                            tokio::task::spawn_local(async move {
                                ...
                                let new_skills = gbuild_agent::prompt::skills::list_skills_with_plugins(
                                    Some(&cwd),
                                    &skills_config,
                                    pr.as_deref(),
                        ...
                        SessionCommand::UpdateMcpServers { mcp_servers, respond_to } => {
                            ...
                            // also fires for `<cwd>/.gbuild/config.toml` edits,
                            ...
                            {
                                let _ = tx.send(
                                    gbuild_mcp::servers::McpClientEvent::ConfigDiff {
                        ...
                        SessionCommand::DispatchSessionStartHook { source } => {
                            let envelope = session.fire_hook(
                                gbuild_hooks::event::HookEventName::SessionStart,
                                None,
                                gbuild_hooks::event::HookPayload::SessionStart {
                                    source,
                                    model_id: None,
                                    agent_type: None,
                                },
...
pub(super) fn turn_texts_for_feedback(
    conversation: &[gbuild_sampling_types::ConversationItem],
    turn_number: usize,
