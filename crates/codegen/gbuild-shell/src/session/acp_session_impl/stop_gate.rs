...
use gbuild_hooks::{dispatcher, result};
...
/// `description` instead.
fn stop_entry_from_task(task: &gbuild_tools::types::TaskSnapshot) -> StopBackgroundTask {
    let command_text =
...
fn stop_entry_from_subagent(
    summary: &gbuild_tools::implementations::gbuild::task::types::ActiveSubagentSummary,
) -> StopBackgroundTask {
    StopBackgroundTask {
...
fn stop_cron_from_scheduled(
    task: &gbuild_tools::implementations::gbuild::scheduler::types::ScheduledTask,
) -> StopSessionCron {
    StopSessionCron {
        id: task.id.clone(),
        schedule: gbuild_tools::implementations::gbuild::scheduler::interval::interval_to_human(
            task.interval_secs,
        ),
...
impl SessionActor {
    ...
    pub(crate) async fn list_active_subagents(
        &self,
    ) -> Vec<gbuild_tools::implementations::gbuild::task::types::ActiveSubagentSummary> {
    ...
    async fn announce_keep_working(
        ...
        for block in blocks {
            ...
            .await;
            gbuild_telemetry::session_ctx::log_event(gbuild_telemetry::events::HookBlocked {
...
mod stop_gate_snapshot_tests {
    ...
    fn task_snapshot(
        kind: gbuild_tools::computer::types::TaskKind,
    ...
    fn subagent_summary_maps_to_stop_entry() {
        let summary = gbuild_tools::implementations::gbuild::task::types::ActiveSubagentSummary {
    ...
    fn scheduled_task_maps_to_stop_cron() {
        let task = gbuild_tools::implementations::gbuild::scheduler::types::ScheduledTask::new(
            300,
