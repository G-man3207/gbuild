...
fn partition_rules_by_scope(
    files: Vec<gbuild_agent::prompt::agents_md::AgentConfigFile>,
    gbuild_home: &std::path::Path,
    vendor_homes: &[(std::path::PathBuf, bool)],
    workspace_root: Option<&std::path::Path>,
) -> (
    Vec<gbuild_agent::prompt::user_message::RuleEntry>,
    Vec<gbuild_agent::prompt::user_message::RuleEntry>,
) {
    ...
    for file in files {
        ...
        let entry = gbuild_agent::prompt::user_message::RuleEntry::from(file);
        if is_user_rule {
            user.push(entry);
        } else {
...
mod partition_rules_by_scope_tests {
    ...
    use gbuild_agent::prompt::agents_md::AgentConfigFile;
    use std::path::Path;
    ...
    fn paths(entries: &[gbuild_agent::prompt::user_message::RuleEntry]) -> Vec<&str> {
        entries.iter().map(|entry| entry.content.as_str()).collect()
    }
    #[test]
    fn partitions_custom_gbuild_and_vendor_home_rules_as_user_scope() {
        let files = vec![
    ...
    #[test]
    fn gbuild_home_nested_in_workspace_keeps_direct_surfaces_user_scoped() {
        let files = vec![
            file("/repo/config/AGENTS.md"),
            file("/repo/config/rules/global.md"),
            file("/repo/config/.gbuild/rules/project.md"),
    ...
    #[test]
    fn nested_gbuild_home_workspace_files_stay_workspace_scoped() {
        let files = vec![
            file("/custom/gbuild/rules/global.md"),
            file("/custom/gbuild/worktrees/repo/.cursor/rules/project.md"),
            file("/custom/gbuild/worktrees/repo/src/AGENTS.md"),
        ];
        let (workspace, user) = partition_rules_by_scope(
            files,
            Path::new("/custom/gbuild"),
            &[],
            Some(Path::new("/custom/gbuild/worktrees/repo")),
        );
        assert_eq!(paths(&user), vec!["/custom/gbuild/rules/global.md"]);
        assert_eq!(
            paths(&workspace),
            vec![
...
/// end for gBuild ordering), so a failed offload never
...
impl SessionActor {
                ...
                !matches!(
                    ...
                    ConversationItem::User(u)
                        if u.synthetic_reason
                            == Some(gbuild_sampling_types::SyntheticReason::SystemReminder)
                )
            });
        }
    }
    ...
    async fn build_templated_user_message(
        ...
        use gbuild_agent::prompt::agents_md::read_agents_config_with_paths;
        use gbuild_agent::prompt::user_message::UserMessageContext;
    ...
    async fn gather_vcs_for_prefix(
        ...
        use gbuild_workspace::file_system::{git_status_short, jj_status};
        use gbuild_workspace::session::git::VcsKind;
    ...
    ) -> Vec<gbuild_agent::prompt::user_message::McpServerEntry> {
        use gbuild_agent::prompt::user_message::McpServerEntry;
    ...
    pub(super) async fn transcribe_user_images(
        ...
        let client = gbuild_sampler::SamplingClient::new(sampler_config).map_err(|e| {
