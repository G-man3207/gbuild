...
impl SessionActor {
    ...
    pub(super) async fn handle_session_mode(&self, session_mode_id: acp::SessionModeId) {
        ...
        *self.current_prompt_mode.lock() = prompt_mode;
        ...
        if mode.is_plan() {
            ...
            gbuild_telemetry::session_ctx::log_event(gbuild_telemetry::events::PlanModeToggled {
                enabled: true,
                trigger: gbuild_telemetry::events::PlanModeTrigger::User,
                turn_in_flight,
    ...
    /// first-entry/reentry distinction), or gBuild's reentry/full variant.
