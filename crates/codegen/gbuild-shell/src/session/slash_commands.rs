...
use gbuild_tools::implementations::skills::skill::format_skill_name;
...
///   matching today's gBuild flow that calls our dedicated `skill` tool.
...
pub(super) async fn build_skill_information_for_refs(
    ...
    use gbuild_tools::implementations::skills::skill::{
...
/// `gbuild-tools` so it stays identical to the pager's `LoopCommand` and the
...
fn build_loop_prompt_blocks(args: &str) -> Vec<acp::ContentBlock> {
    use gbuild_tools::implementations::gbuild::{loop_schedule_instruction, loop_usage_message};
...
mod tests {
    ...
    fn listing(name: &str) -> crate::session::workflow::registry::WorkflowListing {
        crate::session::workflow::registry::WorkflowListing {
            ...
            when_to_use: None,
            source: "project",
            path: Some(format!(".gbuild/workflows/{name}.rhai")),
        }
    }
