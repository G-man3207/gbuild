...
//! other concurrently running gBuild processes (which may wipe or downgrade
...
/// User-configurable overrides via `~/.gbuild/settings.json` are deferred
/// to a follow-up.
...
///
/// TODO: When multiple gBuild processes run concurrently, they each have
...
fn collect_all_indexable_content_single_pass(updates_path: &Path) -> io::Result<(String, u64)> {
    ...
    for line_result in reader.lines() {
        ...
        // ("_gbuild/session/update"). Dispatch on source first, then tag.
        if !is_xai {
            // ── ACP content events ──────────────────────────────────
            match tag {
