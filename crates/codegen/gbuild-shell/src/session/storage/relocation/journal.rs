...
impl RelocationJournal {
    ...
    pub(super) fn validate(&self, gbuild_home: &Path) -> Result<()> {
        ...
        if session_dir_at(gbuild_home, &self.source_cwd, &self.session_id)
            == session_dir_at(gbuild_home, &self.target_cwd, &self.session_id)
        {
            return Err(RelocationError::Inconsistent(
...
pub(super) fn acquire(gbuild_home: &Path, session_id: &str) -> Result<RelocationLease> {
    validate_component("session id", session_id)?;
    let dir = relocation_dir(gbuild_home);
...
pub(super) fn read(gbuild_home: &Path, session_id: &str) -> Result<RelocationJournal> {
    validate_component("session id", session_id)?;
    let path = journal_path(gbuild_home, session_id);
...
pub(super) fn write(
    ...
    journal
        .validate(gbuild_home)
        .map_err(WriteFailure::NotCommitted)?;
    let path = journal_path(gbuild_home, &journal.session_id);
...
pub(super) fn relocation_dir(gbuild_home: &Path) -> PathBuf {
    gbuild_home.join(RELOCATION_DIR)
}

pub(super) fn journal_path(gbuild_home: &Path, session_id: &str) -> PathBuf {
    relocation_dir(gbuild_home).join(format!("{session_id}.json"))
}

pub(super) fn session_dir_at(gbuild_home: &Path, cwd: &str, session_id: &str) -> PathBuf {
    gbuild_home
        .join("sessions")
...
pub(super) fn sync_namespace(gbuild_home: &Path) -> Result<()> {
    let dir = relocation_dir(gbuild_home);
