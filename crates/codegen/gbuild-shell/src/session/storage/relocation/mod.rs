...
impl RelocationStorage {
    pub(crate) fn new(gbuild_home: PathBuf) -> Self {
        Self {
            gbuild_home,
            #[cfg(test)]
            fault: None,
    ...
    #[cfg(test)]
    fn with_fault(gbuild_home: PathBuf, fault: TestFault) -> Self {
        Self {
            gbuild_home,
            fault: Some(fault),
        }
    ...
    pub(crate) fn acquire(&self, session_id: &str) -> Result<RelocationLease> {
        journal::acquire(&self.gbuild_home, session_id)
    }

    pub(crate) fn read_journal(&self, session_id: &str) -> Result<RelocationJournal> {
        journal::read(&self.gbuild_home, session_id)
    }
    ...
    fn write_journal(&self, journal: &RelocationJournal) -> std::result::Result<(), WriteFailure> {
        journal::write(
            &self.gbuild_home,
            journal,
            self.journal_fault(journal.phase),
        )
    }
    ...
    pub(crate) fn sync_journal_namespace(&self) -> Result<()> {
        ...
        journal::sync_namespace(&self.gbuild_home)
    }
    ...
    pub(crate) fn stage_and_publish(
        ...
        journal.validate(&self.gbuild_home)?;
        let source =
            journal::session_dir_at(&self.gbuild_home, &journal.source_cwd, &journal.session_id);
    ...
    pub(crate) fn recover_all(&self) -> Result<()> {
        for session_id in RelocationView::journal_ids(&self.gbuild_home)? {
    ...
    pub(crate) fn finalize_terminal(
        ...
        proof.journal.validate(&self.gbuild_home)?;
        ...
        let path = journal::journal_path(&self.gbuild_home, &lease.session_id);
    ...
    fn finish_commit(&self, journal: &mut RelocationJournal) -> Result<TerminalRelocation> {
        ...
        let result = (|| {
            ...
            self.sync_journal_namespace()?;
            self.validate_target(journal)?;
            let source = journal::session_dir_at(
                &self.gbuild_home,
                &journal.source_cwd,
                &journal.session_id,
            );
    ...
    fn finish_rollback(&self, journal: &mut RelocationJournal) -> Result<TerminalRelocation> {
        ...
        let result = (|| {
            self.validate_source_summary(journal)?;
            self.remove_transaction_directory(&self.staging_dir(journal))?;
            let target = journal::session_dir_at(
                &self.gbuild_home,
                &journal.target_cwd,
                &journal.session_id,
            );
    ...
    fn validate_request(&self, lease: &RelocationLease, request: &RelocationRequest) -> Result<()> {
        ...
        if journal::session_dir_at(&self.gbuild_home, &request.source_cwd, &request.session_id)
            == journal::session_dir_at(&self.gbuild_home, &request.target_cwd, &request.session_id)
        {
            return Err(RelocationError::Inconsistent(
    ...
    pub(super) fn validate_authoritative_dir(
        ...
        if path
            != journal::session_dir_at(&self.gbuild_home, expected_cwd, &journal.session_id)
                .as_path()
        {
            return Err(RelocationError::Inconsistent(
    ...
    fn validate_target(&self, journal: &RelocationJournal) -> Result<()> {
        let target =
            journal::session_dir_at(&self.gbuild_home, &journal.target_cwd, &journal.session_id);
    ...
    fn target_parent(&self, cwd: &str) -> PathBuf {
        self.gbuild_home
            .join("sessions")
            .join(gbuild_config::encode_cwd_dirname(cwd))
    }

    fn ensure_target_parent(&self, cwd: &str) -> Result<PathBuf> {
        let sessions = self.gbuild_home.join("sessions");
        fs::create_dir_durable(&sessions)?;
        let encoded = gbuild_config::encode_cwd_dirname(cwd);
    ...
    fn staging_dir(&self, journal: &RelocationJournal) -> PathBuf {
        self.gbuild_home
            .join("sessions")
            .join(gbuild_config::encode_cwd_dirname(&journal.target_cwd))
