...
/// scratch root (`<temp_dir>/gbuild-goal-<id>`) and the verifier
...
/// Private per-goal scratch root: `<temp_dir>/gbuild-goal-<verifier_id>`.
///
...
pub(crate) fn goal_scratch_root(verifier_id: &str) -> PathBuf {
    std::env::temp_dir().join(format!("gbuild-goal-{verifier_id}"))
}
...
mod tests {
    ...
    /// `gbuild-goal-<12 hex chars>` scratch-root format (and the
    ...
    /// `temp_dir()` + `verifier_id`: a `gbuild-goal-<vid>` root with an
    ...
    fn scratch_path_helpers_derive_pinned_layout() {
        ...
        assert_eq!(root, std::env::temp_dir().join("gbuild-goal-vid123"));
        assert_eq!(
            implementer_scratch_dir("vid123"),
            std::env::temp_dir()
                .join("gbuild-goal-vid123")
                .join("implementer"),
        );
        assert_eq!(
