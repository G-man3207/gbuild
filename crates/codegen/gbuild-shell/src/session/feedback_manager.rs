...
mod author_identity_tests {
    ...
    async fn env_var_identity_reaches_the_wire_end_to_end() {
        let _email =
            gbuild_test_support::env::EnvGuard::set("GBUILD_TEST_WORK_EMAIL", "ada@corp.example");
        let _name =
            gbuild_test_support::env::EnvGuard::set("GBUILD_TEST_WORK_NAME", "Ada Lovelace");

        // The loader expands `$VAR` at load, exactly as a trusted config tier ships it.
        let mut value = toml::from_str::<toml::Value>(
            r#"
[feedback.user]
name = ["$GBUILD_TEST_WORK_NAME"]
email = ["$GBUILD_TEST_WORK_EMAIL"]
"#,
        )
        .unwrap();
    ...
    /// `GBUILD_USER_METADATA` is merged into the submission and travels with it:
    ...
    async fn workflow_merges_user_metadata_into_submission() {
        let _guard = gbuild_test_support::env::EnvGuard::set(
            "GBUILD_USER_METADATA",
