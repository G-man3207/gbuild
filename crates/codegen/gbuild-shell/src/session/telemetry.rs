use gbuild_telemetry::events::SessionHarness;
use gbuild_workspace::permission::Decision;
...
pub(crate) fn skill_source_label(skill_path: &str, cwd: &str) -> &'static str {
    ...
    let user = crate::util::gbuild_home::gbuild_home()
        .parent()
...
pub(crate) fn format_hook_name(spec: &gbuild_hooks::config::HookSpec) -> String {
...
fn format_hook_source(spec: &gbuild_hooks::config::HookSpec) -> &'static str {
    use gbuild_hooks::config::HookOrigin as O;
    match gbuild_hooks::config::hook_origin(spec) {
...
impl HookRegInfo {
    pub(crate) fn from_spec(spec: &gbuild_hooks::config::HookSpec) -> Self {
        Self {
...
pub(crate) struct SessionHarnessMetrics {
    ...
    pub permission_mode: gbuild_telemetry::enums::PermissionMode,
    pub mcp_server_names: Vec<String>,
    pub lsp_server_names: Vec<String>,
    pub memory_enabled: bool,
    pub auto_update: Option<bool>,
    pub cwd: String,
    pub skills_config: gbuild_agent::prompt::skills::SkillsConfig,
    /// Resolved vendor-compat config, so recorded skill / AGENTS.md names match
    /// what the session actually discovers.
    pub compat: gbuild_tools::types::compat::CompatConfig,
...
impl SessionHarnessMetrics {
    pub async fn into_event(self, hooks: Vec<HookRegInfo>) -> SessionHarness {
        ...
        let agents_md_dir_names =
            gbuild_agent::prompt::agents_md::read_agents_config_with_paths(&self.cwd, self.compat)
                .await
                .iter()
                .filter_map(|f| {
                ...
                .collect();
        let skill_names = gbuild_agent::prompt::skills::list_skills_with_plugins(
            Some(&self.cwd),
            &self.skills_config,
        ...
        SessionHarness {
            ...
            is_git_repo: gbuild_telemetry::context::collect_git_context(&self.cwd).is_git_repo,
