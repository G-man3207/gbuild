...
fn install_client_hook(
    actor: &SessionActor,
    event: gbuild_hooks::event::HookEventName,
    callback_ids: &[&str],
) {
...
async fn client_hooks_fire_without_file_registry() {
    ...
    local
        .run_until(async {
            ...
            install_client_hook(&actor, gbuild_hooks::event::HookEventName::Stop, &["cb_0"]);

            actor.fire_hook(
                gbuild_hooks::event::HookEventName::Stop,
                None,
                gbuild_hooks::event::HookPayload::Stop {
            ...
            assert_eq!(args.request.method.as_ref(), "gbuild/hooks/event");
    ...
    local
        .run_until(async {
            ...
            *actor.agent.borrow_mut() =
                test_agent_with_tools(vec![gbuild_tools::registry::types::ToolConfig::for_tool::<
                    gbuild_tools::implementations::use_tool::UseTool,
                >()])
                .await;
            ...
            client_hooks.insert(
                ...
                vec![crate::extensions::hooks::ClientHookGroup {
                    matcher: Some(
                        gbuild_hooks::matcher::HookMatcher::new("linear__save_issue").unwrap(),
                    ),
...
async fn post_tool_use_and_failure_never_double_fire() {
    ...
    local
        .run_until(async {
            ...
            *actor.agent.borrow_mut() = test_gbuild_agent_with_todo().await;
                ...
                |rx: &mut tokio::sync::mpsc::UnboundedReceiver<xai_acp_lib::AcpClientMessage>| {
                    ...
                    while let Ok(msg) = rx.try_recv() {
                        if let xai_acp_lib::AcpClientMessage::ExtNotification(args) = msg
                            && args.request.method.as_ref() == "gbuild/hooks/event"
                        {
                            let params: serde_json::Value =
                                serde_json::from_str(args.request.params.get()).unwrap();
...
fn file_registry_with_stop_spec(
    event: gbuild_hooks::event::HookEventName,
    script: &str,
) -> gbuild_hooks::discovery::HookRegistry {
    let (mut registry, _) = gbuild_hooks::discovery::load_hooks(None, None);
    registry.append_specs(vec![gbuild_hooks::config::HookSpec {
            ...
            *actor.hook_registry.borrow_mut() =
            ...
            tokio::task::spawn_local(async move {
                while let Some(msg) = gateway_rx.recv().await {
                    match msg {
                        xai_acp_lib::AcpClientMessage::ExtMethod(args) => {
                            if args.request.method.as_ref() == "gbuild/hooks/run" {
                                runs.set(runs.get() + 1);
                            }
                        ...
                        xai_acp_lib::AcpClientMessage::ExtNotification(args) => {
                            if args.request.method.as_ref() == "gbuild/hooks/event" {
...
async fn subagent_session_gates_on_subagent_stop() {
    ...
    local
        .run_until(async {
            ...
            *actor.hook_registry.borrow_mut() =
                Some(std::sync::Arc::new(file_registry_with_stop_spec(
                    gbuild_hooks::event::HookEventName::SubagentStop,
...
/// `"subagent_stop"` on the wire, matching `GBUILD_HOOK_EVENT`.
#[tokio::test(flavor = "current_thread")]
async fn alias_envelope_serializes_canonical_event_name() {
    ...
    local
        .run_until(async {
            ...
            let envelope = actor.make_hook_envelope(
                gbuild_hooks::event::HookEventName::SubagentEnd,
                None,
                gbuild_hooks::event::HookPayload::SubagentStop {
                    phase: gbuild_hooks::event::SubagentStopPhase::Observe,
