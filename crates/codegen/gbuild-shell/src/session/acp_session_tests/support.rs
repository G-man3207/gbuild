...
#[cfg(test)]
pub(crate) async fn test_agent_default() -> gbuild_agent::Agent {
    test_agent_with_tools(vec![]).await
}
#[cfg(test)]
pub(crate) async fn test_agent_backend_search(
    hosted_tools: Vec<gbuild_sampling_types::HostedTool>,
) -> gbuild_agent::Agent {
    let base = test_agent_default().await;
    gbuild_agent::Agent::new(
        base.definition().clone(),
        gbuild_agent::PromptContext::default(),
        String::new(),
        base.tool_bridge().clone(),
        gbuild_agent::ReminderPolicy::default(),
        gbuild_agent::CompactionPolicy::default(),
        hosted_tools,
        true,
    )
}
...
#[cfg(test)]
pub(crate) async fn test_agent_with_goal_tool() -> gbuild_agent::Agent {
    use gbuild_tools::implementations::gbuild::update_goal::UpdateGoalTool;
    use gbuild_tools::registry::types::ToolConfig;
    test_agent_with_tools(vec![ToolConfig::for_tool::<UpdateGoalTool>()]).await
}
/// gBuild agent with the real `TodoWriteTool` (id `todo_write`, kind
/// `Plan`) registered, so `tool_for_kind(ToolKind::Plan)` resolves through the
/// live toolset instead of the literal fallback.
#[cfg(test)]
pub(crate) async fn test_gbuild_agent_with_todo() -> gbuild_agent::Agent {
    use gbuild_tools::implementations::gbuild::todo::TodoWriteTool;
    use gbuild_tools::registry::types::ToolConfig;
...
#[cfg(test)]
pub(crate) async fn test_agent_with_plan_tools() -> gbuild_agent::Agent {
    use gbuild_tools::implementations::gbuild::enter_plan_mode::EnterPlanModeTool;
    use gbuild_tools::implementations::gbuild::exit_plan_mode::ExitPlanModeTool;
    use gbuild_tools::registry::types::ToolConfig;
...
pub(crate) async fn test_agent_with_tools(
    tools: Vec<gbuild_tools::registry::types::ToolConfig>,
) -> gbuild_agent::Agent {
    test_agent_from_config(
        gbuild_tools::registry::types::ToolServerConfig {
            tools,
            behavior_preset: None,
        },
        gbuild_agent::AgentDefinition::default_gbuild(),
        std::sync::Arc::new(gbuild_tools::computer::local::LocalTerminalBackend::new()),
    )
    .await
}
#[cfg(test)]
pub(crate) async fn test_agent_with_user_message_template(
    template: gbuild_agent::prompt::user_message::UserMessageTemplate,
) -> gbuild_agent::Agent {
    let mut definition = gbuild_agent::AgentDefinition::default_gbuild();
...
async fn test_agent_from_config(
    ...
    definition: gbuild_agent::AgentDefinition,
    backend: std::sync::Arc<dyn gbuild_tools::computer::types::TerminalBackend>,
) -> gbuild_agent::Agent {
    use gbuild_tools::computer::local::LocalFs;
    use gbuild_tools::computer::types::AsyncFileSystem;
    ...
    let ctx = SessionContext {
        ...
        session_folder: std::env::temp_dir().join("gbuild-test"),
...
pub(crate) async fn create_test_actor_ex(
    ...
    let cwd = gbuild_paths::AbsPathBuf::new(std::path::PathBuf::from("/tmp")).unwrap();
    let fs = Arc::new(gbuild_workspace::file_system::MockFs::new(
        cwd.to_path_buf(),
    ));
    ...
    tool_context.task_completion_reservations =
        Some(gbuild_tools::reminders::task_completion::TaskCompletionReservations::default());
    tool_context.task_wake_suppressed =
        Some(gbuild_tools::reminders::task_completion::TaskWakeSuppressed::default());
    ...
    let actor = SessionActor {
        ...
        permissions: gbuild_workspace::permission::PermissionHandle::allow_all(),
        tool_context,
