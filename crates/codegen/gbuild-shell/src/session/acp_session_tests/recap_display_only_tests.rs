...
fn over_budget_recap_serializes_to_well_formed_messages_request() {
    ...
    use gbuild_sampling_types::messages::{ContentBlock, MessageContent, MessageRole};
    use gbuild_sampling_types::{ConversationRequest, ToolCall, rs};
    ...
    let msg = gbuild_sampling_types::build_messages_request(&req);
    ...
    local
        .run_until(async {
            ...
            *actor.agent.borrow_mut() = test_agent_with_goal_tool().await;
            ...
            cfg.api_backend = gbuild_sampling_types::ApiBackend::Responses;
...
async fn recap_request_sends_hosted_tools_under_backend_search() {
    use gbuild_sampling_types::HostedTool;
    use gbuild_test_support::MockInferenceServer;
        ...
        .run_until(async {
            ...
            *actor.agent.borrow_mut() = test_agent_with_goal_tool().await;
            ...
            // Backend-search fixture: agent carries hosted tools and both gates are on.
            {
                ...
                *agent_slot = gbuild_agent::Agent::new(
...
async fn recap_hosted_tools_reflect_the_active_per_turn_override() {
    use gbuild_sampling_types::{HostedTool, SearchDateBound, ToolOverrides, XSearchOptions};
    use gbuild_test_support::MockInferenceServer;
