...
async fn persist_ack_waits_for_disk_flush_before_success() {
    ...
    local
        .run_until(async {
            ...
            let sampling_client = crate::sampling::Client::new(gbuild_sampler::SamplerConfig {
    ...
    local
        .run_until(async {
            ...
            agent
                .tool_bridge()
                .update_resource(
                    gbuild_tools::implementations::gbuild::task::types::CurrentPromptIdResource(
                        "running".to_string(),
                    ),
                )
                .await;
    ...
    local
        .run_until(async {
            ...
            actor.chat_state_handle.push_assistant_response(
                ConversationItem::assistant_tool_calls(vec![gbuild_sampling_types::ToolCall {
                    id: "call-1".into(),
...
async fn cancel_propagates_to_sampler_handle_so_no_further_emission() {
    ...
    local
        .run_until(async {
            ...
            let cfg = gbuild_sampler::SamplerConfig {
                api_key: Some("test-key".to_string()),
                ...
                temperature: None,
                top_p: None,
                api_backend: gbuild_sampler::ApiBackend::Responses,
                auth_scheme: Default::default(),
                extra_headers: Default::default(),
                query_params: Default::default(),
                env_http_headers: Default::default(),
            ...
            let sampler_handle = gbuild_sampler::SamplerActor::spawn(
                cfg,
                gbuild_sampler::RetryPolicy::default(),
                sampler_event_tx,
            );
            let (gateway_tx, _gateway_rx) =
                tokio::sync::mpsc::unbounded_channel::<xai_acp_lib::AcpClientMessage>();
            ...
            let request = ConversationRequest {
                items: vec![ConversationItem::User(gbuild_sampling_types::UserItem {
                    content: vec![gbuild_sampling_types::ContentPart::Text { text: "hi".into() }],
...
async fn skill_reminder_deferred_while_turn_running_flushed_when_idle() {
    use gbuild_tools::types::skill_discovery_tracker::{SkillUpdateEffects, SkillUpdateKind};
        ...
        actor
            ...
            .filter(|item| {
                matches!(item, ConversationItem::User(u) if u.content.iter().any(|p| matches!(
                    p,
                    gbuild_sampling_types::ContentPart::Text { text } if text.contains("pdf-tools")
                )))
            })
            .count()
    }
