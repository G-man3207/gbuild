...
//! handshake against rmcp 2.1 in `gbuild-mcp/tests/repro_sse_flood.rs`): a
...
use chrono::Utc;
use gbuild_mcp::servers::{ClientStateKind, HttpConfig, McpClient};
...
async fn entry_b_routes_auth_rejection_but_not_policy_denial() {
    use gbuild_mcp::servers::{is_auth_rejection_message, parse_mcp_tool_name};
