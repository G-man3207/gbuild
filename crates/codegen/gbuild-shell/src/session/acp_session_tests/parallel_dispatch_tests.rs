...
//!
//! These tests verify the parallel dispatch path (GBUILD_PARALLEL_TOOL_DISPATCH):
...
/// JSON key. The compat toolset input types use `path`, and gbuild's
...
#[test]
fn lock_path_for_args_matches_gbuild_file_path() {
    // gbuild search_replace / opencode EditTool / WriteTool / etc.
...
#[test]
fn lock_path_for_args_matches_gbuild_target_file() {
    // gbuild read_file uses #[serde(rename = "target_file")].
    let args = serde_json::json!({
...
#[test]
fn lock_path_for_args_buckets_gbuild_and_compat_to_same_lock_for_same_file() {
    // A mixed batch (e.g. gbuild search_replace + StrReplace
    ...
    assert_eq!(lock_path_for_args(&gbuild), lock_path_for_args(&compat));
}
...
fn test_skill_discovery_deferred_during_parallel_batch() {
    use gbuild_sampling_types::{ConversationItem, SyntheticReason};
