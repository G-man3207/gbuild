use gbuild_tools::computer::local::{LocalTerminalBackend, MockFs};
...
async fn tool_bridge_routes_writes_through_injected_fs() {
    ...
    let ctx = SessionContext {
        ...
        fs,
        cwd: cwd.clone(),
        session_folder: std::env::temp_dir().join("gbuild-test-fs"),
        ...
        skills: vec![],
        state_path: std::env::temp_dir().join("gbuild-test-fs/tool_state.json"),
        memory_backend: None,
