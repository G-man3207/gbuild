...
///   `build_recap_items(...)` verbatim (keeps the gBuild prefix KV cache warm;
...
pub(crate) fn budget_recap_items(
    ...
    // verbatim path keeps the gBuild prefix cache warm.
...
mod tests {
    ...
    fn main_turn_count_counts_real_users_only() {
        use gbuild_sampling_types::{ContentPart, SyntheticReason, ToolCall, UserItem};
        use std::sync::Arc;
    ...
    fn mk_tool_call(id: &str, args: &str) -> gbuild_sampling_types::ToolCall {
        use std::sync::Arc;
