//! gBuild's L5 wiring onto the shared full-replace engine
//! (`gbuild_compaction::code_compaction`).
//!
//! The shared engine drives the sample → retry → degenerate/failure
//! classification loop via [`sample_full_replace_summary`](gbuild_compaction::sample_full_replace_summary);
//! this module adapts gBuild's transport and telemetry to its two seams:
//!
...
//! [`FullReplaceError`](gbuild_compaction::FullReplaceError).
...
use gbuild_sampling_types::{ConversationItem, HostedTool, ToolSpec};
use gbuild_telemetry::events::{CompactionRetryDegraded, CompactionTrigger};
...
/// [`CompactionSampler`] for gBuild's full-replace pass.
///
...
/// else the structured gBuild prompt), so the shared `CompactionPrompt`
/// the engine passes is ignored — the engine builds the gBuild prompt,
...
impl CompactionSampler for ShellCompactionSampler {
    ...
    async fn sample_compaction(
        ...
        // message (compat short vs structured gBuild), ignoring the shared
...
/// Map gBuild's [`CompactFailure`] onto the shared engine's
...
/// [`FullReplaceObserver`] that reproduces gBuild's per-attempt telemetry:
    ...
    fn on_attempt(&self, _attempt: u32, outcome: &FullReplaceAttemptOutcome<'_>) {
        ...
        match outcome {
            ...
            FullReplaceAttemptOutcome::Degenerate {
                ...
                if *will_retry {
                    gbuild_telemetry::session_ctx::log_event(CompactionRetryDegraded {
