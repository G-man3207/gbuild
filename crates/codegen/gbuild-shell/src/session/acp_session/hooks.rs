...
use gbuild_hooks::event::{HookEventEnvelope, HookEventName, HookPayload};
use gbuild_telemetry::events::ClientHookGateOutcome;
...
fn matching_callback_ids<'a>(
    ...
    groups
        .iter()
        .filter(|group| gbuild_hooks::matcher::matcher_allows(group.matcher.as_ref(), match_value))
...
impl SessionActor {
    ...
    fn client_gate_responses<'a>(
        ...
        let default_timeout =
            if envelope.hook_event_name.traits().gate == gbuild_hooks::event::GateKind::Stop {
        ...
        groups
            .iter()
            .filter(move |group| {
                gbuild_hooks::matcher::matcher_allows(group.matcher.as_ref(), tool_name)
            })
            .flat_map(move |group| {
            ...
            .map(move |(callback_id, timeout)| {
                ...
                async move {
                    ...
                    gbuild_telemetry::session_ctx::log_event(
                        gbuild_telemetry::events::ClientHookGate {
    ...
    pub(super) async fn run_stop_client_hooks(
        &self,
        envelope: &HookEventEnvelope,
    ) -> gbuild_hooks::dispatcher::StopDispatchResult {
        use gbuild_hooks::result::HookRunResult;

        let mut out = gbuild_hooks::dispatcher::StopDispatchResult::default();
        ...
        for (callback_id, (response, elapsed)) in ordered {
            ...
            let detail = gbuild_hooks::dispatcher::stop_detail(
                stop_reason.is_some(),
            ...
            out.absorb(
                &hook_name,
                gbuild_hooks::dispatcher::StopSignals {
                    block_reason,
                    stop_reason,
...
mod tests {
        ...
        local
            .run_until(async {
                ...
                *actor.hook_registry.borrow_mut() = Some(std::sync::Arc::new(
                    gbuild_hooks::discovery::HookRegistry::default(),
                ));
                assert!(actor.hook_event_active(HookEventName::Stop));
    ...
    fn matching_callback_ids_filters_by_matcher() {
        use gbuild_hooks::matcher::HookMatcher;
