...
//!   under `~/.gbuild/worktree_pool/<instance_id>/` with a `.pid` liveness
...
/// `~/.gbuild/worktree_pool/<instance_id>/` with a `.pid` file recording
...
impl WorktreePool {
    ...
    /// `~/.gbuild/worktree_pool/<instance_id>/`. A `.pid` file is written so
...
pub struct AdoptableWorktree {
    /// Current path: `~/.gbuild/worktree_pool/<old_instance>/<pool_id>/`
...
/// `~/.gbuild/worktree_pool/`, reads each `.pid` file, and checks
...
/// The base pool directory under `~/.gbuild/`.
...
/// `CleanAll` mode. Placed under `~/.gbuild/worktree_pool/<instance_id>/<uuid>/`.
