//! Shim — see `gbuild_telemetry::instrumentation` for the implementation.
//!
...
//!   (i.e. `gbuild_shell::instrumentation_timer!`). Keeping the macro here
...
pub use gbuild_telemetry::instrumentation::{
...
///
/// Stays in shell so callers can keep calling `gbuild_shell::instrumentation::finalize_and_exit`.
...
///
/// Macro stays in shell so `$crate` continues to resolve to `gbuild_shell`
/// for the 12+ existing call sites that spell it as
/// `crate::instrumentation_timer!(...)` or `gbuild_shell::instrumentation_timer!(...)`.
/// The macro body delegates to types and functions in
/// `gbuild_telemetry::instrumentation`.
#[macro_export]
