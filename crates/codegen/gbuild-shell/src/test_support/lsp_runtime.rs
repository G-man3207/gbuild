...
pub(crate) fn ctx_with_toggle(toggle: HashMap<String, bool>) -> SubagentSpawnContext {
    ...
    SubagentSpawnContext {
        ...
        fs: Arc::new(gbuild_workspace::file_system::LocalFs::new(PathBuf::from(
            "/tmp",
        ))),
        ...
        auth_manager: Arc::new(crate::auth::AuthManager::new(
            std::path::Path::new("/tmp/nonexistent-gbuild-test"),
...
impl gbuild_tools::implementations::lsp::LspBackend for DummyLspDispatch {
    ...
    async fn dispatch(
        &self,
        _input: &gbuild_tools::implementations::lsp::LspToolInput,
    ) -> gbuild_tools::implementations::lsp::LspToolResult {
        gbuild_tools::implementations::lsp::LspToolResult {
            text: String::new(),
            is_error: false,
        }
    ...
    ) -> Option<gbuild_tools::implementations::lsp::DiagnosticsSummary> {
        None
    }
    ...
    ) -> Vec<gbuild_tools::implementations::lsp::FileDiagnosticEntry> {
        vec![]
    }
}
