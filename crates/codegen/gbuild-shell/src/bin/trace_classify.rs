...
//!       [--api-key <key> | $XAI_API_KEY | <gbuild-home>/auth.json] \
//!       [--min-confidence 0.7] \
...
use clap::Parser;
use gbuild_shell::trace_classifier::{RunArgs, run, validate_min_confidence};
...
struct Cli {
    ...
    /// `$XAI_API_KEY`, then `<gbuild-home>/auth.json` (`xai::api_key`
    ...
    /// shell uses (`$GBUILD_HOME` or `~/.gbuild`). Exposed primarily for
    /// tests / sandboxed invocations.
    #[arg(long)]
    gbuild_home: Option<PathBuf>,
}
...
mod tests {
    ...
    fn cli_parses_minimal_args() {
        ...
        assert!(cli.gbuild_home.is_none());
    }
    ...
    #[test]
    fn cli_gbuild_home_override_parses() {
        ...
        .expect("parse");
        assert_eq!(cli.gbuild_home, Some(PathBuf::from("/tmp/scratch-gbuild")));
    }
