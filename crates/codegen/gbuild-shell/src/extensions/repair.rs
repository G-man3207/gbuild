...
pub async fn handle(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    match args.method.as_ref() {
        "gbuild/session/repair" => handle_session_repair(agent, args).await,
...
/// back atomically. `gbuild_root` is injectable for tests.
async fn repair_on_disk(
    gbuild_root: &std::path::Path,
    session_id: &str,
    dry_run: bool,
) -> ExtResult {
    let summary = crate::session::persistence::find_summary_by_session_id_in_root(
        session_id,
        &gbuild_root.join("sessions"),
    )
    .ok_or_else(|| {
    ...
    let storage = JsonlStorageAdapter::with_root(gbuild_root.to_path_buf());
