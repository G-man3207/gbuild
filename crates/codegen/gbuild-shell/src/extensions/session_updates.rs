...
//! the update type (`"session/update"` for ACP, `"_gbuild/session/update"` for
...
mod tests {
    ...
    fn make_request(
        ...
        acp::ExtRequest::new("gbuild/session/updates", std::sync::Arc::from(raw))
    }
    ...
    async fn handle_tail_request_with_rewind_returns_only_live_timeline() {
        ...
        std::fs::write(
            ...
            [
                ...
                r#"{"timestamp":5,"method":"_gbuild/session/update","params":{"sessionId":"s","update":{"sessionUpdate":"rewind_marker","target_prompt_index":1}}}"#,
    ...
    async fn handle_falls_back_to_id_lookup_for_divergent_cwd() {
        ...
        // Clean up the dir written under the real gBuild home.
    ...
    fn xai_rewind(target: usize) -> String {
        format!(
            r#"{{"timestamp":0,"method":"_gbuild/session/update","params":{{"sessionId":"s","update":{{"sessionUpdate":"rewind_marker","target_prompt_index":{target}}}}}}}"#
        )
    }
