...
use gbuild_agent::prompt::skills::{
...
fn discover_auto_sources(cwd: &str, skills: &[SkillInfo]) -> Vec<(String, usize)> {
    ...
    for subdir in &subdirs {
        try_add_source(gbuild_home.join(subdir), None);
    }
...
pub async fn handle(
    ...
    match args.method.as_ref() {
        "gbuild/skills/add" => {
            ...
            .await
            {
                gbuild_telemetry::session_ctx::log_event(gbuild_telemetry::events::SkillAdded {
                    added_count: 0,
                    total_skills: 0,
                    success: false,
                });
        ...
        "gbuild/skills/remove" => {
            ...
            .await
            {
                gbuild_telemetry::session_ctx::log_event(gbuild_telemetry::events::SkillRemoved {
                    success: false,
                });
