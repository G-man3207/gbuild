...
use gbuild_telemetry::id::agent_id;
...
async fn handle_session_delete(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    ...
    // Mirrored by the `gbuild sessions delete <id>` CLI path.
...
/// hot-reload watcher when `<cwd>/.gbuild/config.toml`,
...
/// Hot-reload the model catalog from `~/.gbuild/models_cache.json` after an
