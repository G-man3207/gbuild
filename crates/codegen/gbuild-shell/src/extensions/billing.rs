...
//!
//! Fetches the authenticated user's gBuild billing configuration
...
/// Current billing configuration for gBuild coding credits.
///
/// Carries both the newer credits-config fields (`credit_usage_percent`,
/// `current_period`) and the deprecated `GBuildBillingConfig` fields
...
/// `GetGBuildBillingConfig` backend responses.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct BillingConfig {
    ...
    /// absent (legacy `GetGBuildBillingConfig` shape or older servers).
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
///
/// Keeps history to a count + the most recent period so `~/.gbuild/logs/unified.jsonl`
...
async fn handle_get_billing(agent: &MvpAgent) -> ExtResult {
    let auth = super::auth_gate::require_xai_auth(
        ...
        "Billing data requires auth with grok.com. Run `gbuild login` to authenticate.",
    )?;
...
async fn handle_get_auto_topup_rule(agent: &MvpAgent) -> ExtResult {
    let auth = super::auth_gate::require_xai_auth(
        ...
        "Auto top-up data requires auth with grok.com. Run `gbuild login` to authenticate.",
    )?;
    ...
    fn billing_config_deserializes_credits_config_shape() {
        ...
        let json = serde_json::json!({
            "config": {
                ...
                "productUsage": [
                    {"product": "PRODUCT_GBUILD", "usagePercent": 61.2}
                ],
                "history": [
                    {
                        "period": {
