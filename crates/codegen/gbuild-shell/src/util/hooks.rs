...
use gbuild_config::resolve_global_hook_sources;
use gbuild_hooks::discovery::HookSource;
use gbuild_hooks::error::HookError;
...
fn include_claude_hooks(compat: &gbuild_tools::types::compat::CompatConfig) -> bool {
    compat.claude.hooks
        && !crate::claude_import::is_claude_import_marked_with_log("discover_hook_source_paths")
}

fn include_cursor_hooks(compat: &gbuild_tools::types::compat::CompatConfig) -> bool {
    compat.cursor.hooks
}
...
pub fn discover_hook_source_paths(
    ...
    // Soft hooks-paths I/O keeps fixed slots; hard resolve omits gBuild globals.
    let mut global: Vec<PathBuf> =
        match resolve_global_hook_sources(gbuild.as_deref(), /* reject_symlinks */ false) {
            Ok(resolved) => {
                if let Some(e) = &resolved.configured_error {
                    tracing::warn!(
                        error = %e,
                        "hooks-paths unreadable; retaining fixed gBuild hook discovery sources only"
                    );
                }
                resolved
                    .discovery_sources()
            ...
            Err(e) => {
                tracing::warn!(
                    error = %e,
                    "global hook source resolve hard-failed; omitting gBuild global sources"
                );
                Vec::new()
            }
        };
    ...
    if let Some(root) = git_root {
        ...
        project.push(root.join(".gbuild").join("hooks"));
        if include_cursor {
...
/// dedup in [`gbuild_hooks::discovery::registry_from_specs_deduped`], a config
...
pub fn assemble_hooks(
    ...
    (
        gbuild_hooks::discovery::registry_from_specs_deduped(specs),
        errors,
    )
}
