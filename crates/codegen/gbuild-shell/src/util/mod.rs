...
// The foundation utilities live in `gbuild-shell-base` (upstream of this
// crate so they build in parallel). Re-exported at the original paths so
// existing `crate::util::…` / `gbuild_shell::util::…` users compile
// unchanged.
pub use gbuild_shell_base::util::*;
...
pub(crate) fn is_user_instruction_path(
    ...
    let gbuild_rules = gbuild_home.join("rules");
    let is_exact_home_surface = parent
        .is_some_and(|parent| parent == gbuild_home || parent == gbuild_rules)
...
mod is_user_instruction_path_tests {
    ...
    #[test]
    fn gbuild_home_named_file_nested_in_workspace_is_user_scoped() {
    ...
    #[test]
    fn workspace_descendants_under_gbuild_home_stay_project_scoped() {
        assert!(!is_user_instruction_path(
            Path::new("/custom/gbuild/worktrees/repo/src/AGENTS.md"),
