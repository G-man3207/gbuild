...
use gbuild_tools::types::compat::{CompatConfig, CompatConfigToml};
...
pub use gbuild_mcp::oauth_config::{McpOAuthConfig, McpOAuthConfigMap};
// MCP server config value types extracted to `gbuild-config-types` (config
...
pub use gbuild_config_types::{McpConfig, RelaySyncConfig};
// Worktree-pool config value type extracted; re-exported to keep paths stable.
pub use gbuild_config_types::PoolConfig;
...
/// Walks from cwd up to the git repo root checking `.gbuild/config.toml` at each level.
/// Project-scoped `.gbuild/config.toml` entries override global `~/.gbuild/config.toml`
...
/// `.gbuild/config.toml`, otherwise user (global config, `~/.claude.json`,
...
/// Load MCP servers with project-scoped overrides from `.gbuild/config.toml`.
///
/// Merge strategy:
/// 1. Load MCP servers from global `~/.gbuild/config.toml`
/// 2. Walk from git repo root down to `cwd`, loading `.gbuild/config.toml` at each level
        ...
        if let Ok(root) = crate::config::load_config_file(config_path) {
            ...
            if !project_servers.is_empty() {
                tracing::info!(
                    ...
                    "Loaded project-scoped MCP servers from .gbuild/config.toml"
                );
...
pub fn mcp_preferences_path() -> PathBuf {
    gbuild_config::gbuild_home().join("mcp_preferences.json")
}
...
pub fn collect_mcp_setup_configs(
    cwd: &std::path::Path,
    plugin_registry: Option<&gbuild_agent::plugins::PluginRegistry>,
    compat: &CompatConfig,
    ...
    if let Some(registry) = plugin_registry {
        ...
        for plugin in registry.active_plugins() {
            ...
            if let Some(ref inline_value) = plugin.inline_mcp_servers {
                let normalized =
                    gbuild_agent::plugins::manifest::normalize_inline_mcp_servers(inline_value);
...
///
/// Uses the top-level `disabled_mcp_servers` array in `~/.gbuild/config.toml`.
...
/// Upsert an MCP server entry in `~/.gbuild/config.toml`.
///
...
/// config file, e.g. a project-scoped `.gbuild/config.toml`.
...
/// Delete an MCP server entry from `~/.gbuild/config.toml`.
///
...
/// config file, e.g. a project-scoped `.gbuild/config.toml`. OAuth credential
...
pub async fn delete_mcp_server_config_at(
    ...
    if let Ok(mut cred_store) = gbuild_mcp::credentials::McpCredentialStore::load_default() {
...
/// project-scoped `.gbuild/config.toml` files.
...
/// transport-less case is steered to `disabled_mcp_servers`, gBuild's real
/// disable mechanism.
    ...
    let message = if !has_command && !has_url {
        format!(
            "`mcp_servers.{name}` has no transport. To run it, set `command = \"...\"` or \
             ...
             See ~/.gbuild/docs/user-guide/07-mcp-servers.md"
        )
    } else {
        format!(
                    ...
                    problems.push(McpServerConfigProblem {
                        ...
                        message: format!(
                            "`mcp_servers.{name}` has an unrecognized field `{field}`; it is \
                             ignored. See ~/.gbuild/docs/user-guide/07-mcp-servers.md"
                        ),
                    });
                }
                if config.enabled
                    ...
                    problems.push(McpServerConfigProblem {
                        ...
                        message: format!(
                            "`mcp_servers.{name}` is enabled but its `{field}` is blank. \
                             ...
                             off. See ~/.gbuild/docs/user-guide/07-mcp-servers.md"
                        ),
                    });
                    continue;
                }
...
/// (`"user"` or `"project"`).
///
/// Overlays project-scoped `.gbuild/config.toml` files from `cwd` up to the
...
/// [`load_mcp_server_configs_with_project`], for `gbuild inspect`.
...
fn config_path() -> PathBuf {
    crate::util::gbuild_home::gbuild_home().join("config.toml")
}

/// Path to the user-level config file (`~/.gbuild/config.toml`).
pub fn user_config_path() -> PathBuf {
    config_path()
}

/// Path to a project-level config file (`<dir>/.gbuild/config.toml`).
pub fn project_config_path(dir: &std::path::Path) -> PathBuf {
    dir.join(".gbuild").join("config.toml")
}
...
/// Overrides `[cli] session_registry`; usable before `~/.gbuild/config.toml` exists.
pub const SESSION_REGISTRY_ENV_VAR: &str = "GBUILD_SESSION_REGISTRY";

pub fn session_registry_from_env_opt() -> Option<bool> {
    gbuild_config::env_bool(SESSION_REGISTRY_ENV_VAR)
}
...
mod tests {
    ...
    fn session_registry_local_override_precedence() {
        ...
        {
            let _g = gbuild_test_support::EnvGuard::set(SESSION_REGISTRY_ENV_VAR, "false");
            assert_eq!(
        ...
        {
            let _g = gbuild_test_support::EnvGuard::set(SESSION_REGISTRY_ENV_VAR, "bogus");
            assert_eq!(
                session_registry_local_override_sourced(Some(&toml_true)),
        ...
        {
            let _g = gbuild_test_support::EnvGuard::unset(SESSION_REGISTRY_ENV_VAR);
...
"#;
    ...
    fn skills_config_parses_paths_and_ignore() {
        let root = toml::from_str::<TomlValue>(
            r#"
[skills]
paths = ["~/.gbuild/skills", "~/.gbuild/skills/special/SKILL.md"]
ignore = ["~/.gbuild/skills/noisy/SKILL.md"]
"#,
        )
        .unwrap();
        ...
        assert_eq!(
            cfg.paths,
            vec!["~/.gbuild/skills", "~/.gbuild/skills/special/SKILL.md"]
        );
        assert_eq!(cfg.ignore, vec!["~/.gbuild/skills/noisy/SKILL.md"]);
    }
...
"#;
    ...
    fn mcp_json_env_var_default_value() {
        ...
        std::fs::write(
            ...
            r#"{
                "mcpServers": {
                    "api": {
                        "url": "${GBUILD_TEST_MCP_UNSET_VAR_12345:-https://fallback.example.com}/mcp"
                    }
                }
            }"#,
        )
    ...
    fn mcp_json_all_toml_names_includes_disabled() {
        ...
        std::fs::create_dir_all(&gbuild_dir).unwrap();
        std::fs::write(
            gbuild_dir.join("config.toml"),
            r#"
