...
pub fn load_relay_sync_enabled_sync() -> bool {
    toml_bool_sync(Some("GBUILD_RELAY_SYNC_ENABLED"), "relay", "enabled")
}
...
/// 1. Nearest project `.gbuild/config.toml` with `[permission]` section (from cwd upward)
/// 2. Global `~/.gbuild/config.toml` `[permission]` section
///
