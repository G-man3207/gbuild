...
/// [`gbuild_config::env_bool`] parser (`GBUILD_TOOLS_FIND_BFS` /
/// `GBUILD_TOOLS_GREP_UGREP`).
///
/// Pass the **merged** requirements ([`crate::config::load_merged_requirements`])
/// so an org policy in any requirements layer — not only
/// `~/.gbuild/requirements.toml` — is honored. Returns `(find_bfs, grep_ugrep)`,
/// which the caller bakes into a
/// [`gbuild_tools::computer::local::SearchShadowConfig`] on the local terminal
/// backend.
pub fn resolve_search_tools_enabled(
    ...
    let disable = gbuild_config::env_bool("DISABLE_EMBEDDED_SEARCH_TOOLS");
    fn from_toml(v: Option<&TomlValue>, key: &str) -> Option<bool> {
        v?.get("toolset")?.get("bash")?.get(key)?.as_bool()
    }
    let resolve = |env_name: &str, key: &str| -> bool {
        let env = gbuild_config::env_bool(env_name);
    ...
    (
        resolve("GBUILD_TOOLS_FIND_BFS", "find_bfs"),
        resolve("GBUILD_TOOLS_GREP_UGREP", "grep_ugrep"),
    )
}
...
) -> Option<gbuild_tools::util::ShellEnvironmentPolicy> {
    let value = effective_cfg?.get("shell_environment_policy")?.clone();
    match value.try_into::<gbuild_tools::util::ShellEnvironmentPolicy>() {
...
const ENV_LOGIN_SHELL_CAPTURE: &str = "GBUILD_LOGIN_ENV";
...
const ENV_SCHEDULER_BACKGROUND_LOOPS: &str = "GBUILD_SCHEDULER_BACKGROUND_LOOPS";
...
///
/// Precedence: requirements > env (`GBUILD_SCHEDULER_BACKGROUND_LOOPS`) > user
...
/// the shared [`gbuild_config::env_bool`] via `BoolFlag`). The secs env var
/// lives in the tools crate (`RESPONSE_TIMEOUT_ENV`), parsed once there.
const ENV_ASK_USER_QUESTION_TIMEOUT_ENABLED: &str = "GBUILD_ASK_USER_QUESTION_TIMEOUT_ENABLED";
...
///
/// Precedence: requirements > env (`GBUILD_ASK_USER_QUESTION_TIMEOUT_ENABLED`)
...
fn resolve_ask_user_question_timeout_secs_from_tiers(
    ...
    requirement
        ...
        .or(remote)
        .unwrap_or(
            gbuild_tools::implementations::gbuild::ask_user_question::RESPONSE_TIMEOUT.as_secs(),
        )
}

/// Resolve `[toolset.ask_user_question] timeout_secs` (positive seconds).
///
/// Precedence: requirements > env (`GBUILD_ASK_USER_QUESTION_TIMEOUT_SECS`,
...
fn resolve_ask_user_question_timeout_secs(
    ...
    resolve_ask_user_question_timeout_secs_from_tiers(
        ...
        gbuild_tools::implementations::gbuild::ask_user_question::response_timeout_env_secs(),
...
) -> gbuild_tools::implementations::gbuild::ask_user_question::AskUserQuestionParams {
    let requirements = crate::config::load_merged_requirements();
    let layers = match crate::config::ConfigLayers::load() {
        Ok(l) => Some(l),
        Err(e) => {
...
mod ask_user_question_timeout_tests {
    ...
    use gbuild_tools::implementations::gbuild::ask_user_question::RESPONSE_TIMEOUT_ENV;
...
mod tests {
    ...
    // Assumes GBUILD_TOOLS_* / DISABLE_EMBEDDED_SEARCH_TOOLS are unset in the test env.
    #[test]
...
mod shell_env_policy_tests {
    use super::*;
    use gbuild_tools::util::{EnvironmentVariablePattern, ShellEnvironmentPolicyInherit};
