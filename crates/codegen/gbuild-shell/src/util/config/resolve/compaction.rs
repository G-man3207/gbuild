...
pub(crate) const ENV_COMPACTION_TOOL_CHOICE: &str = "GBUILD_COMPACTION_TOOL_CHOICE";
...
pub(crate) const ENV_AUTO_COMPACT_THRESHOLD_PERCENT: &str = "GBUILD_AUTO_COMPACT_THRESHOLD_PERCENT";
...
///   1. env `GBUILD_AUTO_COMPACT_THRESHOLD_PERCENT`
...
///      (populated from `gbuild_models[i].auto_compact_threshold_percent`;
...
///      (populated from `gbuild_settings.auto_compact_threshold_percent`)
...
const ENV_COMPACTION_WALL_CLOCK_BUDGET_SECS: &str = "GBUILD_COMPACTION_WALL_CLOCK_SECS";

/// Resolve the per-compaction wall-clock budget (seconds). Precedence: env
/// `GBUILD_COMPACTION_WALL_CLOCK_SECS` > remote settings global
...
mod compaction_wall_clock_budget_tests {
    ...
    // Assumes GBUILD_COMPACTION_WALL_CLOCK_SECS is unset in the test env.
    #[test]
