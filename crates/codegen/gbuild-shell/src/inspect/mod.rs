//! `gbuild inspect` — configuration introspection.
//!
//! Shows everything gBuild discovers in the current directory: project
...
use gbuild_tools::util::truncate::estimate_tokens;
...
pub struct LoginPolicyReport {
    /// Raw `disable_api_key_auth` knob (env `GBUILD_DISABLE_API_KEY_AUTH`).
...
pub struct ConfigSources {
    ...
    /// .gbuild/config.toml files). Driven from the same resolvers used at runtime
...
/// A single config layer entry for `gbuild inspect`.
#[derive(Debug, Clone, Serialize)]
#[serde(rename_all = "camelCase")]
...
async fn build_report(cwd: &Path) -> InspectReport {
    ...
    InspectReport {
        gbuild_version: gbuild_version::VERSION.to_string(),
...
fn instruction_file_type(
    ...
    if path
        .parent()
        .is_some_and(|parent| parent == gbuild_home.join("rules"))
        || has_rules_directory(file_path, ".gbuild")
...
async fn list_instructions(cwd: &Path) -> Vec<InstructionFile> {
    ...
    let configs = gbuild_agent::prompt::agents_md::read_agents_config_with_paths(
        &cwd.display().to_string(),
        gbuild_agent::prompt::skills::CompatConfig::default(),
    )
    .await;
    ...
    // `gbuild_agent::prompt::agents_md::read_agents_config_with_paths`
    ...
    configs
        ...
        .map(|c| {
            let file_type =
                instruction_file_type(&c.file_path, &gbuild_home, imported, &extra_rule_prefixes);
            let scope =
                instruction_scope(&c.file_path, &gbuild_home, &vendor_homes, &workspace_root);
...
fn list_hooks(
    ...
    let mut entries: Vec<HookEntry> = registry
        ...
        .map(|h| {
            ...
            use gbuild_hooks::config::HookOrigin as O;
            ...
            let source = match gbuild_hooks::config::hook_origin(h) {
                O::SystemManaged | O::Managed => ConfigSource::Managed {
                    path: Some(config_file(gbuild_config::MANAGED_CONFIG_FILENAME)),
                },
                O::Requirements => ConfigSource::Managed {
                    path: Some(config_file(gbuild_config::REQUIREMENTS_FILENAME)),
                },
                O::UserConfig => ConfigSource::ConfigToml {
                    path: config_file(gbuild_config::USER_CONFIG_FILENAME),
                },
...
async fn list_skills(
    cwd: &Path,
    plugin_registry: &gbuild_agent::plugins::PluginRegistry,
    skills_config: &gbuild_agent::prompt::skills::SkillsConfig,
) -> Vec<SkillEntry> {
    // Discover with all vendors ON so inspect shows the full set.
    let skills = gbuild_agent::prompt::skills::list_skills_with_plugins(
...
fn skill_entry_source(s: &gbuild_agent::prompt::skills::SkillInfo) -> ConfigSource {
    use gbuild_tools::implementations::skills::types::SkillScope;
...
fn list_plugins(discovered: &[gbuild_agent::plugins::DiscoveredPlugin]) -> Vec<PluginEntry> {
    discovered
        .iter()
        .map(|p| {
            let scope = match p.scope {
                gbuild_agent::plugins::PluginScope::CliOverride => Scope::Cli,
                gbuild_agent::plugins::PluginScope::Project => Scope::Project,
                gbuild_agent::plugins::PluginScope::User => Scope::User,
                gbuild_agent::plugins::PluginScope::ConfigPath => Scope::Config,
            };
            PluginEntry {
                ...
                provides: PluginProvides {
                    ...
                    skills: gbuild_agent::plugins::registry::skill_md_paths(&p.skill_dirs).len(),
...
fn list_marketplaces(git_root: Option<&Path>) -> Vec<MarketplaceEntry> {
    ...
    gbuild_agent::plugins::marketplace::resolve(root)
        .into_iter()
...
fn list_lsp_servers(
    ...
    let servers = gbuild_tools::implementations::lsp::config::load_servers_with_plugins_sourced(
        cwd,
        &plugin_lsp_paths,
        &inline_values,
        &plugin_names,
...
/// `.gbuild/config.toml` files (via `find_project_configs`). The macOS MDM
...
fn list_config_sources(cwd: &Path) -> ConfigSources {
    ...
    // User managed
    if let Some(home) = crate::config::user_gbuild_home() {
...
fn print_human(r: &InspectReport) {
    println!();
    println!("  Environment");
    println!("  {TREE} Version: {} [{}]", r.gbuild_version, r.channel);
    ...
    if r.mcp_servers.is_empty() {
        ...
        println!("  {TREE} (none) \u{2014} see `gbuild mcp add --help`");
    } else {
        print_columns(
            "MCP Servers",
            &r.mcp_servers,
...
mod tests {
    use super::*;
    use gbuild_agent::prompt::skills::{SkillInfo, SkillsConfig};
    use gbuild_tools::implementations::skills::types::SkillScope;
    ...
    fn vendor_rule_paths_select_rules_compatibility_cells() {
        ...
        for (vendor, path) in [
            ...
            let file_type =
                instruction_file_type(path, Path::new("/home/user/.gbuild"), false, &[]);
        ...
        for path in [
            "/repo/.gbuild/rules/team.md",
            r"C:\repo\.gbuild\rules\team.md",
        ] {
            assert_eq!(
                instruction_file_type(path, Path::new("/home/user/.gbuild"), false, &[]),
                "rules"
        ...
        for path in [
            ...
            assert_eq!(
                instruction_file_type(path, Path::new("/home/user/.gbuild"), true, &[]),
                "rules"
            );
        }
        for path in [
            ...
            let file_type = instruction_file_type(path, Path::new("/home/user/.gbuild"), true, &[]);
    ...
    #[test]
    fn gbuild_home_nested_in_workspace_keeps_direct_surfaces_global() {
        let gbuild_home = Path::new("/repo/config");
        let workspace = Path::new("/repo");
        for path in ["/repo/config/AGENTS.md", "/repo/config/rules/global.md"] {
            assert!(matches!(
                instruction_scope(path, gbuild_home, &[], workspace),
                Scope::Global
            ));
        }
        for path in [
            "/repo/config/.gbuild/rules/project.md",
    ...
    fn vendor_home_nested_in_workspace_keeps_direct_surfaces_global() {
        ...
        for path in ["/repo/.claude/rules/global.md", "/repo/.claude/CLAUDE.md"] {
            assert!(matches!(
                instruction_scope(path, Path::new("/other/gbuild"), &vendor_homes, workspace),
                Scope::Global
            ));
        }
        for path in [
    ...
    #[test]
    fn workspace_scope_wins_inside_gbuild_home() {
        let gbuild_home = Path::new("/custom/gbuild");
        let workspace = Path::new("/custom/gbuild/worktrees/repo");
        for path in [
            "/custom/gbuild/worktrees/repo/.cursor/rules/project.md",
            "/custom/gbuild/worktrees/repo/src/AGENTS.md",
        ] {
            assert!(matches!(
                instruction_scope(path, gbuild_home, &[], workspace),
                Scope::Project
    ...
    #[test]
    fn custom_gbuild_home_rules_are_classified_as_rules() {
        assert_eq!(
    ...
    fn requirements_layer_contributes_requires_non_empty_post_strip_table() {
        ...
        let path = "/home/u/.gbuild/requirements.toml";
    ...
    fn skill_entry_source_maps_scopes() {
        let s = skill_fixture("a", "/repo/.gbuild/skills/a/SKILL.md", SkillScope::Local);
        assert!(matches!(
            skill_entry_source(&s),
            ConfigSource::Project { .. }
        ));

        let s = skill_fixture("b", "/repo/.gbuild/skills/b/SKILL.md", SkillScope::Repo);
        assert!(matches!(
            skill_entry_source(&s),
            ConfigSource::Project { .. }
        ));

        let s = skill_fixture("c", "/home/u/.gbuild/skills/c/SKILL.md", SkillScope::User);
        assert!(matches!(skill_entry_source(&s), ConfigSource::User { .. }));

        let s = skill_fixture(
            "d",
            "/home/u/.gbuild/server-skills/d/SKILL.md",
            SkillScope::Server,
        );
        assert!(matches!(
            skill_entry_source(&s),
        ...
        let s = skill_fixture(
            "e",
            "/home/u/.gbuild/bundled/e/SKILL.md",
            SkillScope::Bundled,
        );
        assert!(matches!(
            skill_entry_source(&s),
    ...
    async fn list_skills_honors_skills_config() {
        ...
        // Test-unique names: discovery also reads this machine's real ~/.gbuild dirs.
        ...
        let registry = gbuild_agent::plugins::PluginRegistry::from_discovered(vec![], &[], &[]);
