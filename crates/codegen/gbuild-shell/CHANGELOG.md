...
- **New /tutorial slash command** opens an opt-in nine-topic onboarding tour of gBuild.
...
- **`gbuild doctor fix`** can now repair common tmux clipboard and passthrough problems.
- **Per-provider auth helpers** now work on Windows and can run from a configurable working directory.
- **/resume** now shows only native gBuild sessions by default and shows a hint when external sessions are hidden.
- **`gbuild --resume`** can now resume a session by its title as well as by ID.
...
- You can now run `gbuild doctor fix` commands directly from inside the TUI instead of only from the CLI.
...
- **npm package** now installs the native binary under `$GBUILD_HOME/bin` (honoring the same override as the Rust CLI).
...
- gBuild now automatically stops a turn that keeps repeating the exact same tool call many times in a row.
...
- When the auto-permission classifier times out or fails, gBuild now shows a normal permission prompt instead of silently denying.
...
- **gbuild doctor fix ssh-wrap** can set up `gbuild wrap ssh` automatically for Bash, zsh, and fish.
...
- **gbuild doctor** checks terminal, tmux, clipboard, and keyboard setup without opening the TUI.

## Bug Fixes

- **Image paste** over gbuild wrap now works on headless remotes.
...
- **Added GBUILD_CLIPBOARD_NO_OSC52** env var to stop clipboard sequences from appearing as garbage in unsupported terminals.
...
- **/btw** now works inside `gbuild --minimal`, showing answers in the live area and committing them to scrollback on Esc.
...
- **Global rules** from ~/.gbuild/rules and compatible vendor homes are now discovered correctly.
...
- **SSH sessions** now show a one-time tip recommending `gbuild wrap ssh <host>` for clipboard and terminal restore.
...
- **gbuild** and the agent binary now stay in sync even when no update is installed.
- **Copying** a multiline queued prompt now copies the complete text instead of a collapsed summary.
- **gbuild wrap** now restores the terminal after SSH disconnects or other abrupt child exits.
- **Voice speech-to-text** now works with per-model API keys in config.toml without requiring `gbuild login`.
...
- **gbuild login** now requests Grok Projects scopes so workspace listing works after consent.
...
- **`gbuild plugin install`** now accepts a marketplace's registered name as a qualifier.
...
- **Fixed background task tracking** when using gbuild -p --no-wait-for-background so tasks are properly reaped on exit.
...
- **gbuild inspect** now shows effective compatibility settings for Cursor, Claude, and Codex sessions.
...
- **Resume hint** after quitting minimal mode now prints the correct gbuild --minimal --resume command.
...
- **gBuild no longer crashes** when printing resume hints after the terminal pane has closed.
...
- **gbuild setup --json** prints your team's managed configuration without installing it.
...
- **How-to Guides** modal now shows a tip linking to Ask gBuild above the footer shortcuts.
...
- **`gbuild worktree ls`** now works as a short alias for `gbuild worktree list`.
- **MCP tool output truncation** can now be set per-repo in `.gbuild/config.toml`.
...
- **File links and media** now work for worktree sessions under ~/.gbuild/worktrees/.
...
- **gbuild inspect** now lists skills from [skills].paths and correctly labels bundled vs user skills.
- **--minimal** and **--fullscreen** choices are now remembered for future plain gBuild launches.

## Bug Fixes

- **Queued bash commands** promoted at turn end now render their output instead of disappearing.
- **Xcode / Foundation ACP clients** can now drive gbuild agent stdio without silent parse drops on session/* calls.
...
- **gbuild --minimal** now supports native drag-select on classic Windows conhost terminals.
...
- **gbuild --minimal** now aligns the prompt, status bar, and messages flush-left with the welcome card.
...
- **gbuild update** on Windows no longer fails when a previous .old executable is still running.
...
- **gbuild models** banner now correctly reports per-model API keys and deployment keys.
...
- You can now add a local directory as a plugin marketplace source with `gbuild plugin marketplace add`.
...
- **Plan files** now default to .gbuild/plan.md to match gBuild conventions.
...
- **--minimal** flag now shows in `gbuild --help`.
...
- **Tool result previews** no longer paint opaque panels in `gbuild --minimal`.
- **gbuild wrap** now correctly handles quoted strings and shell aliases.
...
- **`gbuild update`** downloads have a longer timeout.
...
- **Pasting images** from the local clipboard now works when running commands through `gbuild wrap`.
...
- **gbuild agent stdio** now checks for updates in the background like other modes.
...
- **gbuild wrap** now shows copy success over SSH and suggests native drag-select when paste fails.
...
- **Fixed `gbuild agent stdio` hangs** on Windows when used with persistent clients such as VS Code.
...
- **Added `gbuild wrap`** to run any command with local clipboard support.
...
- **Fixed `gbuild agent stdio` startup hangs** on Windows when used with persistent clients such as VS Code or grok-desktop.
...
- You can now pass --json-schema to gbuild -p and receive a validated JSON object instead of free text.
...
- **Agent-run terminal commands** now set `GBUILD_AGENT=1` so host tools can tell them apart from interactive shells.
...
- **gbuild -w --ref <branch>** now creates worktrees based on the specified ref instead of HEAD.
...
- **--disable-web-search** now honored in gbuild -p and gbuild agent; auxiliary model routing respects catalog overrides.
...
- The legacy `agent` command is now kept in sync with `gbuild` after running `gbuild update`.
- Headless (`gbuild -p`) runs now wait for background tasks and subagents to finish before exiting.
...
- **`gbuild plugin install <name>`** now resolves plugins from registered marketplaces instead of only local paths.
...
- **gbuild sessions delete <id>** command now lets you permanently remove a session from the CLI.
...
- **gbuild quit** no longer hangs when background git or network tasks are slow.
...
- Session lists (welcome screen, /resume, gbuild sessions list) are now sorted by the same activity time shown in the UI.
...
- **Added option** to fully disable the hunk tracker via --hunk-tracker-mode, GBUILD_HUNK_TRACKER, or config.
...
- **gBuild** can now export usage metrics and events to your own OpenTelemetry collector when enabled.
...
- **`gbuild dashboard`** now reliably opens the dashboard instead of silently falling through to a normal session.
...
- **`gbuild mcp add`** now accepts positional arguments (e.g. `gbuild mcp add filesystem -- npx ...`), supports --scope project, and adds -e/-H flags for env/headers.
...
- **`gbuild update`** no longer downloads the same binary twice when multiple updaters or leader checks run concurrently.
...
- **Fixed git clone failures** on Windows when the CLI tries to clone marketplace plugins into ~/.gbuild.
...
- **MCP servers menu** and slash commands now work when starting gbuild outside a project directory.
...
- **gbuild --resume** now correctly finds the real session instead of failing on empty image-only folders.
...
- **Fixed `gbuild --resume`** failing on empty image-only session folders left by cross-directory pastes.
...
- **Ctrl+C** now cancels a blocking `gbuild update` cleanly instead of leaving an orphaned download repainting the terminal.
...
- **`gbuild --debug`** now produces per-session log files under ~/.gbuild/debug/ even with a leader process.
...
- **`gbuild update`** now restarts any older running leader so all clients get the new binary.
...
- **`gbuild login`** now defaults to device code flow, which works reliably in SSH, WSL, VPN, and browser-restricted environments.
...
- **API-key users** can now run `gbuild agent --leader` without forced interactive login or timeouts.
...
- **image_gen** can now be enabled via the harness model using [features] in config.toml or the GBUILD_IMAGE_GEN_HARNESS env var.
...
- **gbuild inspect** now shows the resolved on/off state and source for every Claude/Cursor compatibility toggle.
- **Cursor MCP servers and hooks** are now discovered and can be disabled independently via GBUILD_CURSOR_MCPS_ENABLED / GBUILD_CURSOR_HOOKS_ENABLED.
...
- **Cursor skills and rules** are now discovered alongside gBuild and Claude directories.
