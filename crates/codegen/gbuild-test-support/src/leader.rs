...
/// A `gbuild agent --leader stdio` client subprocess speaking ACP over pipes.
...
impl LeaderFixture {
    ...
    pub async fn start(
        ...
        Self::start_with_binary(&gbuild_binary(), server, cwd, sandbox).await
    }
    ...
    async fn start_with_binary_timeout(
        ...
        let socket = sandbox.gbuild_home().join("leader.sock");
        let lock = sandbox.gbuild_home().join("leader.lock");
        ...
        cmd.envs(xai_tty_utils::pager_env())
            .env("GBUILD_CLI_CHAT_PROXY_BASE_URL", server.url())
            .env("GBUILD_XAI_API_BASE_URL", server.url())
            .env("GBUILD_MODELS_BASE_URL", server.url())
            .env("GBUILD_FEEDBACK_BASE_URL", server.url())
            .env("GBUILD_TRACE_UPLOAD_URL", server.url())
            .env("XAI_API_KEY", "test-key-for-ci")
            .env("GBUILD_LEADER_SOCKET", &socket)
            .env("RUST_LOG", "gbuild_shell=debug");
        let log_path = sandbox.gbuild_home().join("leader.log");
        ...
        let pid = child.id();
        let tree = match TestProcessTree::try_attach(pid, "persistent gBuild test leader") {
            Ok(tree) => tree,
            Err(error) => {
                let _ = child.kill();
    ...
    async fn spawn_with_binary_and_socket(
        ...
        let mut process = TestProcess::spawn(
            ...
            TestProcessConfig::new()
                .label("gbuild leader stdio client")
                ...
                .env("GBUILD_TRACE_UPLOAD_URL", server.url())
                .env("XAI_API_KEY", "test-key-for-ci")
                .env("GBUILD_LEADER_SOCKET", leader_socket)
                .env("RUST_LOG", "gbuild_shell=debug"),
        )
        .map_err(|error| {
            io::Error::new(
...
pub fn leader_lock_path(home: &Path) -> PathBuf {
    home.join(".gbuild").join("leader.lock")
}
...
pub fn leader_log(home: &Path) -> String {
    std::fs::read_to_string(home.join(".gbuild").join("leader.log")).unwrap_or_default()
}
