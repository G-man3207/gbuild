//! Hermetic filesystem and child-environment owner for gBuild integration tests.
...
/// gBuild network kill switches, and explicit overrides.
...
impl TestSandbox {
    ...
    /// Explicit gBuild state root.
    pub fn gbuild_home(&self) -> &Path {
        &self.gbuild_home
    }
    ...
    pub fn diagnostic_summary(&self) -> String {
        let mut summary = format!(
            "root={} home={} gbuild_home={} workspace={} temp={}",
            self.root().display(),
            self.home.display(),
            self.gbuild_home.display(),
...
impl TestSandboxBuilder {
    /// Wire gBuild API, models, feedback, trace, conversation, and web traffic to
    ...
    pub fn build(self) -> TestSandbox {
        ...
        for path in [&home, &gbuild_home, &workspace, &temp] {
        ...
        let mut env = baseline_env(&home, &gbuild_home, &temp, &parent_cwd);
...
impl TestSandbox {
    fn init_git_workspace(&self) {
        ...
        run_git(self, &["config", "user.name", "gBuild Test"]);
...
fn baseline_env(
    ...
    baseline_env_from_parent(home, gbuild_home, temp, parent_cwd, &parent_env)
}
...
fn baseline_env_from_parent(
    ...
    for (key, value) in [
        ("GBUILD_TELEMETRY_ENABLED", "false"),
        ("GBUILD_TELEMETRY_TRACE_UPLOAD", "false"),
        ("GBUILD_FEEDBACK_ENABLED", "false"),
        ("GBUILD_TRACE_UPLOAD", "false"),
        ("GBUILD_INSTRUMENTATION", "disabled"),
        ("OTEL_SDK_DISABLED", "true"),
        ("DISABLE_TELEMETRY", "1"),
        ("DISABLE_FEEDBACK_COMMAND", "1"),
        ("GBUILD_DISABLE_AUTOUPDATER", "1"),
        ("GBUILD_PROMPT_SUGGESTIONS", "false"),
    ...
    env.insert(
        ...
        gbuild_home.join("gitconfig").into_os_string(),
    );
    env
}
...
mod tests {
    ...
    fn owns_distinct_isolated_paths() {
        ...
        assert_eq!(sandbox.gbuild_home(), sandbox.home().join(".gbuild"));
    }
    ...
    fn git_command_uses_sandbox_state_without_process_global_mutation() {
        ...
        let sandbox = TestSandbox {
            ...
            gbuild_home: root.path().join("home/.gbuild"),
    ...
    fn baseline_is_hermetic_and_network_quiet() {
        ...
        assert_eq!(
            env_value(&sandbox, "GBUILD_HOME"),
            Some(sandbox.gbuild_home().into())
        );
        assert_eq!(
        ...
        assert_eq!(
            env_value(&sandbox, "GBUILD_DISABLE_AUTOUPDATER").as_deref(),
            Some(OsStr::new("1"))
        );
        assert_eq!(
            env_value(&sandbox, "GBUILD_TELEMETRY_TRACE_UPLOAD").as_deref(),
        ...
        assert_eq!(env_value(&sandbox, "GBUILD_LEADER_SOCKET"), None);
        assert_eq!(env_value(&sandbox, "GBUILD_DISABLE_WEB_FETCH"), None);
        assert_eq!(env_value(&sandbox, "GBUILD_WEB_FETCH"), None);
    }
    ...
    fn command_application_clears_ambient_env_and_command_override_wins() {
        ...
        cmd.env("AMBIENT_SECRET", "must-disappear")
            .env("GBUILD_PROMPT_SUGGESTIONS", "ambient");
        sandbox.apply_to_std_command(&mut cmd);
        cmd.env("GBUILD_PROMPT_SUGGESTIONS", "command");
        ...
        assert_eq!(
            env.get(OsStr::new("GBUILD_PROMPT_SUGGESTIONS"))
                .map(OsString::as_os_str),
    ...
    fn explicit_overrides_win_and_can_remove_baseline_entries() {
        ...
        sandbox
            ...
            .set_env("GBUILD_PROMPT_SUGGESTIONS", "true")
            .set_env("NO_PROXY", "override.invalid")
            .remove_env("GBUILD_DISABLE_AUTOUPDATER");
        assert_eq!(
            env_value(&sandbox, "TERM_PROGRAM").as_deref(),
            Some(OsStr::new("vscode"))
        );
        assert_eq!(
            env_value(&sandbox, "GBUILD_PROMPT_SUGGESTIONS").as_deref(),
            Some(OsStr::new("true"))
        );
        assert_eq!(
            env_value(&sandbox, "NO_PROXY").as_deref(),
        ...
        assert_eq!(env_value(&sandbox, "GBUILD_DISABLE_AUTOUPDATER"), None);
    }
    ...
    fn diagnostics_fail_closed_for_credential_keys() {
        ...
        for (key, value) in [
            ...
            ("GBUILD_DEPLOYMENT_KEY", "deployment-key-do-not-print"),
            ("GBUILD_EXTRA_AUTH_KEY", "alpha-test-key-do-not-print"),
