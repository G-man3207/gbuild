...
fn local_gbuild_binary_path() -> PathBuf {
    target_dir()
        .join("debug")
        .join(format!("gbuild{}", std::env::consts::EXE_SUFFIX))
}

fn ensure_local_gbuild_binary(binary: &Path) {
    if binary.exists() {
        return;
    }
...
pub fn gbuild_binary() -> PathBuf {
    ...
    let binary = local_gbuild_binary_path();
    ensure_local_gbuild_binary(&binary);
    binary
}
