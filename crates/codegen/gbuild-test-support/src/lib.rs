...
//! Shared test utilities for gBuild crates: mock inference server, SSE
...
//! - [`GBuildStdioClient`] — ACP client that drives `gbuild agent stdio` as a subprocess
//! - [`RawStdioClient`] — raw-wire ACP driver for bytes the typed client can't
//!   produce (Foundation `\/` methods, string UUID ids)
//! - [`leader::LeaderStdioClient`] — ACP client that drives `gbuild agent --leader stdio` (unix)
...
//! - [`run_headless`] — Run `gbuild -p` against the mock server and capture output
//! - [`git_workdir`] — Create a git-initialized [`TestSandbox`]
//! - [`gbuild_binary`] — Resolve the gBuild binary path (`GBUILD_BINARY` or cargo_bin)
...
/// Multiply a harness timeout by `GBUILD_TEST_TIMEOUT_SCALE` (positive integer,
/// default 1). CI lanes on shared runner pools raise it so pool load slows
/// tests instead of failing them (see the gBuild merge CI workflow).
pub fn scaled(base: std::time::Duration) -> std::time::Duration {
    let scale = std::env::var("GBUILD_TEST_TIMEOUT_SCALE")
        .ok()
...
#[cfg(unix)]
pub mod uds_proxy;
pub use acp_client::{GBuildStdioClient, RawStdioClient};
pub use counting_server::spawn_counting_server;
pub use env::{EnvGuard, gbuild_binary, git_workdir};
pub use headless::{
