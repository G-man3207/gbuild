...
//!
//! - **`Tool`**: a natural definition would be `Tool(#[from] gbuild_tools::ToolError)`.
//!   That coupling would force this crate to depend on
//!   `gbuild-tools`, which would defeat the lightweight
...
/// notably `std::io::Error` and `gbuild_tools::ToolError`) happens
