//! Wire types for the `gbuild-workspace` API.
//!
...
//! `gbuild-workspace-grpc` crate. The Rust types defined here are
//! the source of truth.
...
///
/// Lives in `gbuild-workspace-types` (instead of `gbuild-workspace` or
/// `gbuild-mcp`) so both the permission-validation layer and the MCP
/// transport layer can depend on it without dragging the full workspace or
/// rmcp into each other. Re-exported by `gbuild_workspace::permission`
