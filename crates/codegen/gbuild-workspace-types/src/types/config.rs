...
//! agent-session config types in `gbuild-config` and friends.
...
/// `gbuild-tools` once the wire surface is firm.
...
///
/// TODO(workspace): align with `gbuild_config::ProjectConfig`.
#[derive(Debug, Clone, Default, PartialEq, Eq, Serialize, Deserialize)]
