...
//! `gbuild-tools/src/implementations/skills/types.rs` (the type the
...
pub enum SkillScope {
    /// cwd/.gbuild/skills
    Local,
    /// repo_root/.gbuild/skills
    Repo,
    /// ~/.gbuild/skills
    User,
    /// ~/.gbuild/server-skills (synced from the skill store)
    Server,
...
mod tests {
    ...
    // Fixture mirrored field-for-field from the gbuild-tools SkillInfo
    ...
    fn skill_info_deserializes_full_payload() {
        let raw = serde_json::json!({
            ...
            "metadata": {"team": "infra"},
            "path": "/root/.gbuild/server-skills/deploy/SKILL.md",
            "scope": "server",
            "config_source": {"type": "user", "path": "/root/.gbuild/skills"},
            "plugin_name": "infra-plugin",
            "plugin_version": "1.0.0",
            "plugin_root": "/root/.gbuild/plugins/infra-plugin",
            "plugin_data": "/root/.gbuild/plugin-data/infra-plugin",
            "allowed_tools": ["bash"],
            "model": "grok-4",
            "effort": "high",
            "user_invocable": true,
