...
//!   here would force every consumer of `gbuild-workspace-types`
...
//! (`gbuild-workspace`) wraps this in its own `Request<T>` that
//! adds the cancellation token and extensions map.
//!
//! Splitting the envelope this way keeps `gbuild-workspace-types`
