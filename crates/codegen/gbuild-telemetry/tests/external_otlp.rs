...
fn external_stream_end_to_end() {
    ...
    let mut cfg = gbuild_telemetry::external::ExternalOtelConfig::resolve_with(
        |name| match name {
            "GBUILD_EXTERNAL_OTEL" => Some("1".into()),
    ...
    cfg.client = gbuild_telemetry::external::config::ExternalClientInfo {
        service_version: "0.0.0-test".into(),
        client_version: "0.0.0-test".into(),
        app_entrypoint: "cli".into(),
    };

    gbuild_telemetry::external::init(Some(cfg));
    assert!(gbuild_telemetry::external::is_active());
    ...
    assert!(!gbuild_telemetry::is_enabled());
    gbuild_telemetry::log_event(gbuild_telemetry::events::SessionNew {
        session_id: "sess-int-1".into(),
        client_identifier: None,
        client_version: None,
        is_git_repo: true,
        permission_mode: gbuild_telemetry::enums::PermissionMode::Ask,
    });
    gbuild_telemetry::log_event(gbuild_telemetry::events::SessionHarness {
        session_id: "sess-int-1".into(),
    ...
    gbuild_telemetry::log_event(gbuild_telemetry::events::PromptSubmitted {
    ...
    gbuild_telemetry::log_event(gbuild_telemetry::events::ModelResponseReceived {
    ...
    for req in &logs {
        for rl in &req.resource_logs {
            ...
            for sl in &rl.scope_logs {
                assert_eq!(sl.scope.as_ref().map(|s| s.name.as_str()), Some("gbuild"));
    ...
    assert_eq!(
        event_names
            .iter()
            .filter(|n| *n == "gbuild.session_start")
            .count(),
        1
    );
            ...
            for sm in &rm.scope_metrics {
                for metric in &sm.metrics {
                    ...
                    if let Some(Data::Sum(sum)) = &metric.data {
                        ...
                        if metric.name == "gbuild.session.count" {
    ...
    assert!(
        metric_names.iter().any(|n| n == "gbuild.session.count"),
        "missing session.count in {metric_names:?}"
    );
    assert!(metric_names.iter().any(|n| n == "gbuild.token.usage"));
    assert_eq!(
    ...
    gbuild_telemetry::external::shutdown();
    assert!(
