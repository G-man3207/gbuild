...
use gbuild_telemetry::external::{self, ExternalOtelRemotePolicy, IdentityAttrs};
...
fn external_stream_gates_on_end_to_end() {
    ...
    gbuild_telemetry::log_event(gbuild_telemetry::events::ToolCallCompleted {
    ...
    let harness =
        col::find_event(&collected, "gbuild.session_start").expect("session_start must be present");
    assert_eq!(harness.scope_name, "gbuild");
    assert_eq!(
        harness
            .resource
            .get("service.name")
    ...
    let prompt = col::find_event(&collected, "gbuild.user_prompt").expect("user_prompt present");
    ...
    let tool = col::find_event(&collected, "gbuild.tool_result").expect("tool_result present");
    assert_eq!(
    ...
    let tokens = col::find_metric(&collected, "gbuild.token.usage");
    ...
    let sessions = col::find_metric(&collected, "gbuild.session.count");
