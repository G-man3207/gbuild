...
//! file logger that writes to `~/.gbuild/logs/memory.log`.
//!
//! ## When to use
//!
...
//! GBUILD_MEMORY_LOG=0 gbuild                # disable even when enabled
//! tail -f ~/.gbuild/logs/memory.log      # watch in another terminal
//! ```
...
mod inner {
    ...
    const ENV_MEMORY_LOG: &str = "GBUILD_MEMORY_LOG";
    ...
    ///
    /// Writes to `~/.gbuild/logs/memory.log`. Filters to `xai_memory=trace`.
    /// Set `GBUILD_MEMORY_LOG=0` to disable, `GBUILD_MEMORY_LOG=/path` to redirect.
    ...
    fn resolve_log_path() -> Option<PathBuf> {
        let default_path = || gbuild_home().join("logs").join("memory.log");
