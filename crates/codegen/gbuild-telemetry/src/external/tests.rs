...
fn event_names_are_pinned() {
    ...
    let expected: &[(E, &str)] = &[
        (E::SessionStart, "gbuild.session_start"),
        (E::SessionEnd, "gbuild.session_end"),
        (E::UserPrompt, "gbuild.user_prompt"),
        (E::TurnCompleted, "gbuild.turn_completed"),
        (E::ApiRequest, "gbuild.api_request"),
        (E::ApiError, "gbuild.api_error"),
        (E::ToolResult, "gbuild.tool_result"),
        (E::ToolDecision, "gbuild.tool_decision"),
        (E::McpServerConnection, "gbuild.mcp_server_connection"),
        (E::PermissionModeChanged, "gbuild.permission_mode_changed"),
        (E::SkillActivated, "gbuild.skill_activated"),
        (E::PluginLoaded, "gbuild.plugin_loaded"),
        (E::Compaction, "gbuild.compaction"),
        (E::Subagent, "gbuild.subagent"),
        (E::Auth, "gbuild.auth"),
        (E::InternalError, "gbuild.internal_error"),
        (E::ModelSwitched, "gbuild.model_switched"),
        (E::ContextualTip, "gbuild.contextual_tip"),
    ];
...
fn client_identifier_allowlist_is_pinned() {
    ...
    assert_eq!(
        schema::sanitize_client_identifier("gbuild-pager"),
        "gbuild-pager"
    );
    assert_eq!(
...
fn sentinel_session_harness() -> events::SessionHarness {
    events::SessionHarness {
        ...
        client_identifier: Some("gbuild-pager".into()),
        model_id: "grok-4".into(),
        agent_name: "gbuild-plan".into(),
...
fn session_start_snapshot_counts_not_names() {
    ...
    let ev = &events[0];
    assert_eq!(ev.0, "gbuild.session_start");
...
fn session_new_increments_session_count_only() {
    ...
    assert_eq!(names, vec!["gbuild.session.count".to_owned()]);
}
...
fn api_request_snapshot_and_token_usage() {
    ...
    let events = exported_events(&stream);
    let ev = &events[0];
    assert_eq!(ev.0, "gbuild.api_request");
    ...
    assert_eq!(exported_metric_names(&stream), vec!["gbuild.token.usage"]);
}
...
fn one_failed_turn_increments_error_count_exactly_once() {
    ...
    let names: Vec<String> = exported_events(&stream)
        ...
        .map(|e| e.0.clone())
        .collect();
    assert_eq!(names.iter().filter(|n| *n == "gbuild.api_error").count(), 2);
    ...
    let total: u64 = stream
        ...
        .filter(|m| m.name() == "gbuild.error.count")
        .map(|m| match m.data() {
...
fn turn_error_increments_error_count() {
    ...
    names.sort();
    assert_eq!(names, vec!["gbuild.error.count", "gbuild.turn.count"]);
}
...
fn tool_result_gates_off_collapses_and_reduces() {
    ...
    let events = exported_events(&stream);
    let ev = &events[0];
    assert_eq!(ev.0, "gbuild.tool_result");
...
fn user_prompt_gates_off_drops_text() {
    ...
    let events = exported_events(&stream);
    let ev = &events[0];
    assert_eq!(ev.0, "gbuild.user_prompt");
...
fn mcp_connection_collapses_server_name_by_default() {
    ...
    let events = exported_events(&stream);
    let ev = &events[0];
    assert_eq!(ev.0, "gbuild.mcp_server_connection");
...
fn tool_decision_snapshot() {
    ...
    let events = exported_events(&stream);
    let ev = &events[0];
    assert_eq!(ev.0, "gbuild.tool_decision");
    ...
    assert_eq!(exported_metric_names(&stream), vec!["gbuild.tool.decision"]);
}
...
fn contextual_tip_maps_every_tip_and_action() {
    ...
    for (tip, action, tip_label, action_label) in cases {
        ...
        let events = exported_events(&stream);
        let ev = &events[0];
        assert_eq!(ev.0, "gbuild.contextual_tip");
...
/// gbuild-workspace sampler events, which live outside this crate and have
...
fn validating_metric_exporter_drops_export_on_bad_attr_key() {
    ...
    let rogue = meter.u64_counter("gbuild.session.count").build();
    rogue.add(
        1,
...
fn redacting_log_exporter_drops_record_with_closed_gate_key() {
    ...
    record.set_event_name("gbuild.user_prompt");
...
fn redacting_log_exporter_drops_record_with_unknown_key() {
    ...
    let logger = stream.ext.logger.as_ref().unwrap();
    let mut record = logger.create_log_record();
    record.set_event_name("gbuild.api_request");
...
fn metric_increments_pass_model_through_scrub_and_attach_session_id() {
    ...
    let exported = stream.metrics.get_finished_metrics().unwrap();
    let blob = format!("{exported:?}");
    assert!(blob.contains("gbuild.token.usage"));
    assert!(
