//! External OTEL schema v2: event names, attribute keys, typed records, and
...
/// Wire schema version, exported as resource attr `gbuild.schema.version`.
...
pub const SCOPE_NAME: &str = "gbuild";
...
impl ExternalEventName {
    ...
    pub fn as_str(self) -> &'static str {
        match self {
            Self::SessionStart => "gbuild.session_start",
            Self::SessionEnd => "gbuild.session_end",
            Self::UserPrompt => "gbuild.user_prompt",
            Self::TurnCompleted => "gbuild.turn_completed",
            Self::ApiRequest => "gbuild.api_request",
            Self::ApiError => "gbuild.api_error",
            Self::ToolResult => "gbuild.tool_result",
            Self::ToolDecision => "gbuild.tool_decision",
            Self::McpServerConnection => "gbuild.mcp_server_connection",
            Self::PermissionModeChanged => "gbuild.permission_mode_changed",
            Self::SkillActivated => "gbuild.skill_activated",
            Self::PluginLoaded => "gbuild.plugin_loaded",
            Self::Compaction => "gbuild.compaction",
            Self::Subagent => "gbuild.subagent",
            Self::Auth => "gbuild.auth",
            Self::InternalError => "gbuild.internal_error",
            Self::ModelSwitched => "gbuild.model_switched",
            Self::ContextualTip => "gbuild.contextual_tip",
        }
    }
}
...
pub enum MetricIncrement {
    /// `gbuild.session.count`.
    SessionCount,
    /// `gbuild.token.usage` — `token_type` ∈ `input|output|reasoning|cache_read`.
    TokenUsage {
...
pub(crate) const METRIC_SESSION_COUNT: &str = "gbuild.session.count";
pub(crate) const METRIC_TOKEN_USAGE: &str = "gbuild.token.usage";
pub(crate) const METRIC_TURN_COUNT: &str = "gbuild.turn.count";
pub(crate) const METRIC_TOOL_DECISION: &str = "gbuild.tool.decision";
pub(crate) const METRIC_TOOL_USAGE: &str = "gbuild.tool.usage";
pub(crate) const METRIC_ERROR_COUNT: &str = "gbuild.error.count";
...
/// Built-in tool names (closed enum surface in `gbuild-tools`) that pass
...
/// `SessionHarness` → `gbuild.session_start`. Emitted from a spawn outside
...
/// `SessionNew` → `gbuild.session.count` (metric only; the flagship
...
/// `SessionEnded` → `gbuild.session_end`.
...
/// `PromptSubmitted` → `gbuild.user_prompt`. Prompt text rides the
...
/// `TurnCompleted` → `gbuild.turn_completed` + `turn.count`
...
/// `ModelResponseReceived` → `gbuild.api_request` + `token.usage`.
...
/// `RateLimitHit` → `gbuild.api_error` (`error_category = rate_limit`).
...
/// `ApiError` → `gbuild.api_error`. Category/class enums only — no
...
/// `ToolCallCompleted` → `gbuild.tool_result` + `tool.usage`.
...
/// `PermissionDecisionPayload` → `gbuild.tool_decision` + `tool.decision`.
...
/// `McpServerConnected` → `gbuild.mcp_server_connection` (`status=connected`).
...
/// `McpServerFailed` → `gbuild.mcp_server_connection` (`status=failed`).
...
/// `PlanModeToggled` → `gbuild.permission_mode_changed`.
...
/// `ContextualTip` → `gbuild.contextual_tip`. Labels only (no user
...
/// `YoloToggled` → `gbuild.permission_mode_changed`.
...
/// `SkillDispatched` → `gbuild.skill_activated`. Skill names are
...
/// `PluginInstalled` → `gbuild.plugin_loaded`.
...
/// `PluginUsed` → `gbuild.plugin_loaded` (plugin name details-gated).
...
/// `CompactionCompleted` → `gbuild.compaction`.
...
/// `CompactionTriggered` → `gbuild.compaction` trigger attrs ride on the
/// completion event instead (one event per compaction); not mapped.
/// `SubagentLaunched` → `gbuild.subagent` (`phase=launched`).
...
/// `SubagentCompleted` → `gbuild.subagent` (`phase=completed`).
...
/// `InternalError` → `gbuild.internal_error`. Error class only — no
...
/// `ModelSwitched` → `gbuild.model_switched`.
