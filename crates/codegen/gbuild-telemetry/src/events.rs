...
//!
//! These structs were extracted from `gbuild-shell` so they can be
...
pub enum ContextualTipKind {
    ...
    WordSelect,
    /// SSH session without `gbuild wrap` → tip to wrap the ssh command locally.
    SshWrap,
}
...
// ---------------------------------------------------------------------------
// Pager events (called from gbuild-pager via log_event)
// ---------------------------------------------------------------------------
...
/// first prompt when gBuild is launched from a non-project directory).
...
pub struct ClipboardCopy {
    ...
    /// An explicit `gbuild wrap` OSC 52 sink was active.
    pub osc52_sink: bool,
...
/// attributed to gBuild: the user saw the gate in gBuild, went and
...
pub struct SubscriptionActivated {
    ...
    /// is strongly attributable to gBuild's upsell surface.
...
/// `gbuild-shell-manual_auth` (the `manual_auth` binding gets the `gbuild-shell-`
