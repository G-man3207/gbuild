...
//!
//! Extracted from `gbuild-shell::agent::telemetry::track`. The HTTP client is
...
/// whichever [`EmitterOrigin`] prefix it carries (`gbuild-shell-` /
/// `gbuild-workspace-`). Unprefixed names pass through unchanged. Kept in
...
pub async fn track(event_name: &str, request_id: &str, ctx: &UserContext, mut metadata: Metadata) {
    ...
    if let Some(ref mixpanel) = client.mixpanel {
        ...
        props.insert("app_name".into(), json!("gBuild"));
...
mod tests {
    ...
    /// identical to the previous `strip_prefix("gbuild-shell-")` behavior.
    #[test]
    fn event_value_strips_shell_prefix() {
        assert_eq!(event_value("gbuild-shell-turn"), "turn");
        assert_eq!(
            event_value("gbuild-shell-trace_upload_attempted"),
    ...
    fn event_value_strips_workspace_prefix() {
        assert_eq!(event_value("gbuild-workspace-turn"), "turn");
    }
    ...
    fn event_value_strips_only_leading_prefix() {
        assert_eq!(event_value("gbuild-shell-workspace-x"), "workspace-x");
    }
    ...
    /// (e.g. a future `"gbuild-shell-ext-"` origin), an earlier `ALL` entry could
