...
//! file logger that writes to `~/.gbuild/logs/hooks.log`.
//!
//! ## When to use
//!
//! Use regular `tracing::info!` / `tracing::debug!` / `tracing::warn!` with
//! targets `gbuild_hooks` or `gbuild_agent::plugins` at key lifecycle
//! points — discovery, dispatch, execution, errors.
//!
//! ## Enabling
//!
//! ```bash
//! GBUILD_HOOKS_LOG=1 gbuild              # enable, write to ~/.gbuild/logs/hooks.log
//! GBUILD_HOOKS_LOG=/tmp/h.log gbuild     # write to custom path
//! GBUILD_HOOKS_LOG=0 gbuild              # explicitly disable
//! tail -f ~/.gbuild/logs/hooks.log     # watch in another terminal
//! ```
...
const ENV_HOOKS_LOG: &str = "GBUILD_HOOKS_LOG";
...
///
/// Writes to `~/.gbuild/logs/hooks.log` (or custom path via `GBUILD_HOOKS_LOG`).
/// Filters to hooks (`gbuild_hooks`) and plugins (`gbuild_agent::plugins`) targets.
/// Set `GBUILD_HOOKS_LOG=0` to disable, `GBUILD_HOOKS_LOG=/path` to redirect.
    ...
    S: Subscriber + for<'span> LookupSpan<'span>,
{
    ...
    let filter = tracing_subscriber::filter::EnvFilter::new(
        "gbuild_hooks=debug,gbuild_agent::plugins=debug",
    );
...
fn resolve_log_path() -> Option<PathBuf> {
    let default_path = || gbuild_home().join("logs").join("hooks.log");
