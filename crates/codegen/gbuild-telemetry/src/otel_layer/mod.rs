...
//!
//! `gbuild-pager` uses this module to set up OTLP
...
///
/// Built by the binaries (`gbuild-pager`) from their own
/// configuration. The credentials provider is constructed by shell's
/// `gbuild_shell::auth::credential_provider::build_otel_credential_provider`.
...
fn build_export_headers(
    ...
    snapshot: &gbuild_auth::CredentialSnapshot,
...
mod tests {
    ...
    use gbuild_auth::{AuthCredentialProvider, CredentialSnapshot, HttpAuth};
    use std::sync::Mutex;
