//! `pty-bench` — PTY benchmark CLI for `gbuild-pager`.
//!
...
//! ```bash
//! cargo bench -p gbuild-pager-pty-harness \
...
//! ```bash
//! PAGER_BINARY=./artifacts/gbuild-${VERSION}-linux-x86_64 \
//!   cargo bench -p gbuild-pager-pty-harness \
//!   --bench pty_bench -- --all \
...
#[command(
    name = "pty-bench",
    about = "PTY benchmark harness for gbuild-pager",
    long_about = None,
)]
struct Cli {
