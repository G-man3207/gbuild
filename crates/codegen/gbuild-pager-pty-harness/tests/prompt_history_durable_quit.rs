...
//!
//! ```bash
//! cargo test -p gbuild-pager-pty-harness --test prompt_history_durable_quit \
//!   -- --ignored --nocapture
//! ```
...
use gbuild_pager_pty_harness::{ContentController, PtyExitPoll, PtyHarness, keys, pager_binary};
...
/// The per-CWD history file lives at `<home>/.gbuild/sessions/<enc-cwd>/prompt_history.jsonl`.
fn find_prompt_history(home: &Path) -> Result<PathBuf> {
    let root = home.join(".gbuild").join("sessions");
