//! Unified PTY harness for gbuild-pager.
//!
...
//!    `gbuild-pager/tests/scenarios/`) — assert screen contents and
...
//! - **`scroll_matrix`**  — `GBUILD_SCROLL_LOG` JSONL ingestion for the scroll validation matrix.
...
impl PtyHarness {
    ...
    /// (XTVERSION probes and gbuild wrap). Content-backed launches must use
    /// [`Self::new_in_sandbox`].
    ...
    /// Spawn from a canonical [`gbuild_test_support::TestSandbox`] baseline
    ...
    pub fn new_in_sandbox(
        ...
        cols: u16,
        args: &[&str],
        sandbox: &gbuild_test_support::TestSandbox,
        env: &[(&str, &str)],
        cwd: Option<&Path>,
    ) -> Result<Self> {
    ...
    /// # use gbuild_pager_pty_harness::{PtyHarness, ContentController, pager_binary};
    ...
    /// through gBuild's stdout. Used to reproduce the doubled-line class of bugs
    /// where gBuild's diff renderer never re-asserts a region it didn't write
