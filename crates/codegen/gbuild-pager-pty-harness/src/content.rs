...
//! [`MockInferenceServer`] from `gbuild-test-support` and provides the
...
use gbuild_test_support::{MockInferenceServer, TestSandbox};

pub use gbuild_test_support::mock_server::LogEntry;
pub use gbuild_test_support::mock_server::MockModelEntry as MockModel;
pub use gbuild_test_support::mock_server::StorageUpload;
pub use gbuild_test_support::sse;
pub use gbuild_test_support::{
...
impl ContentController {
    ...
    pub async fn start_with_models(models: Vec<MockModel>) -> Result<Self> {
        ...
        sandbox.set_env("GBUILD_PROMPT_SUGGESTIONS", "false");
    ...
    /// Isolated `$HOME` directory that the pager should use (keeps its ~/.gbuild
