...
fn ensure_local_pager_binary(binary: &std::path::Path) -> Result<()> {
    ...
    cmd.current_dir(workspace_root()?)
        .args(["build", "-p", "gbuild-pager-bin", "--bin", "gbuild"])
        .stdin(Stdio::null())
        .envs(xai_tty_utils::pager_env());
    xai_tty_utils::detach_std_command(&mut cmd);
    let output = cmd
        .output()
        .with_context(|| format!("failed to spawn {cargo} to build gbuild"))?;

    if !output.status.success() {
        bail!(
            "failed to build gbuild (exit {:?})\nstdout:\n{}\nstderr:\n{}",
            output.status.code(),
...
/// 2. `CARGO_BIN_EXE_gbuild` (set by `cargo test`)
/// 3. Build locally via `cargo build -p gbuild-pager-bin` (the composition-
///    root package that owns the `gbuild` binary)
