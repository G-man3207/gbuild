//! `scroll-matrix` — scroll validation matrix sweep for gbuild-pager.
//!
...
use gbuild_pager_pty_harness::pager_binary;
use gbuild_pager_pty_harness::scroll_matrix::{
...
#[command(
    name = "scroll-matrix",
    about = "Run the scroll validation matrix against gbuild-pager",
    long_about = None,
)]
struct Cli {
    /// Cell tier to run.
    ...
    /// Pager binary. Defaults to PAGER_BINARY, CARGO_BIN_EXE_gbuild,
...
async fn run() -> Result<ExitCode> {
    ...
    // The pager child resolves GBUILD_SCROLL_LOG against ITS cwd (the
