...
//! `gbuild_sampler::shared_http` (one HTTP/2 pooled client plus
//! a pool-less HTTP/1.1 fallback shared across every
//! `SamplingClient`). The sampler reads `GBUILD_POOL_*` /
//! `GBUILD_CONNECT_TIMEOUT_SECS` once, when its shared client is
//! first built, and `GBUILD_SAMPLER_SHARED_CLIENT=0` falls back to
...
use gbuild_workspace::permission::ClientType;

/// Startup span timer, local to this crate.
///
/// Replaces `gbuild_shell::instrumentation_timer!`, which cannot be referenced
/// here (it lives in the shell crate, which now depends on this one). This is a
/// behavior-preserving copy: it routes to the same
/// `gbuild_telemetry::instrumentation` API and keeps the Chrome trace
/// span for these startup timings.
macro_rules! startup_timer {
    ($name:literal) => {{
        use gbuild_telemetry::instrumentation::{
...
// `OriginClientInfo` is owned by `gbuild-sampler` so `SamplerConfig` can use
// it without taking a circular dependency on `gbuild-shell`. Re-exported
// under the same path (`crate::http::OriginClientInfo`) so existing call-sites
// compile unchanged. The telemetry engine in `gbuild-telemetry` consumes
// the same type via `gbuild_sampler::OriginClientInfo`. The shell-specific
...
/// `GBUILD_CLIENT_VERSION` env vars. Returns `None` when
/// `GBUILD_CLIENT_NAME` is unset.
...
/// Process-level client identifier (`GBUILD_CLIENT_NAME` env var, default `"gbuild-shell"`).
...
/// (`gbuild -p`) run or an interactive session; feeds the `client_mode`
/// metric label.
pub const CLIENT_MODE_HEADER: &str = "x-grok-client-mode";

/// One-way latch: set to `"headless"` at startup by the non-TUI entry points
/// (`run_single_turn` for `gbuild -p`, `run_headless_inner` for
/// `gbuild agent [headless]`), `"interactive"` otherwise.
...
pub fn with_auth_retry(
    ...
    credentials: std::sync::Arc<dyn gbuild_auth::AuthCredentialProvider>,
) -> reqwest_middleware::ClientWithMiddleware {
    reqwest_middleware::ClientBuilder::new(client)
        .with(gbuild_auth::AuthRetryMiddleware::new(credentials, 1))
        .build()
}
...
mod tests {
    ...
    fn origin_client_info_from_meta_uses_client_type_when_identifier_absent() {
        ...
        assert_eq!(
            origin_client_info_from_meta(Some(&meta)),
            Some(OriginClientInfo {
                product: "gbuild-pager".to_string(),
    ...
    fn session_user_agent_string_renders_expected_variants() {
        ...
        assert!(with_version.starts_with("grok-desktop/1.2.3 gbuild-shell/"));
        ...
        assert!(without_version.starts_with("grok-web gbuild-shell/"));
    ...
    fn user_agent_render_collapses_duplicate_origin_and_agent_identity() {
        let ua = UserAgent {
            origin: OriginClientInfo {
                product: "gbuild-shell".to_string(),
        ...
        assert_eq!(ua.render(), "gbuild-shell/0.1.171 (macos; aarch64)");
    }
}
