...
use gbuild_tools::registry::types::{
...
pub(crate) fn resolve_session_toolset(
    ...
    notification_handle: Option<gbuild_tools::notification::types::ToolNotificationHandle>,
) -> WorkspaceResult<(
    ToolServerConfig,
...
pub(crate) fn resolve_session_toolset_rebuild(
    ...
    viewer_ctx: Option<xai_tool_runtime::WorkspaceViewerContext>,
    notification_handle: Option<gbuild_tools::notification::types::ToolNotificationHandle>,
    terminal_backend: Arc<dyn gbuild_tools::computer::types::TerminalBackend>,
    ...
    let toolset = builder
        .finalize_with_trunc_config(
            finalize_config,
            ctx,
            gbuild_tools::types::context::TruncationConfig::default(),
            viewer_ctx,
        )
        .map_err(|errs| {
...
/// enabled (`GBUILD_WORKSPACE_TOOL_STATE_ENABLED=true`; any other value keeps
/// legacy behavior).
pub fn tool_state_enabled() -> bool {
    std::env::var("GBUILD_WORKSPACE_TOOL_STATE_ENABLED").as_deref() == Ok("true")
}
...
/// `GBUILD_WORKSPACE_TOOL_STATE_ENABLED`. Aliased to the crate-wide
...
/// [`LocalTerminalBackend`]: gbuild_tools::computer::local::LocalTerminalBackend
pub struct WorkspaceSessionContextFactory {
    auth: Option<xai_computer_hub_sdk::SharedAuthProvider>,
    api_base_url: Option<String>,
    /// Resolved `$GBUILD_WORKSPACE_HOME` when tool-state persistence is enabled;
...
impl WorkspaceSessionContextFactory {
    ...
    /// (`$GBUILD_WORKSPACE_HOME`). Callers should only invoke this when
...
impl SessionContextFactory for WorkspaceSessionContextFactory {
    fn build_session_context(
        ...
        let fs = Arc::new(gbuild_tools::computer::local::LocalFs)
            as Arc<dyn gbuild_tools::computer::types::AsyncFileSystem>;
        let notification_handle = gbuild_tools::notification::ToolNotificationHandle::noop();
    ...
    fn build_terminal_backend(&self) -> crate::config::SessionTerminalBackend {
        crate::config::SessionTerminalBackend::local(
            gbuild_tools::computer::local::LocalTerminalBackend::new(),
        )
    }
...
fn build_proxy_headers(base_url: &str) -> indexmap::IndexMap<String, String> {
    ...
    let version = gbuild_version::VERSION;
    headers.insert(
        "user-agent".to_string(),
        format!("gbuild-workspace/{version}"),
    );
    headers.insert("x-grok-client-version".to_string(), version.to_string());
    headers.insert(
        "x-grok-client-identifier".to_string(),
        std::env::var("GBUILD_CLIENT_NAME").unwrap_or_else(|_| "gbuild-shell".to_string()),
    );
...
/// `GBUILD_DISABLE_WEB_FETCH=1` is set.
fn build_web_fetch_config() -> gbuild_tools::implementations::gbuild::web_fetch::WebFetchConfig {
    use gbuild_tools::implementations::gbuild::web_fetch::{WebFetchConfig, WebFetchParams};
    if std::env::var("GBUILD_DISABLE_WEB_FETCH").is_ok_and(|v| v == "1" || v == "true") {
        return WebFetchConfig::Disabled;
    }
    let mut params = WebFetchParams::default();
    if let Ok(proxy) = std::env::var("GBUILD_WEB_FETCH_PROXY") {
        params.proxy_endpoint = Some(proxy);
    }
    if gbuild_config::env_bool("GBUILD_WEB_FETCH_ALLOW_LOCAL") == Some(true) {
        params.allow_local = Some(true);
    }
    WebFetchConfig::Enabled { params }
}
fn default_web_search_model() -> String {
    std::env::var("GBUILD_WEB_SEARCH_MODEL").unwrap_or_else(|_| "grok-4.5".to_string())
}
    ...
    pub fn baseline_config() -> ToolServerConfig {
        ToolServerConfig {
            tools: vec![
                ...
                tc("GBuild:search_replace", Some(ToolKind::Edit)),
                tc("GBuild:grep", Some(ToolKind::Search)),
...
mod tests {
    ...
    async fn resolve_session_toolset_mcp_merge_dedup_by_id_baseline_wins() {
        ...
        let baseline = ToolServerConfig {
            tools: vec![test_support::tc("GBuild:read_file", Some(ToolKind::Read))],
            behavior_preset: None,
        };
        let mut mcp_dup = test_support::tc("GBuild:read_file", Some(ToolKind::Read));
    ...
    fn backfill_tool_kinds_fills_known_kindless_ids_only() {
        let kinds = HashMap::from([
            ("GBuild:search_replace".to_owned(), ToolKind::Edit),
            ("GBuild:read_file".to_owned(), ToolKind::Read),
        ]);
        let config = ToolServerConfig {
            tools: vec![
                test_support::tc("GBuild:search_replace", None),
                test_support::tc("adhoc.opaque", None),
                // Pre-set kinds must never be overwritten by the registry.
                test_support::tc("GBuild:read_file", Some(ToolKind::Search)),
            ],
        ...
        assert_eq!(kind_of("GBuild:search_replace"), Some(ToolKind::Edit));
        assert_eq!(
    ...
    async fn resolve_session_toolset_readonly_filters_kindless_pinned_tools() {
        ...
        let baseline = ToolServerConfig {
            tools: vec![
                test_support::tc("GBuild:read_file", None),
                test_support::tc("GBuild:grep", None),
                test_support::tc("GBuild:list_dir", None),
                test_support::tc("GBuild:search_replace", None),
                test_support::tc("GBuild:run_terminal_cmd", None),
            ],
            behavior_preset: None,
        };
        let (eff, ts, _backend) = resolve_session_toolset(
    ...
    async fn resolve_session_toolset_mcp_kind_none_dropped_under_readonly() {
        ...
        let baseline = ToolServerConfig {
            tools: vec![
                test_support::tc("GBuild:read_file", Some(ToolKind::Read)),
        ...
        assert!(
            kept_ids.contains(&"GBuild:read_file"),
    ...
    fn tool_state_enabled_only_true_enables() {
        ...
        let var = "GBUILD_WORKSPACE_TOOL_STATE_ENABLED";
    ...
    async fn tool_state_rehydrates_same_session_and_cold_starts_other() {
        use gbuild_tools::types::resources::{State, WebCitationCounter};
