...
use gbuild_mcp::servers::McpState;
use gbuild_tools::notification::types::{ToolNotification, ToolNotificationHandle};
use gbuild_tools::registry::types::{FinalizedToolset, ToolConfig, ToolServerConfig};
...
impl WorkspaceSession {
    ...
    pub(crate) async fn toolset_terminal_is_session_owned(&self) -> bool {
        ...
        match res.get::<gbuild_tools::types::resources::Terminal>() {
...
pub struct WorkspaceShared {
    ...
    pub(crate) events: tokio::sync::broadcast::Sender<gbuild_workspace_types::WorkspaceEvent>,
    pub(crate) respect_gitignore: bool,
    pub(crate) memory_config: Option<MemoryConfig>,
    pub(crate) hook_registry: Arc<parking_lot::RwLock<gbuild_hooks::discovery::HookRegistry>>,
    pub(crate) hook_load_errors: Vec<gbuild_hooks::error::HookError>,
    ...
    pub(crate) activity_notify_handle:
        arc_swap::ArcSwap<Option<gbuild_tools::notification::types::ToolNotificationHandle>>,
    ...
    pub(crate) lsp: Option<std::sync::Arc<dyn gbuild_tools::implementations::lsp::LspBackend>>,
    ...
    /// (from `GBUILD_WORKSPACE_REWIND_ALL_OUTCOMES`, default off).
    pub(crate) workspace_rewind_all_outcomes: bool,
    /// Resolved `$GBUILD_WORKSPACE_HOME` — the workspace-owned on-disk state root
    /// (`<gbuild_home>/workspace` by default). The upload queue spills here.
    ...
    /// (`GBUILD_WORKSPACE_EVENTS_ENABLED=true`). When `false`, every
    ...
    /// enabled (`GBUILD_WORKSPACE_TOOL_DEFS_ENABLED=true`).
...
impl WorkspaceShared {
    ...
    pub fn subscribe_events(
        &self,
    ) -> tokio::sync::broadcast::Receiver<gbuild_workspace_types::WorkspaceEvent> {
