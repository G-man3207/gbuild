...
/// Directory (under `<cwd>/.gbuild`) holding every session's checkpoint store.
...
pub(crate) struct CheckpointStore {
    /// Per-session store directory: `<cwd>/.gbuild/rewind-checkpoints/<session_id>`.
    dir: PathBuf,
