//! Hub [`AuthProvider`] from `~/.gbuild/auth.json` for the standalone
...
//!
//! The in-leader `gbuild workspace` exposure does NOT use this path — it sources
...
fn default_auth_path() -> anyhow::Result<PathBuf> {
    let gbuild = gbuild_config::user_gbuild_home()
        .ok_or_else(|| anyhow::anyhow!("no user gbuild home (set $GBUILD_HOME or $HOME)"))?;
    Ok(gbuild.join("auth.json"))
}
...
fn read_auth_entry(path: &Path) -> anyhow::Result<(String, AuthEntry)> {
    if !path.exists() {
        anyhow::bail!(
            "No auth credentials found at {}. Run `gbuild login` first.",
            path.display()
        );
    }
    ...
    entries
        ...
        .ok_or_else(|| {
            anyhow::anyhow!(
                "no OIDC auth entry found in {}. Run `gbuild login` first.",
                path.display()
            )
        })
}
...
/// the default credential path (`~/.gbuild/auth.json`).
pub fn provider(
    hub_url: &Url,
