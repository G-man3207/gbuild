...
//!
//! Reads OIDC credentials from `~/.gbuild/auth.json`, connects to a
//! server, exposes workspace tools, and refreshes tokens
//! automatically.
use clap::Parser;
use gbuild_workspace::config::WorkspaceServerMetadata;
use gbuild_workspace::daemonize;
use gbuild_workspace::diag_server::{self, DiagHandle, ErrorClass};
use gbuild_workspace::error::WorkspaceError;
use gbuild_workspace::preview_supervisor::{self, PreviewArgs, PreviewVisibility};
...
const SERVICE_NAME: &str = "prod_gbuild_workspace";
...
struct Args {
    ...
    /// `GBUILD_WORKSPACE_UPLOAD_QUEUE_ENABLED` env var to `false`) to fall back to
    /// the legacy inline path. Accepts `true`/`false`.
    #[arg(
        long,
        env = "GBUILD_WORKSPACE_UPLOAD_QUEUE_ENABLED",
    ...
    /// Trust project-scoped LSP servers from `<repo>/.gbuild/lsp.json`.
    /// Defaults off; sandbox opts in only after workspace trust is established.
    #[arg(
        long,
        env = "GBUILD_WORKSPACE_PROJECT_LSP_TRUSTED",
    ...
    /// Override with `GBUILD_WORKSPACE_CONFINE_FS_TO_ROOT=false` (e.g. local dev).
    #[arg(
        long,
        env = "GBUILD_WORKSPACE_CONFINE_FS_TO_ROOT",
        default_value_t = true,
        action = clap::ArgAction::Set,
    )]
...
async fn run(args: Args, cwd: PathBuf) -> anyhow::Result<()> {
    ...
    tracing_subscriber::registry()
        ...
        .with(donating.clone())
        .init();
    let direct_otlp = match std::env::var("GBUILD_WORKSPACE_OTLP_ENDPOINT") {
    ...
    {
        use gbuild_sandbox::{ProfileName, SandboxManager};
        let profile = match std::env::var("GBUILD_SANDBOX_PROFILE").ok() {
            Some(val) => {
                let parsed = val
                    .parse::<ProfileName>()
                ...
                if matches!(parsed, ProfileName::Custom(_)) {
                    tracing::warn!(value = %val,
                        "Unrecognized GBUILD_SANDBOX_PROFILE, defaulting to workspace");
                    ProfileName::Workspace
                } else {
                    parsed
                }
            }
            None if gbuild_sandbox::trust_bwrap_marker_for_devbox() => ProfileName::Devbox,
            None => ProfileName::Workspace,
        };
        let profile_name = profile.to_string();
        if profile == ProfileName::Off {
            tracing::info!(profile = %profile_name, "Sandbox explicitly disabled via GBUILD_SANDBOX_PROFILE=off");
        } else {
            ...
            sandbox.install();
            let active = gbuild_sandbox::is_active();
            ...
            tracing::info!(
                ...
                active,
                restrict_network_at_known_linux_launches = gbuild_sandbox::should_restrict_child_network(),
                "{status_msg}"
            );
        }
    }
    let auth_provider = gbuild_workspace::hub_auth::provider(&url, args.auth_config.as_deref())?;
    tracing::info!(
        hub_url = %url,
        cwd = %cwd.display(),
    ...
    let session_id = std::env::var("GBUILD_SESSION_ID").ok();
    ...
    let status_config = gbuild_workspace::StatusConfig::from_env();
    ...
    gbuild_workspace::init_metrics();
    let ws_handle = match gbuild_workspace::handle::connect_local_workspace(
        cwd,
        url,
        auth_provider,
        metadata,
    ...
    let grace_budget = gbuild_workspace::handle::termination_grace_from_env();
    ws_handle
        .two_phase_drain(grace_budget, gbuild_workspace::handle::DrainReason::Sigterm)
        .await;
...
mod tests {
    ...
    fn project_lsp_trust_defaults_off_and_is_opt_in() {
        unsafe { std::env::remove_var("GBUILD_WORKSPACE_PROJECT_LSP_TRUSTED") };
        let args = Args::try_parse_from(["xai-workspace-server"]).unwrap();
