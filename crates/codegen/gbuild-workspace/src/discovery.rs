...
//! `gbuild-agent` and `gbuild-tools` rather than duplicating them.
...
//!
//! Re-exports [`AgentsMdTracker`] from `gbuild-tools` for external
...
// workspace crate without a direct gbuild-tools dependency.
pub use gbuild_tools::types::agents_md_tracker::AgentsMdTracker;

// Re-export the config types that callers pass into WorkspaceConfig.
pub use gbuild_agent::plugins::discovery::DiscoveryConfig as PluginDiscoveryConfig;
pub use gbuild_agent::plugins::trust::TrustStore as PluginTrustStore;
pub use gbuild_agent::prompt::skills::SkillsConfig;
...
///
/// Delegates to [`gbuild_agent::prompt::skills::list_skills`] with
...
pub async fn discover_skills(root_cwd: &Path, config: &SkillsConfig) -> Vec<Value> {
    ...
    let skills = gbuild_agent::prompt::skills::list_skills(
        Some(&cwd_str),
        config,
...
pub async fn discover_agents_md(root_cwd: &Path) -> Vec<Value> {
    let cwd_str = root_cwd.to_string_lossy();
    let files = gbuild_agent::prompt::agents_md::read_agents_config_with_paths(
        &cwd_str,
...
///
/// Delegates to [`gbuild_agent::plugins::discover_plugins`] with
...
/// Load the project config from `<root_cwd>/.gbuild/config.toml`.
///
...
pub fn load_project_config(root_cwd: &Path) -> Value {
    let config_path = root_cwd.join(".gbuild").join("config.toml");
    match gbuild_config::load_config_file(&config_path) {
...
mod tests {
    ...
    // `~/.gbuild/skills/`, so on a developer machine the result may be
    ...
    async fn discover_skills_finds_skill_md() {
        ...
        let skills_dir = tmp.path().join(".gbuild").join("skills").join("my-skill");
    ...
    async fn discover_skills_respects_ignore_config() {
        let tmp = tempfile::tempdir().unwrap();
        let skills_dir = tmp.path().join(".gbuild").join("skills").join("ignored");
        fs::create_dir_all(&skills_dir).unwrap();
        fs::write(
            skills_dir.join("SKILL.md"),
            "---\nname: ignored\ndescription: Ignored\n---\n",
    ...
    fn agent_config_file_wire_matches_workspace_types_mirror() {
        // The RPC serializes gBuild's AgentConfigFile and the remote
        ...
        // break discovery.
        let src = gbuild_agent::prompt::agents_md::AgentConfigFile {
            file_name: "AGENTS.md".to_string(),
            file_path: "/repo/AGENTS.md".to_string(),
            content: "# Instructions\n".to_string(),
        };
        let json = serde_json::to_value(&src).unwrap();
        let mirror: gbuild_workspace_types::rpc::agents_md::AgentConfigFile =
    ...
    // Discovery also scans the real `~/.gbuild`, so fixtures use test-unique names.
    #[tokio::test]
    ...
    // from `~/.gbuild/plugins/`, so tests check for specific plugins.
    ...
    // developer machine with gBuild installed it may return non-Null even
