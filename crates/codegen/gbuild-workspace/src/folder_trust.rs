...
//! [`crate::trust::TrustStore`] (`~/.gbuild/trusted_folders.toml`). The
//! consume/gating half (the `DECISIONS` cache, `resolve_and_record`,
//! `project_scope_allowed`, the loader filters) lives in `gbuild-shell`.
//!
...
//! provisional and re-checked rather than cached — is a `gbuild-shell`
//! concern, documented there.)

use std::io::IsTerminal;
use std::path::Path;

use gbuild_config_types::{BoolFlag, RemoteSettings};
...
pub fn decide_inputs_with_interactive(
    ...
    DecideInputs {
        ...
        // can't persist (Case 2: cwd IS $HOME, incl. the default `~/.gbuild`).
...
fn feature_enabled_for_build(remote: Option<&RemoteSettings>, is_local_build: bool) -> bool {
    ...
    let user = gbuild_config::load_from_disk().ok();
    let managed = gbuild_config::load_managed_config().ok();
    BoolFlag::env("GBUILD_FOLDER_TRUST")
...
/// `[permission]` / `.claude` / `.gbuild/agents` / subdir-launch gaps. NOT itself
/// the trust gate.
pub fn repo_config_kinds(cwd: &Path) -> Vec<&'static str> {
    collect_repo_config_kinds(cwd, false)
}

/// Whether a project `.gbuild/config.toml` `[permission]` value would contribute
...
fn collect_repo_config_kinds(cwd: &Path, first_only: bool) -> Vec<&'static str> {
    ...
    let chain = gbuild_agent::repo::RepoDirChain::resolve(cwd);
    ...
    // Project `.gbuild/config.toml` declaring repo-controlled code-exec or
    ...
    for path in crate::project_config::find_project_configs_in(&chain.dirs) {
        let Ok(root) = gbuild_config::load_config_file(&path) else {
            continue;
        };
    ...
    // Project `.gbuild/lsp.json`.
    if cwd.join(".gbuild").join("lsp.json").is_file() {
        hit!("lsp");
    }
    ...
    // must be gated — else a hooks-only clone (e.g. `.gbuild/hooks/evil.json`) would
    ...
    if path_present_or_uncertain(&hook_root.join(".gbuild").join("hooks"))
    ...
    // that must be gated — else a plugin clone (e.g. `.gbuild/plugins/evil/`, even
    // one in a subdir launched via `cd sub && gbuild`) would resolve trusted and
    ...
    if !gbuild_agent::plugins::project_plugin_dirs_in(&chain.dirs).is_empty() {
        hit!("plugins");
    }
    // Project AGENT dirs (`.gbuild/agents` / `.claude/agents`): a project agent
    ...
    if !gbuild_agent::discovery::project_agent_dirs_in(&chain.dirs).is_empty() {
        hit!("agents");
    }
    // Presence matches exact-cwd discovery without parsing repository content.
    let gbuild = cwd.join(".gbuild");
    if directory_present_or_uncertain(&gbuild.join("roles")) {
        hit!("roles");
    }
    if directory_present_or_uncertain(&gbuild.join("personas")) {
        hit!("personas");
    }
    if directory_present_or_uncertain(&hook_root.join(".gbuild").join("workflows")) {
        hit!("workflows");
    }
...
pub fn prompt_for_trust(key: &Path) -> bool {
    ...
    let _ = writeln!(
        err,
        "This folder contains repo-local config (.mcp.json / .gbuild/lsp.json / hooks) \
...
mod tests {
    ...
    #[test]
    fn repo_configs_present_detects_gbuild_config_mcp_servers() {
        let tmp = repo_tmp();
    ...
    #[test]
    fn repo_configs_present_detects_gbuild_lsp_json() {
        let tmp = repo_tmp();
        let gbuild = tmp.path().join(".gbuild");
        std::fs::create_dir_all(&gbuild).unwrap();
        std::fs::write(gbuild.join("lsp.json"), "{}").unwrap();
        assert!(repo_configs_present(tmp.path()));
    }
    ...
    fn repo_configs_present_detects_project_agents() {
        // A `.gbuild/agents`-only clone must be gated: a project agent definition
        // can carry an inline `hooks:` block (code-exec) and can shadow a built-in
        // subagent by name.
        let tmp = repo_tmp();
        std::fs::create_dir_all(tmp.path().join(".gbuild").join("agents")).unwrap();
        assert!(repo_configs_present(tmp.path()));
    }
    ...
    fn repo_configs_present_detects_project_roles() {
        let tmp = repo_tmp();
        std::fs::create_dir_all(tmp.path().join(".gbuild").join("roles")).unwrap();
    ...
    fn repo_configs_present_detects_project_personas() {
        let tmp = repo_tmp();
        std::fs::create_dir_all(tmp.path().join(".gbuild").join("personas")).unwrap();
    ...
    fn project_subagent_marker_regular_file_is_absent() {
        ...
        let gbuild = tmp.path().join(".gbuild");
        std::fs::create_dir_all(&gbuild).unwrap();
        std::fs::write(gbuild.join("roles"), "not a directory").unwrap();
        assert!(!repo_configs_present(tmp.path()));
    }
    ...
    fn project_subagent_marker_at_repo_root_is_absent_from_subdir() {
        let tmp = repo_tmp();
        std::fs::create_dir_all(tmp.path().join(".gbuild/roles")).unwrap();
    ...
    fn project_subagent_marker_symlink_to_directory_is_present() {
        ...
        std::fs::create_dir_all(&gbuild).unwrap();
        std::os::unix::fs::symlink(&target, gbuild.join("roles")).unwrap();
        assert!(repo_configs_present(tmp.path()));
    }
    ...
    fn dangling_project_subagent_marker_is_absent() {
        ...
        let gbuild = tmp.path().join(".gbuild");
        std::fs::create_dir_all(&gbuild).unwrap();
        std::os::unix::fs::symlink("missing", gbuild.join("personas")).unwrap();
        assert!(!repo_configs_present(tmp.path()));
    }
    ...
    fn repo_configs_present_detects_project_workflows_from_subdir() {
        let tmp = repo_tmp();
        std::fs::create_dir_all(tmp.path().join(".gbuild").join("workflows")).unwrap();
        let subdir = tmp.path().join("crates").join("inner");
        std::fs::create_dir_all(&subdir).unwrap();
        assert!(repo_configs_present(&subdir));
    ...
    fn repo_configs_present_detects_project_hooks_file() {
        ...
        let gbuild = tmp.path().join(".gbuild");
        std::fs::create_dir_all(&gbuild).unwrap();
        std::fs::write(gbuild.join("hooks"), "{}").unwrap();
    ...
    fn repo_configs_present_detects_dangling_project_hooks_symlink() {
        ...
        let gbuild = tmp.path().join(".gbuild");
        std::fs::create_dir_all(&gbuild).unwrap();
        std::os::unix::fs::symlink("missing-hooks", gbuild.join("hooks")).unwrap();
    ...
    fn repo_configs_present_detects_project_plugins() {
        ...
        let tmp = repo_tmp();
        std::fs::create_dir_all(tmp.path().join(".gbuild").join("plugins").join("x")).unwrap();
        assert!(repo_configs_present(tmp.path()));
    }
    ...
    fn repo_configs_present_detects_project_plugins_in_subdir() {
        ...
        std::fs::create_dir_all(subdir.join(".gbuild").join("plugins").join("evil")).unwrap();
        assert!(repo_configs_present(&subdir));
    }
    ...
    fn repo_configs_present_false_for_empty_mcp_servers_table() {
        ...
        let gbuild = tmp.path().join(".gbuild");
        std::fs::create_dir_all(&gbuild).unwrap();
        std::fs::write(gbuild.join("config.toml"), "[mcp_servers]\n").unwrap();
        assert!(!repo_configs_present(tmp.path()));
    }

    #[test]
    fn repo_configs_present_detects_gbuild_config_plugins_paths() {
    ...
    #[test]
    fn repo_configs_present_detects_gbuild_config_permission() {
    ...
    fn repo_config_kinds_matches_gate_and_reports_all_kinds() {
        ...
        // `.gbuild/agents` — even when launched from a SUBDIR (the cwd→git-root walk
        // that `first_only` shares). Guards against silent drift between the two.
        let tmp = repo_tmp();
        let gbuild = tmp.path().join(".gbuild");
        std::fs::create_dir_all(gbuild.join("agents")).unwrap();
        std::fs::write(gbuild.join("config.toml"), "[plugins]\npaths = [\"./x\"]\n").unwrap();
    ...
    // GBUILD_HOME-isolation idiom mirrored from this crate's `permission::claude_compat`
    // tests (the workspace crate has no `serial_test`/`gbuild-test-support`
    ...
    fn revoke_folder_trust_store_persists_untrust_for_trusted_folder() {
        ...
        // GBUILD_HOME-isolated so the seed/deny hit a temp store, not the real file.
        let _lock = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let home = tempfile::tempdir().unwrap();
    ...
    fn revoke_folder_trust_store_writes_no_deny_for_never_trusted_folder() {
        ...
        // GBUILD_HOME-isolated so the grant writes to a temp store.
        let _lock = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let home = tempfile::tempdir().unwrap();
        let _env = EnvVarGuard::set("GBUILD_HOME", home.path());
        let _sim = simulate_release_build();
