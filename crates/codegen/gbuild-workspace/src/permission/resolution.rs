//! Permission resolution engine: merges native `.gbuild/config.toml`,
...
/// (fail-safe) with a warn + skip record for `gbuild inspect`.
...
/// user `~/.gbuild` → `Requirements`, so [`is_admin_source`] trusts only the root tier.
fn load_requirements_permissions() -> Vec<Sourced<PermissionRule>> {
    gbuild_config::requirements_layers()
        .into_iter()
        .flat_map(|layer| {
...
/// Load `[permission]` rules from native gBuild TOML config files:
///
...
fn load_config_toml_permissions(cwd: &Path, project_trusted: bool) -> Vec<Sourced<PermissionRule>> {
    ...
    // Gated on user_gbuild_home() so a project's .gbuild/config.toml is never read as
    // global permissions when neither GBUILD_HOME nor a home dir resolves.
    if let Some(global_path) = gbuild_config::user_gbuild_home().map(|g| g.join("config.toml"))
        && global_path.is_file()
    {
        match gbuild_config::load_config_file(&global_path) {
...
fn managed_config_permissions(
    layers: &[gbuild_config::ManagedConfigLayer],
) -> Vec<Sourced<PermissionRule>> {
    layers
        .iter()
        .flat_map(|layer| {
...
/// Resolve permission config, merging native gBuild and Claude sources.
...
/// blocked `--yolo`); keep admin-tier ones. Records each drop for `gbuild inspect`.
...
/// bypass is pinned off via gbuild `requirements.toml`
...
async fn resolve_permissions_with_provenance_inner(
    ...
    // Keep skip-only resolutions alive so the drop reaches `gbuild inspect`; zero
...
fn resolve_claude_settings_inner(
    ...
    for path in claude_settings_paths_for_trust(cwd, project_trusted) {
        ...
        if let Some(perms) = settings.permissions {
            ...
            // `gbuild inspect` (all-invalid allow/deny/ask must not leave
    ...
    // `gbuild inspect` via the outer resolver.
...
/// [`gbuild_config::claude_managed_settings_path`] (platform path such as
...
fn load_managed_settings() -> ManagedSettings {
    let Some(path) = gbuild_config::claude_managed_settings_path() else {
...
/// without `~/.gbuild`); `path` only names the layer in a non-bool warning.
...
mod tests {
    ...
    // Shared so `GBUILD_HOME`/`HOME` mutations here also serialize against the
    ...
    fn load_claude_env_merges_with_precedence() {
        // GBUILD_HOME-isolate so the claude-import marker reads clean (an imported
    ...
    fn load_claude_env_empty_when_no_settings() {
        // Isolate GBUILD_HOME (claude-import marker) AND HOME (global `~/.claude`)
...
"#,
    ...
    fn non_bool_lock_key_warns_and_does_not_lock() {
        ...
        let p = Path::new("/etc/gbuild/requirements.toml");
        let bad = layer("[ui]\ndisable_bypass_permissions_mode = \"true\"\n");
        ...
        assert!(
            out.contains("/etc/gbuild/requirements.toml"),
    ...
    fn admin_source_trusts_only_root_owned_tiers() {
        ...
        // the user-writable `~/.gbuild/requirements.toml` is not, despite its path.
        ...
        assert!(is_admin_source(&RequirementSource::SystemRequirements {
            path: "/etc/gbuild/requirements.toml".into(),
        }));
        assert!(!is_admin_source(&RequirementSource::Requirements {
            path: "/home/u/.gbuild/requirements.toml".into(),
        }));
        assert!(!is_admin_source(&RequirementSource::ManagedConfig {
            path: "/etc/gbuild/managed_config.toml".into(),
        }));
    ...
    fn drop_untrusted_catchall_allows_is_source_aware() {
        ...
        let rules = vec![
            ...
            // Managed config: defaults tier, untrusted even from /etc/gbuild.
            sourced(
                allow_any(Some("*")),
    ...
    fn unrecognized_project_mode_claims_scope_over_global_accept_edits() {
        ...
        assert!(
            ...
            "typo should be recorded for gbuild inspect"
        );
    }
    ...
    async fn managed_config_toml_rules_resolve_as_non_admin_defaults() {
        ...
        let layers =
            gbuild_config::managed_config_layers_at(Some(system.path()), Some(user.path()));
        ...
        .unwrap();
        assert_eq!(
            gbuild_config::managed_config_layers_at(Some(system.path()), Some(user.path())).len(),
            1
        );
