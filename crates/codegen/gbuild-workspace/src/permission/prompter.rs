...
use gbuild_tools::implementations::gbuild::web_fetch::domain_from_url;
use xai_acp_lib::AcpAgentGatewaySender as GatewaySender;
use xai_file_utils::events::{Event, EventWriter, PermissionDecision};

const REJECT_ONCE_LABEL: &str = "No, and tell gBuild what to do differently";
...
///        `~/.gbuild/config.toml` via the `Effect::PersistPermissionMode` effect
...
/// (global yolo) option that is prepended for GBuildTui / GBuildPager / Desktop.
///
...
/// (`GBuildTui`, `GBuildPager`, `Desktop`) that wire the option id through
...
fn client_supports_enable_always_approve(client_type: ClientType) -> bool {
    matches!(
        client_type,
        ClientType::GBuildTui | ClientType::GBuildPager | ClientType::Desktop
    )
}
...
/// Canonical definition lives in `gbuild_workspace_types` (so both the
/// permission-validation layer and the MCP transport in `gbuild-mcp` can
...
/// reached `gbuild_workspace::permission::MCP_TOOL_NAME_DELIMITER`.
...
impl AcpPrompter {
    pub fn new(
        ...
        // Bash options for GBuildTui - interactive selection with expandable/contractable terms
    ...
    fn build_options_inner(
        ...
        match access {
            ...
            AccessKind::Bash(bash_command) => {
                // For GBuildTui clients, use the fancy interactive options with term selection
...
mod tests {
    ...
    fn gate_off_strips_bash_always_allow_keeps_yes_no() {
        let p = prompter_with_gate(ClientType::GBuildPager, false);
    ...
    fn gate_on_includes_bash_always_allow() {
        let p = prompter_with_gate(ClientType::GBuildPager, true);
        let access = AccessKind::Bash("kubectl get pods".to_owned());
        let opts = p.build_options(&access);
        assert!(
            has_option(&opts, "allow-always-command"),
    ...
    fn bash_meta_present_only_when_gate_on_for_fancy_clients() {
        ...
        let on = prompter_with_gate(ClientType::GBuildPager, true);
        ...
        let off = prompter_with_gate(ClientType::GBuildPager, false);
    ...
    fn bash_reject_always_command_maps_selected_words() {
        let p = prompter(ClientType::GBuildPager);
    ...
    fn mcp_prompt_includes_allow_always_with_meta() {
        let p = prompter(ClientType::GBuildTui);
        for (name, server) in [
            ("linear__list", "linear"),
            ("123__lookup", "123"),
    ...
    /// Same pin as above for `GBuildTui` and `Desktop` — both client types
    ...
    fn enable_always_approve_is_first_for_tui_and_desktop() {
        for ct in [ClientType::GBuildTui, ClientType::Desktop] {
            let p = prompter(ct);
