...
impl ProtectedEditReason {
    pub fn kind(self) -> &'static str {
        match self {
            ...
            Self::Etc => "etc",
            Self::GBuildConfig => "gbuild_config",
    ...
    pub fn description(self) -> Option<&'static str> {
        match self {
            ...
            Self::GBuildConfig => Some(
                "Note: This edit contains changes to gBuild config, which can alter permissions, tools, and other behavior in later sessions.",
            ),
...
pub(crate) fn edit_target_protection(path: &Path) -> Option<ProtectedEditReason> {
    ...
    let lexical = gbuild_paths::normalize_lexically(path);
...
fn protected_edit_reason(path: &Path) -> Option<ProtectedEditReason> {
    ...
    if protected_gbuild_hook_root(path, &string_components) {
    ...
    if string_components.ends_with(&[".gbuild", "config.toml"]) {
        return Some(ProtectedEditReason::GBuildConfig);
    }
...
fn path_is_under_user_gbuild_hook_root(path: &Path, gbuild_home: &Path) -> bool {
    path.starts_with(gbuild_home.join("hooks")) || path == gbuild_home.join("hooks-paths")
}

fn protected_gbuild_hook_root(path: &Path, components: &[&str]) -> bool {
    components
        .windows(2)
        .any(|pair| pair == [".gbuild", "hooks"])
        || components.ends_with(&[".gbuild", "hooks-paths"])
        || gbuild_config::user_gbuild_home().is_some_and(|gbuild_home| {
            let lexical_home = gbuild_paths::normalize_lexically(&gbuild_home);
            path_is_under_user_gbuild_hook_root(path, &lexical_home)
                || resolve_following_symlinks(&lexical_home, 0).is_some_and(|resolved_home| {
                    path_is_under_user_gbuild_hook_root(path, &resolved_home)
                })
        })
}
...
mod tests {
    ...
    fn sensitive_edit_targets_and_lexical_aliases_prompt() {
        ...
        for path in [
            "/work/src/main.rs",
            "/work/project/.gbuild/config.toml/backup",
        ] {
            assert!(
    ...
    fn edit_target_protection_classifies_reasons() {
        let cases = [
            (
                "/home/user/.gbuild/hooks/evil.json",
            ...
            (
                "/home/user/.gbuild/config.toml",
                ProtectedEditReason::GBuildConfig,
            ),
            (
    ...
    fn sensitive_edit_targets_include_hook_roots() {
        for path in [
            "/home/user/.gbuild/hooks/evil.json",
            "/home/user/.gbuild/hooks/nested/deep.json",
            "/home/user/.gbuild/hooks-paths",
            "/home/user/.claude/settings.json",
            "/home/user/.claude/settings.local.json",
            "/home/user/.cursor/hooks.json",
            "/work/project/.gbuild/hooks/local.json",
            "/work/project/.gbuild/hooks-paths",
        ] {
            assert!(
                edit_target_protection(Path::new(path)).is_some(),
        ...
        for path in [
            "/home/user/.gbuild/hooks-disabled/note.json",
            "/home/user/.gbuild/hooks-evil/note.json",
    ...
    #[test]
    fn path_is_under_user_gbuild_hook_root_matches_relocated_home() {
        let home = Path::new("/custom/gbuild-home");
        for path in [
            "/custom/gbuild-home/hooks/x.json",
            "/custom/gbuild-home/hooks/nested/deep.json",
            "/custom/gbuild-home/hooks",
            "/custom/gbuild-home/hooks-paths",
        ] {
            assert!(
                path_is_under_user_gbuild_hook_root(Path::new(path), home),
                "must match under custom gBuild home: {path}"
            );
        }
        for path in [
            "/custom/gbuild-home/hooks-disabled/note.json",
            "/custom/gbuild-home/hooks-evil/note.json",
            "/custom/gbuild-home/config.toml",
            "/custom/other/hooks/x.json",
            "/custom/gbuild-home-extra/hooks/x.json",
        ] {
            assert!(
                !path_is_under_user_gbuild_hook_root(Path::new(path), home),
    ...
    fn sensitive_edit_targets_follow_symlinks() {
        ...
        .unwrap();
        let gbuild_hook = outside.path().join(".gbuild/hooks/evil.json");
        std::fs::create_dir_all(gbuild_hook.parent().unwrap()).unwrap();
        std::fs::write(&gbuild_hook, b"{}").unwrap();
        symlink(&gbuild_hook, ws.path().join("gbuild-hook-link")).unwrap();

        for path in [
            ws.path().join("file-link"),
            ws.path().join("hooks-link/new-hook"),
            ws.path().join("module-hooks-link/new-hook"),
            ws.path().join("gbuild-hook-link"),
        ] {
            assert!(
    ...
    fn resolved_root_alias_matches_physical_destination() {
        ...
        assert!(resolved_path_is_within_root(
            &resolved_root.join("gbuild-test"),
            Path::new("/etc")
        ));
        assert!(!resolved_path_is_within_root(
            Path::new("/tmp/gbuild-test"),
            Path::new("/etc")
        ));
    }
