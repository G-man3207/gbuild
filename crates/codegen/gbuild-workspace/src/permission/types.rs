...
pub enum ClientType {
    ...
    #[default]
    #[serde(rename = "generic", alias = "gbuild-shell", alias = "gbuild_shell")]
    Generic,
    /// gBuild TUI client - show interactive bash term selection
    #[serde(rename = "gbuild-tui", alias = "gbuild_tui")]
    GBuildTui,
    ...
    Extension,
    /// gBuild pager client - TUI-like terminal pager with interactive permission UI.
    /// Treated identically to GBuildTui for permission options (gets bash highlights +
    /// interactive selection). Reports as "pager" for telemetry attribution.
    ///
    /// Accepts both the hyphenated `"gbuild-pager"` (what the pager actually
    /// sends over the wire, matching `PAGER_CLIENT_TYPE`) and the underscored
    /// `"gbuild_pager"` form for symmetry with the rest of this enum.
    #[serde(rename = "gbuild-pager", alias = "gbuild_pager")]
    GBuildPager,
...
impl ClientType {
    /// Product token for the `User-Agent` header (e.g. `gbuild-pager`).
    pub fn user_agent_label(&self) -> &'static str {
        match self {
            Self::Generic => "gbuild-shell",
            Self::GBuildTui => "gbuild-tui",
            Self::GrokWeb => "grok-web",
            Self::Nebula => "nebula",
            Self::Extension => "grok-code-extension",
            Self::GBuildPager => "gbuild-pager",
    ...
    pub fn from_client_identifier(id: Option<&str>) -> Self {
        match id {
            ...
            Some("gbuild-pager") => Self::GBuildPager,
            _ => Self::Generic,
        }
    }
    ...
    pub fn feedback_label(&self) -> &'static str {
        match self {
            Self::GBuildTui | Self::GBuildPager => "tui",
            Self::GrokWeb => "web",
            Self::Nebula => "nebula",
...
impl AccessKind {
    ...
    pub fn from_tool_input(tool_name: &str, input: &gbuild_tools::types::ToolInput) -> Self {
...
impl From<&gbuild_tools::types::ToolInput> for AccessKind {
    fn from(input: &gbuild_tools::types::ToolInput) -> Self {
...
pub enum RequirementSource {
    Unknown,
    /// User-writable `~/.gbuild/requirements.toml` — untrusted for keeping a
...
mod tests {
    ...
    fn hashline_edit_maps_to_edit_access() {
        use gbuild_tools::implementations::gbuild_hashline::edit::types::HashlineEditInput;
        use gbuild_tools::types::ToolInput;
    ...
    fn bash_maps_to_bash_access() {
        use gbuild_tools::implementations::gbuild::bash::BashToolInput;
        use gbuild_tools::types::ToolInput;
    ...
    fn use_tool_maps_to_mcp_tool_access() {
        use gbuild_tools::implementations::use_tool::UseToolInput;
        use gbuild_tools::types::ToolInput;
    ...
    fn monitor_maps_to_bash_access() {
        use gbuild_tools::implementations::gbuild::monitor::types::MonitorInput;
        use gbuild_tools::types::ToolInput;
    ...
    fn web_fetch_maps_to_web_fetch_access() {
        use gbuild_tools::implementations::gbuild::web_fetch::WebFetchInput;
        use gbuild_tools::types::ToolInput;
    ...
    fn web_search_maps_to_web_search_access() {
        use gbuild_tools::implementations::gbuild::web_search::WebSearchInput;
        use gbuild_tools::types::ToolInput;
    ...
    #[test]
    fn client_type_deserializes_gbuild_shell_as_generic() {
        assert_eq!(
            serde_json::from_value::<ClientType>("gbuild-shell".into()).unwrap(),
            ClientType::Generic,
        );
        assert_eq!(
            serde_json::from_value::<ClientType>("gbuild_shell".into()).unwrap(),
            ClientType::Generic,
        );
        assert_eq!(
