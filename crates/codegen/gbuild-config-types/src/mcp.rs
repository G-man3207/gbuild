//! MCP server configuration value types, extracted from gbuild-shell
//! (config dependency inversion).

use agent_client_protocol as acp;
use gbuild_mcp::oauth_config::McpOAuthConfig;
...
/// and surfaced through `gbuild inspect`.
...
impl RelaySyncConfig {
    ...
    pub fn is_enabled(&self) -> bool {
        if let Ok(env_val) = std::env::var("GBUILD_RELAY_SYNC_ENABLED") {
