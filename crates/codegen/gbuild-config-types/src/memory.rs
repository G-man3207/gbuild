//! Memory-system configuration value types, extracted from gbuild-shell
...
//! `gbuild-shell` — `resolve()` depends on `toml` and on shell-internal
...
///
/// When enabled, watches `~/.gbuild/memory/` for `.md` file changes (create,
...
///
/// On session init, directories under `~/.gbuild/memory/` are scanned:
