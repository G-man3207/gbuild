...
pub struct RemoteSettings {
    ...
    /// `app::subscription` module). Forwarded from the `gbuild_settings`
    ...
    /// remote settings `gbuild_settings.max_mcp_output_bytes` — global default
    ...
    /// `gbuild_settings` remote settings key) to enable it. See
    ...
    /// `gbuild_telemetry::external::apply_remote_policy`.
    #[serde(default)]
    ...
    /// `GBUILD_TWO_PASS_COMPACTION` / default (off).
    #[serde(default)]
    ...
    /// Controlled via remote settings (`non_git_warning` in `gbuild_settings`).
    ...
    /// ghost text), from the `gbuild_settings` remote settings flag. Sits below
    /// env (`GBUILD_PROMPT_SUGGESTIONS_MODEL`) and `[models] prompt_suggestion`
    ...
    /// Overridable locally via `GBUILD_VOICE_MODE`. Free-tier SuperGrok upsell
    ...
    /// remain coding-data opted-out. From `gbuild_settings`.
    ...
    /// User's subscription tier from remote settings `gbuild_access_gate`.
    ...
    /// Whether the user is allowed to use gBuild. Set by remote settings
    /// `gbuild_access_gate` targeting rules. `None` = no server response
    ...
    /// Server-controlled via the remote settings `gbuild_usage_redirect_url`
    ...
    /// `gbuild_settings`. Per-model override on `ModelInfo`
    /// (`gbuild_models`) takes precedence; user config and env var
    ...
    /// `0` disables. Env (`GBUILD_COMPACTION_WALL_CLOCK_SECS`) overrides it.
    ...
    /// Env (`GBUILD_COMPACTION_MODE`) and user config override it.
    #[serde(default)]
    pub compaction_mode: Option<String>,
    /// Segments verbatim detail (`none` | `minimal` | `balanced` | `verbose`)
    /// from remote settings. Env (`GBUILD_COMPACTION_DETAIL`) and config override it.
    #[serde(default)]
    pub compaction_detail: Option<String>,
    /// remote settings verbatim-input flag; env (`GBUILD_COMPACTION_VERBATIM_INPUT`) and config override it. `None` = default (true).
    #[serde(default)]
    ...
    /// remote settings gate for the `gbuild workspace` CLI command (Computer Hub
    /// workspace exposure), from `gbuild_settings.workspace_command_enabled`.
...
pub struct ContextualHintsRemote {
    ...
    /// SSH wrap session-load tip (recommend `gbuild wrap ssh` for remote sessions).
    #[serde(default)]
...
/// only guaranteed to work with a compatible harness (cursor vs gBuild).
...
pub struct GoalRoleModel {
    ...
    pub model: String,
    /// Harness `agent_type` (e.g. "cursor", "gbuild-plan") whose
    /// `AgentDefinition` decides the role subagent's harness flavor (system
    /// prompt + cursor-vs-gBuild toolset), applied REGARDLESS of the
