...
//!
//! Extracted from `gbuild-shell/src/agent/subagent/` `normalize_forked_context()`.
...
/// `gbuild-shell/src/session/storage/jsonl.rs` (it truncates to the last
...
mod tests {
    use super::*;
    use gbuild_sampling_types::conversation::{ConversationItem, ToolCall, ToolResultItem};
    ...
    fn reasoning_item(text: &str) -> ConversationItem {
        ConversationItem::Reasoning(gbuild_sampling_types::synthesized_reasoning_item(text))
    }
