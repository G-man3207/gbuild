//! gBuild-owned hook write-deny: plan, identity revalidation, and post-reexec checks.
//! Namespace lockdown is in [`crate::child_net`].

use std::path::{Path, PathBuf};

use gbuild_config::{
    GlobalHookSource, ensure_gbuild_hook_slots, missing_configured_sources,
    resolve_global_hook_sources,
};

#[cfg(target_os = "linux")]
use gbuild_config::unique_ancestors_rootward;
#[cfg(unix)]
use gbuild_config::validated_hook_json_files_for_sources;
...
impl From<gbuild_config::GlobalHookSourceError> for HookWriteDenyError {
    fn from(e: gbuild_config::GlobalHookSourceError) -> Self {
...
fn reject_hardlinked_files(sources: &[GlobalHookSource]) -> Result<(), HookWriteDenyError> {
    use gbuild_config::GlobalHookSourceKind;
    use std::os::unix::fs::MetadataExt;
    for s in sources {
...
pub fn resolve_hook_write_deny_snapshot() -> Result<Vec<GlobalHookSource>, HookWriteDenyError> {
    ...
    let resolved =
        resolve_global_hook_sources(Some(gbuild.as_path()), /* reject_symlinks */ true)?;
...
pub fn prepare_hook_write_deny(
    ...
    let gbuild = gbuild_home();
    ensure_gbuild_hook_slots(gbuild.as_path())?;
...
fn capture_dir_json_snapshot(dir: &Path) -> Result<DirJsonSnapshot, HookWriteDenyError> {
    use gbuild_config::{list_direct_hook_json_files, validate_direct_hook_json_file};
