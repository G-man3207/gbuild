//! Integration tests for gbuild-sandbox.
//!
...
fn test_profile_capability_set_construction() {
    use gbuild_sandbox::ProfileName;
...
fn test_sandbox_logger() {
    use gbuild_sandbox::{SandboxEvent, SandboxLogger};
...
fn test_should_restrict_child_network_default() {
    ...
    assert!(!gbuild_sandbox::should_restrict_child_network());
}
