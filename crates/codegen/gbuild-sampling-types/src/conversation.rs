...
// ---------------------------------------------------------------------------
//
// Part of the gBuild compaction unification. Lets the shared, transport-agnostic
// engine in `crates/common/gbuild-compaction` operate over gBuild's
...
impl gbuild_compaction::CompactionItem for ConversationItem {
    fn role(&self) -> gbuild_compaction::CompactionRole {
        use gbuild_compaction::CompactionRole;
        // gBuild has no distinct `Developer` role; everything maps onto
    ...
    fn is_compaction_summary(&self) -> bool {
        // gBuild has no structural marker that uniquely identifies a prior
        ...
        // (add a dedicated marker) before routing gBuild history through
        // the shared `history`/`inter` filter.
        false
    }

    fn attachment_refs(&self) -> Vec<gbuild_compaction::CompactionFileRef> {
        // gBuild `UserItem`s carry only `Text`/`Image { url }` content
...
impl gbuild_compaction::CompactionItemFactory for ConversationItem {
...
/// ([stream/messages.rs:340](crates/codegen/gbuild-sampler/src/stream/messages.rs))
...
///
/// This is `pub` so downstream crates (`gbuild-shell`) can re-use it
...
impl From<ChatResponseMessage> for ConversationItem {
    fn from(msg: ChatResponseMessage) -> Self {
        ...
        // ([crates/codegen/gbuild-sampler/src/stream/chat_completions.rs])
...
pub fn build_messages_request(req: &ConversationRequest) -> crate::messages::MessagesRequest {
    ...
    // output without tools. The gbuild-shell agent does NOT use this path — a
...
/// ([crates/codegen/gbuild-sampler/src/stream/messages.rs]) instead
...
mod compaction_item_bridge_tests {
    use super::*;
    use gbuild_compaction::{CompactionItem, CompactionItemFactory, CompactionRole};
    ...
    fn metadata_accessors_are_conservative() {
        // gBuild has no structural compaction-summary marker, and no
...
mod tests {
    ...
    fn test_transform_cwd_transforms_tool_call_arguments() {
        ...
        let worktree = "/home/user/.gbuild/worktrees/project/ab-uuid-a";
    ...
    fn test_transform_cwd_worktree_to_root_syncback() {
        ...
        let worktree = "/home/user/.gbuild/worktrees/myproject/fork-a";
    ...
    fn test_transform_cwd_assistant_only_tool_calls_no_content() {
        ...
        let worktree = "/home/user/.gbuild/worktrees/proj/fork-a";
    ...
    #[test]
    fn upgrade_legacy_reasoning_singular_gbuild_shape() {
