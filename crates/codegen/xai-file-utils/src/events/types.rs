...
pub enum Event {
    ...
    /// `TODO_GATE_*` discriminator constant in `gbuild-shell::session::events`.
    TodoGateFired {
        fires: u32,
        pending: usize,
        in_progress: usize,
    ...
    /// constants in `gbuild-shell::session::events`.
    ...
    /// in `gbuild-shell::session::events`.
    ...
    /// discriminator constants in `gbuild-shell::session::events`.
    ...
    /// `gbuild-shell::session::events`. Fail-open never pauses the goal.
    ...
    /// enumerated by
    /// `gbuild-shell::session::goal_stop_detector::PATTERN_LABELS`;
...
/// Telemetry mirror of `gbuild-shell`'s `GoalClassifierVerdict`. Two
/// crates due to the orphan rule; the conversion lives in
/// `gbuild-shell/src/session/events.rs`.
#[derive(Debug, Clone, Copy, Serialize)]
#[serde(rename_all = "snake_case")]
...
/// Telemetry mirror of `gbuild-shell`'s `GoalPauseReason`. The two types
