...
This crate extracts a first-class `Agent` type from `gbuild-shell`.
...
`gbuild-shell`, another in-process host, or a headless batch runner.
...
in `.gbuild/agents/` (project-level) or `~/.gbuild/agents/` (user-level).

```rust
use gbuild_agent::{AgentDefinition, AgentBuilder};
use gbuild_tools::notification::ToolNotificationHandle;

// 1. Parse the definition file
let def = AgentDefinition::from_file(".gbuild/agents/code-reviewer.md")?;
...
```rust
use gbuild_agent::discovery;

// Find all .md files in .gbuild/agents/ directories
...
└────────▲─────────┘
         │ depends on
┌────────┴─────────┐
│  gbuild-shell  │  uses AgentBuilder to create
│  (session host)  │  Agent during session setup
└──────────────────┘
```

- **`gbuild-tools`**: Provides `ToolBridge`, `ToolRegistry`,
  `ToolState`, `SystemReminderLayer`, and tool implementations.
  `gbuild-agent` depends on it for tool setup.
- **`gbuild-shell`**: The application shell. Uses `AgentBuilder`
  to construct an `Agent` during session creation. The shell
  re-exports some modules from `gbuild-agent` (AGENTS.md
...
|---|---|---|
| `gbuild` | extend | Default agent for software engineering tasks |
...
```bash
# Check
cargo check -p gbuild-agent

# Test
cargo test -p gbuild-agent

# Clippy
cargo clippy -p gbuild-agent --fix --allow-dirty
