...
//! - `Default`: the legacy gBuild prefix (built by the shell layer).
...
use gbuild_tools::types::skill_discovery_tracker::{XmlRenderMode, format_announcement_xml};
...
pub enum UserMessageTemplate {
    /// Legacy gBuild prefix (`<user_info>` + optional `<git_status>`), built directly by the
...
/// One discovered rule file (AGENTS.md / Claude.md / .gbuild/rules/*.md).
///
...
pub struct UserMessageContext {
    ...
    /// string should use `gbuild_shell::util::uname::os_kernel_and_release`
    /// (or equivalent).
    ...
    /// User-scoped rule files (~/.gbuild/, ~/.claude/).
    ...
    /// (`~/.gbuild/projects/<encoded-cwd>/mcps`). Surfaced in
