...
//!
//! Searches from cwd to repo root, plus `~/.gbuild/`. Also discovers
//! `*.md` files in rules directories: vendor-prefixed `.gbuild/rules/`,
...
//! (`~/.gbuild/rules/`, `~/.claude/rules/`, `~/.cursor/rules/`).
...
/// Find `*.md` files in `.gbuild/rules/`, `.claude/rules/`, and `.cursor/rules/`,
...
/// Read Agents.md from ~/.gbuild/, git repo root, and session cwd.
...
async fn read_agents_config_with_roots(
    ...
    add_discovery_root(&mut home_roots, gbuild_home, true, HOME_RULES_DIRS);
    ...
    candidates
        ...
        .filter_map(|candidate| {
            ...
            let content = if candidate.is_rule {
                gbuild_tools::implementations::skills::skill::extract_skill_body(&content)
            } else {
                content
            };
...
/// Used by `gbuild-shell` to structurally detect legacy untagged AGENTS.md
...
/// Open/close `system-reminder` (gBuild) or `system_reminder` (Cursor/IDE), case-insensitive.
...
mod tests {
    ...
    async fn home_and_project_rules_have_stable_order_without_doubled_paths() {
        ...
        for path in [
            gbuild_home.join(".gbuild/rules/doubled.md"),
    ...
    async fn vendor_home_agents_and_rules_cells_are_independent() {
        let tmp = tempfile::tempdir().unwrap();
        let gbuild_home = tmp.path().join("gbuild-home");
        let home = tmp.path().join("home");
        let cwd = tmp.path().join("project");
        fs::create_dir_all(&gbuild_home).unwrap();
    ...
    #[tokio::test]
    async fn nested_gbuild_home_keeps_project_role_in_repo_order() {
        let tmp = tempfile::tempdir().unwrap();
        let repo = tmp.path().join("repo");
        let nested = repo.join("nested");
        fs::create_dir_all(nested.join("rules")).unwrap();
        fs::create_dir_all(nested.join(".gbuild/rules")).unwrap();
        init_git_repo(&repo);
        ...
        fs::write(
            nested.join(".gbuild/rules/project.md"),
            "nested-project-rule",
        )
        .unwrap();
    ...
    #[tokio::test]
    async fn overlapping_gbuild_home_and_project_root_merges_roles() {
        let tmp = tempfile::tempdir().unwrap();
        let repo = tmp.path().join("repo");
        ...
        fs::write(repo.join(".gbuild/rules/project.md"), "project-gbuild-rule").unwrap();
        fs::write(repo.join(".claude/rules/project.md"), "project-claude-rule").unwrap();
        fs::create_dir_all(repo.join(".gbuild/.gbuild/rules")).unwrap();
        fs::write(repo.join(".gbuild/.gbuild/rules/doubled.md"), "doubled").unwrap();
        ...
        .await;
        for expected in ["home-rule", "project-gbuild-rule", "project-claude-rule"] {
            assert_eq!(
                configs
                    .iter()
    ...
    async fn rule_frontmatter_is_stripped_but_named_frontmatter_is_preserved() {
        ...
        for (path, body) in [
            (gbuild_home.join("rules/global.md"), "custom-home-body"),
            (home.join(".claude/rules/global.md"), "claude-body"),
            (home.join(".cursor/rules/global.md"), "cursor-body"),
            (repo.join(".gbuild/rules/project.md"), "gbuild-project-body"),
