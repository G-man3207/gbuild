...
//!
//! Plugins from project directories (`.gbuild/plugins/`, `.claude/plugins/`)
...
//!
//! **Trust storage**: `~/.gbuild/trusted-plugins` (one canonical path per line).
//!
...
/// Name of the trust-store file under `~/.gbuild/`.
...
impl TrustStore {
    ...
    ///
    /// If `~/.gbuild/trusted-plugins` does not exist, returns an empty store.
    /// If the file cannot be read, logs a warning and returns an empty store.
    pub fn load() -> Self {
        // Gate on user_gbuild_home() so a project's `.gbuild/trusted-plugins` is never
        // read as the user trust store when neither GBUILD_HOME nor a home dir resolves.
        let Some(gbuild_home) = gbuild_config::user_gbuild_home() else {
            return Self {
                trusted: HashSet::new(),
                file_path: PathBuf::new(),
            };
        };
        let file_path = gbuild_home.join(TRUST_FILE_NAME);
    ...
    ///
    /// Canonicalizes the path and appends it to `~/.gbuild/trusted-plugins`.
    ...
    /// rewrites `~/.gbuild/trusted-plugins` without the revoked entry.
    ...
    /// trust via `~/.gbuild/trusted-plugins`.
