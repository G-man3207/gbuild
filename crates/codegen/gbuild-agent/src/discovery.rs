...
//!
//! Searches `.gbuild/agents/` and `.claude/agents/` from cwd to repo root,
//! then `~/.gbuild/agents/`, then `~/.claude/agents/`. Name-based dedup keeps
//! highest priority.
...
/// Project-level agent directories to scan (`.gbuild/agents/` + `.claude/agents/` compat).
const PROJECT_AGENT_SUBDIRS: &[&str] = &[".gbuild/agents", ".claude/agents"];

/// Existing project-level agent dirs (`.gbuild/agents` / `.claude/agents`), walked
...
pub fn all_subagents(cwd: &Path, toggle: &HashMap<String, bool>) -> Vec<SubagentEntry> {
    ...
    all_subagents_with_home(cwd, toggle, dirs::home_dir().as_deref(), gbuild.as_deref())
}
...
fn all_subagents_with_home(
    ...
    home: Option<&Path>,
    gbuild_home: Option<&Path>,
) -> Vec<SubagentEntry> {
    let discovered = discover_with_home(cwd, home, gbuild_home);
...
fn merge_subagents(
    ...
    //
    // A user-level ~/.gbuild/agents/explore.md does NOT shadow built-in explore
...
/// 1. `.gbuild/agents/` walking from `cwd` up to repo root
/// 2. `~/.gbuild/agents/` (user-level)
/// 3. `~/.claude/agents/` (compat user-level)
/// 4. `~/.gbuild/bundled/agents/` (bundled, lowest priority)
///
/// Deduplicates by name — higher-priority definitions win.
/// User-level agent directories in priority order: user gbuild agents, `.claude`
/// compat agents, then bundled. `.gbuild` dirs resolve from `gbuild_home`
/// (GBUILD_HOME-aware) plus the legacy literal `~/.gbuild` when GBUILD_HOME points
/// elsewhere; `.claude` resolves from `home`.
pub(crate) fn user_agent_dirs(
    home: Option<&Path>,
    gbuild_home: Option<&Path>,
) -> Vec<(std::path::PathBuf, AgentScope)> {
    // Legacy literal ~/.gbuild, included only when it differs from gbuild_home
    // (i.e. GBUILD_HOME points elsewhere) so agents left in the old location are
    // still discovered and stay consistent with scope_from_path classification.
    let default_gbuild = home
        .map(|h| h.join(".gbuild"))
        .filter(|legacy| gbuild_home != Some(legacy.as_path()));

    let mut dirs = Vec::new();
    if let Some(g) = gbuild_home {
        dirs.push((g.join("agents"), AgentScope::User));
    }
    if let Some(l) = &default_gbuild {
...
pub fn discover(cwd: &Path) -> Vec<AgentDefinition> {
    let gbuild = gbuild_config::user_gbuild_home();
    discover_with_home(cwd, dirs::home_dir().as_deref(), gbuild.as_deref())
}
...
fn discover_with_home(
    ...
    for (dir, scope) in user_agent_dirs(home, gbuild_home) {
        if dir.is_dir() {
...
pub fn by_name(name: &str) -> Option<AgentDefinition> {
    let gbuild = gbuild_config::user_gbuild_home();
    by_name_with_home(name, dirs::home_dir().as_deref(), gbuild.as_deref())
}
...
fn by_name_with_home(
    ...
    {
        let home_dirs = user_agent_dirs(home, gbuild_home);
...
///
/// Project-level `.gbuild/agents/` has highest priority, then falls back
/// to built-ins, user-level, and finally bundled definitions.
pub fn by_name_in_cwd(name: &str, cwd: &Path) -> Option<AgentDefinition> {
    let gbuild = gbuild_config::user_gbuild_home();
    by_name_in_cwd_with_home(name, cwd, dirs::home_dir().as_deref(), gbuild.as_deref())
}
...
fn by_name_in_cwd_with_home(
    ...
    by_name_with_home(name, home, gbuild_home)
}
...
fn by_name_in_cwd_with_plugins_and_home(
    ...
    if let Some(def) = by_name_in_cwd_with_home(name, cwd, home, gbuild_home) {
        return Some(def);
    }
...
/// Expand `${CLAUDE_PLUGIN_ROOT}` / `${CLAUDE_PLUGIN_DATA}` (and the gBuild
...
/// Load project agent definitions from every `.gbuild/agents` / `.claude/agents`
...
mod tests {
    ...
    fn test_origin_for_scope(scope: PluginScope) -> crate::plugins::PluginOrigin {
        ...
        match scope {
            ...
            PluginScope::Project => PluginOrigin::ProjectGBuild,
            PluginScope::User => PluginOrigin::UserGBuild,
    ...
    #[test]
    fn user_agent_dirs_includes_default_gbuild_when_gbuild_home_differs() {
        let home = Path::new("/home/u");
        let gbuild = Path::new("/custom/gbuildhome");
        let paths: Vec<_> = user_agent_dirs(Some(home), Some(gbuild))
            .into_iter()
            .map(|(p, _)| p)
            .collect();
        assert!(paths.contains(&gbuild.join("agents")));
        assert!(paths.contains(&home.join(".gbuild").join("agents")));
        assert!(paths.contains(&home.join(".claude").join("agents")));
        assert!(paths.contains(&gbuild.join("bundled").join("agents")));
        assert!(paths.contains(&home.join(".gbuild").join("bundled").join("agents")));
    }

    #[test]
    fn user_agent_dirs_dedups_default_when_gbuild_home_is_dot_gbuild() {
        let home = Path::new("/home/u");
        let gbuild = home.join(".gbuild");
        let count = user_agent_dirs(Some(home), Some(&gbuild))
            .into_iter()
            .filter(|(p, _)| *p == gbuild.join("agents"))
            .count();
        assert_eq!(
            count, 1,
            "no duplicate ~/.gbuild/agents when gbuild_home == ~/.gbuild"
        );
    }
    ...
    #[test]
    fn test_by_name_builtin_gbuild() {
        let def = by_name("gbuild");
        assert!(def.is_some());
        assert_eq!(def.unwrap().name, "gbuild");
    }
    ...
    fn test_discover_dedup_by_name() {
        ...
        let agents_dir_1 = tmp.path().join(".gbuild").join("agents");
        let agents_dir_2 = inner_dir.join(".gbuild").join("agents");
    ...
    fn test_discover_includes_bundled_agents_at_lowest_priority() {
        ...
        let bundled_dir = home.join(".gbuild").join("bundled").join("agents");
        ...
        let defs = discover_with_home(&cwd, Some(&home), Some(&home.join(".gbuild")));
        assert_eq!(defs.len(), 1);
    ...
    fn test_by_name_in_cwd_user_beats_bundled() {
        ...
        let cwd = tmp.path().join("workspace");
        let home = tmp.path().join("home");
        let user_dir = home.join(".gbuild").join("agents");
        let bundled_dir = home.join(".gbuild").join("bundled").join("agents");
        fs::create_dir_all(&cwd).unwrap();
        ...
        let def =
            by_name_in_cwd_with_home("reviewer", &cwd, Some(&home), Some(&home.join(".gbuild")))
                .unwrap();
    ...
    fn test_by_name_in_cwd_builtin_beats_bundled_for_builtin_names() {
        ...
        let def =
            by_name_in_cwd_with_home("explore", &cwd, Some(&home), Some(&home.join(".gbuild")))
                .unwrap();
    ...
    fn test_by_name_in_cwd_project_beats_bundled() {
        ...
        let cwd = tmp.path().join("workspace");
        let home = tmp.path().join("home");
        let project_dir = cwd.join(".gbuild").join("agents");
        let bundled_dir = home.join(".gbuild").join("bundled").join("agents");
    ...
    fn test_by_name_in_cwd_project_shadows_builtin() {
        ...
        // Create a project-level "gbuild" that shadows the built-in
        write_agent_file(&agents_dir, "gbuild.md", "gbuild", "Custom gbuild");

        let def = by_name_in_cwd("gbuild", tmp.path());
        assert!(def.is_some());
        let def = def.unwrap();
        assert_eq!(def.name, "gbuild");
        assert_eq!(def.description, "Custom gbuild");
    }

    #[test]
    fn test_by_name_in_cwd_falls_back_to_builtin() {
        let tmp = tempfile::tempdir().unwrap();
        // No .gbuild/agents/ directory — should fall back to built-in
    ...
    fn test_orchestrator_from_str_resolves() {
        use std::str::FromStr;
        let variant = BuiltinAgentName::from_str("gbuild-orchestrator")
            .expect("from_str must resolve gbuild-orchestrator");
        assert_eq!(variant, BuiltinAgentName::GBuildOrchestrator);
        let def = variant.definition();
        assert_eq!(def.name, "gbuild-orchestrator");
        assert!(
    ...
    fn test_orchestrator_by_name_in_cwd() {
        let tmp = tempfile::tempdir().unwrap();
        let def = by_name_in_cwd("gbuild-orchestrator", tmp.path())
            .expect("by_name_in_cwd must find gbuild-orchestrator");
        assert_eq!(def.name, "gbuild-orchestrator");
    ...
    fn test_merge_user_level_builtin_name_is_skipped() {
        // A user-level (~/.gbuild/agents/) agent named "explore" should NOT shadow
