...
use gbuild_tools::bridge::ToolBridge;
use gbuild_tools::computer::types::{AsyncFileSystem, TerminalBackend};
use gbuild_tools::notification::ToolNotificationHandle;
use gbuild_tools::registry::types::SessionContext;
use gbuild_tools::types::tool::ToolKind;
use std::collections::HashMap;
use std::path::PathBuf;
use std::sync::Arc;
/// The gBuild [`ToolKind`] a vendor-compat `tools:` allowlist entry resolves to, so
/// a plugin's upstream allowlist still binds. Backed by the shared vendor-to-gBuild
/// tool registry in `gbuild-tools` (also used by the hook matcher).
fn claude_tool_kind(name: &str) -> Option<ToolKind> {
    gbuild_tools::types::kind_for(name)
}
...
pub struct AgentBuilder {
    ...
    /// path (e.g., `~/.gbuild/worktrees/project/fork-...-overlay`) that must stay
    ...
    parent_scheduler_handle:
        Option<gbuild_tools::implementations::gbuild::scheduler::types::SchedulerHandle>,
    ...
    memory_backend: Option<Arc<dyn gbuild_tools::types::memory_backend::MemoryBackend>>,
    web_search_config: gbuild_tools::implementations::web_search::WebSearchConfig,
    ...
    backend_search: bool,
    web_fetch_config: gbuild_tools::implementations::gbuild::web_fetch::WebFetchConfig,
    lsp: Option<std::sync::Arc<dyn gbuild_tools::implementations::lsp::LspBackend>>,
    image_gen_config: gbuild_tools::implementations::gbuild::image_gen::ImageGenConfig,
    video_gen_config: gbuild_tools::implementations::gbuild::video_gen::VideoGenConfig,
    app_builder_deployer_config:
        gbuild_tools::implementations::gbuild::deploy_app::AppBuilderDeployerConfig,
    ...
    api_key_provider: Option<gbuild_tools::types::SharedApiKeyProvider>,
    attribution_callback: Option<gbuild_tools::SharedAttributionCallback>,
    ...
    /// `resolve_max_mcp_output_bytes_for_cwd` in gbuild-shell).
    ...
    preloaded_skills: Option<Vec<gbuild_tools::implementations::skills::types::SkillInfo>>,
}
/// Ensure plan mode tools (`enter_plan_mode`, `exit_plan_mode`,
/// `ask_user_question`) are present in the tool config.
fn ensure_plan_mode_tools(tool_config: &mut gbuild_tools::registry::types::ToolServerConfig) {
    use gbuild_tools::implementations::gbuild;
    let existing: std::collections::HashSet<&str> =
        tool_config.tools.iter().map(|tc| tc.id.as_str()).collect();
    let missing_enter = !existing.contains("GBuild:enter_plan_mode");
    let missing_exit = !existing.contains("GBuild:exit_plan_mode");
    let missing_ask = !existing.contains("GBuild:ask_user_question");
    drop(existing);
    if missing_enter {
        tool_config.tools.push((&gbuild::EnterPlanModeTool).into());
    }
    if missing_exit {
        tool_config.tools.push((&gbuild::ExitPlanModeTool).into());
    }
    if missing_ask {
        tool_config
            .tools
            .push((&gbuild::AskUserQuestionTool).into());
    }
}
...
fn merge_tool_params(
    tool_config: &mut gbuild_tools::registry::types::ToolServerConfig,
    ids: &[&str],
...
impl AgentBuilder {
    pub fn new(
        ...
        Self {
            ...
            terminal_backend,
            fs_backend: Arc::new(gbuild_tools::computer::local::LocalFs),
            notification_handle,
            ...
            system_reminder_tag: gbuild_tools::reminders::DEFAULT_REMINDER_TAG,
    ...
    pub fn with_preloaded_skills(
        mut self,
        skills: Vec<gbuild_tools::implementations::skills::types::SkillInfo>,
    ) -> Self {
    ...
    pub fn with_memory_backend(
        mut self,
        backend: Arc<dyn gbuild_tools::types::memory_backend::MemoryBackend>,
    ) -> Self {
    ...
    pub fn with_parent_scheduler_handle(
        mut self,
        handle: gbuild_tools::implementations::gbuild::scheduler::types::SchedulerHandle,
    ) -> Self {
    ...
    pub fn with_web_search_config(
        mut self,
        config: gbuild_tools::implementations::web_search::WebSearchConfig,
    ) -> Self {
    ...
    /// `GBUILD_WEB_FETCH` env var.
    pub fn with_web_fetch_config(
        mut self,
        config: gbuild_tools::implementations::gbuild::web_fetch::WebFetchConfig,
    ) -> Self {
    ...
    pub fn with_lsp(
        mut self,
        handle: std::sync::Arc<dyn gbuild_tools::implementations::lsp::LspBackend>,
    ) -> Self {
    ...
    pub fn with_image_gen_config(
        mut self,
        config: gbuild_tools::implementations::gbuild::image_gen::ImageGenConfig,
    ) -> Self {
    ...
    pub fn with_video_gen_config(
        mut self,
        config: gbuild_tools::implementations::gbuild::video_gen::VideoGenConfig,
    ) -> Self {
    ...
    pub fn with_app_builder_deployer_config(
        mut self,
        config: gbuild_tools::implementations::gbuild::deploy_app::AppBuilderDeployerConfig,
    ) -> Self {
    ...
    pub fn with_api_key_provider(
        mut self,
        provider: gbuild_tools::types::SharedApiKeyProvider,
    ) -> Self {
    ...
    /// Set public model slugs advertised in the GBuild Task description.
    ...
    ///
    /// When disabled, `GBuild:ask_user_question` is stripped from the
    ...
    pub fn with_compat_config(mut self, compat: gbuild_tools::types::compat::CompatConfig) -> Self {
        self.compat = compat;
        self
    }
    /// Set the skills config (custom paths, ignore globs) from config.toml.
    /// Without this, only auto-discovered skills (cwd/.gbuild/skills, ~/.gbuild/skills)
    ...
    pub async fn build(mut self) -> Result<Agent, AgentBuildError> {
        ...
        if definition.inject_default_tools {
            ...
            if self.web_search_config.is_enabled() {
                use gbuild_tools::implementations::gbuild;
                tool_config.tools.push((&gbuild::WebSearchTool).into());
            }
            if self.web_fetch_config.is_enabled() {
                use gbuild_tools::implementations::gbuild;
                tool_config.tools.push((&gbuild::WebFetchTool).into());
            }
            if self.lsp.is_some() {
                tool_config
                    .tools
                    .push((&gbuild_tools::implementations::gbuild::LspTool).into());
            }
            if self.image_gen_config.image_gen_enabled() {
                tool_config
                    .tools
                    .push((&gbuild_tools::implementations::gbuild::ImageGenTool).into());
            }
            if self.image_gen_config.image_edit_enabled() {
                tool_config
                    .tools
                    .push((&gbuild_tools::implementations::gbuild::ImageEditTool).into());
            }
            if self.video_gen_config.is_enabled() {
                tool_config
                    .tools
                    .push((&gbuild_tools::implementations::gbuild::ImageToVideoTool).into());
                tool_config
                    .tools
                    .push((&gbuild_tools::implementations::gbuild::ReferenceToVideoTool).into());
            }
            ...
            if self.write_file_enabled && !has_write_tool {
                tool_config
                    .tools
                    .push((&gbuild_tools::implementations::opencode::OpenCodeWriteTool).into());
            }
            ensure_plan_mode_tools(&mut tool_config);
        }
        if self.memory_backend.is_none() {
            let gbuild_ns = gbuild_tools::types::tool::ToolNamespace::GBuild.to_string();
            let mem_search_id = format!(
                "{gbuild_ns}:{}",
                gbuild_tools::implementations::memory::MEMORY_SEARCH_TOOL_NAME
            );
            let mem_get_id = format!(
                "{gbuild_ns}:{}",
                gbuild_tools::implementations::memory::MEMORY_GET_TOOL_NAME
            );
            tool_config
                .tools
        ...
        if !self.ask_user_question_enabled {
            let ask_user_id = format!(
                "{}:ask_user_question",
                gbuild_tools::types::tool::ToolNamespace::GBuild,
            );
        ...
        if task_stripped {
            use gbuild_tools::types::tool::ToolNamespace;
            ...
            if !has_satisfier(ToolNamespace::GBuild, "run_terminal_cmd", true)
                && !has_satisfier(ToolNamespace::GBuildConcise, "run_terminal_cmd", true)
        ...
        if let gbuild_tools::implementations::gbuild::web_fetch::WebFetchConfig::Enabled {
            ref params,
        } = self.web_fetch_config
            && let Ok(params_value) = serde_json::to_value(params)
            && let Some(obj) = params_value.as_object()
        {
            merge_tool_params(&mut tool_config, &["GBuild:web_fetch"], obj);
        }
        if let Some(ref bash_params) = self.bash_params_json {
            merge_tool_params(
                &mut tool_config,
                &["GBuild:run_terminal_cmd", "GBuildConcise:run_terminal_cmd"],
                bash_params,
            );
        }
        if let Some(ref ask_params) = self.ask_user_question_params_json {
            merge_tool_params(&mut tool_config, &["GBuild:ask_user_question"], ask_params);
        }
        ...
        if let Some(bytes) = self.mcp_max_output_bytes {
            tool_bridge.toolset().resources.lock().await.insert(
                gbuild_tools::types::resources::TruncationCfg(
                    gbuild_tools::types::context::TruncationConfig {
        ...
        if use_backend_search {
            if web_search_enabled && definition.hosted_tool_allowed("web_search") {
                hosted_tools.push(gbuild_sampling_types::HostedTool::WebSearch { options: None });
            }
            if definition.hosted_tool_allowed("x_search") {
                hosted_tools.push(gbuild_sampling_types::HostedTool::XSearch { options: None });
            }
            gbuild_sampling_types::apply_tool_overrides(
                &mut hosted_tools,
...
fn resolve_shell_for_prompt() -> String {
    ...
    #[cfg(not(unix))]
    {
        gbuild_config::shell::detect_windows_shell()
            .name()
            .to_string()
    }
}
...
mod tests {
    ...
    fn entry(name: &str, desc: &str, source: SubagentSource) -> SubagentEntry {
        SubagentEntry {
            ...
            config_source: gbuild_tools::types::config_source::ConfigSource::Builtin,
        }
    }
    #[test]
    ...
    fn build_task_description_shadowed_builtin_uses_user_desc() {
        let subagents = vec![SubagentEntry {
            ...
            config_source: gbuild_tools::types::config_source::ConfigSource::Project {
    ...
    fn task_model_guidance_resolves_model_param_override() {
        use gbuild_tools::types::template_renderer::TemplateRenderer;
        use gbuild_tools::types::tool::ToolKind;
    ...
    async fn pager_flag_combinations_satisfy_tool_invariants() {
        ...
        let cases: &[PagerFlagCase] = &[
            PagerFlagCase {
                label: "gbuild / subagents+ask_user",
                profile: AgentDefinition::default_gbuild,
                subagents: true,
                ask_user: true,
            },
            PagerFlagCase {
                label: "gbuild / subagents / no-ask-user",
                profile: AgentDefinition::default_gbuild,
                subagents: true,
                ask_user: false,
            },
            PagerFlagCase {
                label: "gbuild / no-subagents / ask_user",
                profile: AgentDefinition::default_gbuild,
                subagents: false,
                ask_user: true,
            },
            PagerFlagCase {
                label: "gbuild / no-subagents / no-ask-user",
                profile: AgentDefinition::default_gbuild,
                subagents: false,
                ask_user: false,
            },
            PagerFlagCase {
                label: "gbuild-ask-user / subagents",
                profile: AgentDefinition::gbuild_ask_user,
                subagents: true,
                ask_user: true,
            },
            PagerFlagCase {
                label: "gbuild-ask-user / no-subagents",
                profile: AgentDefinition::gbuild_ask_user,
                subagents: false,
                ask_user: true,
            },
            PagerFlagCase {
                label: "gbuild-plan",
                profile: AgentDefinition::gbuild_plan,
                subagents: true,
                ask_user: true,
            },
            PagerFlagCase {
                label: "gbuild-plan / no-ask-user",
                profile: AgentDefinition::gbuild_plan,
                subagents: true,
                ask_user: false,
            },
            PagerFlagCase {
                label: "gbuild-plan-no-subagents",
                profile: AgentDefinition::gbuild_plan_no_subagents,
                subagents: false,
                ask_user: true,
            },
            PagerFlagCase {
                label: "gbuild-plan-no-subagents / no-ask-user",
                profile: AgentDefinition::gbuild_plan_no_subagents,
                subagents: false,
                ask_user: false,
            },
    ...
    async fn plan_mode_injected_ask_user_question_receives_params() {
        use gbuild_tools::computer::local::LocalTerminalBackend;
        use gbuild_tools::implementations::gbuild::ask_user_question::AskUserQuestionParams;
        use gbuild_tools::notification::ToolNotificationHandle;
        use gbuild_tools::types::resources::Params;
        let profile = crate::config::AgentDefinition::default_gbuild();
        assert!(
            !profile
                .tool_config
                .tools
                .iter()
                .any(|tc| tc.id == "GBuild:ask_user_question"),
    ...
    async fn build_with_tools(tools: Vec<String>, disallowed: Vec<String>) -> crate::agent::Agent {
        use gbuild_tools::computer::local::LocalTerminalBackend;
        use gbuild_tools::notification::ToolNotificationHandle;
        let mut def = crate::config::AgentDefinition::default_gbuild();
        def.tools = tools;
    ...
    fn session_tools_allowed_clamp() {
        ...
        assert!(def.session_tools_allowed("GBuild:read_file"));
    ...
    async fn spawning_blocked_disables_all_background_bash_modes() {
        use gbuild_tools::computer::local::LocalTerminalBackend;
        use gbuild_tools::implementations::gbuild::bash::BashParams;
        use gbuild_tools::notification::ToolNotificationHandle;
        use gbuild_tools::types::resources::Params;
    ...
    /// Compat allowlist names (`Read`, `Bash`, `Grep`) map to their gBuild
    /// equivalents by `ToolKind` — a real restricted toolset, not zero tools.
    #[tokio::test]
    async fn claude_tool_names_map_to_gbuild_equivalents() {
    ...
    /// Shell, LSP, ask, and task-lifecycle tool names resolve to their gBuild
    ...
    /// treat `mcp__*` as always-on, so gBuild keeps the MCP meta-tools
    ...
    async fn requested_enabled_web_tools_survive_allowlist() {
        use gbuild_tools::computer::local::LocalTerminalBackend;
        use gbuild_tools::implementations::gbuild::web_fetch::WebFetchConfig;
        use gbuild_tools::implementations::web_search::WebSearchConfig;
        use gbuild_tools::notification::ToolNotificationHandle;
        let mut definition = crate::config::AgentDefinition::default_gbuild();
    ...
    /// gbuild toolsets have no Skill tool — skills are read from
    ...
    /// Compat `ToolSearch` meta-tool maps to gBuild's `search_tool` (MCP
    ...
    async fn build_with_web_search(
        ...
        tool_overrides: Option<gbuild_sampling_types::ToolOverrides>,
    ) -> crate::agent::Agent {
        use gbuild_tools::computer::local::LocalTerminalBackend;
        use gbuild_tools::implementations::web_search::WebSearchConfig;
        use gbuild_tools::notification::ToolNotificationHandle;
    ...
    async fn disallowed_web_search_strips_function_and_hosted_tools() {
        ...
        assert!(
            !hosted
                .iter()
                .any(|t| matches!(t, gbuild_sampling_types::HostedTool::WebSearch { .. })),
            "hosted WebSearch must be removed when web_search is disallowed, got: {hosted:?}"
        );
        assert!(
            hosted
                .iter()
                .any(|t| matches!(t, gbuild_sampling_types::HostedTool::XSearch { .. })),
    ...
    async fn hosted_tools_bake_definition_tool_overrides_into_options() {
        let x_search = gbuild_sampling_types::XSearchOptions {
            date_bound: Some(
                gbuild_sampling_types::SearchDateBound::new(None, Some("2024-03-15".into()))
                    .unwrap(),
            ),
        };
        let agent = build_with_web_search(
            ...
            true,
            &[],
            Some(gbuild_sampling_types::ToolOverrides {
        ...
        assert!(
            agent
                .hosted_tools()
                .contains(&gbuild_sampling_types::HostedTool::XSearch {
