//! Agent definition types — parsed from `.gbuild/agents/*.md` files.
use crate::error::AgentBuildError;
use crate::prompt::context::TemplateOverride;
use crate::prompt::user_message::UserMessageTemplate;
use gbuild_tools::implementations::codex;
use gbuild_tools::implementations::gbuild;
use gbuild_tools::implementations::gbuild_concise;
use gbuild_tools::implementations::memory;
use gbuild_tools::implementations::opencode;
use gbuild_tools::implementations::search_tool;
use gbuild_tools::implementations::use_tool;
use gbuild_tools::registry::types::{ToolConfig, ToolServerConfig};
...
/// Orchestrator-specific prompt body appended to the standard GBuild
...
fn bash_tool_config() -> ToolConfig {
    ToolConfig::from(&gbuild::BashTool)
...
fn task_tool_config() -> ToolConfig {
    ToolConfig::from(&gbuild::TaskTool)
...
fn task_output_tool_config() -> ToolConfig {
    ToolConfig::from(&gbuild::TaskOutputTool).with_name("get_command_or_subagent_output")
}
/// `wait_tasks` → `wait_commands_or_subagents`.
fn wait_tasks_tool_config() -> ToolConfig {
    ToolConfig::from(&gbuild::WaitTasksTool).with_name("wait_commands_or_subagents")
}
/// `kill_task` → `kill_command_or_subagent`.
fn kill_task_tool_config() -> ToolConfig {
    ToolConfig::from(&gbuild::KillTaskTool).with_name("kill_command_or_subagent")
}
/// Complete workspace-executable toolset for hub registration.
///
/// Extends `default_gbuild_toolset()` with tools that are dynamically
...
/// zero local dispatch.
pub fn workspace_gbuild_toolset() -> ToolServerConfig {
    let mut tools = default_gbuild_toolset().tools;
    tools.push((&opencode::OpenCodeWriteTool).into());
    tools.push((&gbuild::EnterPlanModeTool).into());
    tools.push((&gbuild::ExitPlanModeTool).into());
    tools.push((&gbuild::AskUserQuestionTool).into());
    tools.push((&gbuild::WebSearchTool).into());
    tools.push((&gbuild::ImageGenTool).into());
    tools.push((&gbuild::ImageToVideoTool).into());
    tools.push((&gbuild::ReferenceToVideoTool).into());
    tools.push((&gbuild::WebFetchTool).into());
    tools.push((&memory::search_tool::MemorySearchImpl).into());
    tools.push((&memory::get_tool::MemoryGetImpl).into());
    tools.push((&gbuild::LspTool).into());
    ToolServerConfig {
        tools,
        behavior_preset: None,
    }
}
/// Toolset for the `gbuild-computer` (workspace/sandbox) preset.
fn gbuild_computer_toolset() -> ToolServerConfig {
    #[allow(unused_mut)]
    let mut tools = vec![
        bash_tool_config(),
        (&gbuild::ReadFileTool).into(),
        (&gbuild::SearchReplaceTool).into(),
        (&opencode::OpenCodeWriteTool).into(),
        (&gbuild::ListDirTool).into(),
        (&gbuild::GrepTool).into(),
        (&gbuild::KillTerminalCommandTool).into(),
        (&gbuild::GetTerminalCommandOutputTool).into(),
    ];
    ToolServerConfig {
        tools,
        behavior_preset: None,
...
fn native_toolset_presets() -> Vec<(&'static str, ToolServerConfig)> {
    vec![
        ("gbuild", workspace_gbuild_toolset()),
        ("gbuild-concise", gbuild_concise_toolset()),
        ("gbuild-plan", gbuild_plan_toolset()),
        ("codex", codex_toolset()),
        ("explore", explore_toolset()),
        ("plan", plan_toolset()),
        ("gbuild-computer", gbuild_computer_toolset()),
    ]
}
...
fn default_gbuild_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            bash_tool_config(),
            (&gbuild::ReadFileTool).into(),
            ...
            (&gbuild::TodoWriteTool).into(),
            task_output_tool_config(),
            wait_tasks_tool_config(),
            task_tool_config(),
            (&gbuild::SchedulerCreateTool).into(),
            (&gbuild::SchedulerDeleteTool).into(),
            (&gbuild::SchedulerListTool).into(),
            (&gbuild::MonitorTool).into(),
            (&search_tool::SearchTool).into(),
            (&use_tool::UseTool).into(),
            (&gbuild::UpdateGoalTool).into(),
            (&gbuild::WorkflowTool).into(),
        ],
        behavior_preset: None,
    }
}
fn gbuild_concise_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            (&gbuild_concise::BashConciseTool).into(),
            (&gbuild_concise::ReadFileConciseTool).into(),
            (&gbuild_concise::SearchReplaceConciseTool).into(),
            (&gbuild::ListDirTool).into(),
            (&gbuild::GrepTool).into(),
            kill_task_tool_config(),
            (&gbuild::TodoWriteTool).into(),
...
pub fn gbuild_hashline_toolset(
    hashline_tools: Vec<gbuild_tools::registry::types::ToolConfig>,
) -> ToolServerConfig {
    let mut tools: Vec<gbuild_tools::registry::types::ToolConfig> = vec![bash_tool_config()];
...
/// gBuild + plan mode toolset.
///
/// Extends the default `gbuild` toolset with plan mode tools:
...
fn gbuild_plan_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            // Standard gbuild tools
            bash_tool_config(),
            (&gbuild::ReadFileTool).into(),
            (&gbuild::SearchReplaceTool).into(),
            (&gbuild::ListDirTool).into(),
            ...
            (&gbuild::WorkflowTool).into(),
            // Plan mode tools
            (&gbuild::EnterPlanModeTool).into(),
            (&gbuild::ExitPlanModeTool).into(),
            (&gbuild::AskUserQuestionTool).into(),
        ],
        behavior_preset: None,
    }
}
...
fn orchestrator_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            ...
            // Web tools
            (&gbuild::WebSearchTool).into(),
            (&gbuild::WebFetchTool).into(),
            // Imagine
            (&gbuild::ImageGenTool).into(),
            (&gbuild::ImageToVideoTool).into(),
            (&gbuild::ReferenceToVideoTool).into(),
            // Memory
...
/// gBuild + plan mode toolset WITHOUT subagent tools.
///
/// Same as `gbuild_plan_toolset` but excludes `TaskTool`,
/// `TaskOutputTool`, and `KillTaskTool`. Use this when the shell
/// does not have subagent infrastructure wired up.
fn gbuild_plan_no_subagents_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            // Standard gbuild tools (minus TaskTool only — KillTaskTool and
...
/// Default gBuild toolset + `ask_user_question`.
///
/// Same as `default_gbuild_toolset` with the `AskUserQuestionTool` added,
/// allowing the agent to ask structured questions without full plan mode.
fn gbuild_ask_user_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            bash_tool_config(),
            (&gbuild::ReadFileTool).into(),
...
pub enum BuiltinAgentName {
    #[strum(serialize = "gbuild")]
    GBuild,
    #[strum(serialize = "gbuild-concise")]
    GBuildConcise,
    #[strum(serialize = "gbuild-plan")]
    GBuildPlan,
    #[strum(serialize = "gbuild-plan-no-subagents")]
    GBuildPlanNoSubagents,
    #[strum(serialize = "gbuild-ask-user")]
    GBuildAskUser,
    Codex,
    Opencode,
    GeneralPurpose,
    ...
    Plan,
    BrowserUse,
    #[strum(serialize = "gbuild-orchestrator")]
    GBuildOrchestrator,
}
...
impl BuiltinAgentName {
    ...
    pub fn definition(self) -> AgentDefinition {
        match self {
            Self::GBuild => AgentDefinition::default_gbuild(),
            Self::GBuildConcise => AgentDefinition::gbuild_concise(),
            Self::GBuildPlan => AgentDefinition::gbuild_plan(),
            Self::GBuildPlanNoSubagents => AgentDefinition::gbuild_plan_no_subagents(),
            Self::GBuildAskUser => AgentDefinition::gbuild_ask_user(),
            ...
            Self::GBuildOrchestrator => AgentDefinition::gbuild_orchestrator(),
        }
    }
...
/// Portable agent identity — parsed from .gbuild/agents/*.md.
...
pub struct AgentDefinition {
    ...
    #[serde(default = "default_gbuild_toolset")]
    ...
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub tool_overrides: Option<gbuild_sampling_types::ToolOverrides>,
...
pub enum AgentScope {
    /// .gbuild/agents/ (project-level, highest priority)
    Project,
    /// ~/.gbuild/agents/ (user-level)
    User,
    /// ~/.gbuild/bundled/agents/ (lowest-priority bundled cache)
    Bundled,
    /// Built-in agent (e.g., default_gbuild(), browser_use()).
    #[default]
    BuiltIn,
}
impl AgentScope {
...
pub enum MemoryScope {
    /// `~/.gbuild/agent-memory/<name>/`
    User,
    /// `<project>/.gbuild/agent-memory/<name>/`
    Project,
    /// `<project>/.gbuild/agent-memory-local/<name>/`
    Local,
}
impl MemoryScope {
    ...
    pub fn resolve_dir(self, agent_name: &str, project_cwd: &std::path::Path) -> ResolvedMemoryDir {
        match self {
            Self::User => ResolvedMemoryDir {
                path: gbuild_config::gbuild_home()
                    .join("agent-memory")
                    .join(agent_name),
                is_project_scoped: false,
            },
            Self::Project => ResolvedMemoryDir {
                path: project_cwd.join(".gbuild/agent-memory").join(agent_name),
                is_project_scoped: true,
            },
            Self::Local => ResolvedMemoryDir {
                path: project_cwd
                    .join(".gbuild/agent-memory-local")
                    .join(agent_name),
                is_project_scoped: true,
            },
        }
...
/// these into `gbuild_tools::registry::types::ToolsetConfig.bash`
...
impl AgentDefinition {
    ...
    fn scope_from_path(path: &Path) -> AgentScope {
        ...
        let gbuild = gbuild_config::user_gbuild_home();
        let home = dirs::home_dir();
        for (dir, scope) in crate::discovery::user_agent_dirs(home.as_deref(), gbuild.as_deref()) {
            if path.starts_with(&dir) {
                return scope;
            }
        }
        if path_str.contains(".gbuild/agents/") || path_str.contains(".gbuild\\agents\\") {
            return AgentScope::Project;
        }
        if path_str.contains(".gbuild/bundled/agents/")
            || path_str.contains(".gbuild\\bundled\\agents\\")
        {
...
impl AgentDefinition {
    ...
    /// (`!inject_default_tools`). Stock `gbuild*` agents leave all
    ...
    pub fn override_file_tools(
        &mut self,
        file_tools: Vec<gbuild_tools::registry::types::ToolConfig>,
    ) {
        const FILE_TOOL_SLOTS: &[[&str; 2]] = &[
            ["GBuild:read_file", "GBuildHashline:hashline_read"],
            ["GBuild:search_replace", "GBuildHashline:hashline_edit"],
            ["GBuild:grep", "GBuildHashline:hashline_grep"],
        ];
    ...
    pub fn builtin_defaults(name: &str, description: &str) -> Self {
        Self {
            ...
            tool_config: default_gbuild_toolset(),
    ...
    pub fn default_gbuild() -> Self {
        Self::base(
            BuiltinAgentName::GBuild,
            "gBuild agent for software engineering tasks.",
        )
    }
    /// gBuild Concise agent definition — concise output format for SFT/RL.
    pub fn gbuild_concise() -> Self {
        Self {
            tool_config: gbuild_concise_toolset(),
            agents_md: false,
            ..Self::base(
                BuiltinAgentName::GBuildConcise,
                "gBuild agent with concise output format.",
            )
        }
    }
    /// gBuild agent with plan mode tools.
    pub fn gbuild_plan() -> Self {
        Self {
            tool_config: gbuild_plan_toolset(),
            ..Self::base(
                BuiltinAgentName::GBuildPlan,
                "gBuild agent with plan mode support.",
            )
        }
    }
    /// gBuild + plan mode WITHOUT subagent tools.
    pub fn gbuild_plan_no_subagents() -> Self {
        Self {
            tool_config: gbuild_plan_no_subagents_toolset(),
            ..Self::base(
                BuiltinAgentName::GBuildPlanNoSubagents,
                "gBuild agent with plan mode (no subagents).",
            )
        }
    }
    /// Default gBuild agent with the `ask_user_question` tool.
    pub fn gbuild_ask_user() -> Self {
        Self {
            tool_config: gbuild_ask_user_toolset(),
            ..Self::base(
                BuiltinAgentName::GBuildAskUser,
                "gBuild agent with ask-user-question tool.",
            )
        }
    }
    pub fn codex() -> Self {
    ...
    /// gBuild Orchestrator — GBL model with full GBuild tools
    ...
    pub fn gbuild_orchestrator() -> Self {
        Self {
            tool_config: orchestrator_toolset(),
            inject_default_tools: false,
            prompt_body: Some(ORCHESTRATOR_PROMPT_BODY.to_string()),
            ..Self::base(
                BuiltinAgentName::GBuildOrchestrator,
                "GBuild orchestrator that delegates coding to specialized subagents",
            )
        }
    }
    ...
    pub fn from_json(value: &serde_json::Value) -> Result<Self, AgentBuildError> {
        ...
        if !value.get("toolConfig").is_some_and(|v| v.is_object()) {
            def.tool_config = default_gbuild_toolset();
        }
...
mod tests {
    ...
    fn presets_select_distinct_toolsets_by_size() {
        let gb = toolset_for_preset("gbuild").unwrap();
    ...
    fn gbuild_computer_exclusive_ids() -> Vec<String> {
        #[allow(unused_mut)]
        let mut ids: Vec<String> = vec![
            ToolConfig::from(&gbuild::GetTerminalCommandOutputTool).id,
            ToolConfig::from(&gbuild::KillTerminalCommandTool).id,
        ];
        ids
    }
    #[test]
    fn gbuild_computer_preset_is_curated_gbuild_subset() {
        let gc = toolset_for_preset("gbuild-computer").unwrap();
        let gb = toolset_for_preset("gbuild").unwrap();
        let gb_ids: std::collections::HashSet<&str> =
            gb.tools.iter().map(|t| t.id.as_str()).collect();
        let exclusive_ids = gbuild_computer_exclusive_ids();
        ...
        for t in &gc.tools {
            ...
            assert!(
                ...
                "gbuild-computer tool `{}` must also ship in the gbuild preset",
                t.id
            );
        }
        assert!(
            gc.tools.len() < gb.tools.len(),
            "gbuild-computer should be a curated subset of gbuild"
        );
    }
    #[test]
    fn gbuild_computer_uses_subagent_free_background_task_tools() {
        let gc = toolset_for_preset("gbuild-computer").unwrap();
        let ids: std::collections::HashSet<&str> = gc.tools.iter().map(|t| t.id.as_str()).collect();
        assert!(
            ids.contains(
                ToolConfig::from(&gbuild::GetTerminalCommandOutputTool)
                    .id
                    .as_str()
            )
        );
        assert!(
            ids.contains(
                ToolConfig::from(&gbuild::KillTerminalCommandTool)
                    .id
                    .as_str()
            )
        );
        assert!(!ids.contains(ToolConfig::from(&gbuild::TaskOutputTool).id.as_str()));
        assert!(!ids.contains(ToolConfig::from(&gbuild::KillTaskTool).id.as_str()));
        assert!(!ids.contains(ToolConfig::from(&gbuild::TaskTool).id.as_str()));
        for t in &gc.tools {
            if t.id == ToolConfig::from(&gbuild::GetTerminalCommandOutputTool).id
                || t.id == ToolConfig::from(&gbuild::KillTerminalCommandTool).id
            {
                assert!(t.name_override.is_none(), "tool `{}` must not rename", t.id);
            }
        }
    }
    /// The gbuild-computer preset must ship a full-file write tool (legacy
    /// `write_file` parity) — the same OpenCode `write` tool the gbuild
    ...
    #[test]
    fn gbuild_computer_preset_includes_write_tool() {
        let gc = toolset_for_preset("gbuild-computer").unwrap();
        let write_id = ToolConfig::from(&opencode::OpenCodeWriteTool).id;
        assert!(
            gc.tools.iter().any(|t| t.id == write_id),
            "gbuild-computer preset must include the `{write_id}` tool"
        );
    }
    #[test]
    fn gbuild_computer_preset_excludes_plan_and_lsp() {
        let gc = toolset_for_preset("gbuild-computer").unwrap();
        let gc_ids: std::collections::HashSet<&str> =
            gc.tools.iter().map(|t| t.id.as_str()).collect();
        for excluded in [
            ToolConfig::from(&gbuild::LspTool).id,
            ToolConfig::from(&gbuild::EnterPlanModeTool).id,
            ToolConfig::from(&gbuild::ExitPlanModeTool).id,
        ] {
            assert!(
                !gc_ids.contains(excluded.as_str()),
                "gbuild-computer preset must not advertise `{excluded}`"
            );
        }
        let full = workspace_gbuild_toolset();
        ...
        for present in [
            ...
            assert!(
                ...
                "workspace_gbuild_toolset must ship `{present}`"
            );
        }
    }
    ...
    fn expected_strict_harness(name: BuiltinAgentName) -> bool {
        match name {
            BuiltinAgentName::Codex | BuiltinAgentName::GBuildOrchestrator => true,
            BuiltinAgentName::GBuild
            | BuiltinAgentName::GBuildConcise
            | BuiltinAgentName::GBuildPlan
            | BuiltinAgentName::GBuildPlanNoSubagents
    ...
    fn is_strict_harness_agent_type_classifies_by_name() {
        for strict in ["codex", "gbuild-orchestrator"] {
            assert!(
...
"#;
    ...
    fn memory_scope_resolve_dir() {
        ...
        assert_eq!(
            proj.path,
            std::path::PathBuf::from("/project/.gbuild/agent-memory/a")
        );
        assert!(proj.is_project_scoped);
        let local = MemoryScope::Local.resolve_dir("a", cwd);
        assert_eq!(
            local.path,
            std::path::PathBuf::from("/project/.gbuild/agent-memory-local/a")
        );
...
"#;
    ...
    #[test]
    fn test_default_tool_config_has_gbuild_tools() {
        let content = r#"---
name: default-tools
...
"#;
        ...
        assert!(
            ...
            "default tool_config should have gbuild tools"
        );
    }
    #[test]
    ...
    fn test_builtin_agent_name_strum_round_trip() {
        use std::str::FromStr;
        for (s, expected) in [
            ("gbuild", BuiltinAgentName::GBuild),
            ("gbuild-concise", BuiltinAgentName::GBuildConcise),
            ("gbuild-plan", BuiltinAgentName::GBuildPlan),
            (
                "gbuild-plan-no-subagents",
                BuiltinAgentName::GBuildPlanNoSubagents,
            ),
            ("gbuild-ask-user", BuiltinAgentName::GBuildAskUser),
            ("gbuild-orchestrator", BuiltinAgentName::GBuildOrchestrator),
    ...
    fn def_with_template(tpl: crate::prompt::context::TemplateOverride) -> AgentDefinition {
        let mut def = AgentDefinition::default_gbuild();
