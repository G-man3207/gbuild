//! Integration tests for gbuild-hooks.
//!
...
use gbuild_hooks::dispatcher;
use gbuild_hooks::event::*;
use gbuild_hooks::result::HookDecision;
use gbuild_hooks::runner::RunContext;
...
async fn non_blocking_dispatch() {
    ...
    assert!(matches!(
        &results[0],
        gbuild_hooks::result::HookRunResult::Success { .. }
    ));
}
    ...
    let cases = vec![
        ...
        Case {
            ...
            payload: HookPayload::StopFailure {
                error: gbuild_hooks::event::StopFailureKind::RateLimit,
...
/// runner-reserved keys (`GBUILD_HOOK_EVENT`, `GBUILD_HOOK_NAME`,
/// `GBUILD_SESSION_ID`, `GBUILD_WORKSPACE_ROOT`, `CLAUDE_PROJECT_DIR`)
...
async fn runner_injected_vars_override_extra_env_at_spawn() {
    ...
    let cmd = format!(
        r#"echo "EVENT=$GBUILD_HOOK_EVENT" > {f}; echo "NAME=$GBUILD_HOOK_NAME" >> {f}; echo "SESSION=$GBUILD_SESSION_ID" >> {f}; echo "ROOT=$GBUILD_WORKSPACE_ROOT" >> {f}; echo "PROJ=$CLAUDE_PROJECT_DIR" >> {f}; echo "USER_KEY=$USER_KEY" >> {f}; echo '{{"decision":"allow"}}'"#,
                    ...
                    "hooks": [
                        {
                            ...
                            "env": {
                                "GBUILD_HOOK_EVENT": "spoofed_event",
                                "GBUILD_HOOK_NAME": "spoofed_name",
                                "GBUILD_SESSION_ID": "spoofed_session",
                                "GBUILD_WORKSPACE_ROOT": "/spoofed/root",
    ...
    assert!(
        ...
        "GBUILD_HOOK_EVENT must reflect the real event, got:\n{captured}"
    );
    assert!(
        !captured.contains("EVENT=spoofed_event"),
        "spoofed GBUILD_HOOK_EVENT must NOT leak through, got:\n{captured}"
    );
    assert!(
        captured.contains(&format!("SESSION={real_session}")),
        "GBUILD_SESSION_ID must reflect the real session, got:\n{captured}"
    );
    assert!(
        !captured.contains("SESSION=spoofed_session"),
        "spoofed GBUILD_SESSION_ID must NOT leak through"
    );
    assert!(
        captured.contains(&format!("ROOT={real_workspace}")),
        "GBUILD_WORKSPACE_ROOT must reflect the real workspace root, got:\n{captured}"
    );
    assert!(
        !captured.contains("ROOT=/spoofed/root"),
        "spoofed GBUILD_WORKSPACE_ROOT must NOT leak through"
    );
    assert!(
...
async fn http_hook_url_env_expansion_end_to_end() {
    ...
    use gbuild_hooks::result::HookRunResult;
    let info = match run {
