...
pub async fn run_command_hook(
    ...
    let debug_payloads = std::env::var("GBUILD_HOOK_DEBUG").is_ok_and(|v| v == "1");
    if debug_payloads {
        tracing::trace!(
            hook_name = %spec.name,
    ...
    let mut cmd = if is_shell_command {
        ...
        #[cfg(not(unix))]
        {
            let inv = gbuild_config::shell::shell_command_argv(&command_str);
    ...
    // plugin) can spoof `GBUILD_HOOK_EVENT`, `GBUILD_HOOK_NAME`, `GBUILD_SESSION_ID`,
    // `GBUILD_WORKSPACE_ROOT`, or `CLAUDE_PROJECT_DIR`, which are the
    ...
    let mut child = match cmd
        ...
        .env("GBUILD_HOOK_EVENT", envelope.hook_event_name.to_string())
        .env("GBUILD_HOOK_NAME", &spec.name)
        .env("GBUILD_SESSION_ID", ctx.session_id)
        .env("GBUILD_WORKSPACE_ROOT", ctx.workspace_root)
        // Compatibility alias for external hooks that read this env name.
        // Same value as `GBUILD_WORKSPACE_ROOT`; native `.gbuild` hooks should use
        // `GBUILD_WORKSPACE_ROOT`.
...
///   hooks),
/// * the gBuild process's own environment (which is inherited by the child),
...
mod tests {
    ...
    fn resolve_command_path_variants() {
        ...
        assert_eq!(
            resolve_command_path(&spec(
                HandlerType::Command,
                Some("bin/check.sh"),
                "/project/.gbuild/hooks"
            )),
            Some(std::path::PathBuf::from(
                "/project/.gbuild/hooks/bin/check.sh"
            ))
        );
        assert_eq!(
    ...
    fn find_unresolved_skips_resolvable_vars() {
        ...
        let v = find_unresolved_env_vars(
            "${GBUILD_HOOK_EVENT}/${CLAUDE_PROJECT_DIR}/${GBUILD_SESSION_ID}/${CLAUDE_PLUGIN_ROOT}/foo",
            &env,
        );
        assert!(
            v.is_empty(),
    ...
    async fn test_tilde_expansion_runs_via_shell() {
        let tmp = tempfile::tempdir().unwrap();
        // Create the script at <tmp>/.gbuild-test-hooks-gb856/tilde-test.sh
        let hook_dir = tmp.path().join(".gbuild-test-hooks-gb856");
        ...
        // Inject HOME via extra_env so `sh -c "~/.gbuild-test-hooks-gb856/..."`
        ...
        let spec = HookSpec {
            ...
            enabled: true,
            command: Some(std::path::PathBuf::from(
                "~/.gbuild-test-hooks-gb856/tilde-test.sh",
            )),
            command_raw: Some("~/.gbuild-test-hooks-gb856/tilde-test.sh".to_string()),
            url: None,
            url_raw: None,
            timeout_ms: 5000,
            source_dir: std::env::temp_dir(),
