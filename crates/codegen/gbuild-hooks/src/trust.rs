...
// (`~/.gbuild/trusted_folders.toml`) is the single authority for whether a repo's
...
/// (`<user_gbuild_home>/trusted-hook-projects`), or `None` when no user gBuild home
/// resolves. Retained only for the one-time migration into folder-trust.
pub fn legacy_trust_file_path() -> Option<PathBuf> {
    Some(gbuild_config::user_gbuild_home()?.join("trusted-hook-projects"))
}
...
pub fn disable_hook(hook_name: &str) -> Result<(), String> {
    let file = disabled_hooks_file_path()
        .ok_or_else(|| "no user gBuild home (set $GBUILD_HOME or $HOME)".to_string())?;
...
/// Returns the path to `$GBUILD_HOME/disabled-hooks`, or `None` when no user gBuild
/// home resolves.
fn disabled_hooks_file_path() -> Option<PathBuf> {
    Some(gbuild_config::user_gbuild_home()?.join("disabled-hooks"))
}
...
mod tests {
    ...
    fn trust_file_in(dir: &Path) -> PathBuf {
        let gbuild_dir = dir.join(".gbuild");
        std::fs::create_dir_all(&gbuild_dir).unwrap();
        gbuild_dir.join("trusted-hook-projects")
    }
