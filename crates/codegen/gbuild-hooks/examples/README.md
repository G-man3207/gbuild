...
Sample hooks for gBuild. Copy to `~/.gbuild/hooks/` to enable globally, or to `<project>/.gbuild/hooks/` for project-scoped hooks (requires `/hooks-trust`).
...
**Install:**
```sh
mkdir -p ~/.gbuild/hooks/bin
cp examples/hooks/safe-shell.json ~/.gbuild/hooks/
cp examples/hooks/bin/safe-shell-guard.sh ~/.gbuild/hooks/bin/
chmod +x ~/.gbuild/hooks/bin/safe-shell-guard.sh
```
...
```sh
mkdir -p ~/.gbuild/hooks/bin
cp examples/hooks/no-recursive-grep.json ~/.gbuild/hooks/
cp examples/hooks/bin/no-recursive-grep-guard.py ~/.gbuild/hooks/bin/
chmod +x ~/.gbuild/hooks/bin/no-recursive-grep-guard.py
```
...
Appends session metadata to `~/.gbuild/session-audit.log` — event, session ID, cwd, timestamp.

**Install:**
```sh
mkdir -p ~/.gbuild/hooks/bin
cp examples/hooks/session-log.json ~/.gbuild/hooks/
cp examples/hooks/bin/session-log.sh ~/.gbuild/hooks/bin/
chmod +x ~/.gbuild/hooks/bin/session-log.sh
```
...
Logs all tool calls to `~/.gbuild/tool-activity.log` — tool name, event type, effective tool name, backgrounded status.

**Install:**
```sh
mkdir -p ~/.gbuild/hooks/bin
cp examples/hooks/tool-logger.json ~/.gbuild/hooks/
cp examples/hooks/bin/tool-logger.sh ~/.gbuild/hooks/bin/
chmod +x ~/.gbuild/hooks/bin/tool-logger.sh
```
...
```sh
mkdir -p ~/.gbuild/hooks/bin
cp examples/hooks/stop-verify.json ~/.gbuild/hooks/
cp examples/hooks/bin/stop-verify.sh ~/.gbuild/hooks/bin/
chmod +x ~/.gbuild/hooks/bin/stop-verify.sh
```
...
- **Event names:** `SessionStart`, `PreToolUse`, `PostToolUse`, `Stop`, `SubagentStop`, `SessionEnd` (see the [user guide](../../gbuild-pager/docs/user-guide/10-hooks.md) for the full set)
- **Matcher:** regex on tool name. Claude names like `Bash`, `Read`, `Edit` are auto-expanded to also match gBuild names (`run_terminal_cmd`, `read_file`, `search_replace`)
...
Remove the JSON file from `~/.gbuild/hooks/`. The hook stops running on the next session.
