//! Voice input for gBuild CLI: an xAI streaming STT client and the
...
mod intercept_tests {
    ...
    fn capture_subcommand_matches_only_argv1() {
        assert!(is_capture_subcommand(&argv(&["gbuild", "__mic-capture"])));
        ...
        assert!(!is_capture_subcommand(&argv(&["gbuild"])));
        assert!(!is_capture_subcommand(&argv(&["gbuild", "chat"])));
        assert!(!is_capture_subcommand(&argv(&[
            "gbuild",
            "chat",
            "__mic-capture"
