...
//! run) — and can be forced with `GBUILD_VOICE_CAPTURE=inprocess`.
...
/// Env escape hatch: `GBUILD_VOICE_CAPTURE=inprocess` forces the legacy
/// in-process cpal backend (accepting its permanent footprint cost).
const CAPTURE_BACKEND_ENV: &str = "GBUILD_VOICE_CAPTURE";
