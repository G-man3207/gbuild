...
//! ```bash
//! export XAI_API_KEY=...
//! cargo run -p gbuild-voice --bin voice-probe -- --seconds 5
//! ```
...
fn main() -> anyhow::Result<()> {
    ...
    if let Some(code) = gbuild_voice::maybe_run_capture_subprocess() {
...
async fn run() -> anyhow::Result<()> {
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "info,gbuild_voice=debug".into()),
        )
        .init();
    ...
    if args.mic_only {
        ...
        {
            let (bytes, chunks) =
                gbuild_voice::run_mic_only_probe(config.sample_rate, args.seconds)?;
...
fn load_config(path: Option<&std::path::Path>) -> VoiceConfig {
    ...
    let env_base = std::env::var("GBUILD_XAI_API_BASE_URL").ok();
    ...
    if let Ok(home) = std::env::var("GBUILD_HOME")
...
Environment:
  ...
  RUST_LOG        optional (default info,gbuild_voice=debug)

Reads [voice] from ~/.gbuild/config.toml unless --config PATH is set.
"#
    );
}
