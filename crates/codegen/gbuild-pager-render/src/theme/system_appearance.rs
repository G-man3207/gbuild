...
/// in `config.toml`. When `None`, defaults to `GBuildNight` / `GBuildDay`.
///
...
pub fn to_theme_kind(
    ...
    match appearance {
        SystemAppearance::Light => light_theme.unwrap_or(ThemeKind::GBuildDay),
        SystemAppearance::Dark => dark_theme.unwrap_or(ThemeKind::GBuildNight),
    }
}
