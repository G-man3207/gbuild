...
//! The default theme is GBuildNight (neutral gray base with TokyoNight accents).
//!
//! ## Color support
//!
//! GBuildNight is defined in `Color::Rgb` (truecolor). At startup,
...
impl ThemeKind {
    ...
    pub fn available() -> &'static [ThemeKind] {
        ...
        const NO_TRUECOLOR: &[ThemeKind] = &[ThemeKind::GBuildNight, ThemeKind::GBuildDay];
    ...
    pub fn display_name(self) -> &'static str {
        match self {
            Self::GBuildNight => "gbuild_night",
            Self::TokyoNight => "tokyonight",
            Self::GBuildDay => "gbuild_day",
    ...
    /// when quantized to 256 or 16 colors. GBuildNight uses neutral grays
    /// that survive quantization cleanly.
    pub fn requires_truecolor(self) -> bool {
        match self {
            Self::GBuildNight => false,
            Self::TokyoNight => true,
            Self::GBuildDay => false,
    ...
    pub fn from_name(name: &str) -> Option<Self> {
        ...
        match lower.as_str() {
            ...
            "gbuild_night" | "gbuild-night" | "dark" => Some(Self::GBuildNight),
            "tokyonight" | "tokyo-night" | "tokyo" => Some(Self::TokyoNight),
            "gbuild_day" | "gbuild-day" | "light" | "day" => Some(Self::GBuildDay),
...
/// `"gbuild_night"` -> `"gBuild Night"`). Falls back to `value` verbatim.
pub fn display_name_for_canonical(value: &str) -> &str {
    match value {
        "auto" => "Auto",
        "gbuild_night" => "gBuild Night",
        "gbuild_day" => "gBuild Day",
...
impl Theme {
    ...
    ///
    /// Reads the active theme kind (loaded from `~/.gbuild/config.toml` on
    ...
    pub fn current() -> Self {
        ...
        let base = match cache::current_kind() {
            ThemeKind::GBuildNight => Self::gbuild_night(),
            ThemeKind::TokyoNight => Self::tokyonight(),
            ThemeKind::GBuildDay => Self::gbuild_day(),
            ...
            // if reached, fall back to GBuildNight.
            ThemeKind::Auto => Self::gbuild_night(),
        };
        ...
        //      (e.g., `TERM=xterm`, `TERM=ansi`, or `GBUILD_FORCE_COLOR_LEVEL=basic`),
    ...
    ///    on gbuild_night, erasing every state signal). We pin each
    ...
    /// theme (e.g., GBuildNight on a white terminal would show white
    ...
    fn ansi16_chrome_overrides(self, dark: bool) -> Self {
        ...
        Self {
            ...
            // GBuildNight bg field quantizes to Color::Black and these
            // become invisible.
...
pub fn apply_cursor_color() {
    ...
    gbuild_shared::stderr::with_locked_stderr(|stderr| {
...
mod tests {
    ...
    fn is_auto_returns_false_for_concrete_variants() {
        assert!(!ThemeKind::GBuildNight.is_auto());
        assert!(!ThemeKind::GBuildDay.is_auto());
    ...
    fn ansi16_overrides_light_inverts_high_contrast_and_elevated_bg() {
        ...
        use ratatui::style::Color;
        let t = Theme::gbuild_day().ansi16_chrome_overrides(false);
    ...
    fn ansi16_overrides_state_accents_pin_to_polarity_aware_hue() {
        ...
        use ratatui::style::Color;
        let t_dark = Theme::gbuild_night().ansi16_chrome_overrides(true);
        ...
        let t_light = Theme::gbuild_day().ansi16_chrome_overrides(false);
    ...
    fn ansi16_overrides_canvas_matching_surfaces_use_theme_polarity() {
        ...
        // the theme disagrees with the terminal profile (e.g. GBuildNight
    ...
    fn scrollbar_thumb_contrasts_with_track_in_all_themes() {
        ...
        for &kind in ThemeKind::ALL {
            let theme = match kind {
                ThemeKind::GBuildNight => Theme::gbuild_night(),
                ThemeKind::GBuildDay => Theme::gbuild_day(),
    ...
    #[test]
    fn ansi16_quantize_without_override_collapses_gbuild_night_backgrounds() {
        // Regression-ratchet for the override gate in `Theme::current`:
        // naive `Basic` quantization maps every dark GBuildNight bg field
        ...
        use ratatui::style::Color;
        let q = Theme::gbuild_night().quantized(color_support::ColorLevel::Basic);
        for (name, color) in [
            ("bg_base", q.bg_base),
            ("bg_light", q.bg_light),
            ("bg_dark", q.bg_dark),
    ...
    fn from_name_concrete_variants_still_work() {
        assert_eq!(
            ThemeKind::from_name("gbuild_night"),
            Some(ThemeKind::GBuildNight)
        );
        assert_eq!(ThemeKind::from_name("dark"), Some(ThemeKind::GBuildNight));
        assert_eq!(
            ThemeKind::from_name("gbuild_day"),
            Some(ThemeKind::GBuildDay)
        );
        assert_eq!(ThemeKind::from_name("light"), Some(ThemeKind::GBuildDay));
        assert_eq!(
    ...
    fn from_str_matches_from_name_for_all_canonicals() {
        ...
        let cases = [
            ...
            ("gbuild_night", ThemeKind::GBuildNight),
            ("gbuild-night", ThemeKind::GBuildNight),
            ("dark", ThemeKind::GBuildNight),
            ("tokyonight", ThemeKind::TokyoNight),
            ("tokyo-night", ThemeKind::TokyoNight),
            ("tokyo", ThemeKind::TokyoNight),
            ("gbuild_day", ThemeKind::GBuildDay),
            ("gbuild-day", ThemeKind::GBuildDay),
            ("light", ThemeKind::GBuildDay),
            ("day", ThemeKind::GBuildDay),
