...
//! lookup must be cheaper than re-loading from `~/.gbuild/config.toml`.
...
//! `gbuild_shell::util::config::set_theme()` (and friends), invoked
...
static CURRENT: AtomicU8 = AtomicU8::new(ThemeKind::GBuildNight as u8);
...
/// back to `GBuildNight` if the byte is somehow out of range (which
...
fn theme_kind_from_u8(byte: u8) -> ThemeKind {
    match byte {
        x if x == ThemeKind::GBuildNight as u8 => ThemeKind::GBuildNight,
        x if x == ThemeKind::GBuildDay as u8 => ThemeKind::GBuildDay,
...
/// When `None`, `to_theme_kind()` defaults to `GBuildNight` / `GBuildDay`.
...
///
/// On the first call, reads from `~/.gbuild/config.toml` (via the shell's
...
pub fn set_terminal_native_lock(locked: bool) {
    ...
    gbuild_markdown::set_color_level_cap(if locked {
        gbuild_markdown::ColorLevel::Basic
    } else {
        gbuild_markdown::ColorLevel::TrueColor
    });
    gbuild_markdown::set_polarity_safe_syntax(locked);
}
...
///
/// Precedence:
/// 1. Environment variable (`GBUILD_THEME`)
/// 2. Config file (`[ui].theme`)
/// 3. Default: `GBuildNight`
#[must_use]
...
fn resolve_from_appearance(appearance: Option<system_appearance::SystemAppearance>) -> ThemeKind {
    ...
    appearance
        ...
        .unwrap_or(ThemeKind::GBuildNight)
}
...
/// and the user's dark/light theme mapping. Falls back to `GBuildNight`
...
// -- Disk reads --------------------------------------------------------------
//
// All writes go through `gbuild_shell::util::config::set_theme()` (and
...
fn load_from_disk() -> Option<ThemeKind> {
    let root = gbuild_config::load_effective_config_disk_only().ok()?;
...
fn load_auto_theme_config() -> AutoThemeConfig {
    let Ok(root) = gbuild_config::load_effective_config_disk_only() else {
...
pub fn reset_for_test() {
    ...
    CURRENT.store(ThemeKind::GBuildNight as u8, Ordering::Relaxed);
...
mod tests {
    ...
    fn terminal_native_lock_pins_kind_and_blocks_apply_kind() {
        with_test_env(|| {
            set(ThemeKind::GBuildDay);
            set_terminal_native_lock(true);
            assert!(terminal_native_locked());
            assert_eq!(current_kind(), ThemeKind::GBuildNight, "nominal kind");

            let applied = super::super::Theme::apply_kind(ThemeKind::GBuildDay);
            assert_eq!(applied, ThemeKind::GBuildNight, "apply_kind must no-op");
            assert_eq!(current_kind(), ThemeKind::GBuildNight);
    ...
    fn terminal_native_lock_serves_terminal_default_palette() {
        with_test_env(|| {
            ...
            assert_ne!(
                theme.text_primary,
                super::super::Theme::gbuild_day().text_primary,
                "must not serve the cached (GBuildDay) theme"
            );
        });
    }
    ...
    fn terminal_native_lock_enables_polarity_safe_syntax() {
        with_test_env(|| {
            assert!(!gbuild_markdown::polarity_safe_syntax());
            set_terminal_native_lock(true);
            assert!(
                gbuild_markdown::polarity_safe_syntax(),
    ...
    fn terminal_native_lock_caps_quantize_at_ansi16() {
        ...
        with_test_env(|| {
            ...
            for input in [
                Color::Rgb(0x26, 0x26, 0x26), // gbuild_day text_primary
    ...
    fn resolve_no_osc11_explicit_auto_and_default() {
        with_test_env(|| {
            assert_eq!(
                resolve_from_config(Some(ThemeKind::GBuildDay), false),
                ThemeKind::GBuildDay
            );
            assert!(!is_auto_mode());
            ...
            assert_eq!(resolve_from_config(None, false), ThemeKind::GBuildNight);
        });
    }
    ...
    #[test]
    fn resolve_auto_dark_system_returns_gbuild_night() {
        with_test_env(|| {
            system_appearance::set_mock(Some(system_appearance::SystemAppearance::Dark));
            let result = resolve_auto();
            assert_eq!(result, ThemeKind::GBuildNight);
        });
    }

    #[test]
    fn resolve_auto_light_system_returns_gbuild_day() {
        with_test_env(|| {
            system_appearance::set_mock(Some(system_appearance::SystemAppearance::Light));
            let result = resolve_auto();
            assert_eq!(result, ThemeKind::GBuildDay);
        });
    }

    #[test]
    fn resolve_auto_detection_failure_returns_gbuild_night() {
        with_test_env(|| {
    ...
    #[test]
    fn resolve_from_config_no_config_returns_gbuild_night() {
        with_test_env(|| {
    ...
    fn resolve_from_config_explicit_theme_returns_it() {
        with_test_env(|| {
            let result = resolve_from_config(Some(ThemeKind::GBuildDay), true);
            assert_eq!(result, ThemeKind::GBuildDay);
            assert!(
                !is_auto_mode(),
    ...
    fn set_then_current_kind_round_trips() {
        with_test_env(|| {
            ...
            set(ThemeKind::GBuildDay);
            assert_eq!(current_kind(), ThemeKind::GBuildDay);
        });
    }
