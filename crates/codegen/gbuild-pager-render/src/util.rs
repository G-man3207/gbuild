...
pub use gbuild_config::gbuild_home;

/// Path to `$GBUILD_HOME/pager.toml`.
pub fn pager_toml_path() -> PathBuf {
    gbuild_home().join("pager.toml")
}

/// User-facing label for the user gbuild directory (``~/.gbuild`` or ``$GBUILD_HOME``).
///
/// Derived from resolved [`gbuild_home()`] vs `gbuild_config::default_gbuild_home()`,
/// not from whether `GBUILD_HOME` is set in the environment.
pub fn display_gbuild_home_prefix() -> String {
    display_gbuild_home_prefix_for(&gbuild_home())
}

fn display_gbuild_home_prefix_for(home: &Path) -> String {
    if home == gbuild_config::default_gbuild_home() {
        "~/.gbuild".to_string()
    } else {
        "$GBUILD_HOME".to_string()
    }
}

/// User-facing path under [`gbuild_home()`], e.g. ``~/.gbuild/config.toml``.
pub fn display_user_gbuild_path(relative: impl AsRef<Path>) -> String {
    display_user_gbuild_path_for(&gbuild_home(), relative)
}

fn display_user_gbuild_path_for(home: &Path, relative: impl AsRef<Path>) -> String {
    let rel = relative.as_ref();
    let prefix = display_gbuild_home_prefix_for(home);
...
/// Abbreviate an absolute path for display: prefer [`gbuild_home()`], then `$HOME`.
pub fn abbreviate_path(path: &str) -> Cow<'_, str> {
    let path_buf = Path::new(path);
    let gbuild = gbuild_home();
    if let Ok(rest) = path_buf.strip_prefix(&gbuild) {
        let prefix = display_gbuild_home_prefix();
...
/// True when `path` is under user [`gbuild_home()`] (not project `{cwd}/.gbuild`).
pub fn is_under_user_gbuild_home(path: &Path) -> bool {
    path.starts_with(gbuild_home())
}
...
mod tests {
    ...
    #[test]
    fn display_gbuild_home_prefix_default_install() {
        if std::env::var("GBUILD_HOME").is_ok() {
            return;
        }
        assert_eq!(display_gbuild_home_prefix(), "~/.gbuild");
    }

    #[test]
    fn display_user_gbuild_path_joins_relative() {
        let path = display_user_gbuild_path("config.toml");
        assert!(path.ends_with("/config.toml") || path.ends_with("\\config.toml"));
        assert!(path.contains(".gbuild") || path.contains("$GBUILD_HOME"));
    }

    #[test]
    fn display_user_gbuild_path_for_custom_home_uses_override_label() {
        let custom = std::env::temp_dir().join("gbuild-home-display-regression");
        assert_eq!(
            display_user_gbuild_path_for(&custom, "config.toml"),
            "$GBUILD_HOME/config.toml"
        );
        assert_eq!(
            display_user_gbuild_path_for(&custom, "sandbox.toml"),
            "$GBUILD_HOME/sandbox.toml"
        );
    }

    #[test]
    fn abbreviate_path_uses_home_when_under_default_gbuild() {
        if let Ok(home) = std::env::var("HOME") {
            if home.is_empty() {
                return;
            }
            let full = format!("{home}/.gbuild/memory/MEMORY.md");
