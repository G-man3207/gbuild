...
//! Dark themes (GBuildNight, TokyoNight) share `gbuild-night.tmTheme`;
//! GBuildDay uses `gbuild-day.tmTheme` with deepened colors for light backgrounds.
//!
...
//! `current_kind()` is a nominal `GBuildNight` (so leftover kind-keyed paths
...
pub use gbuild_markdown::Syntect;
...
static SYNTECT_GBUILD_NIGHT: OnceLock<Syntect> = OnceLock::new();
static SYNTECT_TOKYONIGHT: OnceLock<Syntect> = OnceLock::new();
static SYNTECT_GBUILD_DAY: OnceLock<Syntect> = OnceLock::new();
...
/// reports a nominal `GBuildNight`, so this returns the night theme. Token
...
pub fn get_syntect() -> &'static Syntect {
    match crate::theme::Theme::current_kind() {
        ...
        | ThemeKind::Auto => SYNTECT_GBUILD_NIGHT
            .get_or_init(|| Syntect::new(include_bytes!("../assets/gbuild-night.tmTheme"))),
        ThemeKind::TokyoNight => SYNTECT_TOKYONIGHT
            .get_or_init(|| Syntect::new(include_bytes!("../assets/tokyo-night.tmTheme"))),
        ThemeKind::GBuildDay => SYNTECT_GBUILD_DAY
            .get_or_init(|| Syntect::new(include_bytes!("../assets/gbuild-day.tmTheme"))),
    }
}
