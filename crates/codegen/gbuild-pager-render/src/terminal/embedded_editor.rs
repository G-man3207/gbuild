//! Detects whether gBuild is running inside an editor's embedded `:terminal`
...
/// Which embedded editor `:terminal` gBuild is running inside.
...
/// `HOST_TERMINAL_ENV_VARS` in `gbuild-pager-pty-harness/src/pty.rs`
/// (test-env hygiene).
