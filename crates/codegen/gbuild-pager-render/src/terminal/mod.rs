...
pub struct TerminalContext {
    ...
    /// Which embedded editor `:terminal` gBuild is running inside, if any.
...
impl TerminalContext {
    ...
    /// keyed off this only fires when a FocusGained actually reaches gBuild, which
    ...
    pub fn telemetry_snapshot(&self) -> gbuild_telemetry::events::TerminalTelemetry {
        ...
        gbuild_telemetry::events::TerminalTelemetry {
            ...
            clipboard_native_tool: gbuild_shared::clipboard::native_tool_name().to_owned(),
            clipboard_data_control: crate::clipboard::wayland_data_control_label().to_owned(),
        }
    }

    /// Extract terminal info for feedback submissions.
    pub fn feedback_info(&self) -> gbuild_shared::session::FeedbackTerminalInfo {
        use gbuild_shared::session::FeedbackTerminalInfo;
        ...
        FeedbackTerminalInfo {
            ...
            clipboard_native_tool: Some(gbuild_shared::clipboard::native_tool_name().to_owned()),
...
fn detect_terminal_context() -> TerminalContext {
    ...
    // first client, not the current one). Revisit when `gbuild ssh` can
    // forward env vars.
    let mut ctx = build_terminal_context_from_env(&env);
...
/// `gbuild-pager-pty-harness/src/pty.rs` (test-env hygiene — the PTY
...
///
/// Parsed from `[terminal] alt_screen` in `~/.gbuild/pager.toml` and
