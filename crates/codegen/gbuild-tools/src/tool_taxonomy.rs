...
///   share it (gBuild `read_file` → `"Read"`).
...
mod tests {
    ...
    fn identity(kind: ToolKind) -> ToolIdentity {
        ToolIdentity {
            tool_kind: kind,
            namespace: ToolNamespace::GBuild,
    ...
    fn namespace_round_trips_snake_case_with_pascal_aliases() {
        ...
        fn wire_and_pascal(ns: ToolNamespace) -> (&'static str, &'static str) {
            match ns {
                ToolNamespace::GBuild => ("gbuild", "GBuild"),
                ToolNamespace::GBuildConcise => ("gbuild_concise", "GBuildConcise"),
                ToolNamespace::GBuildHashline => ("gbuild_hashline", "GBuildHashline"),
