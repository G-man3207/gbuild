...
//! after every tool call to check for SKILL.md files in `.gbuild/skills/`,
...
/// Directories that contain skill definitions (`.gbuild/skills/`, `.agents/skills/`,
/// `.claude/skills/`, `.cursor/skills/`). Shared between startup skill discovery
/// and runtime `SkillDiscoveryReminder`.
pub const SKILL_CONFIG_DIRS: &[&str] = &[".gbuild", ".agents", ".claude", ".cursor"];
...
impl SkillDiscoveryReminder {
    ...
    /// (`.gbuild/skills/`, `.agents/skills/`, or `.claude/skills/`).
...
impl Reminder for SkillDiscoveryReminder {
    ...
    async fn collect_reminders(
        ...
        // discovery cannot find these because it looks for `.gbuild/skills/`
        // sub-directories in *ancestor* dirs, and user-scope skills
        // (~/.gbuild/) are outside the git root so the walk breaks early.
