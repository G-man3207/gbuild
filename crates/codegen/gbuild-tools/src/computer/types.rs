...
/// (`gbuild-shell::extensions::task::KillTaskResponse`) and deserialized
/// by clients (gbuild-pager), so it derives both serde directions.
...
///
/// Implemented by:
/// - `LocalTerminalBackend` (in gbuild-tools, spawns processes)
/// - `AcpTerminalBackend` (in gbuild-shell, calls ACP protocol)
#[async_trait::async_trait]
