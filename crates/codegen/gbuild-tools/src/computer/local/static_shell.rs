...
pub use gbuild_config::shell::UnixShellKind;

const INIT_MARKER: &str = "__GBUILD_STATIC_SHELL_MARKER__";
...
fn shell_binary(shell: UnixShellKind) -> &'static str {
    gbuild_config::shell::unix_shell_path(shell)
}
...
impl StaticShellSnapshot {
    ...
    pub async fn init(cwd: &Path) -> Self {
        let shell = gbuild_config::shell::detect_unix_shell_kind();
...
mod tests {
    ...
    async fn replays_aliases_and_functions() {
        ...
        let output = run_static(
            "alias gbuild_alias_probe='echo ALIAS_OK'\ngbuild_fn_probe() { echo FN_OK; }\n",
            "gbuild_alias_probe && gbuild_fn_probe",
        )
        .await;
