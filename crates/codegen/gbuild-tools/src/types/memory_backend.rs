...
//!
//! `MemoryBackend` is defined in `gbuild-tools` to keep the tool crate
...
///
/// Use `tracing::info!(target: MEMORY_LOG_TARGET, ...)` in `gbuild-tools`.
/// Mirrors `gbuild_shell::session::memory_log::TARGET`.
