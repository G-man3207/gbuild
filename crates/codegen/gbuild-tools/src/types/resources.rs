...
/// `"namespace.Name"` (e.g., `"gbuild.ReadFile"`). The ID is used as
...
pub trait ResourceType: Any + 'static {
    /// Unique identifier, e.g. `"gbuild.ReadFile"`.
    const ID: &'static str;
...
///
/// ```ignore
/// register_resource!("gbuild", "ReadFile", ReadHistory);
/// ```
///
/// This generates:
/// ```ignore
/// impl ResourceType for ReadHistory {
///     const ID: &'static str = "gbuild.ReadFile";
/// }
/// ```
#[macro_export]
...
struct ResourceEntry {
    type_id: TypeId,
    /// The `ResourceType::ID` string (e.g., `"gbuild.ReadFile"`).
    id: String,
...
/// tool falls back to `Cwd/.gbuild/plan.md`.
#[derive(Debug, Clone)]
...
pub const PLAN_FILE_RELATIVE_PATH: &str = ".gbuild/plan.md";
...
/// [`Cwd`]`/.gbuild/plan.md`, else the bare relative `.gbuild/plan.md`.
...
/// `GBUILD_SCHEDULER_BACKGROUND_LOOPS` env var, or the
...
register_resource!("gbuild", "WebCitation", WebCitationCounter);
...
/// depending on `gbuild-mcp` directly.  Follows the same pattern as
...
mod tests {
    ...
    register_resource!("gbuild", "Edit", EditConfig);
    #[derive(Debug, Clone, Default, PartialEq, Eq, serde::Serialize, serde::Deserialize)]
    struct ReadHistory {
    ...
    register_resource!("gbuild", "Todo", TodoData);
    #[test]
    ...
    fn ephemeral_types_silently_skipped_during_serialization() {
        ...
        assert!(state.get("gbuild.ReadFile").is_some());
    ...
    fn load_from_populates_registered_types() {
        ...
        state_map.insert(
            "gbuild.ReadFile".to_string(),
            serde_json::json!({"files_read": ["loaded.rs"]}),
        );
        let mut params_map = HashMap::new();
        params_map.insert(
            "gbuild.Edit".to_string(),
    ...
    fn get_json_returns_registered_value() {
        ...
        let val = res.get_json("params", "gbuild.Edit").unwrap();
    ...
    fn get_json_returns_none_for_missing_value() {
        let mut res = Resources::new();
        res.register_params::<EditConfig>();
        assert!(res.get_json("params", "gbuild.Edit").is_none());
    }
    #[test]
