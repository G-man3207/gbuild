...
use crate::implementations::gbuild::todo::{TodoItem, TodoState};
...
/// protocol boundary in `gbuild-shell`.
#[derive(Debug, Clone, Serialize, Deserialize, schemars::JsonSchema)]
...
pub struct BashOutput {
    ...
    /// Set by the gbuild `run_terminal_cmd` implementation when the
    /// command was detected as a bare `echo "<msg>"` (or close variant:
    /// echo -n, echo -e, simple printf for literal output, etc.).
    ///
    /// Used for:
    /// - Telemetry / statistics on this pattern for the gbuild backend.
...
pub struct WebFetchContent {
    ...
    pub bytes: usize,
    /// Internal path to the complete converted body when GBuild persisted overflow.
    #[serde(skip)]
    #[schemars(skip)]
...
pub enum ToolOutput {
    ...
    Monitor(crate::implementations::gbuild::monitor::types::MonitorOutput),
    SchedulerCreate(crate::implementations::gbuild::scheduler::create::SchedulerCreateOutput),
    SchedulerDelete(crate::implementations::gbuild::scheduler::delete::SchedulerDeleteOutput),
    SchedulerList(crate::implementations::gbuild::scheduler::list::SchedulerListOutput),
    UpdateGoal(crate::implementations::gbuild::update_goal::UpdateGoalOutput),
    Workflow(crate::implementations::gbuild::workflow::WorkflowToolOutput),
...
mod tests {
    use super::*;
    use crate::implementations::gbuild::todo::{TodoPriority, TodoStatus};
    use serde_json::json;
    ...
    fn subagent_completed_prompt_format_with_worktree() {
        let output = ToolOutput::SubagentCompleted(SubagentCompletedOutput {
            ...
            turns: 1,
            duration_ms: 2000,
            worktree_path: Some("/tmp/gbuild-worktree/wt-agent".into()),
            persona: None,
        ...
        let rendered = output.to_prompt_format();
        assert!(
            rendered.contains("<worktree_path>/tmp/gbuild-worktree/wt-agent</worktree_path>"),
