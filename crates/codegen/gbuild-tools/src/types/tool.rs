...
///
/// Serializes to snake_case (`gbuild`, `mcp`, ...) for the canonical tool
/// `_meta` wire contract. The `Display` impl remains PascalCase for qualified
/// IDs such as `"GBuild:read_file"`; only the serde form goes on the wire.
#[derive(
    Debug,
    Clone,
    Copy,
...
pub enum ToolNamespace {
    #[serde(rename = "gbuild")]
    GBuild,
    #[serde(rename = "gbuild_concise")]
    GBuildConcise,
    #[serde(rename = "gbuild_hashline")]
    GBuildHashline,
    #[serde(alias = "Codex")]
    Codex,
