...
//!
//! `ToolSearchIndex` is defined in `gbuild-tools` to keep the tool crate
//! backend-agnostic. The concrete implementation lives in `gbuild-shell`
...
/// in `gbuild-shell` maps `mcp_initialized` to `is_ready`.
...
/// serialized), injected by `gbuild-shell` after MCP initialization.
#[derive(Clone)]
