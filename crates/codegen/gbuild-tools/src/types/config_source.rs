...
pub enum ConfigSource {
    ...
    Builtin,
    /// Bundled skill shipped with the binary (extracted to ~/.gbuild/skills/
    /// or injected via bundled skill dirs).
    Bundled { path: PathBuf },
    /// Server-synced (e.g. ~/.gbuild/server-skills from the skill store).
    Server { path: PathBuf },
    /// Project-scoped: cwd/.gbuild/ or cwd/.claude/.
    Project { path: PathBuf },
    /// User-scoped: ~/.gbuild/ or ~/.claude/.
    User { path: PathBuf },
