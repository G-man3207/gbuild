...
impl<'a> SkillEntry<'a> {
    ...
    // ── Budgeted XML rendering (gBuild harness) ─────────────
...
///
/// `"/path/.gbuild/skills/my-skill/SKILL.md"` -> `"/path/.gbuild/skills/"`
...
/// Used by the budgeted (gBuild) XML rendering path.
...
pub enum XmlRenderMode {
    ...
    Verbatim,
    /// gBuild harness: budget-capped three-tier rendering with full
    /// XML entity escaping.
    Budgeted {
        ...
        skills
            ...
            .filter(|s| {
                s.enabled
                    ...
                    // Compat mode renders all skills verbatim; only the gBuild
                    // path drops description-less plugin skills.
...
mod tests {
    ...
    fn verbatim_bypasses_is_listable_filter() {
        // gBuild drops a description-less skill; compat mode renders all.
    ...
    // ── budgeted mode: gBuild harness (budgeted XML) ───────────
