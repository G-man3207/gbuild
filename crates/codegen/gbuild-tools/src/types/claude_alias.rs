//! Canonical external-settings tool name ↔ gBuild tool correspondence: one table
//! replacing two that drifted apart.
//!
//! Two consumers read it independently. The hook matcher (`gbuild-hooks`) needs the
//! gBuild tool **names** an external settings term maps to (and the reverse, for regex
//! matchers); the agent builder (`gbuild-agent`) needs the [`ToolKind`] a `tools:`
...
//!
//! The `gbuild` names are test-checked against the live registry.

use super::tool::ToolKind;
use ToolKind::*;

/// One Claude tool's correspondence to gBuild, read via the accessor functions below.
struct ClaudeTool {
    claude: &'static str,
    /// gBuild [`ToolKind`] for allowlist resolution; `None` for names that are matchable
    /// (spawn/plan-mode directives) but must not resolve an allowlist.
    kind: Option<ToolKind>,
    /// gBuild tool names this Claude tool maps to (empty when there is no direct
    /// gBuild tool — the entry then only contributes a `kind`).
    gbuild: &'static [&'static str],
}

/// Row that resolves an allowlist (carries a [`ToolKind`]) — the common case.
const fn k(claude: &'static str, kind: ToolKind, gbuild: &'static [&'static str]) -> ClaudeTool {
    ClaudeTool {
        claude,
        kind: Some(kind),
        gbuild,
...
const fn match_only(claude: &'static str, gbuild: &'static [&'static str]) -> ClaudeTool {
    ClaudeTool {
        claude,
        kind: None,
        gbuild,
...
const CLAUDE_TOOLS: &[ClaudeTool] = &[
    ...
    k("Skill",           Read,                 &["skill"]),                           // matcher: opencode's `skill` tool; allowlist Read (gBuild reads SKILL.md)
...
/// The gBuild [`ToolKind`] a Claude allowlist entry resolves to, if any.
...
/// The gBuild tool names a Claude matcher term fires on.
pub fn gbuild_names_for(claude: &str) -> impl Iterator<Item = &'static str> {
    CLAUDE_TOOLS
        .iter()
        .find(|t| t.claude == claude)
        .map(|t| t.gbuild)
...
/// The Claude names that map to `gbuild_name` (reverse lookup, for regex matchers).
pub fn claude_names_for(gbuild_name: &str) -> impl Iterator<Item = &'static str> + '_ {
    CLAUDE_TOOLS
        .iter()
        .filter(move |t| t.gbuild.contains(&gbuild_name))
        .map(|t| t.claude)
}

/// Every distinct gBuild name the table references, for the `gbuild-agent` drift-check
/// test that asserts each is a real client tool name.
pub fn gbuild_names() -> impl Iterator<Item = &'static str> {
    let mut seen = std::collections::HashSet::new();
    CLAUDE_TOOLS
        .iter()
        .flat_map(|t| t.gbuild.iter().copied())
...
mod tests {
    ...
    fn every_row_contributes() {
        // A row with neither a kind nor a gBuild name is dead weight (and signals a typo).
        for t in CLAUDE_TOOLS {
            assert!(
                t.kind.is_some() || !t.gbuild.is_empty(),
                "dead row: {}",
                t.claude
            );
        }
