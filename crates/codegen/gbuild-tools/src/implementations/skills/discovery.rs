...
///
/// `skills` is the standard layout (`.gbuild/skills/`, `.claude/skills/`,
/// `.cursor/skills/`). The product-specific `skills-cursor/` layout is no
/// longer scanned — it pulled vendor default skills into gBuild sessions.
...
/// name (e.g. `~/.gbuild/skills/shell`) is NOT dropped — only skills physically
...
pub fn parse_skill_files(skill_files: Vec<(PathBuf, SkillScope)>) -> Vec<SkillInfo> {
    ...
    // per-vendor toggle, so vendor builtins never leak into gBuild.
...
/// `cwd` (exclusive). At each directory, checks for `.gbuild/skills/`,
...
/// visits known config roots (`.gbuild`, `.agents`, `.claude`, …); those are
...
pub fn discover_skills_for_paths(
    ...
    // `.gbuild` and `.agents` are always scanned; `.claude` is gated on the
    // claude-vendor skills cell. (`.cursor` is excluded here by design — see fn docs.)
    let mut config_dir_names: Vec<&str> = vec![".gbuild", ".agents"];
...
";
    ...
    fn is_vendor_default_skill_spares_user_skill_outside_vendor_dir() {
        // A user's own "shell" skill in ~/.gbuild is NOT a vendor builtin.
        assert!(!is_vendor_default_skill(
            "/home/u/.gbuild/skills/shell/SKILL.md",
            "shell"
        ));
    }
    ...
    fn parse_skill_files_drops_denylisted_cursor_skill() {
        ...
        .unwrap();
        // Same name under /.gbuild/ → kept (user content).
        let gbuild_shell = tmp.path().join(".gbuild").join("skills").join("shell");
        std::fs::create_dir_all(&gbuild_shell).unwrap();
        std::fs::write(
            gbuild_shell.join("SKILL.md"),
        ...
        let skills = parse_skill_files(vec![
            ...
            (gbuild_shell.join("SKILL.md"), SkillScope::User),
        ]);
        assert_eq!(skills.len(), 1, "cursor builtin must be dropped");
        assert!(skills[0].path.contains("/.gbuild/"));
    }
    ...
    fn discover_skills_for_paths_gates_claude_dir() {
        ...
        // A .claude skill and a .gbuild skill in an intermediate dir.
        ...
        .unwrap();
        let gbuild_skill = sub.join(".gbuild").join("skills").join("gbuild-dyn");
        std::fs::create_dir_all(&gbuild_skill).unwrap();
        std::fs::write(
            gbuild_skill.join("SKILL.md"),
            "---\nname: gbuild-dyn\n---\n",
        )
        .unwrap();
        ...
        assert!(
            names_on.contains(&"gbuild-dyn"),
            "gbuild-dyn missing: {names_on:?}"
        );
        assert!(
        ...
        // claude.skills OFF → only gbuild-dyn discovered.
        ...
        assert!(
            names_off.contains(&"gbuild-dyn"),
            "gbuild-dyn missing: {names_off:?}"
        );
        assert!(
