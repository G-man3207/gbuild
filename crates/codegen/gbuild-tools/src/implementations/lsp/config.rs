//! LSP server configuration from `.gbuild/lsp.json`.
...
/// This is the canonical merge function — both session startup and `gbuild inspect` call it.
...
pub fn load_servers_with_plugins_sourced(
    ...
    let user_path = crate::util::gbuild_home::gbuild_home().join("lsp.json");
    let project_path = cwd.join(".gbuild").join("lsp.json");
