...
/// params are injected, `GBUILD_ASK_USER_QUESTION_TIMEOUT_SECS` (positive
/// integer seconds) still overrides this default directly —
/// e.g. `GBUILD_ASK_USER_QUESTION_TIMEOUT_SECS=8` for tests / TUI repro.
...
pub const RESPONSE_TIMEOUT_ENV: &str = "GBUILD_ASK_USER_QUESTION_TIMEOUT_SECS";
...
crate::register_resource!("gbuild", "AskUserQuestion", AskUserQuestionParams);
...
pub struct QuestionOption {
    ...
    /// Opaque id; hidden from the model. gBuild callers leave it `None`.
...
/// session-owned coordinator (in gbuild-shell), which performs an ACP
