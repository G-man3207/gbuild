...
//!
//! - **`gbuild-tools`** — tool blocks on a oneshot, formats the result.
//! - **`gbuild-shell`** — coordinator receives requests over mpsc, calls the
//!   client via ACP `ext_method`, sends results back over the oneshot.
//! - **`gbuild-pager`** — handles the `ExtMethod`, renders UI, returns a
//!   typed response.
//!
//! All three crates import these types from `gbuild-tools`.
...
register_resource!("gbuild", "UserQuestionSender", UserQuestionSender);
