...
//! `LspTool` (Tool trait impl) under the `GBuild` namespace.
            ...
            handle = res
                ...
                .ok_or_else(|| {
                    xai_tool_runtime::ToolError::custom(
                        "process_manager",
                        "LSP tool is unavailable. Configure ~/.gbuild/lsp.json or <cwd>/.gbuild/lsp.json and ensure the language server can start.",
                    )
                })?
                .clone();
        }
