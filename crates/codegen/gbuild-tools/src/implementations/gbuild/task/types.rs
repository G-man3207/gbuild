...
pub struct SubagentRuntimeOverrides {
    ...
    /// `"gbuild-plan"`) whose `AgentDefinition` decides the child's harness
...
/// defense-in-depth check in `gbuild-shell`'s subagent coordinator.
...
fn is_background_capable_bash_tool(tc: &crate::registry::types::ToolConfig) -> bool {
    match tc.id.as_str() {
        "GBuild:run_terminal_cmd" | "GBuildConcise:run_terminal_cmd" => tc
            .params
            .as_ref()
...
register_resource!("gbuild", "SubagentEventSender", SubagentEventSender);
...
pub struct MonitorEventNotification {
    ...
    /// events inside another session's turn. `None` for legacy / non-gBuild
...
register_resource!("gbuild", "MonitorEventBuffer", MonitorEventBuffer);
...
register_resource!("gbuild", "SubagentDepthCounter", SubagentDepthCounter);
...
register_resource!("gbuild", "TaskModelValidator", TaskModelValidator);
...
register_resource!("gbuild", "SessionIdResource", SessionIdResource);
...
register_resource!("gbuild", "SubagentForegroundWait", SubagentForegroundWait);

/// Carries the current parent prompt/turn ID for TaskTool subagent scoping.
///
/// Set by gbuild-shell immediately before a prompt turn begins executing so
/// subagents launched during that turn can be cancelled together if the user
/// aborts the turn.
#[derive(Debug, Clone)]
pub struct CurrentPromptIdResource(pub String);

register_resource!("gbuild", "CurrentPromptIdResource", CurrentPromptIdResource);

/// True while a `/goal` loop is active. Set by gbuild-shell at turn start.
...
register_resource!("gbuild", "GoalLoopActive", GoalLoopActive);
...
mod tests {
    ...
    fn read_only_filter_prunes_orphaned_background_task_tools() {
        let mut config = ToolServerConfig {
            tools: vec![
                tc("GBuild:run_terminal_cmd", ToolKind::Execute),
                tc("GBuild:read_file", ToolKind::Read),
                tc("GBuild:list_dir", ToolKind::List),
                tc("GBuild:grep", ToolKind::Search),
                tc("GBuild:kill_task", ToolKind::KillTaskAction),
                tc("GBuild:get_task_output", ToolKind::BackgroundTaskAction),
            ],
        ...
        assert_eq!(
            ids,
            vec!["GBuild:read_file", "GBuild:list_dir", "GBuild:grep",]
        );
    }
    ...
    fn read_write_filter_keeps_background_capable_bash_when_explicitly_enabled() {
        let mut bash = tc("GBuild:run_terminal_cmd", ToolKind::Execute);
        bash.params = Some(
