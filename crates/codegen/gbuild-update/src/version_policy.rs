...
use crate::version::get_installed_gbuild_version;
use gbuild_shell::util::config::VersionPolicy;
use semver::Version;
use tracing::warn;
...
pub(crate) enum VersionPolicyError {
    #[error(
        "Cannot install gBuild {target}: the minimum allowed version is {minimum}. \
         Run `gbuild update` to install the latest allowed version."
    )]
