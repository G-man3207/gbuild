...
use gbuild_shell::env::GBuildEnvironment;
use gbuild_shell::util::gbuild_home::gbuild_home;
...
///
/// Constructed once from `GBuildEnvironment` at startup and threaded through the
/// update call chain so that `auto_update` and `version` never need to know
/// about the `GBuildEnvironment` enum directly.
#[derive(Debug, Clone)]
pub struct UpdateConfig {
    /// Chat API proxy base URL (versioned `https://cli-chat-proxy.grok.com/v1` endpoint).
    pub proxy_base_url: String,
    /// Auth scope key for `~/.gbuild/auth.json`.
    pub auth_scope: String,
    /// Enterprise deployment key (GBUILD_DEPLOYMENT_KEY).
...
impl UpdateConfig {
    pub fn from_environment(env: &GBuildEnvironment) -> Self {
        Self {
            proxy_base_url: env.cli_chat_proxy_base_url(),
            auth_scope: gbuild_shell::auth::GrokComConfig::default().auth_scope(),
            deployment_key: None,
            alpha_test_key: None,
...
pub async fn write_version_cache(version: &str, stable_version: Option<&str>) {
    let version_path = gbuild_home().join("version.json");
    let now = time::OffsetDateTime::now_utc();
    let json = GBuildVersion::new(
        version.to_string(),
...
pub async fn is_version_cache_fresh() -> bool {
    ...
    if let Ok(version_str) = fs::read_to_string(&version_path).await
        && let Ok(version) = serde_json::from_str::<GBuildVersion>(&version_str)
...
pub use gbuild_version::installed as get_installed_gbuild_version;

/// Version of the managed gbuild binary currently on disk, read from the
/// `~/.gbuild/bin/gbuild` symlink target (`../downloads/gbuild-<version>-<platform>`)
/// without exec'ing anything.
///
/// Concurrent updaters (TUI background download, leader hourly checker,
/// explicit `gbuild update`) decide staleness from this instead of their own
...
/// link whose target binary was deleted (e.g. manual `~/.gbuild/downloads`
...
pub fn installed_on_disk_version() -> Option<String> {
    #[cfg(unix)]
    {
        let app = gbuild_shell::util::gbuild_home::gbuild_application();
        ...
        version_from_versioned_binary_name(target.file_name()?.to_str()?, "gbuild")
    }
    #[cfg(not(unix))]
    {
        None
...
///
/// Handles the internal layout (`gbuild-0.1.150-macos-aarch64`, including
/// pre-releases: `gbuild-0.1.150-alpha.1-linux-x86_64` → `0.1.150-alpha.1`)
/// and the npm layout without a platform suffix (`gbuild-0.1.150`,
/// `gbuild-0.1.150-alpha.1`): everything between the `{bin_prefix}-` prefix
/// and the first platform-OS component is the version, validated as semver
/// so unknown layouts (`gbuild-latest`, `gbuild-pager-*` when `bin_prefix` is
/// `gbuild`) return `None` instead of garbage.
///
...
/// Read the cached stable version from `~/.gbuild/version.json` (sync, for display).
///
...
pub fn cached_stable_version() -> Option<String> {
    ...
    let gv: GBuildVersion = serde_json::from_str(&content).ok()?;
    gv.stable_version
}
...
pub fn channel_name() -> Option<&'static str> {
    ...
    *NAME.get_or_init(|| {
        ...
        derive_channel(gbuild_version::VERSION, &stable)
    })
}
...
/// `~/.gbuild/version.json` (written by the auto-updater):
...
pub fn channel_label() -> &'static str {
    ...
    LABEL.get_or_init(|| {
        ...
        match derive_channel(gbuild_version::VERSION, &stable) {
            Some("alpha") => " [alpha]",
            Some(_) => " [stable]",
            None => "",
        }
...
mod tests {
    ...
    fn test_is_fresh_rejects_future_timestamp() {
        ...
        let v = GBuildVersion::new("0.1.200".to_string(), None, future);
        assert!(
    ...
    /// symlink-target file name (`gbuild-<version>-<platform>`).
    #[test]
    fn test_version_from_versioned_binary_name() {
        let cases: &[(&str, Option<&str>)] = &[
            ("gbuild-0.2.46-darwin-arm64", Some("0.2.46")),
            ("gbuild-0.1.220-linux-x86_64", Some("0.1.220")),
            ("gbuild-0.2.5-windows-x86_64.exe", Some("0.2.5")),
            ...
            (
                "gbuild-0.1.220-alpha.4-linux-x86_64",
                Some("0.1.220-alpha.4"),
            ),
            ("gbuild-0.1.220-alpha.4", Some("0.1.220-alpha.4")), // npm layout
            ("gbuild-pager-0.1.5-darwin-arm64", None),           // "pager" is not a version
            ("gbuild-garbage-darwin-arm64", None),               // unparseable version
            ("gbuild-0.2.46", Some("0.2.46")),                   // no platform suffix
            ("other-0.2.46-darwin-arm64", None),                 // wrong prefix
            ("gbuild-latest", None),                             // symlink alias, not a version
            ("gbuild", None),                                    // bare name
            ("", None),
        ];
        for (name, expected) in cases {
            assert_eq!(
                version_from_versioned_binary_name(name, "gbuild").as_deref(),
                *expected,
        ...
        // prefix but not under "gbuild".
        assert_eq!(
            version_from_versioned_binary_name("gbuild-pager-0.1.5-darwin-arm64", "gbuild-pager")
                .as_deref(),
            Some("0.1.5")
        );
    }
    ...
    // ──────────────────────────────────────────────────────────────────────
    // GBuildVersion JSON shape — backward compatibility invariants
    // ──────────────────────────────────────────────────────────────────────
    ...
    fn test_version_json_backward_compat() {
        ...
        let v: GBuildVersion = serde_json::from_str(old).unwrap();
        assert_eq!(v.version, "0.1.180");
        assert!(v.stable_version.is_none());

        // New format with stable_version round-trips correctly.
        let now = time::OffsetDateTime::now_utc();
        let new = GBuildVersion::new("0.2.5".to_string(), Some("0.2.3".to_string()), now);
        let json = serde_json::to_string(&new).unwrap();
        let parsed: GBuildVersion = serde_json::from_str(&json).unwrap();
        ...
        assert!(serde_json::from_str::<GBuildVersion>(future).is_ok());

        // Missing required field (checked_at) is rejected.
        let missing = r#"{"version":"0.1.180"}"#;
        assert!(serde_json::from_str::<GBuildVersion>(missing).is_err());
    }
    ...
    fn test_is_fresh_ttl_boundaries() {
        let now = time::OffsetDateTime::now_utc();
        let v = GBuildVersion::new("0.1.200".to_string(), None, now);
    ...
    fn test_update_config_default_channel_is_stable() {
        use gbuild_shell::env::GBuildEnvironment;
        let cfg = UpdateConfig::from_environment(&GBuildEnvironment::Production);
