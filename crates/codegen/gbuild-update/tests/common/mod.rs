...
//! its own `OnceLock<GBUILD_HOME>`. The helpers below ensure the per-binary
//! initialization is identical: same env-var set, same isolation guarantees,
//! same reset between tests.
//!
//! Mirrors the GBUILD_HOME isolation pattern used in other integration tests.
//!
//! ## Usage
//!
//! ```ignore
...
//! #[serial_test::serial]
//! async fn my_test() {
//!     let _ = test_home();   // initializes GBUILD_HOME once per binary
...
// ─────────────────────────────────────────────────────────────────────────────
// GBUILD_HOME isolation
// ─────────────────────────────────────────────────────────────────────────────

/// Returns a process-wide test `GBUILD_HOME`, initialized exactly once per test
/// binary. Once initialized, `gbuild_config::gbuild_home()` will resolve to
...
pub fn test_home() -> &'static PathBuf {
    ...
    HOME.get_or_init(|| {
        ...
        unsafe {
            std::env::set_var("GBUILD_HOME", &path);
            std::env::remove_var("GBUILD_TEST_VERSION");
            std::env::remove_var("NPM_TOKEN");
            std::env::remove_var("GBUILD_INSTALLER");
            std::env::remove_var("GBUILD_MANAGED_BY_NPM");
            std::env::remove_var("GBUILD_MANAGED_BY_INTERNAL");
        }
        path
    })
}

/// Wipe state in `GBUILD_HOME` between tests so each test sees a clean home.
...
/// Override the version reported by `get_installed_gbuild_version()` for the
/// duration of the test (until [`reset_home`] or process exit).
pub fn set_test_version(v: &str) {
    // SAFETY: tests using this helper must be `#[serial]`.
    unsafe { std::env::set_var("GBUILD_TEST_VERSION", v) };
}
...
/// Minimal [`gbuild_update::UpdateConfig`] for install tests.
pub fn make_update_config(channel: &str) -> gbuild_update::UpdateConfig {
    gbuild_update::UpdateConfig {
...
/// Backdate every file in `GBUILD_HOME/downloads` by ~2 hours.
///
