...
//!
//! > `~/.gbuild/bin/gbuild` resolves to a binary that passes the smoke-test, OR it
...
use gbuild_update::auto_update::install_internal_from_base;
...
/// (`gbuild` and `agent` — see `swap_managed_bin_links`). Returns the
...
/// What the active `gbuild` should resolve to after an install attempt.
...
/// to both `gbuild` and `agent` — `swap_managed_bin_links` moves them together.
...
/// `cargo nextest run -p gbuild-update --run-ignored all`.
#[tokio::test(flavor = "multi_thread")]
#[serial]
...
async fn blitz_fuzz_stress() {
    ...
    let iterations: usize = std::env::var("GBUILD_BLITZ_ITERS")
        .ok()
