...
//!
//! Wires together a wiremock-mocked GCS bucket + an isolated `GBUILD_HOME`
//! tempdir so we can verify the full install pipeline:
//!   fetch version → download gbuild binary → chmod → atomic symlink →
//!   cleanup_old_downloads → persist installer config.
//!
//! The function reads `gbuild_home()` (a process-wide `OnceLock`), so all
//! tests in this binary share a single `GBUILD_HOME` and run serially via
//! `#[serial]`.
...
use gbuild_update::UpdateConfig;
use gbuild_update::auto_update::{install_internal_from_base, install_internal_from_bases};
...
async fn mount_gcs(version: &str, platform: &str) -> MockServer {
    ...
    // Main gbuild binary download.
    Mock::given(method("GET"))
        .and(path(format!("/gbuild-{version}-{platform}")))
...
async fn install_internal_pinned_version_writes_binary_and_symlink() {
    ...
    let downloaded = home
        .join("downloads")
        .join(format!("gbuild-0.1.181-{platform}"));
    assert!(downloaded.exists(), "binary downloaded: {downloaded:?}");
    assert_eq!(std::fs::read(&downloaded).unwrap(), b"#!/bin/sh\nexit 0\n");

    let symlink = home.join("bin").join("gbuild");
    assert!(symlink.is_symlink(), "gbuild symlink created");
    let target = std::fs::read_link(&symlink).unwrap();
    assert_eq!(
        target.file_name().unwrap(),
        format!("gbuild-0.1.181-{platform}").as_str()
    );

    // `gbuild` and `agent` move together — see `swap_managed_bin_links`.
    ...
    assert_eq!(
        agent_target, target,
        "agent and gbuild point at same target"
    );
}
...
async fn install_internal_updates_stale_agent_symlink_to_new_version() {
    ...
    let old_binary = download_dir.join(format!("gbuild-0.1.180-{platform}"));
    std::fs::write(&old_binary, b"#!/bin/sh\nexit 0\n").unwrap();
    let rel_old = std::path::Path::new("..")
        .join("downloads")
        .join(format!("gbuild-0.1.180-{platform}"));
    std::os::unix::fs::symlink(&rel_old, bin_dir.join("gbuild")).unwrap();
...
/// Rollback regression: if `agent` swap fails after `gbuild` succeeded,
/// `gbuild` must roll back to its prior target (all-or-nothing).
#[tokio::test]
#[serial]
async fn install_internal_rolls_back_gbuild_when_agent_swap_fails() {
    let _ = test_home();
    reset_home();
    let platform = host_platform();
    let server = mount_gcs("0.1.181", &platform).await;
    ...
    // gbuild must be rolled back to the prior version.
    let gbuild_target = std::fs::read_link(bin_dir.join("gbuild")).unwrap();
    assert_eq!(
        gbuild_target.file_name().unwrap(),
        format!("gbuild-0.1.180-{platform}").as_str(),
        "gbuild must be rolled back when agent swap fails"
    );
}

/// Absent-prior rollback regression: fresh install (no prior `gbuild` /
/// `agent`), sabotaged `agent` swap must *remove* the just-created `gbuild`
/// link so we don't leave it on the new binary while `agent` is absent.
#[tokio::test]
#[serial]
async fn install_internal_rollback_removes_absent_prior_gbuild_link() {
    let _ = test_home();
    reset_home();
    let platform = host_platform();
    let server = mount_gcs("0.1.181", &platform).await;
    ...
    // No prior `gbuild`. Sabotage `agent` swap: non-empty directory → EISDIR.
    let agent_dir = bin_dir.join("agent");
    std::fs::create_dir(&agent_dir).unwrap();
    std::fs::write(agent_dir.join("blocker"), b"x").unwrap();
    assert!(
        !bin_dir.join("gbuild").exists() && !bin_dir.join("gbuild").is_symlink(),
        "precondition: gbuild must not exist before install",
    );
    ...
    let gbuild_path = bin_dir.join("gbuild");
    assert!(
        !gbuild_path.is_symlink() && !gbuild_path.exists(),
        "gbuild must be removed on rollback when there was no prior link",
    );
}
...
async fn install_internal_cleans_up_stale_pager_symlink() {
    // Old installations shipped a separate gbuild-pager binary. Verify the
    // update removes the stale symlink from ~/.gbuild/bin/.
    let _ = test_home();
    reset_home();
    let platform = host_platform();
    let server = mount_gcs("0.1.181", &platform).await;
    ...
    let bin_dir = home.join("bin");
    std::fs::create_dir_all(&bin_dir).unwrap();
    let pager_link = bin_dir.join("gbuild-pager");
    ...
    assert!(
        ...
        "stale gbuild-pager symlink should be removed"
    );
}
...
async fn install_internal_alpha_channel_resolves_max_of_alpha_and_stable() {
    ...
    Mock::given(method("GET"))
        ...
        .await;
    Mock::given(method("GET"))
        .and(path(format!("/gbuild-0.1.181-{platform}")))
        .respond_with(ResponseTemplate::new(200).set_body_bytes(b"#!/bin/sh\nexit 0\n".to_vec()))
        .mount(&server)
        .await;
...
#[tokio::test]
#[serial]
async fn install_internal_fails_on_gbuild_binary_404() {
    let _ = test_home();
    reset_home();
    let platform = host_platform();
    let server = MockServer::start().await;
...
async fn install_internal_cleans_up_old_versions_keeping_n_minus_one() {
    ...
    assert!(
        !downloads
            .join(format!("gbuild-0.1.179-{platform}"))
            .exists(),
        "oldest deleted"
    );

    // Symlink updated to latest.
    let target = std::fs::read_link(home.join("bin").join("gbuild")).unwrap();
    assert!(
        target
            .file_name()
            .unwrap()
...
async fn install_internal_idempotent_for_same_version() {
    ...
    let target = std::fs::read_link(test_home().join("bin").join("gbuild")).unwrap();
    assert!(target.to_string_lossy().contains("0.1.181"));
}

#[tokio::test]
#[serial]
async fn install_internal_creates_gbuild_home_subdirs_if_missing() {
    let _ = test_home();
    reset_home();
