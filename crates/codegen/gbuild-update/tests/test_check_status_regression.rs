...
//! exact JSON shape produced by `gbuild update --check --json` for the failure
...
use gbuild_update::auto_update::check_update_status;

/// Set up a fake `npm` on PATH, set `GBUILD_INSTALLER=npm` so the auto-update
