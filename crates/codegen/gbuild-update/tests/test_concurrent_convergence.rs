...
//! release:
//!
//! 1. TUI startup: `check_update_background` spawns a detached `gbuild update`
//!    (the Ctrl+U path now adopts this child instead of spawning a second).
//! 2. Explicit `gbuild update` (incl. the Ctrl+U fallback when there is no
//!    live child).
...
//!   temp-name fix, every `0.1.x` download shared one `gbuild-0.1.tmp`
...
use gbuild_update::auto_update::{ensure_latest_on_disk, install_internal_from_base, run_update};
use gbuild_update::version::installed_on_disk_version;

/// Assert the active `~/.gbuild/bin/gbuild` resolves to the expected versioned
...
fn assert_active_binary(home: &Path, version: &str, platform: &str, expected_content: &[u8]) {
    let link = home.join("bin").join("gbuild");
    assert!(link.is_symlink(), "gbuild must be a symlink");
    let resolved = dunce::canonicalize(&link)
        .unwrap_or_else(|e| panic!("active gbuild symlink does not resolve: {e}"));
    assert_eq!(
        resolved.file_name().unwrap().to_string_lossy(),
        format!("gbuild-{version}-{platform}"),
        "active gbuild must be the expected version"
    );
    assert_eq!(
    ...
    let ran_ok = std::process::Command::new(&resolved)
        ...
        .map(|s| s.success())
        .unwrap_or(false);
    assert!(ran_ok, "active gbuild must pass the smoke-test");
}

/// Lay down a managed-install layout in the test GBUILD_HOME:
/// `bin/gbuild -> ../downloads/gbuild-<version>-<platform>` (what
...
fn fake_managed_install(version: &str) {
    ...
    let name = format!("gbuild-{version}-{}", host_platform());
...
// ─────────────────────────────────────────────────────────────────────────────
// Convergence: explicit `gbuild update` (the Ctrl+U fallback path) finds the
...
// installers that actually maintain the managed `~/.gbuild/bin/gbuild` symlink
...
async fn disk_probe_rejects_dangling_symlink() {
    ...
    // ~/.gbuild/downloads cleanup), the probe must report None — otherwise
    ...
    std::fs::remove_file(
        home.join("downloads")
            .join(format!("gbuild-0.2.7-{platform}")),
    )
    .unwrap();
...
// shared one `gbuild-0.1.tmp`, so a concurrent racer could atomically rename a
...
async fn concurrent_different_version_installs_do_not_corrupt_each_other() {
    ...
    // Pre-fix, BOTH of these wrote to downloads/gbuild-0.1.tmp concurrently
    ...
    for version in ["0.1.181", "0.1.182"] {
        let path = home
            .join("downloads")
            .join(format!("gbuild-{version}-{platform}"));
        assert_eq!(
