//! Headless markdown analysis sharing gBuild's exact `pulldown-cmark` config.
//!
...
//! feature set, shared with `gbuild-markdown` so analysis matches what gBuild
//! actually renders 1:1.
//!
//! After parsing, gBuild applies [`offset_events`]: only `~~…~~` is strikethrough.
...
/// The exact `pulldown-cmark` option set gBuild uses to render markdown.
///
...
/// Offset event stream from gBuild's parser, with single-tilde strikethrough demoted.
///
...
/// Parse `text` with gBuild's options; count elements and flag structural issues.
