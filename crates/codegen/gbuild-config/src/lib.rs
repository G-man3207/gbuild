//! Config file loading for gBuild.
//!
//! Merge order (lowest → highest priority):
//! 1. `/etc/gbuild/managed_config.toml`
//! 2. `$GBUILD_HOME/managed_config.toml`
//! 3. `$GBUILD_HOME/config.toml`
//! 4. `$GBUILD_HOME/requirements.toml` (cloud cache; Ed25519-signed at rest once a
//!    key is embedded — see [`signed_policy`] — below the OS-protected layers)
//! 5. `/etc/gbuild/requirements.toml`
//! 6. macOS MDM managed preferences (`ai.x.gbuild`, admin-forced) — macOS only
//!
...
pub use global_hook_sources::{
    ...
    ensure_gbuild_hook_slots, existing_ancestor_chain, is_direct_hook_json_name,
...
pub use paths::{
    ...
    default_gbuild_home, encode_cwd_dirname, ensure_sessions_cwd_dir, gbuild_application,
    gbuild_application_in, gbuild_home, sessions_cwd_dir, system_config_dir, user_gbuild_home,
};
pub use validation::{
