//! gBuild-owned direct global hook paths shared by shell discovery and sandbox
//! write-deny: `$GBUILD_HOME/hooks`, `hooks-paths`, and absolute registry targets.
...
pub enum GlobalHookSourceKind {
    /// `$GBUILD_HOME/hooks/` (discovered + protected).
    HookDirectory,
    /// `$GBUILD_HOME/hooks-paths` (protected; never loaded as hook JSON).
    RegistryFile,
...
pub enum GlobalHookSourceError {
    ...
    #[error("symlinked GBUILD_HOME is not allowed under sandbox write-deny: {path}")]
    SymlinkedGBuildHome { path: PathBuf },
    ...
    #[error("gBuild hooks directory has wrong type (expected real directory): {path}")]
    InvalidHooksDir { path: PathBuf },
    #[error("gBuild hooks-paths registry has wrong type (expected real file): {path}")]
    InvalidRegistryFile { path: PathBuf },
    #[error("cannot create gBuild hooks directory {path}: {source}")]
    CreateHooksDir {
        path: PathBuf,
        #[source]
        source: io::Error,
    },
    #[error("cannot create gBuild hooks-paths registry {path}: {source}")]
    CreateRegistryFile {
        path: PathBuf,
        #[source]
        source: io::Error,
...
/// Ensure real `$GBUILD_HOME/hooks` dir + `hooks-paths` file (create if missing).
/// Race-resistant create (`create_dir` / `create_new`+`O_NOFOLLOW`); never
/// truncates an existing registry; rejects symlinks/wrong types.
pub fn ensure_gbuild_hook_slots(gbuild_home: &Path) -> Result<(), GlobalHookSourceError> {
    if path_has_symlink_component(gbuild_home) {
        return Err(GlobalHookSourceError::SymlinkedGBuildHome {
            path: gbuild_home.to_path_buf(),
        });
    }

    match std::fs::create_dir(gbuild_home) {
        Ok(()) => {}
        Err(e) if e.kind() == io::ErrorKind::AlreadyExists => {}
        Err(e) if e.kind() == io::ErrorKind::NotFound => {
            std::fs::create_dir_all(gbuild_home).map_err(|source| {
                GlobalHookSourceError::CreateHooksDir {
                    path: gbuild_home.to_path_buf(),
                    source,
                }
    ...
    let gbuild_meta = std::fs::symlink_metadata(gbuild_home).map_err(|source| {
        GlobalHookSourceError::CreateHooksDir {
            path: gbuild_home.to_path_buf(),
            source,
        }
    })?;
    if gbuild_meta.file_type().is_symlink() || !gbuild_meta.file_type().is_dir() {
        return Err(GlobalHookSourceError::SymlinkedGBuildHome {
            path: gbuild_home.to_path_buf(),
        });
    }

    let hooks = gbuild_home.join("hooks");
    ...
    let registry = gbuild_home.join("hooks-paths");
...
/// Resolve gBuild-owned direct global hook sources (`reject_symlinks` for sandbox).
pub fn resolve_global_hook_sources(
    ...
    if let Some(gbuild_home) = gbuild_home {
        if reject_symlinks && path_has_symlink_component(gbuild_home) {
            return Err(GlobalHookSourceError::SymlinkedGBuildHome {
                path: gbuild_home.to_path_buf(),
            });
        }
        ...
        let hooks = gbuild_home.join("hooks");
        let hooks_paths = gbuild_home.join("hooks-paths");
        if reject_symlinks {
