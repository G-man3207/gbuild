//! Filesystem locations for gBuild config files and binaries.

use std::path::PathBuf;
use std::sync::OnceLock;

static GBUILD_HOME: OnceLock<PathBuf> = OnceLock::new();
...
/// The default user gBuild directory (`~/.gbuild`, canonicalized) used when
/// `GBUILD_HOME` is unset. Exposed so callers (e.g. display helpers) can detect
/// whether [`gbuild_home()`] is the default without duplicating the computation.
///
...
/// `~/.gbuild/marketplace-cache`. `dunce` strips the prefix whenever the path
...
/// `xai_fast_worktree::db::resolve_gbuild_home` (deliberately standalone crate).
pub fn default_gbuild_home() -> PathBuf {
    #[allow(deprecated)]
    let home = std::env::home_dir().unwrap_or_else(|| PathBuf::from("."));
    dunce::canonicalize(&home).unwrap_or(home).join(".gbuild")
}

/// Per-user config directory: `$GBUILD_HOME` or `~/.gbuild`. Created if needed.
pub fn gbuild_home() -> PathBuf {
    GBUILD_HOME
        .get_or_init(|| {
            let gbuild_home = if let Ok(v) = std::env::var("GBUILD_HOME") {
                PathBuf::from(v)
            } else {
                default_gbuild_home()
            };
...
/// The user-global gBuild home, but only when one genuinely resolves: `Some` when
/// `$GBUILD_HOME` is set or a home directory is found, `None` otherwise. Unlike
/// [`gbuild_home()`], this never falls back to a cwd-relative `.gbuild`, so callers
/// that *scan* user-global gBuild resources (hooks, marketplace sources, ...) don't
/// mistake a project's `.gbuild` tree for the user-global one when no home resolves.
pub fn user_gbuild_home() -> Option<PathBuf> {
    #[allow(deprecated)]
    let resolvable = std::env::var_os("GBUILD_HOME").is_some() || std::env::home_dir().is_some();
    resolvable.then(gbuild_home)
}

/// Canonical gBuild application path: `$GBUILD_HOME/bin/gbuild` (Unix) or `gbuild.exe` (Windows).
pub fn gbuild_application() -> PathBuf {
    gbuild_application_in(&gbuild_home())
}

/// [`gbuild_application`] under an explicit home instead of `$GBUILD_HOME`.
pub fn gbuild_application_in(home: &std::path::Path) -> PathBuf {
...
/// System-wide config directory: `/etc/gbuild/` on Unix, `None` on Windows.
pub fn system_config_dir() -> Option<PathBuf> {
    if cfg!(unix) {
        Some(PathBuf::from("/etc/gbuild"))
    } else {
        None
    }
}
...
/// `gbuild_home()/sessions/{encode_cwd_dirname(cwd)}`.
///
...
pub fn sessions_cwd_dir(cwd: &str) -> PathBuf {
    gbuild_home().join("sessions").join(encode_cwd_dirname(cwd))
}
...
pub fn ensure_sessions_cwd_dir(cwd: &str) -> std::io::Result<PathBuf> {
    ...
    let dir = gbuild_home().join("sessions").join(&encoded_name);
...
mod tests {
    ...
    #[test]
    fn default_gbuild_home_has_no_verbatim_prefix() {
        ...
        let home = default_gbuild_home();
        assert!(!home.to_string_lossy().starts_with(r"\\?\"));
        assert!(home.ends_with(".gbuild"));
    }
