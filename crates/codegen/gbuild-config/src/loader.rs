...
use crate::paths::{system_config_dir, user_gbuild_home};
...
/// gBuild config files; use [`load_toml_file`] directly for unrelated TOML.
...
pub fn load_from_disk() -> std::io::Result<toml::Value> {
    load_user_config_layer(user_gbuild_home().as_deref(), USER_CONFIG_FILENAME)
}

/// User config filename (`$GBUILD_HOME/config.toml`), shared by the loaders here.
...
pub fn load_managed_config() -> std::io::Result<toml::Value> {
    load_user_config_layer(user_gbuild_home().as_deref(), MANAGED_CONFIG_FILENAME)
}
...
/// `.gbuild/<filename>` (the cwd-fallback would silently promote an untrusted
/// project `.gbuild` to the user tier).
...
pub struct ManagedConfigLayer {
    ...
    /// `true` for the root-owned system layer (`/etc/gbuild`), derived from the
    /// load directory.
    pub is_system: bool,
}
...
/// A hook's origin (held by `gbuild_hooks::HookSpec::layer`). Defined here, not
/// in `gbuild-hooks`, since the dep direction is `gbuild-hooks -> gbuild-config`;
/// this crate sets the config tiers, `File`/`Plugin` are set downstream.
#[derive(Debug, Clone, Copy, PartialEq, Eq, serde::Serialize, serde::Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum HookProvenance {
    /// `/etc/gbuild/managed_config.toml`.
    SystemManaged,
    /// `$GBUILD_HOME/managed_config.toml` (server-synced).
    Managed,
...
impl ConfigLayers {
    ...
    /// wraps this with the `GBUILD_CAMPAIGNS_OVERRIDE` env layer.
...
/// `GBUILD_CAMPAIGNS=0` or `[features] campaigns = false` on pre-campaign base.
pub fn campaigns_application_disabled(base_effective: &toml::Value) -> bool {
    if crate::env_bool("GBUILD_CAMPAIGNS") == Some(false) {
        return true;
    }
    base_effective
        .get("features")
...
/// (`gbuild_shell::util::config::load_effective_config`) when remote campaigns
/// or `GBUILD_CAMPAIGNS_OVERRIDE` must be honored. The name mirrors the
...
/// can't be parsed (broken `GBUILD_TEST_VERSION` in dev), silently strips
...
pub fn apply_version_overrides_with_registered(value: &mut toml::Value) -> std::io::Result<()> {
    match gbuild_version::installed_semver() {
...
mod tests {
    ...
    fn load_user_config_layer_reads_file_when_home_present() {
        ...
        let dir = std::env::temp_dir().join(format!("gbuild-load-layer-{}", std::process::id()));
    ...
    fn parse_error_keeps_kind_but_not_snippet() {
        let dir = std::env::temp_dir().join(format!("gbuild-toml-leak-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
    ...
    /// `GBUILD_CAMPAIGNS=0` disables campaign application regardless of config.
    /// `GBUILD_CAMPAIGNS` is process-global, so this test serializes itself with a
    ...
    fn kill_switch_env_var_disables() {
        ...
        let prior = std::env::var_os("GBUILD_CAMPAIGNS");
        let empty = toml::Value::Table(Default::default());

        // SAFETY: ENV_GUARD serializes this against itself; no other test in the
        // crate mutates or reads GBUILD_CAMPAIGNS concurrently.
        unsafe { std::env::set_var("GBUILD_CAMPAIGNS", "0") };
        assert!(campaigns_application_disabled(&empty));

        unsafe { std::env::remove_var("GBUILD_CAMPAIGNS") };
        assert!(!campaigns_application_disabled(&empty));

        match prior {
            Some(v) => unsafe { std::env::set_var("GBUILD_CAMPAIGNS", v) },
            None => unsafe { std::env::remove_var("GBUILD_CAMPAIGNS") },
        }
    }
}
