...
>
> - env `GROK_MAX_MCP_OUTPUT_BYTES` or `MAX_MCP_OUTPUT_BYTES` (bytes; gBuild-native
...
gBuild walks from the current directory up to the git repo root, loading `.grok/config.toml` at each level:
...
Project-scoped files contribute `[mcp_servers]`, `[plugins]`, and `[permission]` entries. gBuild reads most other config sections only from `~/.grok/config.toml`.
...
|--------|--------|----------|-------------|
| `config.toml` | Native gBuild config | `~/.grok/config.toml`, `.grok/config.toml` | Always on |
...
For stdio servers, gBuild captures the process's standard error to `~/.grok/logs/mcp/<server>.stderr.log`, truncated on each launch. Check this file when a server starts but fails to handshake:
...
```bash
RUST_LOG=debug GROK_LOG_FILE=/tmp/grok.log gbuild
tail -f /tmp/grok.log
```
