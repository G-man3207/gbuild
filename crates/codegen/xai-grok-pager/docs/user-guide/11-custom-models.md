...
gBuild connects to custom model endpoints for alternative providers, self-hosted models, and overriding built-in settings. This guide explains how to select models, configure endpoints, and integrate third-party providers.

---

## Default Models

By default, gBuild uses models hosted by SpaceXAI, and new sessions start with `grok-4.5`. Default models require no configuration. Authenticate with `gbuild login` or an API key, then start a session.
...
gBuild supports three API backends. Set `api_backend` in your `[model.*]` config to choose which protocol the model uses:
...
When you omit `api_backend`, gBuild uses `chat_completions`.

To send provider-specific authentication or version headers -- for example, Anthropic's `x-api-key` -- use the `extra_headers` field described below. gBuild sends those headers verbatim with every request to the endpoint.
...
gBuild resolves credentials in this order:
...
3. A configured model or provider authentication helper
4. For HTTPS xAI origins only, your signed-in xAI session token or `XAI_API_KEY`

Custom origins never receive ambient xAI credentials. A custom provider must
declare its own key/header, or it is called without authentication. This makes
credential-less local servers safe to use while signed in to xAI.

### Context Window

The `context_window` value tells gBuild when to trigger auto-compaction. When you override a known model, gBuild inherits that model's context window. When you define a new model and omit `context_window`, gBuild defaults to 200,000 tokens, so set it explicitly to match your provider.
...
Some gateways route or version on the query string. `query_params` appends percent-encoded query parameters to every request gBuild makes for a model. For example, a gateway that selects an API version this way:
...
When you override a built-in model, gBuild starts with the default configuration (including the correct `base_url`), then applies only the fields you specify. Unspecified fields inherit from the default.
...
The `messages` backend uses the Anthropic Messages protocol. Anthropic authenticates with an `x-api-key` header rather than `Authorization: Bearer`, so pass your key through `extra_headers`, which gBuild sends verbatim.
...
Point gBuild at a custom OpenAI-compatible `/v1/models` endpoint instead of the default. Use this when your models sit behind a corporate gateway or a self-hosted inference service.

### Environment Variables

| Variable | Required | Description |
|----------|----------|-------------|
| `GROK_MODELS_BASE_URL` | Yes | Base URL for inference. gBuild fetches the model list from `{base_url}/models`. |
| `XAI_API_KEY` | Yes | API key sent as `Authorization: Bearer`. gBuild also accepts `GROK_CODE_XAI_API_KEY`. |
...
When you use `[endpoints]` with partial model overrides, gBuild inherits the `base_url` from the endpoints config, so you do not need to specify it in each `[model.*]` section.
...
If you point web search at a custom model, you also need a `[model.*]` entry so gBuild can reach it. Server-side ("backend") web search runs only when the model sets `supports_backend_search = true` (and the build enables backend search); it does not depend on `api_backend`:
