...
pub struct McpState {
    ...
    /// Persisted to `~/.gbuild/config.toml` under `[mcp_servers.<name>].disabled_tools`.
...
impl McpState {
    ...
    /// `GBuildClientHandler` cloned during `try_handshake` reads
...
/// MCP initialization strategy. Defined in `gbuild-telemetry`; re-exported
/// here so existing call sites continue to work.
pub use gbuild_telemetry::enums::McpInitStrategy;
...
impl xai_tool_runtime::Tool for McpErasedTool {
    ...
    async fn run(
        ...
        gbuild_telemetry::session_ctx::log_event(gbuild_telemetry::events::McpToolCalled {
            server_name: server.clone(),
            tool_name: tool.clone(),
            qualified_name,
            success,
...
/// Uses [`GBuildClientHandler`] rather than rmcp's default `ClientInfo`
...
pub type McpService = Arc<RunningService<RoleClient, GBuildClientHandler>>;
...
///    (`TransportClosed`).
/// 2. [`GBuildClientHandler`] when the server pushes a notification we
...
pub struct McpClient {
    ...
    /// [`GBuildClientHandler`]** constructed by
...
/// and the [`GBuildClientHandler`] it constructs during
...
/// External-config overrides for an MCP server, surfaced to gbuild-mcp
...
impl McpClient {
    ...
    pub async fn ensure_initialized(&self) -> Result<McpService, McpError> {
        ...
        tracing::info!(target: gbuild_telemetry::instrumentation::TARGET, event = "timing", name = "mcp_try_handshake", elapsed_us = handshake_elapsed);
    ...
    async fn try_handshake(
        &self,
        pending: PendingTransport,
    ) -> Result<rmcp::service::RunningService<RoleClient, GBuildClientHandler>, McpError> {
    ...
    fn make_client_info(server_name: &str) -> ClientInfo {
        ...
        ClientInfo::new(
            capabilities,
            Implementation::new(
                format!("gbuild-shell-{server_name}"),
    ...
    /// Build the [`GBuildClientHandler`] that drives `client.serve(...)`.
    ///
    ...
    fn make_client_handler(&self) -> GBuildClientHandler {
        GBuildClientHandler {
    ...
    /// references (the [`GBuildClientHandler`] handed to
    ...
    pub async fn get_tool_registrations(
        ...
        let _ensure_init_timer = gbuild_telemetry::instrumentation::timer("mcp_ensure_initialized");
        let mcp_service = self.ensure_initialized().await?;

        let mut all_tools = Vec::new();
        let mut cursor: Option<String> = None;

        let _list_tools_timer = gbuild_telemetry::instrumentation::timer("mcp_list_tools");
        loop {
...
/// Copy an MCP server's stderr to `~/.gbuild/logs/mcp/<server>.stderr.log`
/// in a background task. Truncated per spawn.
fn drain_mcp_stderr_to_log(server_name: &str, mut stderr: tokio::process::ChildStderr) {
    let log_dir = gbuild_config::gbuild_home().join("logs").join("mcp");
...
pub async fn start_mcp_server(
    ...
    let _per_server_timer = gbuild_telemetry::instrumentation::timer("mcp_start_one_server");
    match mcp_server {
        acp::McpServer::Stdio(acp::McpServerStdio {
            ...
            let _stdio_spawn_timer = gbuild_telemetry::instrumentation::timer("mcp_stdio_spawn");
                ...
                SafeTokioChildProcess::spawn(cmd, name.clone(), event_writer.clone()).map_err(
                    |e| {
                        ...
                        gbuild_telemetry::session_ctx::log_event(
                            gbuild_telemetry::events::McpServerFailed {
                                server_name: name.clone(),
                                error_type: gbuild_telemetry::events::McpErrorType::SpawnFailed,
        ...
        | acp::McpServer::Sse(acp::McpServerSse {
            ...
            let auth_prep = if has_existing_auth {
                ...
                let _auth_discovery_timer =
                    gbuild_telemetry::instrumentation::timer("mcp_http_auth_discovery");
...
pub async fn start_mcp_servers(
    ...
    let _mcp_start_timer = gbuild_telemetry::instrumentation::timer("mcp_start_servers");
...
#[derive(Debug)]
pub struct GBuildClientHandler {
...
impl ClientHandler for GBuildClientHandler {
    ...
    // See the [`GBuildClientHandler`] doc-comment for the full RPIT
    // contract.
...
mod tests {
    ...
    async fn try_call_tool_reconnects_then_succeeds_after_retriable_transport_error() {
        ...
        async fn dead_service() -> McpService {
            ...
            let handler = GBuildClientHandler {
