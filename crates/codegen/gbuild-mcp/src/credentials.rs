...
//!
//! Credentials are stored in `$GBUILD_HOME/mcp_credentials.json`, keyed by a
...
///
/// Local helper (not shell-base): `gbuild-mcp` sits below `config-types` in the
...
/// File name for the credential store inside `$GBUILD_HOME`.
const CREDENTIALS_FILENAME: &str = "mcp_credentials.json";

/// On-disk credential store: `$GBUILD_HOME/mcp_credentials.json`.
///
...
impl McpCredentialStore {
    ...
    /// Load the credential store from the default path (`$GBUILD_HOME/mcp_credentials.json`).
    ///
    ...
    pub fn save_default(&self) -> Result<()> {
        let path = Self::default_path().ok_or_else(|| {
            McpCredentialError::Other("no user gBuild home (set $GBUILD_HOME or $HOME)".into())
        })?;
        self.save_to(&path)
    }
    ...
    /// Default path: `$GBUILD_HOME/mcp_credentials.json`.
    fn default_path() -> Option<PathBuf> {
        Some(gbuild_config::user_gbuild_home()?.join(CREDENTIALS_FILENAME))
    }
}
...
mod tests {
    ...
    /// `$GBUILD_HOME/mcp_credentials.json`. Existing credential files must keep
    ...
    fn save_and_load_from_file() {
        let dir = std::env::temp_dir().join("gbuild-mcp-credentials-test");
