...
/// emitter in `gbuild-shell` and the upsert/detection here — a drift would
...
pub enum AuthType {
    /// From AuthManager (gbuild login, OIDC, external binary). Refreshable.
    #[default]
    SessionToken,
...
/// `gbuild_sampling_types::SamplingConfig` (which is secret-free).
