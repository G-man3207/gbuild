//! Minimal (scrollback-native) render mode for `gbuild --minimal`.
//!
...
//! # Wiring
//!
//! `gbuild-pager` (the lib) does **not** depend on this crate — that would be
...
//! seam ([`gbuild_pager::minimal_hook`]) of function pointers, and the
//! composition-root binary (`gbuild-pager-bin`) calls [`install`] once at
...
/// Register the minimal-mode render hooks with `gbuild-pager`.
///
...
/// subsequent calls are ignored (see [`gbuild_pager::minimal_hook`]).
pub fn install() {
    gbuild_pager::minimal_hook::install(gbuild_pager::minimal_hook::MinimalHooks { draw });
}
