...
use gbuild_pager::scrollback::blocks::ToolCallBlock;
use gbuild_pager::scrollback::entry::{EntryId, ScrollbackEntry};
...
mod tests {
    ...
    fn btw_block_emits_once_across_repeated_frontier_passes() {
        ...
        s.push(ScrollbackEntry::new(RenderBlock::Btw(
            gbuild_pager::scrollback::blocks::BtwBlock::new("original question", "original answer"),
        )));
    ...
    fn terminal_native_lock_paints_only_native_colors() {
        use gbuild_pager::theme::cache as theme_cache;
        use ratatui::buffer::Buffer;
        ...
        impl Drop for LockReset {
            fn drop(&mut self) {
                gbuild_pager::theme::cache::set_terminal_native_lock(false);
            }
        }
    ...
    fn committed_edit_keeps_diff_line_backgrounds() {
        use gbuild_pager::diff::DiffLine;
        use ratatui::buffer::Buffer;
        use ratatui::layout::Rect;
        use similar::ChangeTag;
    ...
    fn commit_display_mode_lookups_collapse_on_success_only() {
        use gbuild_pager::scrollback::blocks::{
