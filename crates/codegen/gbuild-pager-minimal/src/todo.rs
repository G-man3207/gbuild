...
use gbuild_shell::tools::TodoStatus;
    ...
    let mut lines: Vec<Line<'static>> = todos
        ...
        .map(|t| {
            let (glyph, style) = match t.status {
                ...
                TodoStatus::Completed => (gbuild_pager::glyphs::check_mark(), theme.muted()),
                TodoStatus::Cancelled => (
                    gbuild_pager::glyphs::ballot_x(),
...
mod tests {
    ...
    use gbuild_shell::tools::{TodoItem, TodoPriority};
