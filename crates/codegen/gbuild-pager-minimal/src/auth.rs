...
//!
//! Before any agent session exists (unauthenticated) the minimal live region
//! shows the sign-in flow itself — device or external-command flow, a sign-in
//! error, or a brief "starting" transient once the auth gate is open — since
//! minimal has no welcome screen. [`draw_live`](super::live::draw_live)
//! computes a [`MinimalAuthHint`] from the app's [`AuthState`] and renders it
//! via [`render_auth`].
...
use gbuild_pager::app::app_view::AuthState;
...
/// brief "starting" transient once authenticated. Computed before the draw
/// closure so the closure can own it.
...
/// Map the app's auth state to what the no-agent live region should show.
...
/// Rows the no-agent live region needs for `hint` (before URL wrap). Used by
/// the overlay host so the viewport grows enough to show the sign-in flow
...
mod tests {
    ...
    fn auth_hint_maps_auth_state() {
        use gbuild_pager::app::app_view::AuthMode;
