...
pub fn apply_voice_probe(report: &mut DiagnosticReport, emit_missing_issue: bool) {
    ...
    match gbuild_voice::input_device_info() {
        Ok(device) => {
        ...
        Err(err) => {
            let error = match err {
                gbuild_voice::VoiceError::Config(message) => message,
...
pub(crate) fn collect_startup_warnings_from(
    ...
    if ctx.brand == TerminalName::AppleTerminal && ctx.is_ssh {
        ...
        warning.note = Some(
            "gBuild also saves each copy to the backup file shown in the copy message. To copy \
    ...
    if ctx.is_tmux_backed() && matches!(tmux.control_mode, probes::TmuxProbeResult::Available(true))
    {
        let message = match fullscreen_active {
            ...
            Some(false) => {
                "gBuild is using inline mode because tmux control mode limits fullscreen"
            }
...
pub(crate) fn wezterm_kitty_keyboard_warning_from(
    ...
    if shape == WezTermShape::SshXtversion {
        ...
        warning.note = Some(
            "For this session, type `\\` and then press Enter. gBuild can't negotiate the Kitty \
...
pub fn sandbox_profile_conflict_warning(workspace: &Path) -> Option<TerminalWarning> {
    sandbox_profile_conflict_warning_from(gbuild_sandbox::sandbox_profile_conflicts(workspace))
}
...
fn sandbox_profile_conflict_warning_from(conflicts: Vec<String>) -> Option<TerminalWarning> {
    ...
    Some(TerminalWarning {
        ...
        config_path: None,
        note: Some(format!(
            "gBuild is using the user profile. Compare `.gbuild/sandbox.toml` with {}, then rename \
             or remove the conflicting project profile. Project settings can add profile names \
             but can't redefine a user profile.",
            crate::util::display_user_gbuild_path("sandbox.toml")
        )),
    })
}
...
/// - `!osc52_sink_active` — no wrap is already capturing our output. `gbuild
...
pub(crate) fn collect_notification_warnings_with_method(
        ...
        && ctx.brand == TerminalName::Unknown
    {
        let mut warning = TerminalWarning::new(
            ...
            "gBuild is using the terminal bell because the terminal was not recognized",
            None,
            None,
        );
        warning.note = Some(format!(
            ...
            crate::util::display_user_gbuild_path("config.toml")
        ));
        warnings.push(warning);
    }
    ...
    if condition == NotificationCondition::Unfocused && !supports_focus_tracking(ctx.brand) {
        let mut warning = TerminalWarning::new(
            ...
            Some(&crate::util::display_user_gbuild_path("config.toml")),
        );
        warning.note = Some(
...
pub fn color_support_warning(
    ...
    if level == ColorLevel::None {
        ...
        warning.note = Some("Unset `NO_COLOR`, then restart gBuild.".to_string());
        return Some(warning);
    }
    ...
    if is_tmux_backed {
        ...
        warning.note = Some(format!(
            "In the same tmux config, also add `set -g default-terminal \"tmux-256color\"`. Add \
             ...
             `tmux source-file {tmux_config_path}`, or detach and reattach, and restart gBuild."
        ));
        return Some(warning);
    }
...
mod tests {
    ...
    fn sandbox_profile_conflict_warning_reports_conflicts() {
        ...
        assert!(w.note.as_deref().is_some_and(|note| {
            note.contains("rename or remove")
                && note.contains(".gbuild/sandbox.toml")
                && note.contains(&crate::util::display_user_gbuild_path("sandbox.toml"))
    ...
    // -- ssh_wrap_hint: `gbuild wrap ssh` recommendation --------------------------
    ...
    fn ssh_wrap_hint_fires_over_plain_ssh() {
        ...
        assert_eq!(w.fix.as_deref(), Some("gbuild wrap ssh <host>"));
        assert!(
    ...
    fn ssh_wrap_hint_suppressed_when_sink_active() {
        ...
        // `gbuild wrap` — adoption silences the hint by itself.
        ...
        assert!(findings[1].remediation.as_ref().is_some_and(|remediation| {
            remediation.fix.contains("condition = \"always\"")
                && remediation.config_path.as_deref()
                    == Some(crate::util::display_user_gbuild_path("config.toml").as_str())
        }));
    }
