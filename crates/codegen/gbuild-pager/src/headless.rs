//! Headless single-turn mode (`gbuild -p "prompt"`).
//!
//! Runs the agent in-process via
//! `spawn_gbuild_shell`, sends the ACP lifecycle (init → auth → session → prompt),
...
use gbuild_shell::agent::auth_method::AuthMethodKind;
use gbuild_shell::agent::config::Config as AgentConfig;
use gbuild_shell::extensions::task::{CancelSubagentRequest, KillTaskRequest};
...
use gbuild_shell::util::config as cli_config;
use xai_acp_lib::{AcpAgentTx, AcpClientMessageBox, AcpClientRx, acp_send};

use crate::acp::model_state::{EffortTokenError, ModelState};
use crate::acp::spawn::{AgentShutdownGuard, spawn_gbuild_shell};
...
pub fn parse_permission_rules_strict(
    allow: &[String],
    deny: &[String],
) -> anyhow::Result<Vec<gbuild_workspace::permission::types::PermissionRule>> {
...
pub(crate) fn parse_permission_rules_inner(
    ...
    use gbuild_workspace::permission::types::RuleAction;
...
fn parse_cli_agents(
    json: &str,
) -> anyhow::Result<Vec<gbuild_shell::agent::config::AgentDefinition>> {
    ...
    for (name, mut value) in map {
        ...
        let mut def = gbuild_shell::agent::config::AgentDefinition::from_json(&value)
...
fn apply_agent_flag(agent: &Option<String>, config: &mut gbuild_shell::agent::config::Config) {
...
fn attach_result_usage(result: &mut serde_json::Value, usage: &serde_json::Value) {
    gbuild_shell::extensions::notification::attach_result_usage_fail_closed(result, usage);
}
...
async fn authenticate(
    ...
    let method_id = crate::acp::select_eager_auth_method(auths, default_auth_method_id)
        .ok_or_else(|| {
            ...
            let interactive = std::io::stdin().is_terminal()
                && !gbuild_shell::util::clipboard::is_remote_session();
    ...
    if kind.needs_interactive_login() {
        use std::io::IsTerminal;
        let interactive =
            std::io::stdin().is_terminal() && !gbuild_shell::util::clipboard::is_remote_session();
...
async fn open_session(
    ...
    let mcp_servers =
        cli_config::load_mcp_servers(cwd, &gbuild_tools::types::compat::CompatConfig::default());
...
async fn apply_headless_model_and_effort(
    ...
    .map_err(|e| {
        if let Some(name) = model_name {
            anyhow::anyhow!(
                "Couldn't set model '{}': {}. Run 'gbuild models' to see available models.",
                name,
                e
            )
        } else {
...
pub async fn run_single_turn(
    ...
    gbuild_shell::http::set_process_client_mode_headless();
    ...
    agent_config.mode = gbuild_shell::agent::config::AgentMode::Headless;
    ...
    // hub_url lives on `gbuild workspace` / WorkspaceStartArgs only).
    ...
    agent_config.cli_agent_overrides = gbuild_shell::agent::config::CliAgentOverrides {
    ...
    let spawned = match spawn_gbuild_shell(agent_config, &cancel, memory_config).await {
        Ok(s) => s,
        Err(e) => {
    ...
    let track_active = std::env::var("GBUILD_TRACK_HEADLESS").is_ok();
    if track_active {
        let _ =
            gbuild_shell::active_sessions::register(gbuild_shell::active_sessions::ActiveSession {
    ...
    // Handle result
    if track_active {
        // Non-blocking flock so a slow/network ~/.gbuild can't hang exit.
        let _ = gbuild_shell::active_sessions::try_unregister(&session_id);
    }
    ...
    match prompt_result {
        ...
        Some(Err(err)) => {
            ...
            if let Some(usage) = gbuild_shell::sampling::error::prompt_usage_from_error(&err)
...
mod tests {
    ...
    use super::*;
    use gbuild_workspace::permission::types::{RuleAction, ToolFilter};
    ...
    fn domain_mode_web_fetch() {
        ...
        assert_eq!(
            ...
            gbuild_workspace::permission::types::PatternMode::Domain
        );
