...
/// Rows the subtitle occupies (always shown — the info slot is gone).
fn subtitle_rows() -> u16 {
    1
}
...
pub(super) fn compute_hero_box(
    ...
    let hero_subtitle = if subtitle_rows() > 0 {
        Rect {
            x: right_x,
            y: inner.y + 1,
            width: right_width,
    ...
    let right_header_rows = 1 + subtitle_rows() + 1;
