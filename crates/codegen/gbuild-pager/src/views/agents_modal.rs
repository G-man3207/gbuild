...
use gbuild_agent::config::{AgentDefinition, AgentScope, BuiltinAgentName};
use gbuild_shell::agent::config::AgentSelectionConfig;
use gbuild_tools::implementations::skills::discovery::extract_first_paragraph;
...
/// User-level vs project-level config files (`~/.gbuild` vs `{cwd}/.gbuild`).
...
pub struct AgentsModalState {
    ...
    bundle: BundleState,
    /// Resolved startup agent name (same chain as shell: `[agent]`, `GBUILD_AGENT`,
    /// model `agentType`, then `gbuild`).
...
/// Skips internal variants (GBuildConcise, GBuildPlan,
/// GBuildPlanNoSubagents, GBuildAskUser, Codex, Opencode,
/// CursorExtended, GBuildOrchestrator).
...
pub fn build_agent_list(cwd: &Path, toggle: &HashMap<String, bool>) -> Vec<AgentListEntry> {
    ...
    let subagent_names: Vec<String> = BuiltinAgentName::subagent_variants()
        ...
        .collect();
    let discovered = gbuild_agent::discovery::discover(cwd);
...
/// Union bundled personas with local `~/.gbuild/personas` and `{cwd}/.gbuild/personas`.
///
...
pub fn merge_persona_lists(bundle: &BundleState, cwd: &Path) -> Vec<PersonaDetail> {
    ...
    let bundled_dir = gbuild_home.join("bundled").join("personas");
    ...
    let dirs = [
        (
            ...
            cwd.join(".gbuild").join("personas"),
        ),
        (ConfigFileScope::User, gbuild_home.join("personas")),
    ];
...
pub fn load_agent_toggle() -> HashMap<String, bool> {
    let root = match gbuild_shell::config::load_effective_config() {
        Ok(r) => r,
...
fn personas_dir_for_scope(scope: ConfigFileScope, cwd: &Path) -> PathBuf {
    match scope {
        ConfigFileScope::User => gbuild_config::gbuild_home().join("personas"),
        ConfigFileScope::Project => cwd.join(".gbuild").join("personas"),
    }
}
...
/// True when `path` is a deletable local persona file (user or project `.gbuild/personas`).
pub fn persona_path_is_deletable(path: &Path) -> bool {
    config_path_is_user_or_project(path, "personas")
}
/// Shared guard: canonical path under `~/.gbuild/{subdir}` or `{cwd}/.gbuild/{subdir}`, not bundled.
fn config_path_is_user_or_project(path: &Path, subdir: &str) -> bool {
    ...
    let gbuild_home = gbuild_config::gbuild_home();
    let in_user = dunce::canonicalize(gbuild_home.join(subdir))
        .ok()
        .is_some_and(|d| canonical.starts_with(&d));
    let project_suffix = std::path::Path::new(".gbuild").join(subdir);
...
fn load_agent_selection_config() -> AgentSelectionConfig {
    gbuild_shell::config::load_effective_config()
        .ok()
        .and_then(|root| gbuild_shell::agent::config::Config::new_from_toml_cfg(&root).ok())
        .map(|cfg| cfg.agent)
        .unwrap_or_default()
}
...
/// `MvpAgent::resolve_agent_definition` in gbuild-shell.
pub fn resolve_default_agent_name(cwd: &Path, model_agent_type: Option<&str>) -> String {
    ...
    gbuild_shell::agent::mvp_agent::MvpAgent::resolve_agent_definition(
        cwd,
        None,
        &agent_config,
        None,
...
pub fn format_agent_detail(entry: &AgentListEntry) -> Vec<String> {
    ...
    let mode_label = match def.prompt_mode {
        gbuild_agent::config::PromptMode::Extend => "extend",
        gbuild_agent::config::PromptMode::Full => "full",
    };
...
mod tests {
    use super::*;
    use gbuild_shell::agent::config::DEFAULT_AGENT_TYPE;
    #[test]
    ...
    fn merge_persona_lists_appends_local_only() {
        ...
        let personas_dir = dir.path().join(".gbuild").join("personas");
    ...
    fn persona_is_deletable_local_vs_bundled() {
        let dir = tempfile::tempdir().expect("tempdir");
        let local = dir.path().join(".gbuild").join("personas").join("p.toml");
        ...
        let bundled_detail = PersonaDetail {
            ...
            has_inputs: false,
            has_outputs: false,
            source_path: Some("/home/user/.gbuild/bundled/personas/b.toml".into()),
            scope_label: None,
        };
