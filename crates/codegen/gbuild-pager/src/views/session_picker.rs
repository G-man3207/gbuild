...
/// matters because the fast foreign scan can land rows the default gBuild view
...
///
/// Default is [`Self::GBuild`]: native gBuild sessions only (local / remote /
/// conversation), so `/resume` does not mix Claude/Codex/Cursor foreign
/// sessions into the list. `f` cycles gBuild → External → All → Local →
/// Remote — External first so one press from the default reveals foreign
/// sessions.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Default)]
pub enum SourceFilter {
    /// Native gBuild sessions only — excludes Claude/Codex/Cursor foreign rows.
    #[default]
    GBuild,
    Local,
    Remote,
...
impl SourceFilter {
    ...
    pub fn next(self) -> Self {
        match self {
            Self::GBuild => Self::External,
            Self::External => Self::All,
            Self::All => Self::Local,
            Self::Local => Self::Remote,
            Self::Remote => Self::GBuild,
        }
    }
    ...
    /// so they pass the `Remote` filter (and `gBuild` / `All`) but not `Local`.
    /// Foreign sources (`claude` / `codex` / `cursor`) only pass `External` and
    /// `All`.
    pub fn matches(self, source: &str) -> bool {
        match self {
            Self::GBuild => !crate::app::is_foreign_picker_source(source),
...
pub(crate) fn capture_picker_selection(
    entries: Option<&[SessionPickerEntry]>,
    content_results: Option<&[gbuild_shell::extensions::session_search::SearchSessionHit]>,
    state: &PickerState,
    query: &str,
    grouped: bool,
...
pub(crate) fn build_content_entry_data(
    hits: &[gbuild_shell::extensions::session_search::SearchSessionHit],
    entries_data: &[SessionPickerEntry],
    filtered_indices: &[usize],
    state: &PickerState,
    content_start: usize,
...
/// Hint shown on the default `gBuild` view when the foreign-session scan loaded
/// Claude/Codex/Cursor entries it hides. gBuild-only: `next(gBuild) == External`
...
pub(crate) fn hidden_external_hint(
    ...
    if source_filter != SourceFilter::GBuild {
        return None;
    }
    let hidden = entries?
        .iter()
...
mod tests {
    ...
    fn source_filter_matches() {
        // Default gBuild filter: native only (not Claude/Codex/Cursor).
        assert!(SourceFilter::GBuild.matches("local"));
        assert!(SourceFilter::GBuild.matches("remote"));
        assert!(SourceFilter::GBuild.matches("both"));
        assert!(SourceFilter::GBuild.matches("conversation"));
        assert!(!SourceFilter::GBuild.matches("claude"));
        assert!(!SourceFilter::GBuild.matches("codex"));
        assert!(!SourceFilter::GBuild.matches("cursor"));
        ...
        // grok.com conversations are remote: visible under gBuild + All + Remote, not Local.
    ...
    fn source_filter_cycles() {
        ...
        assert_eq!(SourceFilter::GBuild.next(), SourceFilter::External);
        ...
        assert_eq!(SourceFilter::Remote.next(), SourceFilter::GBuild);
        assert_eq!(SourceFilter::GBuild.label(), "gBuild");
        assert_eq!(SourceFilter::External.label(), "External");
        assert_eq!(SourceFilter::default(), SourceFilter::GBuild);
    }
    ...
    fn source_filter_filters_entries() {
        ...
        let gbuild = filter_session_entries(Some(&entries), "", SourceFilter::GBuild);
        assert_eq!(gbuild, vec![0, 1, 2]); // local + remote + both, no foreign
    ...
    fn source_filter_empty_and_unknown_source() {
        ...
        // foreign, so it passes gBuild + All but never Local, Remote, or External.
        assert!(SourceFilter::GBuild.matches(""));
        ...
        assert!(SourceFilter::GBuild.matches("unknown"));
    ...
    fn source_filter_is_active() {
        assert!(!SourceFilter::GBuild.is_active());
    ...
    fn hidden_external_hint_visibility() {
        ...
        // Only the default gBuild view surfaces the hint (with the count).
        assert_eq!(
            hidden_external_hint(Some(&entries), SourceFilter::GBuild).as_deref(),
        ...
        assert_eq!(
            hidden_external_hint(Some(&one), SourceFilter::GBuild).as_deref(),
        ...
        assert!(hidden_external_hint(Some(&native), SourceFilter::GBuild).is_none());
        assert!(hidden_external_hint(None, SourceFilter::GBuild).is_none());
    }
