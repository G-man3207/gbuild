...
fn picking_enum_esc_returns_to_browse() {
    let mut s = make_state();
    s.transition_to_picking_enum("theme", 0, SettingValue::Enum("gbuild_night"), true);
    let outcome = handle_settings_key(&mut s, &KeyEvent::new(KeyCode::Esc, KeyModifiers::NONE));
    match outcome {
        ...
        other => panic!("expected Action::PreviewTheme(\"gbuild_night\") on Esc, got {other:?}"),
    }
    assert!(matches!(s.mode(), SettingsModalMode::Browse));
}
...
//    the "Tip - Ask gBuild..." docs footer and the keybindings hints.
//
...
fn docs_footer_tip_is_centered() {
    ...
    // SHORT = "Tip - Ask gBuild to change a setting" (34 cells);
    ...
    assert!(
        !row_short.contains("gbuild_day"),
        "width=40 must NOT render LONG path (no `gbuild_day`): {row_short:?}",
    );
    assert!(
...
fn click_settings_breadcrumb_collapses_picker_to_browse() {
    ...
    // `"gbuild_night"`. Tightened from the previous `Action(_) |
    ...
    match outcome {
        ...
        other => panic!(
            "expected Action(PreviewTheme(\"gbuild_night\")) — the keyboard \
...
fn click_settings_breadcrumb_after_nav_reverts_to_original() {
    ...
    // Pick a different index. The default theme is `gbuild_night`
...
fn d_key_in_picking_enum_dispatches_open_reset_confirm() {
    ...
    match outcome {
        SettingsKeyOutcome::ActionPair(
            ...
            // Default theme is `gbuild_night`; entering the picker
            // captures `original_value = current value = gbuild_night`,
...
/// misleading: on dark themes (GBuildNight, TokyoNight, RosePine
/// Moon) `bg_visual` is actually *lighter* than `bg_highlight`;
/// only on the GBuildDay light theme is title darker. The
...
fn max_thoughts_width_preview_title_styling_distinguishes_from_content() {
    ...
    let raw_theme = match crate::theme::Theme::current_kind() {
        crate::theme::ThemeKind::GBuildNight => crate::theme::Theme::gbuild_night(),
        crate::theme::ThemeKind::TokyoNight => crate::theme::Theme::tokyonight(),
        crate::theme::ThemeKind::GBuildDay => crate::theme::Theme::gbuild_day(),
        ...
        crate::theme::ThemeKind::Auto => crate::theme::Theme::gbuild_night(),
    };
    assert_ne!(
