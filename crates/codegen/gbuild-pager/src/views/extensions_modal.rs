...
use gbuild_tools::implementations::skills::types::SkillInfo;
...
pub fn plugin_group(plugin: &xai_hooks_plugins_types::PluginInfo) -> PluginGroup {
    ...
    match &plugin.origin {
        Some(PluginOrigin::ProjectGBuild) => PluginGroup::new(0, "origin:project", "Project"),
        Some(PluginOrigin::ProjectClaude) => {
            PluginGroup::new(1, "origin:project-claude", "Project (Claude)")
        }
        Some(PluginOrigin::UserGBuild) => PluginGroup::new(2, "origin:user", "User"),
...
impl ExtensionsTab {
    ...
    pub fn telemetry_tab(self) -> gbuild_telemetry::events::ExtensionsModalTab {
        use gbuild_telemetry::events::ExtensionsModalTab;
        match self {
...
pub enum ButtonAction {
    ...
    AddMcpServer {
        name: String,
        config: Box<gbuild_shell::util::config::McpServerConfig>,
    },
...
pub fn derive_source_label(source_dir: &str) -> (String, bool) {
    let gbuild = gbuild_config::gbuild_home();
    let source_path = std::path::Path::new(source_dir);
    // Plugin / installed-plugin dirs, under the user gbuild home (GBUILD_HOME-aware)
    // or a project-scoped `{cwd}/.gbuild/<subdir>/`. Returns the first path
    ...
    let plugin_name = |subdir: &str| -> Option<String> {
        ...
        // User gbuild home (GBUILD_HOME-aware).
        if let Ok(rest) = source_path.strip_prefix(gbuild.join(subdir))
            && let Some(name) = first_comp(rest)
        {
            return Some(name);
        }
        // Project-scoped `.gbuild/<subdir>/<name>` anywhere in the path.
        ...
        comps
            .windows(3)
            .find(|w| w[0] == ".gbuild" && w[1] == subdir && !w[2].is_empty())
            .map(|w| w[2].clone())
    };
    if let Some(name) = plugin_name("plugins").or_else(|| plugin_name("installed-plugins")) {
        return (format!("Plugin: {name}"), false);
    }
    // Global hooks under $GBUILD_HOME/hooks
    let global_hooks = gbuild.join("hooks");
    ...
    // Project hooks
    if source_dir.ends_with("/.gbuild/hooks") || source_dir.contains("/.gbuild/hooks/") {
        return ("Project hooks".into(), false);
    }
    // Custom directory — removable
    let display = {
        if let Ok(rest) = source_path.strip_prefix(&gbuild) {
            let prefix = crate::util::display_gbuild_home_prefix();
...
fn skill_source_str(skill: &SkillInfo) -> String {
    if let Some(ref cs) = skill.config_source {
        match cs {
            gbuild_tools::types::config_source::ConfigSource::User { path } => {
                if crate::util::is_under_user_gbuild_home(path) {
                    crate::util::display_user_gbuild_path("skills")
            ...
            gbuild_tools::types::config_source::ConfigSource::Project { path } => {
                let s = path.display().to_string();
                if s.contains("/.gbuild/") {
                    ".gbuild/skills".into()
            ...
            gbuild_tools::types::config_source::ConfigSource::Plugin { plugin_name, .. } => {
...
mod tests {
    ...
    fn derive_source_label_detects_project_scoped_plugins() {
        // Regression: project-scoped `{cwd}/.gbuild/plugins/<name>/` must label as
        // a (non-removable) plugin, not a removable "Custom" source. The user
        // gbuild-home branch is GBUILD_HOME-aware; this covers the project fallback.
        let (label, is_custom) = derive_source_label("/repo/work/.gbuild/plugins/my-plugin/hooks");
        assert_eq!(label, "Plugin: my-plugin");
        assert!(!is_custom);

        let (label, is_custom) =
            derive_source_label("/repo/work/.gbuild/installed-plugins/vendor-abc123/skills");
    ...
    fn make_skill(
        name: &str,
        desc: &str,
    ) -> gbuild_tools::implementations::skills::types::SkillInfo {
        gbuild_tools::implementations::skills::types::SkillInfo {
            name: name.to_string(),
            display_name: None,
            ...
            metadata: None,
            path: "test".to_string(),
            scope: gbuild_tools::implementations::skills::types::SkillScope::User,
            config_source: None,
            plugin_name: None,
            plugin_version: None,
            plugin_root: None,
    ...
    fn make_plugin_skill(
        ...
        skill.scope = gbuild_tools::implementations::skills::types::SkillScope::Plugin;
    ...
    fn mcp_add_url_derives_name() {
        ...
        match action {
            Some(ButtonAction::AddMcpServer { name, config }) => {
                ...
                assert!(matches!(
                    config.transport,
                    gbuild_shell::util::config::McpServerTransportConfig::StreamableHttp { .. }
                ));
            }
        ...
        match action {
            Some(ButtonAction::AddMcpServer { name, config }) => {
                ...
                match config.transport {
                    gbuild_shell::util::config::McpServerTransportConfig::Stdio {
                        command,
                        args,
                        ..
                    } => {
    ...
    fn local_plugins_source() -> xai_hooks_plugins_types::MarketplaceScanResult {
        xai_hooks_plugins_types::MarketplaceScanResult {
            ...
            source_url_or_path: "/home/user/.gbuild/marketplace/local".into(),
            plugins: vec![
                TestPlugin {
                    name: "my-linter",
                    version: Some("0.1.0"),
    ...
    fn plugin_group_maps_each_origin_variant() {
        ...
        for (origin, rank, key, label) in [
            (PluginOrigin::ProjectGBuild, 0, "origin:project", "Project"),
            (
            ...
            (PluginOrigin::UserGBuild, 2, "origin:user", "User"),
            (
    ...
    fn plugin_group_fallback_without_origin() {
        ...
        assert_eq!(group.key, "gbuild-mp:xAI Official");
    ...
    fn plugin_group_unknown_origin_uses_scope_fallback() {
        ...
        assert_eq!(plugin_group(&unknown).key, "gbuild-mp:xAI Official");
    }
    ...
    fn plugins_render_groups_with_headers_in_rank_order() {
        ...
        let mut state = plugins_modal_state(vec![
            ...
            make_plugin_with_origin("user-tool", PluginOrigin::UserGBuild),
    ...
    fn plugins_render_multiple_plugins_under_one_group() {
        use xai_hooks_plugins_types::PluginOrigin;
        let mut state = plugins_modal_state(vec![
            make_plugin_with_origin("solo-tool", PluginOrigin::UserGBuild),
            make_plugin_with_origin(
                "catalog-tool",
                PluginOrigin::ClaudeMarketplace {
                    marketplace: "claude-market".into(),
    ...
    fn plugins_collapsed_group_hides_rows_and_search_forces_open() {
        use xai_hooks_plugins_types::PluginOrigin;
        let mut plugin = make_plugin_with_origin("user-tool", PluginOrigin::UserGBuild);
