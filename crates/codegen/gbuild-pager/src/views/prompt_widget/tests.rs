#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    fn cmd_a_image_chip_selection_is_path_free() {
        ...
        img.source_path = Some(PathBuf::from("/tmp/gbuild-test-image.png"));
        ...
        assert!(
            !full.contains("/tmp/gbuild-test-image.png"),
            "source path must not appear in the buffer chip: {full:?}"
        );
        assert_eq!(
            pw.images[0].source_path.as_deref(),
            Some(std::path::Path::new("/tmp/gbuild-test-image.png")),
        ...
        assert!(selected.contains("[Image #1]"));
        assert!(
            !selected.contains("/tmp/gbuild-test-image.png"),
    ...
    fn fuzzy_result(path: &str, is_dir: bool) -> gbuild_workspace::file_system::FuzzyMatchResult {
        gbuild_workspace::file_system::FuzzyMatchResult {
