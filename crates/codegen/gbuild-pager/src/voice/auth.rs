...
use gbuild_voice::{SharedVoiceAuth, VoiceAuthProvider};
...
pub fn build_voice_auth(auth_manager: Arc<gbuild_shell::auth::AuthManager>) -> SharedVoiceAuth {
    Arc::new(AuthManagerVoiceAuth(
        gbuild_shell::auth::shared_api_key_provider(auth_manager),
    ))
}
