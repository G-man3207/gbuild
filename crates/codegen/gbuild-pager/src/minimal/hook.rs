...
//! render mode.
//!
//! `gbuild-pager` cannot depend on `gbuild-pager-minimal`: that crate reads
...
//!
//! The composition-root binary (`gbuild-pager-bin`) wires it up once at
//! startup by calling `gbuild_pager_minimal::install()`. When nothing has
