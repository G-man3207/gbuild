...
/// firehose (`GBUILD_DEBUG_LOG` / `GBUILD_LOG_FILE`).
pub use gbuild_telemetry::debug_log::ACP_UPDATE_PAYLOAD_TARGET;
/// Target for the always-on compact ACP update summary line (kind, ids,
/// status, payload sizes). Cheap to format at streaming rate.
///
/// Defined in `gbuild-telemetry` so the firehose directives and the pager
/// filter share one constant (re-exported here for callsites).
pub use gbuild_telemetry::debug_log::ACP_UPDATE_TARGET;
...
pub fn init_tracing() -> TracingHandle {
    ...
    let directives = format!(
        "gbuild_shell=info,gbuild_pager=trace,gbuild_tools=info,xai_acp_lib=info,{RMCP_SSE_NOISE_TARGET}=error,sampling_log=off,{ACP_UPDATE_TARGET}=debug,{ACP_UPDATE_PAYLOAD_TARGET}={payload_level}"
    );
    ...
    let otel_layer = gbuild_telemetry::otel_layer::build_otel_layer(
    ...
    let registry = tracing_subscriber::registry()
        ...
        .with(otel_layer);
    gbuild_telemetry::debug_log::install_firehose(registry, "tui");
