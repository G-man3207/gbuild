//! New, exit, cloud, and worktree session dispatchers plus startup actions.
...
/// Replay the session-startup actions deferred until auth resolved
/// (`--resume` / `--worktree` / initial-prompt / `gbuild dashboard`). Extracted
/// from the `AuthComplete` handler.
