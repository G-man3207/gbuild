...
use crate::app::app_view::{
    ActiveView, AppView, AuthMode, AuthState, VoiceState, VoiceTarget,
};
...
/// Plant a Build session under the process `gbuild_home()` (OnceLock-cached;
/// do not rely on setting `GBUILD_HOME` mid-process). Caller must remove `sess_dir`.
fn plant_local_build_session(cwd: &std::path::Path, session_id: &str) -> std::path::PathBuf {
    ...
    let encoded = gbuild_shell::util::gbuild_home::encode_cwd_dirname(&cwd.to_string_lossy());
...
fn make_ask_user_question_args(
    ...
    let req = AskUserQuestionExtRequest {
        ...
        mode:
            gbuild_tools::implementations::gbuild::ask_user_question::AskUserQuestionMode::Default,
        ...
        vec![
            acp::PermissionOption::new(
                acp::PermissionOptionId::new(Arc::from(
                    gbuild_workspace::permission::ENABLE_ALWAYS_APPROVE_OPTION_ID,
                )),
...
const POLICY_WARNING: &str = gbuild_workspace::permission::resolution::YOLO_PIN_REASON_REQUIREMENTS;
