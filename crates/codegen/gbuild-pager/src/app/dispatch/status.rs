...
pub(super) fn dispatch_show_privacy_info(app: &mut AppView) -> Vec<Effect> {
    ...
    lines.push("  - [features] telemetry / GBUILD_TELEMETRY_ENABLED");
    lines.push("  - [telemetry] trace_upload / GBUILD_TELEMETRY_TRACE_UPLOAD");
    lines.push("  - GBUILD_EXTERNAL_OTEL / OTEL_*");
    lines.push("");
...
/// (persists via ACP ext-request, NOT `~/.gbuild/config.toml`).
...
pub(super) fn append_consumer_billing_surface(app: &mut AppView, agent_id: AgentId) -> Vec<Effect> {
    ...
    // Remote-settings kill switch (`gbuild_usage_redirect_url`): link out
