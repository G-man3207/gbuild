...
/// Whether a completed clipboard probe should fall through to the `gbuild wrap`
/// host-image request. A clean `FullMiss` always qualifies; a remote read
/// *error* (`AttachmentRead`) also qualifies because inside `gbuild wrap` the
...
/// `osc52_sink_active()`, so this is inert outside `gbuild wrap`.
        ...
        TaskResult::ForeignResumeCwdCanonicalized {
                ...
                && let Some(canonical_cwd) = accepted_cwd
            {
                vec![Effect::DetectForeignResumeHint {
                    ...
                    gbuild_home: gbuild_tools::util::gbuild_home::gbuild_home(),
                    launch_token,
                }]
            } else {
                vec![]
