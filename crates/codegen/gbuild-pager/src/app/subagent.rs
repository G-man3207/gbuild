...
thread_local! {
    static REPLAY_GBUILD_HOME: std::cell::RefCell<Option<std::path::PathBuf>> =
        const { std::cell::RefCell::new(None) };
}
/// Override gBuild home for disk-replay unit tests (thread-local; production never sets this).
#[cfg(test)]
pub(crate) fn set_replay_gbuild_home_for_tests(home: Option<std::path::PathBuf>) {
    REPLAY_GBUILD_HOME.with(|h| *h.borrow_mut() = home);
}
fn effective_gbuild_home() -> std::path::PathBuf {
    if let Some(home) = REPLAY_GBUILD_HOME.with(|h| h.borrow().clone()) {
        return home;
    }
...
pub(crate) fn replay_inherited_updates(
    ...
    let home = effective_gbuild_home();
    let updates = match gbuild_shell::session::storage::load_updates_for_replay_at(
        child_session_id,
        &home,
    ) {
        Ok(Some(u)) => u,
...
mod tests {
    ...
    fn ensure_subagent_child_replayed_releases_retained_memory_once() {
        ...
        set_replay_gbuild_home_for_tests(Some(home.path().to_path_buf()));
        let mut parent = make_min_child_view();
        parent
            .subagent_views
        ...
        set_replay_gbuild_home_for_tests(None);
    }
    #[test]
