...
pub(crate) async fn run(
    ...
    if args.log_sampling {
        ...
        unsafe { std::env::set_var("GBUILD_LOG_SAMPLING", "1") };
    }
    ...
    let launch_auto = gbuild_shell::util::config::effective_auto_for_launch(
        args.yolo,
        args.permission_mode_flag.as_deref(),
        remote_permission_mode,
    );
    ...
    let launch_effective_ui = gbuild_shell::config::load_effective_config()
        .ok()
    ...
    let toml_owns_mode = launch_effective_ui
        .as_ref()
        .and_then(gbuild_shell::util::config::permission_mode_from_ui_if_set)
        .is_some();
    ...
    if let Some(ref agent) = args.agent {
        match crate::headless::resolve_agent_arg(agent) {
            crate::headless::ResolvedAgent::FilePath(path) => {
                match gbuild_shell::agent::config::AgentDefinition::from_file(&path) {
    ...
    app.privacy_notice_rollout = gbuild_config::env_bool("GBUILD_PRIVACY_NOTICE_ROLLOUT")
        ...
        .unwrap_or(false);
    app.privacy_banner_reshow_days = std::env::var("GBUILD_PRIVACY_BANNER_RESHOW_DAYS")
        .ok()
    ...
    app.privacy_banner_acked = gbuild_shell::config::load_from_disk()
        .ok()
        .and_then(|root| {
            gbuild_shell::util::config::load_config_from_toml(&root)
                .privacy
                .privacy_banner_acked
        });
    app.plugin_cta_enabled = gbuild_config::env_bool("GBUILD_PLUGIN_CTA")
        .or_else(|| remote_settings.as_ref().and_then(|s| s.plugin_cta))
        .unwrap_or(false);
    // Voice is applied after auth_meta so API-key detection is accurate.
    app.session_picker_grouped = std::env::var("GBUILD_SESSION_PICKER_GROUPED")
        .ok()
        ...
        .or_else(|| {
            gbuild_shell::config::load_effective_config()
                .ok()
    ...
    let mut post_render_effects = if needs_interactive_login {
        if connection.auth_methods.is_empty() {
            ...
            app.auth_state = super::app_view::AuthState::Pending {
                error: Some(
                    gbuild_shell::agent::auth_method::PREFERRED_API_KEY_UNAVAILABLE.to_string(),
                ),
            };
            vec![]
        } else {
    ...
    if let Some(meta) = connection.auth_meta.as_ref() {
        match serde_json::from_value::<gbuild_shell::auth::AuthMeta>(meta.clone()) {
        ...
        app.is_api_key_auth = app
            .auth_methods
            .iter()
            .any(|m| m.id().0.as_ref() == gbuild_shell::agent::auth_method::XAI_API_KEY_METHOD_ID);
    ...
    let effective_config = match gbuild_shell::config::load_effective_config() {
        Ok(raw) => Some(raw),
        Err(e) => {
    ...
    let compat = gbuild_shell::agent::config::resolve_compat_sessions_from_raw(
        effective_config.as_ref().ok_or(()),
        remote_settings.as_ref(),
    );
    app.foreign_session_compat = gbuild_workspace::foreign_sessions::EnabledForeignSessionSources {
    ...
    if let Some(ref raw) = effective_config {
        ...
        if let Some(table) = raw.as_table() {
            ...
            // (config > GBUILD_XAI_API_BASE_URL env > default).
            let endpoints_base =
                gbuild_shell::agent::config::EndpointsConfig::from_config_value(raw)
                    .xai_api_base_url;
            app.voice_config =
                gbuild_voice::VoiceConfig::from_config_table(table, Some(&endpoints_base));
        }
    }
    ...
    app.zdr_access_enabled = gbuild_shell::util::config::resolve_zdr_access_enabled(
        requirements.as_ref(),
        user_config.as_ref(),
    ...
    crate::appearance::cache::set_show_thinking_blocks(
        gbuild_shell::util::config::resolve_show_thinking_blocks(
            requirements.as_ref(),
            user_config.as_ref(),
            managed_config.as_ref(),
            remote_settings.as_ref(),
    ...
    crate::appearance::cache::set_group_tool_verbs(
        gbuild_shell::util::config::resolve_group_tool_verbs(
            requirements.as_ref(),
            user_config.as_ref(),
            managed_config.as_ref(),
            remote_settings.as_ref(),
    ...
    crate::appearance::cache::set_collapsed_edit_blocks(
        gbuild_shell::util::config::resolve_collapsed_edit_blocks(
            requirements.as_ref(),
            user_config.as_ref(),
            managed_config.as_ref(),
            remote_settings.as_ref(),
...

    {
        ...
        if !app.tips.is_empty() {
            ...
            app.tip = gbuild_shell::util::tips::pick_and_advance(&app.tips, &gbuild_home);
        }
    ...
    let hints = gbuild_shell::util::config::resolve_hints(
    ...
    let display_mode: &'static str = if launch_auto {
        ...
        gbuild_shell::util::config::clamped_display_permission_mode(
            gbuild_shell::util::config::parse_permission_mode_canonical(cli),
        )
    } else {
        gbuild_shell::util::config::resolved_display_permission_mode(
    ...
    let resolved_hints = gbuild_shell::util::config::resolve_contextual_hints(
    ...
    let mouse_toggle = gbuild_shell::util::config::resolve_mouse_reporting_toggle(
        effective_config.as_ref(),
        &app.current_ui,
    );
    ...
    // GBUILD_SCROLL_SPEED/_MODE/_LINES + GBUILD_INVERT_SCROLL env overrides
    // apply on first load).
    ...
    let mut voice_rx = None::<tokio::sync::mpsc::Receiver<gbuild_voice::VoiceEvent>>;
    ...
    // Initial prompt from the CLI positional (`gbuild "fix the bug"`). When
    ...
    // `gbuild dashboard` startup: open the dashboard view immediately. The
    // CLI subcommand wrote a `GBUILD_OPEN_DASHBOARD_AT_STARTUP=1` env var
    // so we don't have to thread a flag through every arg struct.
    if std::env::var("GBUILD_OPEN_DASHBOARD_AT_STARTUP").as_deref() == Ok("1") {
    ...
    loop {
        ...
        if let VoiceState::ColdStart { hold, target } = app.voice_state {
            if app.voice_cmd_tx.is_none() && app.voice_can_start_pipeline() {
                ...
                tokio::spawn(gbuild_voice::run_voice_pipeline(
                    voice_config,
                    voice_auth.clone(),
                    cmd_rx,
                    event_tx,
                ...
                app.voice_state = VoiceState::Idle;
                app.voice_ui_active = false;
                app.show_toast("Voice could not start. Restart gBuild.");
            } else {
        ...
        tokio::select! {
            ...
            Ok(()) = config_watcher.changed() => {
                ...
                // Preserve fields persisted via `~/.gbuild/config.toml [ui]`
                // rather than `~/.gbuild/pager.toml`. The watcher only knows
                        ...
                        let join_handle = tokio::spawn(async move {
                            ...
                            let ok = tokio::time::timeout(timeout, async {
                                ...
                                for (agent_id, plan) in load_plans {
                                    ...
                                    let mcp_servers = gbuild_shell::util::config::load_mcp_servers(
                                        &plan.cwd,
...
pub(crate) fn load_initial_ui_config() -> gbuild_shell::agent::config::UiConfig {
    use gbuild_shell::agent::config::UiConfig;
    let Ok(root) = gbuild_shell::config::load_effective_config() else {
