...
//!
//! Env sandboxing follows this crate's `serial(GBUILD_HOME)` idiom; note
//! `gbuild_home()` is process-cached (OnceLock), so disk assertions always go
//! through [`effective_gbuild_home`] rather than assuming the temp dir won.
//!
//! The scenarios are `#[ignore]`d in the shared lib test binary: the harness
//! mutates process-global env (proxy URLs, `XAI_API_KEY`,
//! `GBUILD_LEADER_SOCKET`, `GBUILD_HOME`) for a real agent's whole lifetime, and
//! in a several-thousand-test process that poisons concurrently-running tests
//! (and `gbuild_home()`'s OnceLock is usually already pinned). Run on demand:
//!
//! ```bash
//! cargo test -p gbuild-pager --lib -- app::leader_cluster --ignored --test-threads=1
//! ```
//!
...
/// The gBuild home the agent actually persisted under: `gbuild_home()` is
/// process-cached, so an earlier test in this binary may have pinned it.
fn effective_gbuild_home() -> PathBuf {
    gbuild_config::gbuild_home()
}
...
struct PagerLeaderCluster {
    ...
    /// agent on the same GBUILD_HOME (two agents on one updates.jsonl is the
    ...
    _gbuild_home: TempDir,
}

impl PagerLeaderCluster {
    /// Stand up the cluster. Callers MUST be `#[serial_test::serial(GBUILD_HOME)]`
    ...
    async fn start() -> Self {
        ...
        let sock_path = gbuild_home.path().join("leader-cluster.sock");

        let env = vec![
            crate::test_util::EnvVarGuard::set("GBUILD_HOME", gbuild_home.path()),
            crate::test_util::EnvVarGuard::set("GBUILD_CLI_CHAT_PROXY_BASE_URL", server.url()),
            crate::test_util::EnvVarGuard::set("GBUILD_XAI_API_BASE_URL", server.url()),
            crate::test_util::EnvVarGuard::set("XAI_API_KEY", "test-key-for-ci"),
            crate::test_util::EnvVarGuard::set("GBUILD_TELEMETRY_ENABLED", "false"),
            crate::test_util::EnvVarGuard::set("GBUILD_FEEDBACK_ENABLED", "false"),
            crate::test_util::EnvVarGuard::set("GBUILD_TRACE_UPLOAD", "false"),
    ...
    async fn spawn_leader_generation(&mut self) {
        ...
        // gbuild-shell/src/agent/app.rs (inside its LocalSet body) — a
        // deliberate copy so production stays untouched. Second copy of the
        // same wiring: gbuild-shell/tests/test_leader_soak.rs ("Real agent
    ...
    async fn kill_leader(&mut self) {
        ...
        // be running against the same GBUILD_HOME when the next generation's
    ...
    /// `LeaderReconnector` (socket pinned via `GBUILD_LEADER_SOCKET`, flock held
