...
/// `gbuild_settings.subscription_watch_interval_secs` field.
...
/// Floor for the `GBUILD_SUBSCRIPTION_WATCH_INTERVAL_SECS` env override
...
impl AppView {
    ...
    pub fn subscription_watch_interval(&self) -> Option<std::time::Duration> {
        if let Ok(v) = std::env::var("GBUILD_SUBSCRIPTION_WATCH_INTERVAL_SECS")
    ...
    #[must_use]
    pub fn impose_gate(&mut self, gate: gbuild_shell::auth::GateInfo) -> Vec<Effect> {
    ...
    pub fn lift_gate(&mut self) -> Vec<Effect> {
        ...
        gbuild_telemetry::session_ctx::log_event(gbuild_telemetry::events::SubscriptionActivated {
    ...
    #[must_use]
    fn defer_gate_for_verification(&mut self, gate: gbuild_shell::auth::GateInfo) -> Vec<Effect> {
...
mod tests {
    ...
    fn watch_gate() -> gbuild_shell::auth::GateInfo {
    ...
    fn impose_gate_direct_for_non_consumer_and_already_gated() {
        ...
        let new_copy = gbuild_shell::auth::GateInfo {
