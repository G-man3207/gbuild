...
pub enum DeferredSessionStartup {
    ...
    /// Fresh plain gBuild session whose first prompt resumes a foreign tool session.
    ForeignResume {
        tool: gbuild_workspace::foreign_sessions::ForeignSessionTool,
        native_id: String,
    },
}
...
pub fn fork_session_params(
    ...
    let source_cwd = gbuild_shell::session::resolve_local_session_any_cwd(parent_session_id)
...
pub fn parent_session_is_worktree(session_id: &str, cwd: &Path) -> bool {
    ...
    let sessions_root = gbuild_shell::util::gbuild_home::gbuild_home().join("sessions");
    let encoded = gbuild_shell::util::gbuild_home::encode_cwd_dirname(&cwd_str);
...
pub fn local_build_session_on_disk(session_id: &str, cwd: &Path) -> bool {
    let cwd_str = cwd.to_string_lossy();
    gbuild_shell::session::resolve_local_session(session_id, &cwd_str).is_some()
}
...
async fn most_recent_session_id(cwd: &str) -> anyhow::Result<(String, Option<String>)> {
    let summaries = gbuild_shell::session::persistence::list_summaries(Some(cwd)).await?;
    let first = summaries.first().ok_or_else(|| {
        anyhow::anyhow!(
            "No session found for current directory. \
             Use 'gbuild' to start a new session."
        )
    })?;
...
pub(crate) fn pre_acp_auth_manager(
    agent_config: &gbuild_shell::agent::config::Config,
) -> std::sync::Arc<gbuild_shell::auth::AuthManager> {
    let auth = std::sync::Arc::new(gbuild_shell::auth::AuthManager::new(
...
pub fn ensure_session_id_available(session_id: &str, cwd: &str) -> anyhow::Result<()> {
    ...
    if gbuild_shell::session::persistence::session_exists_for_cwd(session_id, cwd) {
...
async fn resolve_existing_session(
    ...
    if let Some(local_id) = gbuild_shell::session::resolve_local_session(session_id, cwd) {
    ...
    if let Some(original_cwd) = gbuild_shell::session::resolve_local_session_any_cwd(session_id) {
        tracing::info!(
...
async fn restore_session_from_remote(
    ...
    if let Some((false, source)) =
        gbuild_shell::util::config::session_registry_local_override_sourced(Some(&raw_config))
    {
        anyhow::bail!(
    ...
    let agent_config = gbuild_shell::agent::config::Config::new_from_toml_cfg(&raw_config)
        .map_err(|e| anyhow::anyhow!("Failed to create agent config: {}", e))?;
    use gbuild_shell::agent::session_registry_client::SessionRegistryClient;
    use gbuild_shell::auth::{AuthManager, ensure_authenticated_or_noninteractive};
    ...
    let storage_client = gbuild_shell::auth::credential_provider::build_storage_client_for_proxy(
    ...
    let progress: gbuild_shell::session::restore::ProgressCallback =
...
mod tests {
    ...
    fn intent_default_is_new_auto() {
        assert_eq!(
            parse(&["gbuild"]).session_startup_intent().unwrap(),
    ...
    fn intent_continue() {
        assert_eq!(
            parse(&["gbuild", "-c"]).session_startup_intent().unwrap(),
            SessionStartupIntent::Resume {
                session_id: None,
                most_recent_for_cwd: true,
            }
    ...
    fn intent_fork_with_resume() {
        assert_eq!(
            parse(&["gbuild", "-r", "old", "--fork-session"])
                .session_startup_intent()
                .unwrap(),
            SessionStartupIntent::ForkFrom {
    ...
    fn intent_fork_with_resume_and_new_id() {
        assert_eq!(
            parse(&["gbuild", "-r", "old", "--fork-session", "-s", "new"])
                .session_startup_intent()
                .unwrap(),
            SessionStartupIntent::ForkFrom {
                source_session_id: Some("old".into()),
    ...
    fn intent_fork_alone_errors() {
        let err = parse(&["gbuild", "--fork-session"])
            .session_startup_intent()
            .unwrap_err();
    ...
    fn intent_fork_with_worktree_errors() {
        let err = parse(&["gbuild", "-r", "a", "--fork-session", "-w"])
            .session_startup_intent()
            .unwrap_err();
    ...
    fn intent_from_flags_matches_pager_args() {
        let args = parse(&["gbuild", "-r", "old", "--fork-session", "-s", "new"]);
    ...
    fn materialize_ctx_chat_mode_from_args() {
        assert!(!MaterializeCtx::from_pager_args(&parse(&["gbuild"])).chat_mode);
    }
    /// hardcoded `false` here once disabled it everywhere.
    #[test]
    fn remote_restore_follows_compiled_restore_stack() {
        assert_eq!(
            MaterializeCtx::from_pager_args(&parse(&["gbuild"])).allow_remote_restore,
            false
        );
    }
    ...
    async fn chat_resume_passthrough_keeps_cwd_collision_refusal() {
        ...
        let encoded = gbuild_shell::util::gbuild_home::encode_cwd_dirname(&cwd_str);
        let sessions_cwd_dir = gbuild_shell::util::gbuild_home::gbuild_home()
            .join("sessions")
            .join(&encoded);
    ...
    mod resume_by_title {
        use super::*;
        use crate::test_util::GBuildHomeFixture;
        ...
        async fn title_fallback_resumes_single_match_case_insensitively() {
            let mut fx = GBuildHomeFixture::new();
