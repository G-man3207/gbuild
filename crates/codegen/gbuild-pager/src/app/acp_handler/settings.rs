...
pub(super) fn handle_settings_update(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    ...
    if let Some(v) = update.auto_permission_mode_enabled {
        ...
        // without a restart.
        gbuild_shell::util::config::cache_remote_auto_permission_mode_enabled(Some(v));
        app.auto_mode_gate = gbuild_shell::util::config::auto_permission_mode_enabled_from_disk();
        ...
        && app.permission_mode_from_soft_default
    {
        ...
        let root = gbuild_shell::config::load_effective_config().ok();
    ...
    if let Some(v) = update.privacy_notice_rollout {
        app.privacy_notice_rollout =
            gbuild_config::env_bool("GBUILD_PRIVACY_NOTICE_ROLLOUT").unwrap_or(v);
    }
    if let Some(v) = update.privacy_banner_reshow_days {
        app.privacy_banner_reshow_days = Some(
            std::env::var("GBUILD_PRIVACY_BANNER_RESHOW_DAYS")
                .ok()
    ...
    if let Some(v) = update.subscription_tier_display {
        ...
        if was_api_key
            ...
            && app
                .subscription_tier
                .as_deref()
                .is_some_and(gbuild_shell::tier::is_restricted_tier_name)
        {
            app.voice_reset();
    ...
    if let Some(remote_val) = update.session_picker_grouped {
        let resolved = std::env::var("GBUILD_SESSION_PICKER_GROUPED")
            .ok()
        ...
        && !msg.is_empty()
    {
        ...
        let effs = app.impose_gate(gbuild_shell::auth::GateInfo {
            message: msg.clone(),
    ...
    let (requirements, user_config, managed_config) = (
        gbuild_shell::config::load_merged_requirements(),
        gbuild_shell::config::load_from_disk().ok(),
        gbuild_shell::config::load_managed_config().ok(),
    );
    ...
    // until restart.
    let remote = gbuild_shell::util::config::RemoteSettings {
        group_tool_verbs: update.group_tool_verbs,
        ..Default::default()
    };
    let resolved = gbuild_shell::util::config::resolve_group_tool_verbs(
        requirements.as_ref(),
        user_config.as_ref(),
    ...
    let resolved = gbuild_shell::util::config::resolve_collapsed_edit_blocks(
        requirements.as_ref(),
        user_config.as_ref(),
        managed_config.as_ref(),
        Some(&remote),
    ...
    if let Some(remote_tips) = update.tips {
        use gbuild_shell::util::config::resolve_tips;
    ...
    if let Some(remote_tags) = update.slash_command_tags.as_ref() {
        use gbuild_shell::util::config::resolve_slash_command_tags;
        let effective_config = gbuild_shell::config::load_effective_config().ok();
...
/// `gbuild-shell/src/agent/mvp_agent.rs`. The shell side derives `Serialize`
...
/// `gbuild-shell/src/agent/mvp_agent.rs` when adding fields that both sides
/// need.
