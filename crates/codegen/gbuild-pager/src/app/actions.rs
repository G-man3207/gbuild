...
pub enum SwitchModelError {
    ...
    /// `acp::Error.data`.
    IncompatibleAgent {
        error: gbuild_shell::agent::config::ModelSwitchIncompatibleAgentError,
...
pub enum Action {
    ...
    OpenTranscriptPager,
    /// Minimal mode (`gbuild --minimal`): re-print the most-recently committed
    ...
    OpenExtensionsModal {
        ...
        trigger: gbuild_telemetry::events::ExtensionsModalTrigger,
    },
    ...
    /// Commit the active theme (canonical name, e.g. `"gbuild_night"`, `"auto"`).
    SetTheme(String),
    ...
    /// Open the Agent Dashboard view (`/dashboard`, `Ctrl+\`, `gbuild dashboard`).
    OpenDashboard,
...
///
/// Both variants write to `~/.gbuild/config.toml` and route ACP
...
pub enum Effect {
    ...
    ScanForeignSessions {
        cwd: std::path::PathBuf,
        compat: gbuild_workspace::foreign_sessions::EnabledForeignSessionSources,
        gbuild_home: std::path::PathBuf,
    ...
    /// Persist the dashboard's `[dashboard]` configuration to `~/.gbuild/config.toml`.
    ...
    /// Persist a typed setting to `~/.gbuild/config.toml`. On failure,
    ...
    /// registry (`~/.gbuild/active_sessions.json`).
...
pub enum TaskResult {
    ...
    SessionListLoaded {
        ...
        scope: gbuild_shell::session::unified_list::ListScope,
    ...
    ForeignResumeHintDetected {
        canonical_cwd: std::path::PathBuf,
        launch_token: u64,
        hint: Option<gbuild_workspace::foreign_sessions::RecentForeignSession>,
    },
    ...
    BgTaskKilled {
        session_id: String,
        task_id: String,
        outcome: Option<gbuild_tools::types::KillOutcome>,
    },
    ...
    SkillsListLoaded {
        agent_id: AgentId,
        result: Result<Vec<gbuild_tools::implementations::skills::types::SkillInfo>, String>,
    },
    WorkflowsListLoaded {
        agent_id: AgentId,
        session_id: acp::SessionId,
    ...
    SessionInfoComplete {
        agent_id: AgentId,
        info: Box<gbuild_shell::session::SessionInfoResponse>,
        text: String,
    },
    ...
    SessionUsageComplete {
        agent_id: AgentId,
        session_id: acp::SessionId,
        usage: Box<gbuild_shell::extensions::notification::PromptUsage>,
    },
    ...
    DeepSearchResults {
        results: Vec<gbuild_shell::extensions::session_search::SearchSessionHit>,
        seq: u64,
    },
