...
See ~/.gbuild/README.md for more information.
")]
    Wrap(WrapArgs),
    ...
    /// `GBUILD_WORKSPACE_COMMAND=1` to enable it locally for testing.
    #[command(hide = true)]
    ...
    /// `~/.gbuild/config.toml` or when the `GBUILD_AGENT_DASHBOARD=0` env
    /// var is set.
    Dashboard,
}
...
pub struct ServeArgs {
    ...
    #[arg(long, env = "GBUILD_AGENT_SECRET")]
...
pub struct PagerArgs {
    ...
    #[arg(long)]
    pub cwd: Option<PathBuf>,
    /// Use a custom leader socket path instead of the default `~/.gbuild/leader.sock`.
    #[arg(
        long = "leader-socket",
        value_name = "PATH",
        global = true,
    ...
    /// persists per-segment markdown to grep. Sets `GBUILD_COMPACTION_MODE`.
    ...
    #[arg(
        ...
        value_parser = clap::builder::PossibleValuesParser::new(
            gbuild_shell::agent::config::PermissionMode::VALID_VALUES
        )
    )]
    ...
    #[arg(long, env = "GBUILD_SANDBOX", value_name = "PROFILE")]
    ...
    /// minimal, set `[ui] screen_mode = "minimal"` in ~/.gbuild/config.toml.
    #[arg(long = "minimal")]
    pub minimal: bool,
    ...
    pub fullscreen: bool,
    /// Write sampling events to ~/.gbuild/logs/sampling.jsonl.
    #[arg(long = "log-sampling", env = "GBUILD_LOG_SAMPLING", hide = true)]
...
impl PagerArgs {
    ...
    /// explicit `--sandbox`/`GBUILD_SANDBOX` that differs from the saved profile is
    ...
    pub fn saved_resume_profile_for_cwd(&self, cwd: Option<&str>) -> Option<String> {
        ...
        match self.resume_target() {
            ResumeTarget::SessionId(id) => {
                gbuild_shell::session::persistence::resumed_session_sandbox_profile(Some(&id), cwd)
            }
            ResumeTarget::MostRecentForCwd => {
                gbuild_shell::session::persistence::resumed_session_sandbox_profile(None, cwd)
            }
    ...
    fn resolve_startup_sandbox(explicit: Option<&str>, saved: Option<String>) -> SandboxStartup {
        match (explicit, saved) {
            (Some(x), Some(s))
                if x.parse::<gbuild_sandbox::ProfileName>().ok()
                    != s.parse::<gbuild_sandbox::ProfileName>().ok() =>
            {
    ...
    /// whitespace. This is the `gbuild "<prompt>"` launch form; the headless
...
mod tests {
    ...
    fn version_flags_parse_as_early_intent_without_exiting() {
        for flag in ["--version", "-v", "-V"] {
            let args = PagerArgs::try_parse_from(["gbuild", flag]).expect("version flag parses");
    ...
    fn ordinary_and_doctor_parsing_do_not_set_version_intent() {
        assert!(!PagerArgs::try_parse_from(["gbuild"]).unwrap().version);
        assert!(
            !PagerArgs::try_parse_from(["gbuild", "doctor"])
                .unwrap()
                .version
        );
        assert!(matches!(
            PagerArgs::try_parse_from(["gbuild", "version"])
                .unwrap()
                .command,
    ...
    fn doctor_accepts_report_and_explicit_fix_forms() {
        let bare = PagerArgs::try_parse_from(["gbuild", "doctor"]).expect("bare doctor parses");
        assert!(matches!(
            bare.command,
        ...
        let json = PagerArgs::try_parse_from(["gbuild", "doctor", "--json"])
        ...
        for id in [
            ...
            let fix = PagerArgs::try_parse_from(["gbuild", "doctor", "fix", id, "--yes"])
        ...
        let list = PagerArgs::try_parse_from(["gbuild", "doctor", "fix"])
        ...
        for unsupported in [
            vec!["gbuild", "doctor", "all"],
            vec!["gbuild", "doctor", "fix", "ssh-wrap", "extra"],
            vec!["gbuild", "doctor", "fix", "--yes"],
            vec!["gbuild", "doctor", "--json", "fix", "terminal.ssh-wrap"],
        ] {
    ...
    fn resume_target_classifies_flags() {
        ...
        assert_eq!(
            PagerArgs::try_parse_from(["gbuild", "-c"])
                .unwrap()
                .resume_target(),
            ResumeTarget::MostRecentForCwd
        );
        assert_eq!(
            PagerArgs::try_parse_from(["gbuild", "--resume"])
                .unwrap()
                .resume_target(),
            ResumeTarget::MostRecentForCwd
        );
        assert_eq!(
            PagerArgs::try_parse_from(["gbuild", "--resume", "sess-1"])
                .unwrap()
                .resume_target(),
            ResumeTarget::SessionId("sess-1".to_string())
        );
        assert_eq!(
            PagerArgs::try_parse_from(["gbuild", "-s", "sess-2"])
                .unwrap()
                .resume_target(),
            ResumeTarget::None
        );
        assert_eq!(
            PagerArgs::try_parse_from(["gbuild", "-r", "old", "--fork-session"])
                .unwrap()
                .resume_target(),
    ...
    fn minimal_and_fullscreen_flags_conflict() {
        let args = PagerArgs::try_parse_from(["gbuild", "--minimal"]).unwrap();
        assert!(args.minimal && !args.fullscreen);
        let args = PagerArgs::try_parse_from(["gbuild", "--fullscreen"]).unwrap();
        assert!(args.fullscreen && !args.minimal);
        let err = PagerArgs::try_parse_from(["gbuild", "--minimal", "--fullscreen"]).unwrap_err();
    ...
    fn startup_sandbox_profile_no_resume() {
        assert_eq!(
            PagerArgs::try_parse_from(["gbuild", "--sandbox", "strict"])
                .unwrap()
                .startup_sandbox_profile(None),
            SandboxStartup::Apply(Some("strict".to_string()))
        );
        assert_eq!(
            PagerArgs::try_parse_from(["gbuild", "--sandbox", ""])
                .unwrap()
                .startup_sandbox_profile(None),
    ...
    fn leader_socket_flag_parses_at_root() {
        let args = PagerArgs::try_parse_from(["gbuild", "--leader-socket", "/tmp/leader-x.sock"])
    ...
    fn leader_socket_flag_defaults_to_none() {
        let args = PagerArgs::try_parse_from(["gbuild"]).expect("bare gbuild parses");
        assert!(args.leader_socket.is_none());
    }
    #[test]
    fn leader_mgmt_list_info_kill_parse() {
        let list = PagerArgs::try_parse_from(["gbuild", "leader", "list", "--json"])
            .expect("gbuild leader list --json");
        assert!(matches!(
            list.command,
        ...
        let info = PagerArgs::try_parse_from(["gbuild", "leader", "info", "--pid", "42"])
            .expect("gbuild leader info --pid");
        assert!(matches!(
            info.command,
            Some(Command::Leader(LeaderMgmtArgs {
        ...
        let kill =
            PagerArgs::try_parse_from(["gbuild", "leader", "kill"]).expect("gbuild leader kill");
        assert!(matches!(
            kill.command,
            Some(Command::Leader(LeaderMgmtArgs {
        ...
        assert!(PagerArgs::try_parse_from(["gbuild", "leader", "profile"]).is_err());
    }
    #[test]
    fn debug_file_flag_parses_and_is_global() {
        let root = PagerArgs::try_parse_from(["gbuild", "--debug-file", "/tmp/fire.txt"])
        ...
        let sub =
            PagerArgs::try_parse_from(["gbuild", "agent", "stdio", "--debug-file", "/tmp/f.txt"])
    ...
    fn positional_prompt_seeds_interactive_session() {
        let args =
            PagerArgs::try_parse_from(["gbuild", "fix the bug"]).expect("positional prompt parses");
        assert_eq!(args.initial_prompt(), Some("fix the bug"));
        assert!(args.command.is_none());
        assert!(args.single.is_none());
    }
    #[test]
    fn bare_gbuild_has_no_initial_prompt() {
        let args = PagerArgs::try_parse_from(["gbuild"]).expect("bare gbuild parses");
    ...
    fn initial_prompt_trims_and_ignores_whitespace_only() {
        let args =
            PagerArgs::try_parse_from(["gbuild", "  spaced  "]).expect("padded prompt parses");
        assert_eq!(args.initial_prompt(), Some("spaced"));
        let blank = PagerArgs::try_parse_from(["gbuild", "   "]).expect("blank prompt parses");
    ...
    fn reasoning_effort_and_effort_alias_parse_same_field() {
        let long = PagerArgs::try_parse_from(["gbuild", "--reasoning-effort", "high"])
            .expect("--reasoning-effort parses");
        assert_eq!(long.reasoning_effort.as_deref(), Some("high"));
        let alias = PagerArgs::try_parse_from(["gbuild", "--effort", "high"])
    ...
    fn reasoning_effort_accepts_max_and_remapped_ids() {
        let max = PagerArgs::try_parse_from(["gbuild", "--effort", "max"]).expect("max parses");
        assert_eq!(max.reasoning_effort.as_deref(), Some("max"));
        let deep = PagerArgs::try_parse_from(["gbuild", "--reasoning-effort", "deep"])
            .expect("deep parses");
    ...
    fn reasoning_effort_last_flag_wins_when_both_names_set() {
        let args =
            PagerArgs::try_parse_from(["gbuild", "--reasoning-effort", "low", "--effort", "high"])
                .expect("both effort flag names parse");
        assert_eq!(args.reasoning_effort.as_deref(), Some("high"));
        let reverse =
            PagerArgs::try_parse_from(["gbuild", "--effort", "high", "--reasoning-effort", "low"])
    ...
    fn agent_args_effort_alias_parses() {
        let args = PagerArgs::try_parse_from(["gbuild", "agent", "--effort", "max", "stdio"])
