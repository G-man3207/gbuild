...
/// [`gbuild_shell::util::config::resolve_hints`]; this type is the pager's
/// in-memory mirror.
...
impl From<gbuild_shell::util::config::WorktreeHintMode> for WorktreeMode {
    fn from(mode: gbuild_shell::util::config::WorktreeHintMode) -> Self {
        use gbuild_shell::util::config::WorktreeHintMode;
        match mode {
...
impl WorktreeMode {
    ...
    pub fn from_config_str(s: &str) -> Self {
        gbuild_shell::util::config::WorktreeHintMode::from_config_str(s).into()
    }
    ...
    pub fn resolve_from_hints_value(hints: Option<&toml::Value>) -> (Self, Self) {
        let (new_session, fork) = gbuild_shell::util::config::WorktreeHintMode::resolve_pair(hints);
...
/// Cap for the `GBUILD_ESC_DOUBLE_PRESS_MS` override; the pty_e2e suite sets
/// exactly this value.
pub const ESC_DOUBLE_PRESS_TEST_MS: u64 = 60_000;
/// Idle-Esc double-press confirm window, `GBUILD_ESC_DOUBLE_PRESS_MS`-overridable
...
pub(crate) fn esc_double_press_ttl() -> Duration {
    ...
    *TTL.get_or_init(|| parse_esc_ttl(std::env::var("GBUILD_ESC_DOUBLE_PRESS_MS").ok()))
}
...
/// (toolset) gate via [`gbuild_shell::tier::is_restricted_tier_name`] so
...
fn is_restricted_tier(tier: Option<&str>) -> bool {
    match tier {
        None => true,
        Some(t) => gbuild_shell::tier::is_restricted_tier_name(t),
    }
}
...
pub struct AppView {
    ...
    /// stays sans-IO.
    pub current_ui: gbuild_shell::agent::config::UiConfig,
    /// Working directory.
    pub cwd: PathBuf,
    /// Whether the project picker question has already been shown this session.
    pub project_picker_shown: bool,
    /// "Don't ask me again" opt-out from [`gbuild_shell::util::config::resolve_hints`];
    ...
    /// Current appearance config (hot-reloadable from ~/.gbuild/pager.toml).
    ...
    /// Scroll-diagnostics HUD (`GBUILD_SCROLL_DEBUG` env / `/scroll-debug`).
    ...
    /// Dashboard entry points (`/dashboard`, `Ctrl+\`, `gbuild dashboard`, the
    ...
    /// non-selectable headers. Gated by `GBUILD_SESSION_PICKER_GROUPED` env var
    ...
    /// back into the input box. Gated by `GBUILD_CANCEL_REWIND` env /
    ...
    /// empty and inert outside `--minimal`; the `gbuild-pager-minimal` crate
    ...
    pub session_picker_content_results:
        Option<Vec<gbuild_shell::extensions::session_search::SearchSessionHit>>,
    ...
    pub(crate) foreign_session_compat:
        gbuild_workspace::foreign_sessions::EnabledForeignSessionSources,
    ...
    /// `gbuild_shell::util::config::resolve_auto_permission_mode_enabled`.
    ...
    /// at startup and on settings toggles from `GBUILD_CONTEXTUAL_HINTS` (master)
    /// > `[ui.contextual_hints]` user config > remote tier > default.
    pub contextual_hints: gbuild_shell::util::config::ResolvedContextualHints,
    ...
    pub remote_contextual_hints: Option<gbuild_shell::util::config::ContextualHintsRemote>,
    ...
    /// One-shot gate for the SSH `gbuild wrap` tip: set after the first
    ...
    /// `gbuild_usage_redirect_url`, targeted at personal-team users).
    ...
    pub pending_gate_verification: Option<gbuild_shell::auth::GateInfo>,
    ...
    /// `~/.gbuild/config.toml`. `None` when the file/section is absent
    ...
    /// kill switch or `GBUILD_VOICE_MODE=0` — the STT pipeline is not started and
...
impl AppView {
    ...
    pub fn gate_from_settings(
        rs: &gbuild_shell::util::config::RemoteSettings,
    ) -> Option<gbuild_shell::auth::GateInfo> {
        let msg = rs.gate_message.as_ref()?;
        if msg.is_empty() {
            return None;
        }
        Some(gbuild_shell::auth::GateInfo {
            message: msg.clone(),
    ...
    pub fn apply_auth_meta(&mut self, meta: &gbuild_shell::auth::AuthMeta) {
        ...
        if was_gated && self.gate.is_none() {
            ...
            gbuild_telemetry::session_ctx::log_event(
                gbuild_telemetry::events::SubscriptionActivated {
    ...
    pub fn new(
        ...
        Self {
            ...
            current_ui: gbuild_shell::agent::config::UiConfig::default(),
            ...
            voice_config: gbuild_voice::VoiceConfig::default(),
            voice_auth: None,
            voice_cmd_tx: None,
    ...
    pub fn deferred_model_switch_from_cli(
        ...
        acp::ModelId,
        Option<gbuild_shell::sampling::types::ReasoningEffort>,
    )> {
    ...
    pub fn voice_can_start_pipeline(&self) -> bool {
        self.voice_mode_enabled && gbuild_voice::AUDIO_SUPPORTED
    }
    ...
    fn voice_send(&self, cmd: gbuild_voice::VoiceCommand) {
    ...
    pub(crate) fn voice_begin_recording(&mut self, target: VoiceTarget, hold: bool) {
        self.voice_send(gbuild_voice::VoiceCommand::PttPress);
    ...
    pub(crate) fn voice_stop_keeping_final(&mut self) {
        ...
        self.voice_send(gbuild_voice::VoiceCommand::PttRelease);
    ...
    /// Rows the dev `GBUILD_FPS` overlay occupies (0 in non-dev builds), so
...
struct WelcomeInputCtx<'a> {
    ...
    sp_content_results: &'a Option<Vec<gbuild_shell::extensions::session_search::SearchSessionHit>>,
    ...
    fn draw_inner(&mut self, terminal: &mut PagerTerminal) {
            ...
            )
        {
            ...
            if crate::terminal::terminal_context()
                ...
                .osc22_cursor
            {
                gbuild_shell::util::with_locked_stderr(|stderr| {
                ...
                match *active_view {
                    ActiveView::Welcome => {
                        ...
                        if !has_access && !self.access_gate_shown_logged {
                            ...
                            gbuild_telemetry::session_ctx::log_event(
                                gbuild_telemetry::events::SuperGrokUpsellShown {
                                    source:
                                        gbuild_telemetry::events::SuperGrokUpsell::WelcomeScreen,
                                    auth_method: self
                                        .login_method_id
                                        .as_ref()
...
impl AppView {
    ...
    pub fn apply_contextual_hints(
        &mut self,
        resolved: gbuild_shell::util::config::ResolvedContextualHints,
    ) {
    ...
    /// One-shot SSH `gbuild wrap` tip trigger, run at the top of every `draw`
    ...
    fn apply_clipboard_probe(
        ...
        if agent.show_ephemeral_tip(
            ...
            gbuild_telemetry::session_ctx::log_event(gbuild_telemetry::events::ContextualTip {
                tip: gbuild_telemetry::events::ContextualTipKind::ImageInput,
                action: gbuild_telemetry::events::ContextualTipAction::Shown,
            });
            return true;
        }
        false
...
pub(crate) mod tests {
    ...
    /// including after the fast foreign scan lands rows the default gBuild
    ...
    fn tick_demand_fast_while_modal_session_picker_loads() {
        ...
        assert_eq!(
            app.tick_demand(),
            TickDemand::Fast,
            "foreign rows hidden by the gBuild filter must not end the loading spinner"
        );
    ...
    fn needs_animation_gates_todo_badge_flash() {
        ...
        app.agents
            ...
            .unwrap()
            .todo
            .update_todos(vec![gbuild_shell::tools::TodoItem {
                content: "do the thing".into(),
                priority: Default::default(),
                status: gbuild_shell::tools::TodoStatus::InProgress,
                meta: None,
            }]);
        assert!(
    ...
    fn apply_auth_meta_disables_billing_surface_for_team_users() {
        ...
        let meta = gbuild_shell::auth::AuthMeta {
    ...
    fn apply_auth_meta_enables_billing_surface_for_personal_users() {
        ...
        let meta = gbuild_shell::auth::AuthMeta::default();
        app.apply_auth_meta(&meta);
        assert!(app.usage_visible);
    }
    #[test]
    fn apply_auth_meta_clears_api_key_flag_and_restores_billing_on_personal_login() {
        ...
        app.usage_visible = false;
        app.apply_auth_meta(&gbuild_shell::auth::AuthMeta::default());
    ...
    fn apply_auth_meta_clears_gate_on_subscription() {
        let mut app = test_app();
        app.gate = Some(gbuild_shell::auth::GateInfo {
    ...
    fn apply_auth_meta_gate_unchanged_when_still_gated() {
        let mut app = test_app();
        let gate = gbuild_shell::auth::GateInfo {
    ...
    fn welcome_ctrl_u_update_keeps_priority_over_foreign_resume() {
        ...
        app.apply_foreign_resume_detection(
            launch_token,
            &canonical_cwd,
            Some(gbuild_workspace::foreign_sessions::RecentForeignSession {
    ...
    fn esc_owned_before_agent_covers_app_level_owners() {
        ...
        app.import_claude_modal = Some(
            crate::views::import_claude_modal::ImportClaudeModalState::new(
                gbuild_shell::claude_import::ImportPlan::default(),
    ...
    fn overlay_left_arrow_file_search_open_does_not_exit() {
        let (mut app, id) = neutral_overlay_app();
        {
            ...
            agent.prompt.file_search.set_test_state(
                ctx,
                vec![gbuild_workspace::file_system::FuzzyMatchResult {
    ...
    fn install_question_overlay(
        ...
        use gbuild_tools::implementations::gbuild::ask_user_question::{Question, QuestionOption};
    ...
    fn welcome_picker_f_cycle_disabled_under_chat_mode() {
        ...
        assert_eq!(
            ...
            crate::views::session_picker::SourceFilter::GBuild,
