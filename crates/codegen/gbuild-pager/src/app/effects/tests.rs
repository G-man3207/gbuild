...
use super::*;
use gbuild_shell::extensions::billing::{BillingConfig, Cent, UsagePeriod};
...
fn parse_kill_outcome_reads_result_envelope() {
    use gbuild_tools::types::KillOutcome;
...
fn parse_worktree_restore_payload_full() {
    use gbuild_workspace::session::git::RestoreDegree;
...
/// Redirect `GBUILD_HOME` to a tempdir for test isolation.
fn setup_gbuild_home_in_tempdir() -> tempfile::TempDir {
...
fn register_session_in(root: &std::path::Path, id: &str) -> acp::SessionId {
    use gbuild_shell::active_sessions::{ActiveSession, register_in};
...
fn unregister_best_effort_removes_entry_when_lock_free() {
    ...
    assert!(
            gbuild_shell::active_sessions::list_in(dir.path())
                .expect("list")
                .is_empty(),
...
async fn persist_permission_mode_acp_notification_fires_once_on_best_effort() {
    ...
    let _guard = setup_gbuild_home_in_tempdir();
...
async fn foreign_scan_task_echoes_sequence_without_enabled_sources() {
    ...
    execute(
        Effect::ScanForeignSessions {
            ...
            compat: gbuild_workspace::foreign_sessions::EnabledForeignSessionSources::default(),
            gbuild_home: PathBuf::from("/path/that/must/not/be-read"),
...
fn agent_profile_names_are_valid_builtins() {
    use std::str::FromStr;
    use gbuild_agent::config::BuiltinAgentName;
...
/// Default flags produce no agent profile (uses gbuild default).
#[test]
fn default_flags_produce_no_profile() {
    let flags = SessionFlags::default();
    assert_eq!(flags.agent_profile(), None);
}
/// --subagents alone produces no profile (gbuild already has TaskTool).
#[test]
...
/// Neutralize `GBUILD_AGENT` for the profile-matrix tests below: agent-driven
...
/// Callers must be `#[serial_test::serial(GBUILD_AGENT)]` (process-global env).
fn without_gbuild_agent() -> crate::test_util::EnvVarGuard {
    crate::test_util::EnvVarGuard::set("GBUILD_AGENT", "")
}
...
#[serial_test::serial(GBUILD_AGENT)]
#[test]
fn runtime_default_flags_produce_plan_meta() {
    let _env = without_gbuild_agent();
    let flags = SessionFlags {
        plan_mode: true,
        subagents: true,
        ask_user: true,
    ...
    assert_eq!(meta["agentProfile"], "gbuild-plan");
...
fn plan_only_meta() {
    ...
    let meta = flags.to_meta().unwrap();
    assert_eq!(meta["agentProfile"], "gbuild-plan-no-subagents");
...
/// --ask-user alone selects the gbuild-ask-user profile.
#[serial_test::serial(GBUILD_AGENT)]
#[test]
fn ask_user_alone_meta() {
    ...
    let meta = flags.to_meta().unwrap();
    assert_eq!(meta["agentProfile"], "gbuild-ask-user");
    assert!(meta.get("askUserQuestion").is_none());
    assert_eq!(meta["yoloMode"], false);
}
...
/// does not need an `agentProfile` (default `gbuild` already has it).
#[test]
...
/// All three flags on at the runtime default produce gbuild-plan
...
fn make_session_info(
    ...
    used: u64,
    total: u64,
) -> gbuild_shell::session::SessionInfoResponse {
    use gbuild_shell::session::acp_types::{ContextInfo, SessionInfoData};
    gbuild_shell::session::SessionInfoResponse {
