...
pub struct AcpConnection {
    ...
    /// Whether the agent is a gbuild-shell instance.
    pub is_gbuild_shell: bool,
    ...
    pub auth_manager: std::sync::Arc<gbuild_shell::auth::AuthManager>,
}
...
pub struct ConnectFlags {
    ...
    pub remote_settings: Option<gbuild_shell::util::config::RemoteSettings>,
...
pub async fn connect(cancel: &CancellationToken, flags: ConnectFlags) -> Result<AcpConnection> {
    ...
    let spawned = spawn::spawn_gbuild_shell(agent_config, cancel, memory_config).await?;
...
pub async fn connect_via_leader(
    ...
    let auth_manager = std::sync::Arc::new(gbuild_shell::auth::AuthManager::new(
...
fn apply_config_writes(flags: &ConnectFlags) {
    ...
    let config_path = gbuild_shell::util::gbuild_home::gbuild_home().join("config.toml");
...
async fn initialize(
    ...
    // Check if this is a gbuild-shell agent
    let is_gbuild_shell = resp
        .meta
        .as_ref()
...
mod tests {
    ...
    ///
    /// Enterprise/BYOK configs (e.g. an enterprise `~/.gbuild/config.toml` with a
    ...
    fn shell_built_auth_methods_for_byok_user_skip_login_screen() {
        use gbuild_shell::agent::auth_method::{AuthMethodsBuildInputs, build_auth_methods};
    ...
    fn startup_auth_xai_api_key_not_first_still_requires_login() {
        use gbuild_shell::agent::auth_method::{GROK_COM_METHOD_ID, XAI_API_KEY_METHOD_ID};
