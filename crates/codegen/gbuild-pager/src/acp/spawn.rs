...
//!
//! Simplified to only support GBuildShell (in-process) mode.
...
///
/// Hold one from every site that calls [`spawn_gbuild_shell`] (headless, the TUI,
...
/// Spawn a GBuildShell agent in a background thread.
///
/// Returns the ACP client channel for communication and a cancellation token.
pub async fn spawn_gbuild_shell(
    agent_config: AgentConfig,
    cancel: &CancellationToken,
    memory_config: Option<gbuild_shell::config::MemoryConfig>,
) -> Result<SpawnedAgent> {
    ...
    let (agent_config, models_manager) =
        gbuild_shell::agent::init::bootstrap(&agent_config, &auth_manager, None)
            ...
            local.block_on(&rt, async move {
                ...
                let _skills_watcher = {
                    ...
                    let workspace_user_dir =
                        gbuild_agent::prompt::workspace_user::optional_workspace_user_dir();
                    gbuild_shell::config::watcher::SkillsFileWatcher::start(
                        Some(cwd.as_path()),
                    ...
                    .map(|(mut watcher, mut skills_rx)| {
                        ...
                        tokio::task::spawn_local(async move {
                            while let Some(change) = skills_rx.recv().await {
                                ...
                                match change {
                                    gbuild_shell::config::watcher::DiscoveryChange::Skills => {
                                        tracing::info!(
                                    ...
                                    gbuild_shell::config::watcher::DiscoveryChange::Workflows => {
                                        tracing::info!(
