...
/// Shared GBUILD_HOME boundary fixture for the resume-by-title startup and
/// pre-sandbox tests.
///
/// `gbuild_home()` is OnceLock-cached process-wide, so summaries land under the
/// *resolved* home (possibly the real `~/.gbuild` when another test pinned the
...
impl Default for GBuildHomeFixture {
    fn default() -> Self {
        Self::new()
    }
}
