...
//!
//! Generalizes the `gbuild ssh` wrapper: spawns an arbitrary command inside a
...
//! run — a single shell-quoted string (`gbuild wrap "mycli ssh host"`) or a shell
...
pub fn run(args: &WrapArgs) -> Result<()> {
    ...
    let (wrapped, fallback) = {
        ...
        let in_path = gbuild_config::shell::is_command_available(program);
        (
...
/// The program and argv `gbuild wrap` will actually spawn.
#[derive(Clone)]
struct SpawnPlan {
    program: String,
    args: Vec<String>,
...
fn derive_spawn(
    ...
    // (`gbuild wrap "mycli ssh host"`), not a program name: hand it to the shell
    ...
    // name, and an empty one (`gbuild wrap "$PROG" ...` with `$PROG` unset)
...
/// deliberately not `gbuild_config::shell::unix_shell_path`, which coerces
...
///
/// Unlike `gbuild ssh`, `gbuild wrap` does not gate on the terminal brand: the user
