...
fn execute_hook(
    ...
    cmd.arg("-c")
        .arg(command)
        .env("GBUILD_EVENT", event_str)
        .env("GBUILD_MESSAGE", message)
        .stdin(Stdio::null())
        .stdout(Stdio::null())
        .stderr(Stdio::null());
    if let Some(sid) = session_id {
        cmd.env("GBUILD_SESSION_ID", sid);
    }
...
mod tests {
    ...
    fn sets_environment_variables() {
        ...
        let command = format!(
            "printf 'GBUILD_EVENT=%s\\nGBUILD_MESSAGE=%s\\nGBUILD_SESSION_ID=%s\\n' \
             \"$GBUILD_EVENT\" \"$GBUILD_MESSAGE\" \"$GBUILD_SESSION_ID\" > {}",
            out.display()
        );
        ...
        assert!(
            content.contains("GBUILD_EVENT=Turn complete"),
            "missing GBUILD_EVENT: {content}"
        );
        assert!(
            content.contains("GBUILD_MESSAGE=hello world"),
            "missing GBUILD_MESSAGE: {content}"
        );
        assert!(
            content.contains("GBUILD_SESSION_ID=sess-42"),
            "missing GBUILD_SESSION_ID: {content}"
        );
    }
    ...
    fn omits_session_id_when_none() {
        ...
        let content = std::fs::read_to_string(&out).unwrap();
        assert!(
            !content.contains("GBUILD_SESSION_ID"),
            "GBUILD_SESSION_ID should not be set: {content}"
        );
    }
    ...
    fn run_hook_passes_correct_env_via_thread() {
        ...
        assert!(content.contains("GBUILD_EVENT=Turn complete"));
        assert!(content.contains("GBUILD_MESSAGE=test body payload"));
        assert!(content.contains("GBUILD_SESSION_ID=test-session-123"));
    }
}
