...
fn shorten_path(path: &str) -> &str {
    let memory_root = gbuild_config::gbuild_home().join("memory");
...
**File:** /root/.gbuild/memory/MEMORY.md (lines 0-10)
```
## Project Conventions
...
"#;
        ...
        assert_eq!(results[0].path, "/root/.gbuild/memory/MEMORY.md");
...
**File:** /root/.gbuild/memory/ws/MEMORY.md (lines 1-5)
```
workspace content
```

### Result 2 (score: 0.42, source: session)
**File:** /root/.gbuild/memory/ws/sessions/2026-05-01.md (lines 10-20)
```
session content
```
"#;
    ...
    fn shorten_memory_path() {
        // Paths under the configured gbuild memory root keep one trailing segment group.
        let memory_root = gbuild_config::gbuild_home().join("memory");
