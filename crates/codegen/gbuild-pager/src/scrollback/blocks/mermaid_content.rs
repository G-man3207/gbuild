...
//! the generic [`CodeBlockSpan`](gbuild_markdown::CodeBlockSpan) API) and
...
use gbuild_markdown::MarkdownRenderView;
...
pub struct MermaidBlock {
    ...
    /// from [`CodeBlockSpan::body`](gbuild_markdown::CodeBlockSpan::body). For
    ...
    /// [`CodeBlockSpan::output_line_range`](gbuild_markdown::CodeBlockSpan::output_line_range).
...
) -> impl Iterator<Item = &'a gbuild_markdown::CodeBlockSpan> {
    view.code_blocks
        .iter()
...
///
/// `GBuildDay` is the only light theme; every other concrete theme (and the
/// `GBuildNight` default that `Auto` resolves to before it reaches the cache) is
/// dark. The render worker maps this to `gbuild_mermaid::MermaidTheme`; it
...
pub fn theme_is_dark(theme: ThemeKind) -> bool {
    !matches!(theme, ThemeKind::GBuildDay)
}
...
mod tests {
    ...
    use gbuild_markdown::StreamingMarkdownRenderer;
    ...
    #[test]
    fn theme_is_dark_maps_gbuild_day_to_light_only() {
        assert!(
            !theme_is_dark(ThemeKind::GBuildDay),
            "GBuildDay is the light theme"
        );
        for dark in [
            ThemeKind::GBuildNight,
