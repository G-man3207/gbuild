...
use crate::common::*;
use gbuild_pager_pty_harness::{InferenceEndpoint, InferenceRequestMatcher};
...
async fn minimal_commits_thinking_body_to_scrollback() {
    ...
    std::fs::create_dir_all(content.home().join(".gbuild")).expect("mk .gbuild");
    std::fs::write(
        content.home().join(".gbuild/config.toml"),
