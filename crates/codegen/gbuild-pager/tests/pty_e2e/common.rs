...
pub(crate) use gbuild_pager_pty_harness::{
...
pub(crate) const CONTEXTUAL_HINTS_ENV: &[(&str, &str)] = &[("GBUILD_CONTEXTUAL_HINTS", "1")];
...
/// would never fetch `/v1/settings`. Spawns WITHOUT `GBUILD_ANNOUNCEMENTS_OVERRIDE`
...
// `wait_for_labels_absent` lives in `gbuild_pager_pty_harness::flows`
// (re-exported above).
...
pub(crate) fn seed_mcp_server_config(content: &ContentController) {
    ...
    let gbuild_home = content.home().join(".gbuild");
    std::fs::create_dir_all(&gbuild_home).expect("create fake GBUILD_HOME");
    let config = format!(
        "[mcp_servers.{MCP_TEST_SERVER}]\ncommand = \"{command}\"\nargs = []\nstartup_timeout_sec = 2\n"
    );
    std::fs::write(gbuild_home.join("config.toml"), config).expect("write config.toml");
}
...
// unit tests in gbuild-hooks::discovery::tests. The PTY E2E test requires
...
/// Seed `~/.gbuild/config.toml` with a `[ui]` section body (e.g.
/// `"vim_mode = true"`). Same `{GBUILD_HOME|HOME}/.gbuild/config.toml` location
...
pub(crate) fn seed_ui_config(content: &ContentController, ui_body: &str) {
    let gbuild_home = content.home().join(".gbuild");
    std::fs::create_dir_all(&gbuild_home).expect("create .gbuild");
...
pub(crate) fn seed_mouse_reporting_toggle_config(content: &ContentController, enabled: bool) {
    let gbuild_home = content.home().join(".gbuild");
    std::fs::create_dir_all(&gbuild_home).expect("create .gbuild");
    // Minimal opt-in only — matches load_config's `{GBUILD_HOME|HOME}/.gbuild/config.toml`.
...
pub(crate) fn spawn_mouse_toggle_pager(content: &ContentController) -> PtyHarness {
    ...
    PtyHarness::spawn_with_content_env_ops(
        ...
        content,
        &[],
        &[EnvOp::set("GBUILD_MOUSE_REPORTING_TOGGLE", "true")],
    )
    .expect("spawn pager")
}
...
pub(crate) const ESC_DOUBLE_PRESS_ENV: &str = "GBUILD_ESC_DOUBLE_PRESS_MS";
...
pub(crate) fn spawn_esc_double_press_pager(content: &ContentController) -> PtyHarness {
    let binary = pager_binary().expect("resolve pager binary");
    let value = gbuild_pager::app::app_view::ESC_DOUBLE_PRESS_TEST_MS.to_string();
    PtyHarness::spawn_with_content_env_ops(
        &binary,
        DEFAULT_ROWS,
        DEFAULT_COLS,
...
/// `gbuild wrap` run budget. Same contention math as the requirements-version
...
/// Run `gbuild wrap <wrap_args...>` to completion inside a PTY with an isolated
/// `GBUILD_HOME`, returning the exit code (`None` only while still running at
...
pub(crate) fn run_wrap_driving(
    ...
    let mut env: Vec<(&str, &str)> = vec![("GBUILD_HOME", &home_str), ("NO_COLOR", "1")];
    env.extend_from_slice(extra_env);

    let mut harness =
        PtyHarness::new_inherited_env(&binary, DEFAULT_ROWS, DEFAULT_COLS, &args, &env, None)
            .expect("spawn gbuild wrap");
    ...
    let code = match wait_for_exit_status(&mut harness, WRAP_TIMEOUT) {
        ...
        Ok(PtyExitPoll::Running) => {
            harness.quit().expect("kill gbuild wrap after timeout");
            None
        }
        Ok(PtyExitPoll::PendingStatus) => {
            panic!(
                "gbuild wrap exited but portable status remained unavailable for {WRAP_TIMEOUT:?}"
            )
        }
        Err(error) => panic!("poll gbuild wrap exit: {error:#}"),
    };
...
/// `gbuild wrap` hands to the user's shell without depending on any real
...
/// Dump an asciinema cast of `harness` into `$GBUILD_PTY_CAST_DIR/<file_name>`
...
pub(crate) fn write_cast_if_requested(harness: &PtyHarness, file_name: &str) {
    let Ok(dir) = std::env::var("GBUILD_PTY_CAST_DIR") else {
        return;
    };
    if dir.is_empty() {
        return;
...
pub(crate) use gbuild_pager_pty_harness::host_clipboard::{
...
pub(crate) use gbuild_pager_pty_harness::host_clipboard::clipboard_roundtrip_works;
