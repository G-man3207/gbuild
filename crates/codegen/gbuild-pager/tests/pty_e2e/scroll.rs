...
use gbuild_pager_pty_harness::PtyHarness;
pub(crate) use gbuild_pager_pty_harness::{SGR_SCROLL_DOWN, SGR_SCROLL_UP};
...
/// (`gbuild-pager-pty-harness/src/scroll_matrix/runner.rs`) ports this
...
/// `gbuild-pager-pty-harness/src/scroll_matrix/session.rs` is a port of
...
/// (e.g. `GBUILD_SCROLL_SPEED`) appended to the controller's env.
