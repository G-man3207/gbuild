...
/// Like [`seed_fake_oauth`], but under the `GBUILD_LOCAL_AUTH` dev issuer
...
/// `GBUILD_LOCAL_AUTH=1` in the spawn env so the shell's scope-key lookup
/// resolves this entry.
fn seed_fake_oauth_local_issuer(content: &ContentController, user: &str) {
    let gbuild_home = content.home().join(".gbuild");
    std::fs::create_dir_all(&gbuild_home).expect("create temp .gbuild");
    std::fs::write(
...
fn spawn_subscription_pager(
    ...
    overrides.push(EnvOp::set("GBUILD_LOCAL_AUTH", "1"));
...
async fn subscription_watch_polls_free_tier_then_goes_dormant_after_upgrade() {
    ...
    let mut harness = spawn_subscription_session(
        &content,
        "pty-subwatch",
        &[EnvOp::set("GBUILD_SUBSCRIPTION_WATCH_INTERVAL_SECS", "1")],
    );
...
async fn stale_gate_push_never_flashes_paywall_for_subscribed_user() {
    ...
    let mut harness = spawn_subscription_session(
        &content,
        "pty-subgate-paid",
        &[EnvOp::set("GBUILD_SUBSCRIPTION_WATCH_INTERVAL_SECS", "0")],
    );
