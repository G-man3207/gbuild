...
//!
//! Uses `gbuild-pager-pty-harness` (`PtyHarness`) + Shift+Tab (CSI Z,
...
//! Always-Approve.
//!
//! Auth: seeds `HOME/.gbuild/auth.json` from `GBUILD_AUTH_JSON` (path) or the
//! developer's `~/.gbuild/auth.json` so the pager skips device-login when
...
//!
//! Run with:
//! `cargo test -p gbuild-pager --test pty_auto_mode -- --ignored --nocapture`

use std::path::PathBuf;
use std::time::Duration;

use gbuild_pager_pty_harness::{PtyHarness, pager_binary};
use gbuild_test_support::TestSandbox;
...
/// Prefer explicit path, else the user's real `~/.gbuild/auth.json`.
fn auth_json_source() -> Option<PathBuf> {
    if let Ok(p) = std::env::var("GBUILD_AUTH_JSON") {
...
fn prepare_sandbox(sandbox: &mut TestSandbox, gate_on: bool) -> Vec<(String, String)> {
    ...
    let gbuild = sandbox.gbuild_home();
    let _ = std::fs::create_dir_all(gbuild);
    if let Some(src) = auth_json_source() {
        let dest = gbuild.join("auth.json");
        ...
        eprintln!("pty_auto_mode: no ~/.gbuild/auth.json — may hit device login");
    }
    ...
    // of the developer's shell. `GBUILD_AUTO_PERMISSION_MODE` is the highest gate
    // layer below requirements; "1"/"0" parse to on/off (gbuild_config::
    ...
    env.push((
        "GBUILD_AUTO_PERMISSION_MODE".into(),
...
fn require_auth() -> bool {
    std::env::var("GBUILD_PTY_REQUIRE_AUTH").is_ok_and(|v| v == "1" || v == "true")
}
...
fn pty_shift_tab_cycles_to_auto_mode_banner() {
    ...
    let mut harness =
        PtyHarness::new_in_sandbox(&binary, ROWS, COLS, &[], &sandbox, &env_refs, None)
            .expect("spawn pager in PTY (gbuild-pager-pty-harness)");
    ...
    if is_login_screen(&early) {
        assert!(
            !require_auth(),
            "GBUILD_PTY_REQUIRE_AUTH set but pager hit the login screen — auth seeding broke"
        );
