...
//!
//! These tests exercise the real `gbuild-pager` binary through a PTY using
...
/// enterprise deploy report: with `GBUILD_GOAL=1`, `/goal` must
...
/// explicitly off (`GBUILD_GOAL=0`; goal mode defaults on), `/goal` must stay
...
/// also pins `GBUILD_CONTEXTUAL_HINTS=0` (belt-and-suspenders); the same planning
