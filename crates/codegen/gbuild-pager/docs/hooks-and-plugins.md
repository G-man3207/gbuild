...
gBuild supports **hooks** (event-driven shell commands) and **plugins** (bundles of skills, agents, hooks, and MCP servers). Both are managed through a unified modal interface.
...
- **Global hooks** — from `~/.gbuild/hooks/`
- **Project hooks** — from `.gbuild/hooks/` in your repo
...
Plugin hooks automatically receive `GBUILD_PLUGIN_ROOT` and `GBUILD_PLUGIN_DATA` environment variables (see the [Plugins guide](../user-guide/09-plugins.md#environment-variables-in-plugin-hooks)).
...
2. **settings.json** — `extraKnownMarketplaces` from `~/.gbuild/settings.json` or `~/.claude/settings.json`
...
Press `a` on the Marketplace tab (or run `gbuild plugin marketplace add <source>`)
...
Sources land in `~/.gbuild/config.toml`:
...
Or in `~/.gbuild/settings.json` / `~/.claude/settings.json`:
...
- [Hook Examples](../../../gbuild-hooks/examples/README.md) — ready-to-use sample hooks
