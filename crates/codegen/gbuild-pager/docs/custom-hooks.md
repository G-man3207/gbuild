...
Hooks let you run custom scripts or HTTP requests at key moments during a gBuild session - for example, before or after a tool runs, when a session starts or ends, or when the agent sends a notification.
...
1. Create the hooks directory:
   ```sh
   mkdir -p ~/.gbuild/hooks
   ```

2. Create a simple hook file, e.g. `~/.gbuild/hooks/session-start.json`:
   ```json
   {
     "hooks": {
       "SessionStart": [
...
**Tool name aliases**: Claude-style names like `Bash`, `Edit`, `Read` automatically match gBuild's internal names (`run_terminal_cmd`, `search_replace`, `read_file`).
...
gBuild injects the following variables into every hook process:

- `GBUILD_HOOK_EVENT` — the event name (e.g. `pre_tool_use`, `session_start`, `post_tool_use`)
- `GBUILD_HOOK_NAME` — the full configured name of this hook
- `GBUILD_SESSION_ID` — the current session identifier
- `GBUILD_WORKSPACE_ROOT` — absolute path to the workspace root

For hooks provided by plugins, the following are also set:

- `GBUILD_PLUGIN_ROOT` — absolute path to the plugin's installation directory
- `GBUILD_PLUGIN_DATA` — absolute path to the plugin's writable data directory

These runner- and plugin-injected variables always take precedence. Attempts to override the reserved runner keys via the `env` field are stripped at load time (with a warning logged). For plugin hooks, `GBUILD_PLUGIN_ROOT` and `GBUILD_PLUGIN_DATA` similarly override any user-supplied values for those keys.
...
`GBUILD_PLUGIN_ROOT` and `GBUILD_PLUGIN_DATA`. These keys override any user-declared
...
{
  "type": "command",
  "command": "${HOME}/.config/gbuild-hooks/check.sh"
}
```

Lookup order for each reference:
1. The handler's own `env` map.
2. The current process environment (the env gBuild itself sees).
...
`${GBUILD_PLUGIN_ROOT}/check` resolve against the plugin's actual path.
...
Hooks from `~/.gbuild/hooks/` appear under **Global**, project ones under **Project**, etc.
...
```json
{ "type": "http", "url": "https://hooks.example.com/gbuild-event", "timeout": 15 }
```
...
5. **Version control project hooks** — commit `.gbuild/hooks/` (but never secrets).
...
- **See errors?** → Check the pager logs (usually in the tracing pane or `~/.gbuild/logs`).

## More Examples

See the built-in examples in the `gbuild-hooks` crate:

- [Safe Shell Guard](../../../gbuild-hooks/examples/hooks/safe-shell.json)
- [No Recursive Grep](../../../gbuild-hooks/examples/hooks/no-recursive-grep.json) — hard-blocks `grep -r`/`grep -R`/`rgrep` (OOM guard)
- [Session Audit Log](../../../gbuild-hooks/examples/hooks/session-log.json)
- [Tool Activity Logger](../../../gbuild-hooks/examples/hooks/tool-logger.json)

Copy them to `~/.gbuild/hooks/` and customize.
