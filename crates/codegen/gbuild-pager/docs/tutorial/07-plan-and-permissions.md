...
gBuild asks before doing anything risky — and can plan before it codes.

## Permissions

When gBuild wants to run a risky command or edit a file, it pauses and asks:
...
For bigger or more ambiguous tasks, use **plan mode**: gBuild explores the
...
`s` requests changes — gBuild iterates until you're happy, then implements.
...
background — gBuild keeps working and you're notified when it finishes
