...
gBuild runs as a full-screen TUI. It relies on terminal support for color,
...
Run `/doctor` in gBuild to check the current session and see available fixes. If
gBuild cannot start, run `gbuild doctor` in your shell. Use `gbuild doctor --json`
...
`gbuild doctor --json` reports the same color capability when piped. Microphone
...
`/doctor fix tmux-clipboard` or `gbuild doctor fix dcs-passthrough --yes`.
...
to guess if that directory is unavailable or unsafe. gBuild preserves the file's
...
gBuild deliberately does **not** run `tmux source-file` or change the live tmux
...
gBuild detects these terminal emulators from environment variables:
...
- Inside tmux, variables that identify the outer terminal may not reach gBuild.
...
gBuild writes through up to three routes, shown in `/doctor` under **Clipboard**:

- **native** — the local operating-system clipboard.
- **tmux** — the tmux paste buffer when gBuild runs inside tmux.
...
terminal focused. Older compositors may require gBuild to remain focused until
the copy message appears. gBuild shows a startup warning when this applies; run
`/doctor` for the detected status and steps.

`GBUILD_CLIPBOARD_NO_DATA_CONTROL=1` is an advanced fallback that disables the
data-control route. Copies then use command-line clipboard tools.

#### OSC 52 kill switch

gBuild emits OSC 52 on Linux and across tmux, SSH, or displayless containers when
that route is enabled. A terminal that does not implement OSC 52 may display the
encoded payload as text. Set `GBUILD_CLIPBOARD_NO_OSC52=1` before starting gBuild to
...
A remote gBuild process normally cannot read the local terminal's selection. Use
...
When gBuild cannot identify the outer terminal over SSH, it predicts that OSC 52
...
clipboard. Each copy is still saved to a backup file (`~/.gbuild/last-copy.txt` by
default; override with `GBUILD_COPY_FILE`); the toast names that path when delivery
...
through `gbuild wrap`, for example `gbuild wrap ssh user@host`. The same command can
wrap container and pod shells. It also restores terminal modes after a dropped
connection.

When an SSH session is not using `gbuild wrap`, gBuild shows the one-time tip
...
`[ui.contextual_hints]` in `$GBUILD_HOME/config.toml`. This setting does not hide
the Doctor recommendation.

For repeated SSH use, Doctor offers `gbuild doctor fix ssh-wrap`. It also shows
...
> **Warning**: `gbuild wrap` is experimental and may not work in every setup.
...
Zellij and tmux control mode can limit the alternate screen. gBuild normally uses
inline mode in those environments. Run `/doctor` to see the detected condition.
You can configure `[terminal] alt_screen` in `~/.gbuild/pager.toml`, or run
`gbuild --no-alt-screen` to confirm inline mode works.

### Zellij keybindings interfere with gBuild

Zellij can intercept Ctrl/Alt keys before they reach gBuild. On Zellij 0.41 or
...
mode, if `Ctrl+G` still does not reach gBuild, open the command palette and select
...
WezTerm ships with the Kitty keyboard protocol disabled. Run `/doctor` in gBuild.
...
keys. gBuild therefore does not negotiate the protocol there, and Shift+Enter can
...
If gBuild stops receiving mouse input, re-enable mouse reporting in the terminal:
...
After about 10 seconds without a transcript, gBuild stops capture and shows
...
to the terminal hosting gBuild. Open **System Settings → Privacy & Security →
...
Run `gbuild doctor`, or run `/doctor` while voice mode is on. The **Voice** section
shows the microphone gBuild would use. If no input device is available, Doctor
...
problem, set `GBUILD_VOICE_CAPTURE=inprocess` to use the in-process fallback for
comparison.
