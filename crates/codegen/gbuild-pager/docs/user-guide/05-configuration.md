...
2. **Environment variables** (e.g. `XAI_API_KEY`, `GBUILD_MEMORY`)
3. **config.toml** (`~/.gbuild/config.toml`)
...
Location: `~/.gbuild/config.toml`. If the file is missing, gBuild uses its built-in defaults, so you only need to set the values you want to override.
...
You can also override this with `GBUILD_DEFAULT_SELECTED_PERMISSION`, which is handy for headless or agent test runs that shouldn't mutate `config.toml`. Precedence: env var → `config.toml` → `always_allow_all_sessions`.
...
Each setting also has an environment-variable override, applied on first load only (again, handy for headless / test runs): `GBUILD_SCROLL_SPEED`, `GBUILD_SCROLL_MODE`, `GBUILD_INVERT_SCROLL` (`1`/`true`/`0`/`false`), and `GBUILD_SCROLL_LINES`. Precedence: env var → `config.toml` → default. Unrecognized values fall back to the default, and out-of-range numbers clamp.
...
MCP servers can also be set per-project in `.gbuild/config.toml`. Project-scoped config contributes `[mcp_servers]`, `[plugins]`, and `[permission]` rules; every other section loads only from `~/.gbuild/config.toml`.

Priority for `[mcp_servers]` and `[plugins]`: `.gbuild/config.toml` (current dir) > `<repo-root>/.gbuild/config.toml` > `~/.gbuild/config.toml`. `[permission]` rules aren't overridden by priority — they merge across all files with `deny` > `ask` > `allow` (see [22-permissions-and-safety.md](22-permissions-and-safety.md)).

### Memory

Persist knowledge across sessions (requires `--experimental-memory` or `GBUILD_MEMORY=1`).
...
Background workflows — the `workflow` tool, named `.gbuild/workflows/*.rhai` scripts, `/deep-research`, and `/workflow` launches — are **on by default**. Disable with config, env, or remote settings.

```toml
[workflows]
enabled = false                       # disable background workflows (or GBUILD_WORKFLOWS=0)
```

Project workflows are discovered from `<repo-root>/.gbuild/workflows/`; user workflows from `~/.gbuild/workflows/`. Discovery and invocation key off the script's `meta.name`, so keep each filename aligned with its `meta.name`. Built-ins win over project names, and project names win over user names, so keep names unique across scopes.
...
`[hints]` is read from the **effective config merge**, with the usual precedence: system managed → user `managed_config.toml` → user `config.toml` → user `requirements.toml` → system `requirements.toml`, higher layers winning. The TUI only ever **writes** opt-outs to your user `~/.gbuild/config.toml`.
...
Run your own commands when events fire. Hooks receive `$GBUILD_EVENT`, `$GBUILD_MESSAGE`, and `$GBUILD_SESSION_ID` in the environment.

```toml
# macOS native notification
[[ui.notifications.hooks]]
command = "terminal-notifier -title 'gBuild' -message '$GBUILD_MESSAGE'"
events = ["turn_complete", "approval_required"]
only_unfocused = true
timeout_secs = 10
...
# Push to ntfy server
[[ui.notifications.hooks]]
command = "curl -s -d '$GBUILD_MESSAGE' ntfy.sh/my-gbuild-alerts"
...
- **`[features] telemetry`** / `GBUILD_TELEMETRY_ENABLED` — the product-analytics master switch. `/privacy` doesn't change it.
- **`/privacy`** / Settings — coding-data sharing, separate from telemetry.
- **`[telemetry] trace_upload`** / `GBUILD_TELEMETRY_TRACE_UPLOAD` — session traces; follows telemetry when unset.
- **`[telemetry] otel_*`** / `GBUILD_EXTERNAL_OTEL` — external OTEL to your own collector (below).
...
```toml
[telemetry]
otel_enabled = true                                       # external OTEL master switch (= GBUILD_EXTERNAL_OTEL)
...
Location: `~/.gbuild/pager.toml`. This controls the TUI's look and feel. Changes apply on restart.
...
| `GBUILD_AUTH_PROVIDER_COMMAND` | External auth binary path |
| `GBUILD_AUTH_PROVIDER_LABEL` | Display name on TUI login screen |
| `GBUILD_AUTH_TOKEN_TTL` | Token lifetime in seconds |
| `GBUILD_AUTH_EARLY_INVALIDATION_SECS` | Seconds before expiry to refresh (default: 300) |
| `GBUILD_OIDC_ISSUER` | OIDC issuer URL |
| `GBUILD_OIDC_CLIENT_ID` | OIDC client ID |

### Endpoints

| Variable | Description |
|----------|-------------|
| `GBUILD_CLI_CHAT_PROXY_BASE_URL` | Override API proxy base URL |
...
| `GBUILD_SUBAGENTS` | Enable (`1`) or disable (`0`) subagents |
| `GBUILD_WORKFLOWS` | Enable (`1`) or disable (`0`) background workflows and select the `/goal` driver (default on: host-owned workflow driver; off: legacy `update_goal`) |
| `GBUILD_WEB_FETCH` | Enable (`1`) or disable (`0`) the web_fetch tool |
| `GBUILD_WEB_FETCH_ALLOW_LOCAL` | Allow `web_fetch` to explicit loopback hosts only (`localhost` / `127.0.0.0/8` / `::1`). Same as `[toolset.web_fetch] allow_local`. Default off; private/metadata stay blocked. |
| `GBUILD_AGENT` | Custom agent definition path or name |
| `GBUILD_SANDBOX` | Sandbox profile (off, workspace, devbox, read-only, strict; or a custom profile name) |

### Logging

| Variable | Description |
|----------|-------------|
| `GBUILD_LOG_FILE` | Write logs to this file path (used verbatim as the path) |
| `RUST_LOG` | Log level filter (e.g. `debug`); controls the `GBUILD_LOG_FILE` log and headless stderr output |
...
| `GBUILD_RESPECT_GITIGNORE` | Force gitignore filtering on (`1`) or off (`0`); overrides `[tools] respect_gitignore` |

### Telemetry

| Variable | Description |
|----------|-------------|
| `GBUILD_TELEMETRY_ENABLED` | Enable/disable telemetry |
| `GBUILD_TELEMETRY_TRACE_UPLOAD` | Enable/disable session trace upload |
| `GBUILD_TELEMETRY_MIXPANEL_ENABLED` | Enable/disable Mixpanel specifically |
| `GBUILD_EXTERNAL_OTEL` | External OTEL to your collector (see [24-monitoring-usage.md](24-monitoring-usage.md)) |
| `GBUILD_FEEDBACK_ENABLED` | Enable/disable feedback system |
| `GBUILD_DEPLOYMENT_KEY` | Management API key for enterprise |
...
| Path | Description |
|------|-------------|
| `~/.gbuild/config.toml` | Main configuration file |
| `~/.gbuild/pager.toml` | TUI appearance configuration |
...
| `~/.gbuild/lsp.json` | LSP server configuration (user-scoped) |
| `~/.gbuild/logs/` | Internal log files (e.g. `unified.jsonl`, MCP server logs) |
| `.gbuild/config.toml` | Project-scoped MCP servers, plugins, and permission rules |
...
Some settings can be set per-project by placing files in `.gbuild/` inside your repository:

| File | What it configures |
|------|--------------------|
| `.gbuild/config.toml` | MCP servers, plugins, permission rules, and the `[mcp] max_output_bytes` tool-result cap (other sections load only from `~/.gbuild/config.toml`) |
| `.gbuild/skills/` | Project-specific skills |
| `.gbuild/hooks/` | Project-specific lifecycle hooks |
| `.gbuild/agents/` | Project-specific agent definitions |
| `.gbuild/lsp.json` | LSP server configuration |
| `.gbuild/sandbox.toml` | Custom sandbox profiles |
...
|--------|----------|-------|
| User | `~/.gbuild/lsp.json` | All projects |
| Project | `.gbuild/lsp.json` | Current repository |
...
1. **Project** — `.gbuild/lsp.json`
2. **User** — `~/.gbuild/lsp.json`
