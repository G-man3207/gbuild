...
> **Status: alpha.** The schema below is versioned (`gbuild.schema.version = v1`);
> additive changes may occur without notice, renames/removals will bump the
> version and be called out in the changelog.

gBuild CLI can export usage **metrics** and **events** to your organization's
...
|---------|---------------|
| Telemetry master switch | `[features] telemetry` / `GBUILD_TELEMETRY_ENABLED` |
| `/privacy` | `/privacy opt-in` / `/privacy opt-out`, or Settings |
| Trace upload | `[telemetry] trace_upload` / `GBUILD_TELEMETRY_TRACE_UPLOAD` |
| External OpenTelemetry | `GBUILD_EXTERNAL_OTEL` / `[telemetry] otel_*` (this guide) |
...
```bash
export GBUILD_EXTERNAL_OTEL=1                  # master switch
...
`GBUILD_EXTERNAL_OTEL=1` alone enables **nothing** — you must also select at
...
|---|---|---|
| `GBUILD_EXTERNAL_OTEL` | `0` | Master switch. Distinct from `GBUILD_TELEMETRY_ENABLED`, which controls SpaceXAI-internal product analytics — the two govern opposite-pointing data flows. |
...
| `OTEL_LOG_USER_PROMPTS` | `0` | Content gate: prompt text on `gbuild.user_prompt` (60 KB cap, secret-scrubbed). |
...
> `GBUILD_EXTERNAL_OTEL` is set, product analytics ignores those vars, and the
...
standard OTEL names** (`GBUILD_EXTERNAL_OTEL`, `OTEL_*`) for ecosystem
...
## Metrics (meter scope `gbuild`)

| Metric | Unit | Attributes |
|---|---|---|
| `gbuild.session.count` | `{session}` | base attrs only |
| `gbuild.token.usage` | `{token}` | `type` = `input` \| `output` \| `reasoning` \| `cache_read`; `model` |
| `gbuild.turn.count` | `{turn}` | `outcome` = `completed` \| `cancelled` \| `error`; `model` |
| `gbuild.tool.decision` | `{decision}` | `tool_name`, `decision` = `allow` \| `deny` \| `cancelled` \| `followup`, `access_kind`, `permission_mode` |
| `gbuild.tool.usage` | `{call}` | `tool_name`, `outcome` |
| `gbuild.error.count` | `{error}` | `error_category`, `model` |

There is no `cost.usage` metric: join `gbuild.token.usage` with your own
...
|---|---|
| `gbuild.session_start` | `model`, `permission_mode`, `mcp_server_count`, `plugin_count`, `skill_count`, `hook_count`, `memory_enabled`, `is_git_repo`, `client_identifier` |
| `gbuild.session_end` | `duration_secs`, `turn_count`, `tool_call_count`, `compaction_count`, `model` |
| `gbuild.user_prompt` | `prompt_length`, `model`, `screen_mode?` (`fullscreen` \| `inline` \| `minimal` \| `headless` \| `other`); `prompt` (**prompts**) |
| `gbuild.turn_completed` | `outcome`, `duration_ms`, `tool_call_count`, `model`, `error_category?`, `cancellation_category?` |
| `gbuild.api_request` | `model`, `duration_ms`, `stop_reason?`, `input_tokens`, `output_tokens`, `reasoning_tokens`, `cache_read_tokens` |
| `gbuild.api_error` | `error_category`, `model`, `status_code?`, `duration_ms?` |
| `gbuild.tool_result` | `tool_name`, `outcome`, `success`, `duration_ms`, `file_extension`; `tool_parameters`, `file_path` (**details**) |
| `gbuild.tool_decision` | `tool_name`, `decision`, `access_kind`, `permission_mode`, `source` |
| `gbuild.mcp_server_connection` | `status`, `transport_type`, `duration_ms`, `tool_count?`, `error_type?`; `mcp_server.name` (**details**; collapsed to `mcp_server` otherwise) |
| `gbuild.permission_mode_changed` | `to_mode`, `trigger` |
| `gbuild.skill_activated` | `skill_source`; `skill.name` (**details**) |
| `gbuild.plugin_loaded` | `install_kind?`, `success`, `error_category?`; `plugin_name` (**details**) |
| `gbuild.compaction` | `duration_ms`, `tokens_before`, `tokens_after`, `model?` |
| `gbuild.subagent` | `phase` = `launched` \| `completed`, `subagent_type?`, `outcome?`, `duration_ms?` |
| `gbuild.auth` | `auth_method` |
| `gbuild.internal_error` | `error_type` (class only — no message, no location) |
| `gbuild.model_switched` | `from_model`, `to_model`, `success`, `error_code?` |
...
sum by (model, type) (rate(gbuild_token_usage_total[1h]))

# Sessions per team per day
sum by (team_id) (increase(gbuild_session_count_total[1d]))

# Tool-permission denial ratio
sum(rate(gbuild_tool_decision_total{decision="deny"}[1h]))
  / sum(rate(gbuild_tool_decision_total[1h]))
```
