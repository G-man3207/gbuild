...
2. Create a hook file, e.g. `~/.gbuild/hooks/session-start.json`:
...
|------|------|-------------|
| `~/.gbuild/config.toml` | User | You |
| `managed_config.toml` (`$GBUILD_HOME`, `/etc/gbuild`) | Managed / system | Your organization |
...
- **`/goal`**: gBuild's goal loop is a separate feature that runs before the stop gate; it is not a prompt-type Stop hook.
...
|-----------------------|-------------|
| `GBUILD_HOOK_EVENT`     | The name of the event that triggered the hook (e.g. `pre_tool_use`, `session_start`, `post_tool_use`, `session_end`, `stop`, `notification`). |
| `GBUILD_HOOK_NAME`      | The configured name of this specific hook (includes the plugin prefix for plugin-provided hooks). |
| `GBUILD_SESSION_ID`     | The unique identifier of the current gBuild session. |
| `GBUILD_WORKSPACE_ROOT` | Absolute path to the root of the current workspace. |
| `CLAUDE_PROJECT_DIR`  | Absolute path to the workspace root. A Claude Code-compatible alias for `GBUILD_WORKSPACE_ROOT`, set for every hook. |
...
| `GBUILD_PLUGIN_DATA`   | Absolute path to the plugin's writable data directory (for storing plugin state, caches, etc.). |

These values are provided by the plugin system. For the four plugin-related keys (`GBUILD_PLUGIN_ROOT`, `GBUILD_PLUGIN_DATA`, and their Claude aliases), the plugin adapter ensures the official plugin values always win over any user-declared values in the hook's `env` map.
