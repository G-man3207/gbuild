...
Commands come from two places: **shell builtins**, handled by the agent backend (gbuild-shell), and **pager builtins**, handled by the TUI frontend (gbuild-pager). Both show up in the same menu, and any enabled skill with `user-invocable: true` appears there too.
...
Every copy is also written to a backup file — `~/.gbuild/last-copy.txt` by default, or `GBUILD_COPY_FILE` if set. Confirmed copies toast briefly (e.g. `Copied!`). Unverified OSC 52 deliveries and clipboard-unreachable fallbacks name the backup path so you can recover the text.
...
`/flush`, `/dream`, and `/memory` require memory to be enabled (`--experimental-memory` or `GBUILD_MEMORY=1`); `/memory` also needs a configured memory backend. `/remember` is always available.
...
Project workflows live in `.gbuild/workflows/*.rhai`; user workflows live in `~/.gbuild/workflows/*.rhai`. A same-process pause/resume continues the original immutable script, args, and `agent_budget` cap from committed host-call results — to iterate, edit the returned script copy and launch it as a new run.
...
Any enabled skill with `user-invocable: true` in its SKILL.md frontmatter shows up as a slash command. (Turn a skill off via `/skills` and it stops being advertised.) So a skill at `~/.gbuild/skills/commit/SKILL.md` runs as:
