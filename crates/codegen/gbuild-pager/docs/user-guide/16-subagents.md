...
| **How you set them** | At startup, or with agent definitions (`.md` files in `.gbuild/agents/` or `~/.gbuild/agents/`) | In `config.toml` (`[subagents.personas]`) or `.toml` files under `.gbuild/personas/`; applied during subagent resolution |
...
gBuild discovers file-based personas from these locations, in priority order:

- `.gbuild/personas/*.toml` (project)
- `~/.gbuild/personas/*.toml` (user)
...
> **Note:** gBuild applies personas through subagent resolution and roles, not through a `spawn_subagent` parameter. The main agent does not pass a persona name when it spawns a child.
...
When a persona applies, gBuild resolves the effective model and reasoning effort in this order, highest priority first:
...
gBuild manages worktrees through the `x.ai/git/worktree/*` extension methods, including an apply operation that merges changes back into the main working directory.
...
# instructions_file = ".gbuild/personas/concise.md"  # or load from a file
```

gBuild also discovers roles from `.gbuild/roles/*.toml` and personas from `.gbuild/personas/*.toml`. Inline `config.toml` definitions take precedence over files.

---

## The Tasks Pane (TUI)

gBuild shows running and finished work in side panes on the agent screen:
