...
Set a persistent default in `~/.gbuild/config.toml`:
...
Add custom model endpoints in `~/.gbuild/config.toml` under `[model.<name>]` sections:
...
```bash
export GBUILD_WEB_SEARCH_MODEL="grok-4.5"
```
