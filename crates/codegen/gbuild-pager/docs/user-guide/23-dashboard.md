...
- **`gbuild dashboard`** — launches the TUI directly into the dashboard.
...
- **Ctrl+\\** — same as the slash command, two keystrokes. Configurable
  in `~/.gbuild/config.toml` under `[keybindings]` like every other shortcut.
...
```
 gBuild · Dashboard — 4 agents · 2 awaiting
...
The state icon matches gBuild's sibling views (
`tasks_pane`):
...
rebound via `~/.gbuild/config.toml`.
...
Set `GBUILD_AGENT_DASHBOARD=0` to force-disable the feature for a single
...
process. The plan's Phase 4 ("supervisor / `gbuild --bg`") would list
