...
//! it" buckets.
//!
//! `gbuild-sampler` is intentionally decoupled from `gbuild-shell`
...
/// tools live in `gbuild-tools`
/// (`crates/codegen/gbuild-tools/src/implementations/`), have their
/// own HTTP clients that do not flow through `SamplingClient`, and
/// hook into the `gbuild_tools::ApiKeyProvider` trait rather than
/// this enum.
...
/// `gbuild_shell::auth::token_suffix` (which truncates to 12 chars
...
/// `gbuild-shell/src/auth/manager.rs` to match, otherwise the
