...
use gbuild_sampling_types::error::{try_parse_stream_error, user_facing_api_error_message};
...
pub use gbuild_sampling_types::ApiBackend;

/// Process-level fallback for the `x-grok-client-identifier` header.
const DEFAULT_CLIENT_IDENTIFIER: &str = "gbuild-shell";
...
fn apply_terminal_event_overrides(event: &mut rs::ResponseStreamEvent, data: &str) {
    ...
    if let Some(ticks) = gbuild_sampling_types::reported_cost_ticks(
        value
...
struct ClientDefaults {
    ...
    doom_loop_recovery: Option<gbuild_sampling_types::DoomLoopRecoveryPolicy>,
}
...
impl SamplingClient {
    ...
    pub async fn conversation_stream_responses(
        ...
        let extra_tools = gbuild_sampling_types::extra_tool_entries(&request.hosted_tools);
...
mod tests {
    use super::*;
    use gbuild_sampling_types::types::ChatRequestMessage;
    use indexmap::IndexMap;
    ...
    fn user_agent_omits_origin_version_when_absent() {
        ...
        let ua = user_agent_string_for(&origin);
        // No slash between product and the gbuild-shell agent product.
        assert!(ua.starts_with("my-client gbuild-shell/"));
    }

    #[test]
    fn user_agent_collapses_when_origin_matches_agent() {
        let agent_version = gbuild_version::VERSION.to_string();
        let origin = OriginClientInfo {
            product: AGENT_PRODUCT.to_string(),
