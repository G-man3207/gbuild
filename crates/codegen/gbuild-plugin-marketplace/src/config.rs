//! Parse marketplace sources from `~/.gbuild/config.toml`.
//!
//! Expected format:
//! ```toml
//! [[marketplace.sources]]
...
/// `GBUILD_MARKETPLACE_REQUIRE_SHA=1`. Tighten-only: either source can enable,
...
pub fn env_require_sha() -> bool {
    gbuild_config::env_bool("GBUILD_MARKETPLACE_REQUIRE_SHA").unwrap_or(false)
}
...
/// instead of `~/.gbuild`/`~/.claude`. Each root is checked for
...
mod tests {
    ...
    fn require_sha_policy_composition() {
        ...
        unsafe { std::env::remove_var("GBUILD_MARKETPLACE_REQUIRE_SHA") };
        assert!(!load_require_sha(&empty), "absent everywhere → off");
        assert!(load_require_sha(&enabled), "config alone can enable");

        unsafe { std::env::set_var("GBUILD_MARKETPLACE_REQUIRE_SHA", "1") };
        assert!(load_require_sha(&empty), "env alone can enable");

        unsafe { std::env::set_var("GBUILD_MARKETPLACE_REQUIRE_SHA", "0") };
        assert!(
