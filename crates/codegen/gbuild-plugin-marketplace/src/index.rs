...
//!
//! Catalog file lookup, in order: `.gbuild-plugin/marketplace.json` (preferred),
//! `.gbuild-plugin/plugin.json`, `.claude-plugin/marketplace.json`,
...
///
/// Checks (in order):
/// 1. `.gbuild-plugin/marketplace.json` (preferred xAI convention)
/// 2. `.gbuild-plugin/plugin.json`
...
mod tests {
    ...
    #[test]
    fn load_index_valid_gbuild_dir() {
        let dir = tempfile::tempdir().unwrap();
        let gbuild_dir = dir.path().join(".gbuild-plugin");
        std::fs::create_dir_all(&gbuild_dir).unwrap();
        std::fs::write(
            gbuild_dir.join("marketplace.json"),
            r#"{"name": "gbuild", "plugins": []}"#,
        )
        .unwrap();
        let result = load_index(dir.path()).unwrap();
        assert!(result.is_some());
        assert_eq!(result.unwrap().name, "gbuild");
    }

    #[test]
    fn load_index_gbuild_dir_takes_precedence_over_claude_dir() {
        let dir = tempfile::tempdir().unwrap();
        for (sub, name) in [(".gbuild-plugin", "gbuild"), (".claude-plugin", "claude")] {
        ...
        assert_eq!(load_index(dir.path()).unwrap().unwrap().name, "gbuild");
    }
