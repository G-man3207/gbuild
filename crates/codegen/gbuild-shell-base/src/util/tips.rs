...
//! persisted to `~/.gbuild/tip_cursor.json`.
...
fn cursor_path(gbuild_home: &Path) -> PathBuf {
    gbuild_home.join(CURSOR_FILE)
}

/// Load the cursor from `~/.gbuild/tip_cursor.json`. Returns 0 on any error.
fn load_cursor(gbuild_home: &Path) -> u64 {
    let text = match std::fs::read_to_string(cursor_path(gbuild_home)) {
        Ok(t) => t,
        Err(_) => return 0,
    };
...
/// Save the cursor to `~/.gbuild/tip_cursor.json`. Silently ignores write errors.
fn save_cursor(gbuild_home: &Path, cursor: u64) {
    if let Ok(text) = serde_json::to_string(&TipState { cursor }) {
        let _ = std::fs::write(cursor_path(gbuild_home), text);
    }
}
...
/// cursor in `~/.gbuild/tip_cursor.json`, so every session sees the next tip
...
pub fn pick_and_advance(tips: &[String], gbuild_home: &Path) -> Option<String> {
    if tips.is_empty() {
        return None;
    }
    let cursor = load_cursor(gbuild_home);
    let tip = tips[cursor as usize % tips.len()].clone();
    save_cursor(gbuild_home, cursor + 1);
    Some(tip)
}
