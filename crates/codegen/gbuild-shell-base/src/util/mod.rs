...
pub mod tips;
pub mod uname;
pub use gbuild_shared::clipboard;
pub use gbuild_shared::stderr::{stderr_lock, with_locked_stderr};
...
/// toggles via `GBUILD_CLI_CHAT_PROXY_BASE_URL` (or similar) pointing at an
...
pub fn is_gbuild_process(pid: u32) -> bool {
    #[cfg(target_os = "linux")]
    {
...
/// Stricter [`is_gbuild_process`] for the auto-kill zombie path: on macOS/BSD it
...
/// exactly, so this delegates there. Use the permissive [`is_gbuild_process`] for
/// operator-driven `gbuild leader kill`.
pub fn is_gbuild_process_strict(pid: u32) -> bool {
    #[cfg(all(not(target_os = "linux"), not(windows)))]
    {
...
mod tests {
    ...
    #[test]
    fn is_gbuild_process_impossible_pid_false() {
        assert!(!is_gbuild_process(u32::MAX));
    }
    #[test]
    fn is_gbuild_process_strict_impossible_pid_false() {
        assert!(!is_gbuild_process_strict(u32::MAX));
    }
    #[cfg(unix)]
    #[test]
