...
use gbuild_shell::agent::app::{run_headless, run_leader, run_stdio_agent};
use gbuild_shell::agent::config::Config as AgentConfig;
...
fn apply_agent_endpoint_args(agent_args: &gbuild_pager::app::AgentArgs, config: &mut AgentConfig) {
...
fn print_serve_startup_info(bind_addr: SocketAddr, secret: &str) {
    eprintln!();
    eprintln!("   gBuild agent server starting...");
    eprintln!();
...
/// Entrypoint tag for `gbuild -p`; keys the quiet stderr default in `init_tracing_simple`.
...
fn init_tracing_simple(app_entrypoint: &'static str) {
    use gbuild_telemetry::debug_log::RMCP_SSE_NOISE_TARGET;
    ...
    let registry = tracing_subscriber::registry()
        ...
        .with(gbuild_telemetry::sampling_log::layer())
        .with(gbuild_telemetry::instrumentation::layer())
        .with(gbuild_telemetry::hooks_log::layer())
        .with(gbuild_telemetry::otel_layer::build_otel_layer(
            gbuild_telemetry::otel_layer::OtelClientInfo {
                client_name: "gbuild-pager",
                client_version: gbuild_version::VERSION,
    ...
    gbuild_telemetry::external::init(gbuild_shell::agent::config::resolve_external_otel_config(
        gbuild_telemetry::external::config::ExternalClientInfo {
            ...
            client_version: gbuild_version::VERSION.to_owned(),
...
async fn run_leader_mgmt(args: LeaderMgmtArgs) -> Result<()> {
    match args.command {
        ...
        LeaderMgmtCommand::List { json } => {
            let leaders = gbuild_shell::leader::discover_leaders().await;
            if json {
...
async fn kill_leaders() -> Result<()> {
    ...
    for d in &leaders {
        ...
        if !gbuild_shell::util::is_gbuild_process(pid) {
            if let Some(ref lock) = d.lock_path {
                eprintln!("  PID {pid} is not a gBuild process, removing stale lock");
        ...
        if let Err(e) = gbuild_shell::util::kill_process_by_pid(pid) {
...
fn resolve_target(args: &LeaderTargetArgs) -> LeaderTarget {
    match args.pid {
        ...
        None => LeaderTarget::Environment(gbuild_shell::env::GBuildEnvironment::Production),
    }
}
async fn connect_to_leader(
    args: &LeaderTargetArgs,
) -> Result<(LeaderDescriptor, gbuild_shell::leader::LeaderClient)> {
    let target = resolve_target(args);
    let selection = gbuild_shell::leader::resolve_leader_target(target)
        .await
        .map_err(|e| anyhow::anyhow!("{}", e.message))?;
    let socket_path = selection
        .socket_path()
        .ok_or_else(|| anyhow::anyhow!("resolved leader target did not include a socket path"))?;
    let client = gbuild_shell::leader::LeaderClient::connect(
        socket_path.to_path_buf(),
        "gbuild-pager-leader-cli",
        ClientMode::Stdio,
...
fn leader_info_json(
    ...
    info: Option<&gbuild_shell::leader::ControlPayload>,
...
/// Env override for the `gbuild workspace` gate: any truthy value enables the
/// command locally, a falsy one disables it — bypassing the remote settings flag.
const WORKSPACE_COMMAND_ENV: &str = "GBUILD_WORKSPACE_COMMAND";
/// Resolution of the `gbuild workspace` gate. `Unknown` is kept separate from
...
/// The `GBUILD_WORKSPACE_COMMAND` override, if set (`Some(true)`/`Some(false)`);
...
/// Truthy parse for gBuild on/off env vars: everything enables except the common
...
async fn run_workspace_mgmt(args: WorkspaceMgmtArgs) -> Result<()> {
    ...
    match workspace_command_gate(env_override, remote_settings.as_ref()) {
        ...
        WorkspaceGate::Disabled => {
            anyhow::bail!(
                "`gbuild workspace` is not enabled for this account \
    ...
    .map_err(|e| {
        anyhow::anyhow!(
            "no running leader for this environment ({e}). \
             Start a gBuild session, or run `gbuild workspace start`."
        )
    })
}
async fn workspace_control(
    ...
    let raw_config = gbuild_shell::config::load_effective_config_disk_only()
...
async fn workspace_start(
    ...
    use gbuild_shell::auth::ensure_authenticated;
    ...
    if !use_leader {
        anyhow::bail!(
            "`gbuild workspace` requires leader mode (the workspace is shared via the leader).\n\
             Enable it with `[cli] use_leader = true` in ~/.gbuild/config.toml, or pass --leader."
        );
    }
    ensure_authenticated(
        &agent_config.grok_com_config,
        false,
        Some("No cached credentials found. Run `gbuild login` first."),
    )
    .await?;
...
fn shutdown_and_flush_telemetry(exit_code: i32) -> ! {
    gbuild_telemetry::sentry::flush_on_shutdown();
    gbuild_telemetry::otel_layer::shutdown_otel();
    gbuild_telemetry::debug_log::flush();
...
/// can't drift.
const PLUGIN_DIR_LEADER_WARNING: &str = "gbuild: --plugin-dir is ignored in leader mode; run with --no-leader to \
...
async fn run_agent_command(
    ...
    let _signal_flush = tokio::spawn(async {
        #[cfg(unix)]
        {
            use gbuild_pager::app::signal_handler::next_signal_code;
    ...
    agent_config.reasoning_effort_override = agent_args
        .reasoning_effort
        .as_deref()
        .and_then(gbuild_shell::sampling::types::parse_canonical_effort_token);
    let launch_yolo = gbuild_shell::util::config::effective_yolo_for_launch(
        agent_args.yolo,
        permission_mode_flag.as_deref(),
        None,
    );
    if let Some(warning) = launch_yolo.blocked_warning {
        eprintln!("gbuild: {warning}");
    }
    agent_config.default_yolo_mode = launch_yolo.yolo;
    agent_config.default_auto_mode = gbuild_shell::util::config::effective_auto_for_launch(
        agent_args.yolo,
        permission_mode_flag.as_deref(),
        None,
    );
    ...
    agent_config.resolve_runtime_fields(&gbuild_shell::agent::config::RuntimeResolutionContext {
    ...
    if use_leader {
        ...
        let env_urls = gbuild_shell::leader::LeaderEnvUrls::from(&agent_config.grok_com_config);
    ...
    match agent_args.mode {
        ...
        Some(AgentCmd::Serve(a)) => {
            ...
            let server_config = gbuild_shell::agent::ServerConfig {
                bind_addr: a.bind,
                secret: secret.clone(),
            };
            print_serve_startup_info(a.bind, &secret);
            gbuild_shell::agent::run_agent_server(server_config, agent_config).await
        }
...
/// Sets `GBUILD_OPEN_DASHBOARD_AT_STARTUP=1` if the user asked for
/// `gbuild dashboard`, and clears `args.command` so the regular
...
/// the leader roster — so `gbuild dashboard` does NOT force leader mode and
/// is compatible with `--no-leader`.
///
/// The only gate is the feature flag: a disabled dashboard
/// (`[dashboard].enabled = false` / `GBUILD_AGENT_DASHBOARD=0`) is a CLI
...
fn flag_dashboard_at_startup_if_requested(args: &mut PagerArgs) -> Result<()> {
    ...
    if !gbuild_pager::views::dashboard::dashboard_enabled() {
        anyhow::bail!(
            "the Agent Dashboard is disabled. Enable it by removing \
             `[dashboard] enabled = false` from ~/.gbuild/config.toml and \
             unsetting GBUILD_AGENT_DASHBOARD=0."
        );
    }
    args.command = None;
    unsafe { std::env::set_var("GBUILD_OPEN_DASHBOARD_AT_STARTUP", "1") };
    Ok(())
}
...
#[cfg(all(feature = "jemalloc", unix))]
fn jemalloc_allocator_stats() -> Option<gbuild_pager::memory_trace::AllocatorStats> {
    ...
    unsafe {
        ...
        Some(gbuild_pager::memory_trace::AllocatorStats {
...
#[cfg(all(feature = "jemalloc", unix))]
fn jemalloc_heap_stats() -> Option<gbuild_shell::heap_profile::JemallocStats> {
    unsafe {
        tikv_jemalloc_ctl::raw::write(b"epoch\0", 1u64).ok()?;
        ...
        Some(gbuild_shell::heap_profile::JemallocStats {
            allocated,
            resident,
        })
    }
...
fn install_heap_profile_hooks() {
    gbuild_shell::heap_profile::install(gbuild_shell::heap_profile::HeapProfileHooks {
...
fn dispatch_doctor_if_requested(args: &PagerArgs) -> bool {
    ...
    if let Err(error) = gbuild_pager::doctor_cmd::run(doctor_args.clone()) {
        eprintln!("Error: {error:#}");
        std::process::exit(1);
    }
    true
...
fn main() {
    if let Some(code) = gbuild_pager::app::mermaid_worker::maybe_run_render_subprocess() {
        std::process::exit(code);
    }
    if let Some(code) = gbuild_pager::voice::maybe_run_capture_subprocess() {
        std::process::exit(code);
    }
    ...
    gbuild_pager_minimal::install();
    #[cfg(all(feature = "jemalloc", unix))]
    gbuild_pager::memory_release::install_release_hook(purge_jemalloc_retained_pages);
    #[cfg(all(feature = "jemalloc", unix))]
    {
        gbuild_pager::memory_trace::install_allocator_stats_provider(jemalloc_allocator_stats);
        gbuild_pager::memory_trace::install_allocator_dump_provider(jemalloc_stats_dump);
    }
    #[cfg(all(feature = "jemalloc", unix))]
    install_heap_profile_hooks();
    gbuild_pager::memory_trace::start(
        gbuild_shell::util::gbuild_home::gbuild_home().join("memtrace"),
    );
    raise_fd_limit();
    if let Err(e) = gbuild_config::validate_requirements() {
        eprintln!("Couldn't start gBuild: {e}");
        eprintln!();
        eprintln!(
            "Update gBuild to a version the policy allows, or ask your administrator \
             to fix the managed requirements."
        );
        std::process::exit(2);
    }
    let _sentry_guard = gbuild_telemetry::sentry::init(gbuild_telemetry::sentry::Config {
        client: "gbuild-pager",
        client_version: PAGER_CLIENT_VERSION,
        release: env!("VERSION_WITH_COMMIT"),
        disabled: gbuild_shell::agent::config::is_error_reporting_disabled_sync(),
    });
    gbuild_pager::docs::extract_user_guide_docs(&gbuild_shell::util::gbuild_home::gbuild_home());
    xai_crash_handler::install_terminal_restore_only();
    if gbuild_shell::util::config::load_crash_handler_enabled_sync() {
        let crash_dir = gbuild_shell::util::gbuild_home::gbuild_home().join("crash");
        if let Some(report) = xai_crash_handler::check_previous_crash(&crash_dir) {
            eprintln!("gBuild crashed during your last session.");
    ...
    let crashed = gbuild_shell::active_sessions::collect_crashed().unwrap_or_default();
    if !crashed.is_empty() {
        tracing::info!(
            count = crashed.len(),
...
async fn async_main(args: PagerArgs) -> Result<()> {
    ...
    if let Some(ref mode) = args.compaction_mode {
        unsafe { std::env::set_var("GBUILD_COMPACTION_MODE", mode) };
    }
    if let Some(ref detail) = args.compaction_detail {
        unsafe { std::env::set_var("GBUILD_COMPACTION_DETAIL", detail) };
    }
    if args.chat() {
        unsafe {
            std::env::set_var(gbuild_shell::agent::chat_modes::GBUILD_CHAT_MODE_ENV, "1");
        }
    }
    if let Some(ref socket) = args.leader_socket {
        unsafe { std::env::set_var(gbuild_shell::leader::LEADER_SOCKET_ENV, socket) };
    }
    if let Some(ref path) = args.debug_file {
        unsafe {
            std::env::set_var("GBUILD_DEBUG_LOG", path);
            std::env::remove_var("GBUILD_LOG_FILE");
        }
    }
    if args.debug || args.debug_file.is_some() {
        ...
        set_if_unset("GBUILD_DEBUG_LOG", "1");
        set_if_unset("GBUILD_HOOKS_LOG", "1");
    }
    if let Some(Command::Completions { shell }) = &args.command {
        gbuild_pager::completions_cmd::run(*shell);
        return Ok(());
    }
    if let Some(Command::Wrap(ref wrap_args)) = args.command {
        return gbuild_pager::wrap_cmd::run(wrap_args);
    }
    ...
    let sandbox_profile_arg = match args.startup_sandbox_profile(saved_profile.as_deref()) {
        gbuild_pager::app::cli::SandboxStartup::Apply(profile) => profile,
        gbuild_pager::app::cli::SandboxStartup::Conflict { requested, saved } => {
            eprintln!(
    ...
    gbuild_shell::config::apply_sandbox(None, sandbox_profile_arg.as_deref(), args.cwd.as_deref());
    ...
    gbuild_shell::http::set_client_name(if is_interactive {
        gbuild_workspace::permission::ClientType::GBuildPager
    } else {
    ...
    if let Some(command) = args.command.take() {
        match command {
            ...
            Command::Mcp(mcp_args) => {
                ...
                return gbuild_pager::mcp_cmd::run(mcp_args).await;
            }
            Command::Plugin(plugin_args) => {
                init_tracing_simple("cli");
                let _otel_guard = gbuild_telemetry::otel_layer::otel_guard();
                return gbuild_pager::plugin_cmd::run(plugin_args).await;
            }
            Command::Models => {
                init_tracing_simple("cli");
                let _otel_guard = gbuild_telemetry::otel_layer::otel_guard();
                ...
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return gbuild_pager::models::list_available_models(&agent_config).await;
            }
            ...
            Command::Worktree(worktree_args) => {
                ...
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return gbuild_pager::worktree_cmd::run(worktree_args, &agent_config).await;
            }
            ...
            Command::Trace(trace_args) => {
                ...
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return gbuild_pager::trace_cmd::run(trace_args, &agent_config).await;
            }
            ...
            Command::Login {
                ...
                if let Some(provider) = provider {
                    ...
                    if api_key.is_none() && spec.id == "codex" {
                        // ChatGPT subscription sign-in (OpenAI OAuth + PKCE).
                ...
                let config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                gbuild_shell::auth::run_cli_login(&config, oauth, device_auth, devbox).await?;
                println!();
                gbuild_shell::instrumentation::finalize_and_exit(0);
            }
            Command::Logout { provider } => {
                ...
                let config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                gbuild_shell::auth::run_cli_logout(&config)?;
                gbuild_shell::instrumentation::finalize_and_exit(0);
            }
    ...
    let headless_prompt = gbuild_pager::headless::HeadlessPrompt::from_args(
    ...
    if let Some(prompt) = headless_prompt {
        ...
        let json_schema = args
            .json_schema
            .as_deref()
            .map(gbuild_pager::headless::parse_json_schema)
            .transpose()?;
        if json_schema.is_some()
            && args.output_format == gbuild_pager::headless::OutputFormat::Plain
        {
            args.output_format = gbuild_pager::headless::OutputFormat::Json;
        }
        return gbuild_pager::headless::run_single_turn(
            prompt,
            args.verbatim,
            gbuild_pager::headless::HeadlessOptions {
...
mod tests {
    ...
    fn version_flags_and_doctor_are_distinct_early_intents() {
        let version = PagerArgs::try_parse_from(["gbuild", "--version"]).unwrap();
        assert!(version.version);
        assert!(version.command.is_none());
        let short = PagerArgs::try_parse_from(["gbuild", "-v"]).unwrap();
        assert!(short.version);
        assert!(short.command.is_none());
        let subcommand = PagerArgs::try_parse_from(["gbuild", "version"]).unwrap();
    ...
    fn require_opt_prof() -> bool {
        ...
        eprintln!(
            ...
            gbuild_shell::heap_profile::LG_PROF_SAMPLE
        );
        false
    }
    #[cfg(all(feature = "jemalloc", unix))]
    ...
    #[cfg(all(feature = "jemalloc", unix))]
    fn assert_stats_sane(stats: gbuild_shell::heap_profile::JemallocStats) {
    ...
    fn jemalloc_dump_rejects_interior_nul_path() {
        ...
        let path = Path::new(OsStr::from_bytes(b"/tmp/gbuild-jemalloc-\0.heap"));
    ...
    fn install_heap_profile_hooks_wires_shell_apis() {
        install_heap_profile_hooks();
        assert_stats_sane(gbuild_shell::heap_profile::stats().expect("shell stats after install"));
        if !require_opt_prof() {
            assert!(!gbuild_shell::heap_profile::prof_available());
            return;
        }
        assert!(gbuild_shell::heap_profile::prof_available());
        assert_prof_active(false);
        {
            let _guard = ProfActiveGuard::set(true);
            assert_prof_active(true);
            assert!(gbuild_shell::heap_profile::set_prof_active(true));
            assert_prof_active(true);
        }
        assert_prof_active(false);
        assert!(gbuild_shell::heap_profile::set_prof_active(false));
        assert_prof_active(false);
    ...
    #[serial_test::serial(GBUILD_AGENT_DASHBOARD)]
    #[test]
    fn dashboard_subcommand_flags_startup_without_forcing_leader() {
        let mut args = PagerArgs::try_parse_from(["gbuild", "dashboard"]).unwrap();
        ...
        assert_eq!(
            std::env::var("GBUILD_OPEN_DASHBOARD_AT_STARTUP").as_deref(),
            Ok("1"),
            "startup hook flag must be set",
        );
        unsafe { std::env::remove_var("GBUILD_OPEN_DASHBOARD_AT_STARTUP") };
    }
    /// `gbuild dashboard --no-leader` is allowed — the dashboard does not
    /// require a leader, so the combination launches into the dashboard in
    /// non-leader mode.
    #[serial_test::serial(GBUILD_AGENT_DASHBOARD)]
    #[test]
    fn dashboard_subcommand_allows_no_leader() {
        let mut args = PagerArgs::try_parse_from(["gbuild", "--no-leader", "dashboard"]).unwrap();
        flag_dashboard_at_startup_if_requested(&mut args)
    ...
    #[serial_test::serial(GBUILD_WORKSPACE_COMMAND)]
    #[test]
    fn workspace_command_env_override_parsing() {
        unsafe { std::env::remove_var("GBUILD_WORKSPACE_COMMAND") };
        assert_eq!(workspace_command_env_override(), None);
        unsafe { std::env::set_var("GBUILD_WORKSPACE_COMMAND", "1") };
        assert_eq!(workspace_command_env_override(), Some(true));
        unsafe { std::env::set_var("GBUILD_WORKSPACE_COMMAND", "off") };
