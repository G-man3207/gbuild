//! Shared announcement types, persistence, and formatting for gBuild apps.
//!
//! This crate provides the common logic used by `gbuild-shell` and
//! `gbuild-pager` for handling announcements (banner notifications).
...
/// Read hidden announcement ids from `~/.gbuild/announcements.json`.
...
/// Write hidden announcement ids to `~/.gbuild/announcements.json`.
...
fn announcements_state_path() -> PathBuf {
    gbuild_tools::util::gbuild_home::gbuild_home().join("announcements.json")
}
...
///
/// Precedence: `GBUILD_ANNOUNCEMENTS_OVERRIDE` env var (JSON override) → remote announcements.
...
pub fn resolve_startup(
    ...
    if let Ok(raw) = std::env::var("GBUILD_ANNOUNCEMENTS_OVERRIDE") {
        match serde_json::from_str::<Vec<RemoteAnnouncement>>(&raw) {
            Ok(list) => return Some(list),
            Err(e) => {
                tracing::warn!(error = %e, "invalid GBUILD_ANNOUNCEMENTS_OVERRIDE JSON; ignoring override");
            }
        }
    }
...
mod tests {
    ...
    fn resolve_startup_env_override() {
        ...
        unsafe {
            std::env::set_var("GBUILD_ANNOUNCEMENTS_OVERRIDE", r#"[{"id":"test"}]"#);
        }
        ...
        // SAFETY: test-only
        unsafe {
            std::env::remove_var("GBUILD_ANNOUNCEMENTS_OVERRIDE");
        }
    }
